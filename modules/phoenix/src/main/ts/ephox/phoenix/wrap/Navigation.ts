...
const toLast = function <E, D>(
  ...
  if (universe.property().isText(node)) {
    ...
    return children.length > 0
      ? toLast(universe, children[children.length - 1])
...
const toLeaf = function <E, D>(
  ...
  if (children.length > 0 && offset < children.length) {
    ...
    children.length > 0 &&
    universe.property().isElement(element) &&
...
const freefallLtr = function <E, D>(
  universe: Universe<E, D>,
  element: E
): SpotPoint<E> {
  const candidate = scan(universe, element, universe.query().nextSibling).getOr(
    element
  );
  ...
  return children.length > 0
    ? freefallLtr(universe, children[0])
    : Spot.point(candidate, 0);
};
...
const freefallRtl = function <E, D>(
  universe: Universe<E, D>,
  element: E
): SpotPoint<E> {
  const candidate = scan(universe, element, universe.query().prevSibling).getOr(
    element
  );
  if (universe.property().isText(candidate)) {
  ...
  return children.length > 0
    ...
    : Spot.point(candidate, toEnd(universe, candidate));
};

export { toLast, toLeaf, toLower, freefallLtr, freefallRtl };
