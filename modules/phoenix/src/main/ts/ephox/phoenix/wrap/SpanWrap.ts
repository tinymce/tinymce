...
const point = function <E, D>(
  ...
  const range = Spot.points(Spot.point(cursor.element(), cursor.offset()), Spot.point(cursor.element(), cursor.offset()));
...
const scan = function <E, D>(universe: Universe<E, D>, start: E, soffset: number, exclusions: (e: E) => boolean): SpanWrapPoint<E> {
  return universe
    ...
    .bind(function (parent): Option<SpanWrapPoint<E>> {
      ...
      const canReuse =
        isSpan(universe, exclusions)(parent) && universe.property().children(parent).length === 1 && isUnicode(universe, start);
      return canReuse
...
const wrap = function <E, D>(
  ...
  return Option.from(wrappers[wrappers.length - 1]).map(function (lastSpan) {
    ...
    const range = Spot.points(Spot.point(wrappers[0], 0), Spot.point(lastSpan, lastOffset));
