...
const point = function <E, D>(
  universe: Universe<E, D>,
  start: E,
  soffset: number,
  _finish: E,
  _foffset: number,
  exclusions: (e: E) => boolean
): Option<SpanWrapRange<E>> {
...
const temporary = function <E, D>(
  ...
  const injectAt = universe.property().isEmptyTag(start)
    ? universe.property().parent(start)
    : Option.some(start);
...
const scan = function <E, D>(
  ...
  return universe
    .property()
    .parent(start)
    .bind(function (parent): Option<SpanWrapPoint<E>> {
      const cursor = Spot.point(start, soffset);
      const canReuse =
        isSpan(universe, exclusions)(parent) &&
        universe.property().children(parent).length === 1 &&
        isUnicode(universe, start);
      return canReuse
        ? Option.some<SpanWrapPoint<E>>({
...
const isUnicode = function <E, D>(universe: Universe<E, D>, element: E) {
  return (
    ...
    universe.property().getText(element) === Unicode.zeroWidth
  );
};
...
const wrap = function <E, D>(
  ...
  const wrappers = Wrapper.reuse(
    universe,
    start,
    soffset,
    finish,
    foffset,
    isSpan(universe, exclusions),
    nuSpan
  );
...
const isCollapsed = function <E, D>(
  universe: Universe<E, D>,
  start: E,
  soffset: number,
  finish: E,
...
const spans = function <E, D>(
  universe: Universe<E, D>,
  start: E,
  soffset: number,
  finish: E,
  foffset: number,
  exclusions: (e: E) => boolean = Fun.never
) {
  const wrapper = isCollapsed(universe, start, soffset, finish, foffset)
    ? point
    : wrap;
