...
UnitTest.test('SplitterTest', function () {
  ...
  const checkSubdivide = function (
    ...
    assert.eq(
      expected.length,
      actual.length,
      'Incorrect size for subdivide test'
    );
  ...
  checkSubdivide(
    ['_', 'abcdefghijklm', 'n', 'opq', 'rstuvwxyz'],
    [
    ...
    'a',
    [1, 14, 15, 18],
    Gene('root', 'root', [TextGene('a', '_abcdefghijklmnopqrstuvwxyz')])
  );
