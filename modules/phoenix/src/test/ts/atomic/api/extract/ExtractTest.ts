...
UnitTest.test('api.Extract.(from,all,extract,extractTo)', function () {
  ...
  const checkExtract = function (
    expected: { id: string; offset: number },
    childId: string,
    offset: number
  ) {
  ...
  checkExtract(
    { id: '1.2', offset: 'This is textinside a span'.length },
    '1.2.3',
    0
  );
  checkExtract(
  ...
  checkExtractTo(
    { id: '1.2', offset: 'This is textinside a spanMore text'.length + 2 },
    '1.2.4.1',
    2,
    function (item) {
      return item.name === 'p';
