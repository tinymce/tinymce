...
type GenerateApi = <T, R extends { finish: () => number }>(
  xs: T[],
  f: (x: T, offset: number) => Option<R>,
  start?: number
) => R[];
...
type SplitsApi = <T extends PRange>(
  parray: T[],
  positions: number[],
  subdivide: (unit: T, positions: number[]) => T[]
) => T[];
...
type SublistApi = <T extends PRange>(
  parray: T[],
  start: number,
  finish: number
) => T[];
