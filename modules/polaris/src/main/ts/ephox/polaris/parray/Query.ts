...
const tryend = function (parray: PRange[], finish: number) {
  const finishes =
    parray[parray.length - 1] && parray[parray.length - 1].finish() === finish;
 ...
 */
const sublist = function <T extends PRange>(
  parray: T[],
  start: number,
  finish: number
) {
  ...
  return first
    .bind(function (fIndex): Option<T[]> {
      const last = rawlast.getOr(tryend(parray, finish));
      return last > -1
        ? Option.some(parray.slice(fIndex, last))
        : Option.none();
    })
    .getOr([]);
};
