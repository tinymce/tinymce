...
const generateColorMenuItems = (editor: Editor, setting: string, defaultValues: Menu.ChoiceMenuItemSpec[]): Menu.ChoiceMenuItemSpec[] => {
  const colorMap = editor.getParam(setting);

  if (Type.isUndefined(colorMap)) {
...
const getColors = (editor: Editor): Menu.ChoiceMenuItemSpec[] => {
  return generateColorMenuItems(editor, 'color_map', defaultColors);
};
