...
type MenuItem =
  ...
  | Menu.SeparatorMenuItemApi;

const isSeparator = (item: MenuItem): boolean =>
  Type.isString(item) ? item === '|' : item.type === 'separator';
...
const addContextMenuGroup = (
  xs: Array<MenuItem>,
  groupItems: Array<MenuItem>
) => {
...
const generateContextMenu = (
  contextMenus: Record<string, Menu.ContextMenuApi>,
  menuConfig: string[],
  selectedElement: DomElement
) => {
  const sections = Arr.foldl(
    menuConfig,
    (acc, name) => {
...
const isNativeOverrideKeyEvent = (editor: Editor, e: PointerEvent) =>
  e.ctrlKey && !Settings.shouldNeverUseNative(editor);
...
export const setup = (
  ...
  const showContextMenu = (e) => {
    ...
    if (
      ...
      Settings.isContextMenuDisabled(editor)
    ) {
      return;
    }
    ...
    const initAndShow = isTouch()
      ? MobileContextMenu.initAndShow
      : DesktopContextMenu.initAndShow;
    initAndShow(
      editor,
      e,
      buildMenu,
