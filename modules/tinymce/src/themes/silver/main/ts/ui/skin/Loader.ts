...
const loadSkin = (isInline: boolean, editor: Editor) => {
  ...
  if (isSkinDisabled(editor) === false && skinUiCss) {
    ...
    DOMUtils.DOM.styleSheetLoader.loadInRoot(rootNode, skinUiCss, SkinLoaded.fireSkinLoaded(editor), SkinLoaded.fireSkinLoadError(editor, 'Skin could not be loaded'));
  } else {
