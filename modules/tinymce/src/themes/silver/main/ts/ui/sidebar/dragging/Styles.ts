...
import type { AllowedOverflow, Origin, CssPosition, CssSize, Position, Shift } from './DragTypes';
...
const getPosition = (position: CssPosition, allowedOverflow: AllowedOverflow, origin: Origin, size: Optional<CssSize>): PositioningStyles => {
  const { x, y } = size.fold(
...
const getPositioningStyles = (shift: Shift, position: CssPosition | Position, origin: Origin, allowedOverflow: AllowedOverflow, isDragging: boolean, declaredSize: Optional<CssSize>): PositioningStyles =>
  isDragging ?
