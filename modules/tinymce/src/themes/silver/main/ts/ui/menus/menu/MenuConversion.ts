...
const getFromExpandingItem = (item: Menu.NestedMenuItemApi, menuItems: MenuItemRegistry) => {
  ...
  const newMenus = Merger.deepMerge(rest.menus, Objects.wrap(item.value, rest.items));
  const newExpansions = Merger.deepMerge(rest.expansions, Objects.wrap(item.value, item.value));
