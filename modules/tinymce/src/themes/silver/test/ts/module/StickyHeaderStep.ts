...
const sTestStickyHeader = (
  ...
  return Step.label(
    'Test editor with toolbar_mode: ' + toolbarMode,
    Step.raw((_, next, die, logs) => {
      TinyLoader.setup(
        (editor: Editor, onSuccess, onFailure) => {
          ...
          Pipeline.async(
            ...
            [
              ...
              Step.label(
                'Checking startup structure',
                GeneralSteps.sequence([
                  StickyUtils.sAssertEditorContainer(
                    isToolbarTop,
                    StickyUtils.expectedInFullView
                  ),
                  StickyUtils.sAssertEditorClasses(false)
                ])
              ),
              Step.label(
                'Checking scroll event listeners are bound, scroll by 1px then assert',
                StickyUtils.sScrollAndAssertStructure(
                  isToolbarTop,
                  1,
                  StickyUtils.expectedScrollEventBound
                )
              ),
              Step.label(
                'Scroll to half the editor should have sticky css markings',
                GeneralSteps.sequence([
                  Step.stateful((_value, next, _die) =>
                    next(editor.getContentAreaContainer().clientHeight)
                  ),
                  StickyUtils.sScrollAndAssertStructure(
                    isToolbarTop,
                    200,
                    StickyUtils.expectedHalfView
                  ),
              ...
              Step.label(
                'Scroll down so the editor is hidden from view, it should have hidden css markings',
                StickyUtils.sScrollAndAssertStructure(
                  isToolbarTop,
                  500,
                  StickyUtils.expectedEditorHidden
                )
              ),
              StickyUtils.sAssertHeaderDocked(isToolbarTop),
              ...
              Step.label(
                'Scroll back should not have sticky',
                GeneralSteps.sequence([
                  StickyUtils.sScrollAndAssertStructure(
                    isToolbarTop,
                    -100,
              ...
              Step.label(
                'Open align menu and check sticky states',
                StickyUtils.sOpenMenuAndTestScrolling(
                  MenuUtils.sOpenAlignMenu('open align'),
                  1,
                  isToolbarTop
                )
              ),
              Step.label(
                'Open nested Formats menu Align and check sticky states',
                StickyUtils.sOpenMenuAndTestScrolling(
                  MenuUtils.sOpenNestedMenus([
                    {
              ...
              Step.label(
                'Open menubar Formats menu => Formats => Inline => check sticky states',
                StickyUtils.sOpenMenuAndTestScrolling(
                  MenuUtils.sOpenNestedMenus([
                    {
              ...
              Step.label(
                'Open text color palette => check sticky states',
                StickyUtils.sOpenMenuAndTestScrolling(
                  MenuUtils.sOpenNestedMenus([
                    {
              ...
              ...(toolbarMode === ToolbarMode.default
                ...
                : [
                    Step.label(
                      'Close the more drawer',
                      MenuUtils.sCloseMore(toolbarMode)
                    )
                  ]),

              Step.label(
                'Toggle fullscreen mode and ensure header moves from docked -> undocked -> docked',
                GeneralSteps.sequence([
                  StickyUtils.sScrollAndAssertStructure(
                    isToolbarTop,
                    200,
                  ...
                  UiFinder.sWaitForVisible(
                    'Wait for fullscreen to be activated',
                    Body.body(),
                    '.tox-fullscreen'
                  ),
                  StickyUtils.sAssertEditorContainer(
                  ...
                  tinyApis.sExecCommand('mceFullscreen'),
                  Waiter.sTryUntil(
                    'Wait for fullscreen to be deactivated',
                    UiFinder.sNotExists(Body.body(), '.tox-fullscreen')
                  ),
        ...
        },
        {
          ...
          toolbar:
            'align | fontsizeselect | fontselect | formatselect | styleselect | insertfile | forecolor | backcolor ',
          resize: 'both',
          min_height: 300,
          min_width: 300,
          height: 400,
