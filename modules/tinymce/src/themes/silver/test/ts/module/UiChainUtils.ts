...
const cCountNumber = (selector) =>
  Chain.fromChains([
    UiFinder.cFindAllIn(selector),
    Chain.mapper((ts) => ts.length)
  ]);

const cExtractOnlyOne = (selector) =>
  Chain.fromChains([
    UiFinder.cFindAllIn(selector),
    Chain.binder((ts) =>
      ts.length === 1
        ? Result.value(ts[0])
        : Result.error(
            'Did not find exactly 1 of ' + selector + '. Found: ' + ts.length
          )
    )
  ]);
...
const cResizeToPos = (
  ...
  const numMoves =
    sy === dy ? Math.abs(dx - sx) / delta : Math.abs(dy - sy) / delta;
  ...
  return Chain.control(
    Chain.fromChains(
      [UiFinder.cFindIn('.tox-blocker'), Mouse.cMouseMoveTo(sx, sy)]
        .concat(
        ...
        .concat([Mouse.cMouseMoveTo(dx, dy), Mouse.cMouseUp])
    ),
    Guard.addLogging(`Resizing from (${sx}, ${sy}) to (${dx}, ${dy})`)
  );
};

const cScrollRelativeEditor = (
  editor: Editor,
  relative: 'top' | 'bottom' = 'top',
  deltaY: number
) =>
  Chain.op(() => {
    const target = Element.fromDom(
      editor.inline ? editor.getBody() : editor.getContainer()
    );
    target.dom().scrollIntoView(relative === 'top');
    Scroll.to(0, window.pageYOffset + deltaY);
  });

export { cCountNumber, cExtractOnlyOne, cResizeToPos, cScrollRelativeEditor };
