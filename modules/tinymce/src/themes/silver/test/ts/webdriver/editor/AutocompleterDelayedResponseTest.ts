...
UnitTest.asynctest(
  'Editor Autocompleter delay response test',
  (success, failure) => {
    SilverTheme();
      ...
      (editor, onSuccess, onFailure) => {
        ...
        const sTestAutocompleter = (scenario: Scenario) => {
          const initialContent =
            scenario.initialContent || scenario.triggerChar;
        ...
        Pipeline.async(
          ...
          Logger.ts('Trigger autocompleter', [
            tinyApis.sFocus(),
            Logger.t(
              'Checking delayed autocomplete, (columns = auto), trigger: "$"',
              sTestDelayedResponseAutocomplete
            )
          ]),
          onSuccess,
          onFailure
