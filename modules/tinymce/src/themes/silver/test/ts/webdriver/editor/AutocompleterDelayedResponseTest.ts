    ...
    (editor, onSuccess, onFailure) => {
      ...
      const sTestAutocompleter = (scenario: Scenario) => {
        ...
        return GeneralSteps.sequence([
          ...
          RealKeys.sSendKeysOn('iframe => body => p', [RealKeys.text(initialContent)]),
          // Wait 50ms for the keypress handler to run
          Step.wait(50),
          ...(additionalContent ? [RealKeys.sSendKeysOn('iframe => body => p', [RealKeys.text(additionalContent)])] : []),
      ...
      const sTestDelayedResponseAutocomplete = sTestAutocompleter({
        ...
        choice: GeneralSteps.sequence([Keyboard.sKeydown(eDoc, Keys.enter(), {})]),
