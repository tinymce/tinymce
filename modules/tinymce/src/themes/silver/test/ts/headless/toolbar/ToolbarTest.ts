...
describe('headless.tinymce.themes.silver.toolbar.ToolbarTest', () => {
  ...
  const hook = TestHelpers.GuiSetup.bddSetup((store, _doc, _body) => GuiFactory.build(
    renderToolbar({
      ...
      initGroups: [
        {
          title: Optional.none(), items: Arr.map([ 'one', 'two', 'three' ], makeButton), label: Optional.none()
        },
        {
          title: Optional.some('group title'), items: Arr.map([ 'four', 'five' ], makeButton), label: Optional.none()
        },
        {
          title: Optional.some('another group title'), items: Arr.map([ 'six' ], makeButton), label: Optional.none()
        }
      ]
    })
  ));
  ...
  it('Check structure of toolbars', () => {
    ...
    it('Changing the toolbar contents and checking the keyboard navigation', async () => {
      ...
      const groups = Arr.map([
        {
          title: Optional.none<string>(), items: Arr.map([ 'A', 'B' ], makeButton), label: Optional.none()
        },
        {
          title: Optional.none<string>(), items: Arr.map([ 'C' ], makeButton), label: Optional.none()
        }
      ], renderToolbarGroup);
