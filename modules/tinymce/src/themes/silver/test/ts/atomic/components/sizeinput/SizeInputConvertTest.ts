...
UnitTest.test('SizeInputConvertTest', () => {
  const check = (expected: Option<number>, size: Size, unit: SizeUnit) => {
    ...
    assert.eq(true, expected.equals(result), 'Expected conversion of ' + JSON.stringify(size) + ' to ' + unit + ' = ' + result.toString());
  };
  ...
  Jsc.property('All units can convert to themselves', Jsc.number(0, largeSensible), Jsc.oneof(Jsc.elements(units)), function (
    value: number,
    unit: SizeUnit
  ) {
  ...
  Jsc.property(
    ...
    Jsc.oneof(Jsc.elements(convertableUnits)),
    Jsc.oneof(Jsc.elements(convertableUnits)),
    function (value: number, unit1: SizeUnit, unit2: SizeUnit) {
      const outValue = convertUnit(nuSize(value, unit1), unit2)
        .bind((unit2Value) => convertUnit(nuSize(unit2Value, unit2), unit1))
        .getOrNull();
  ...
  Jsc.property(
    ...
    Jsc.number(0, largeSensible),
    Jsc.oneof(Jsc.elements(nonConvertable)),
