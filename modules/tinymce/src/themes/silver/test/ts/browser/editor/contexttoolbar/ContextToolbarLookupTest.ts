...
UnitTest.asynctest(
  'Inline editor Context Toolbar Lookup test',
  (success, failure) => {
    SilverTheme();
      ...
      (editor, onSuccess, onFailure) => {
        ...
        Pipeline.async(
          ...
          [
            ...
            Log.stepsAsStep(
              'TINY-4571',
              'Context toolbar initial load lookups',
              [
            ...
            Log.stepsAsStep('TINY-4571', 'Context toolbar node scope lookup', [
              ...
              UiFinder.sWaitForVisible(
                'Waiting for node toolbar to appear',
                Body.body(),
                '.tox-tbtn:contains(Node)'
              ),
              sAssertNames(['a'], [], [])
            ]),
            ...
            Log.stepsAsStep(
              'TINY-4571',
              'Context toolbar parent node scope lookup',
              [
                tinyApis.sSetContent(
                ...
                UiFinder.sWaitForVisible(
                  'Waiting for parent node toolbar to appear',
                  Body.body(),
                  '.tox-tbtn:contains(Parent)'
                ),
                sAssertNames(
                  ['em', 'strong', 'p'],
                  ['em', 'strong', 'p'],
            ...
            Log.stepsAsStep(
              'TINY-4571',
              'Context toolbar editor scope lookup',
              [
                tinyApis.sSetContent(
                  '<p><span style="color: red">content</span></p>'
                ),
                sResetNames(),
                tinyApis.sSetCursor([0, 0, 0], 1),
                UiFinder.sWaitForVisible(
                  'Waiting for editor toolbar to appear',
                  Body.body(),
                  '.tox-tbtn:contains(Editor)'
                ),
            ...
            Log.stepsAsStep('TINY-4571', 'Context toolbar root node lookup', [
              ...
              Env.browser.isIE()
                ? sAssertNames(['div', 'div'], ['div', 'div'], ['div', 'div'])
                : sAssertNames(['div'], ['div'], ['div'])
            ]),

            Log.stepsAsStep(
              'TINY-4571',
              'Context toolbar click outside to inside',
              [
                tinyApis.sSetContent(
                  '<p><strong><em>bold italic</em></strong></p>'
                ),
          ...
          ed.ui.registry.addContextToolbar('test-parent-node-toolbar', {
            predicate: (node) => {
              ...
              return (
                childNode &&
                childNode.nodeName &&
                childNode.nodeName.toLowerCase() === 'strong'
              );
            },
            items: 'parentnode',
            scope: 'node'
