...
} from '../../../module/ToolbarUtils';

UnitTest.asynctest(
  'Inline Editor Floating Toolbar Drawer Position test',
  (success, failure) => {
    Theme();
    ...
    const sWithEditor = (
      ...
      getSteps: (editor: Editor, tinyApis: TinyApis) => Array<Step<any, any>>
    ) =>
      Chain.asStep({}, [
        McEditor.cFromSettings({
          ...
          toolbar:
            'undo redo | styleselect | bold italic underline | strikethrough superscript subscript | alignleft aligncenter alignright aligncenter | outdent indent | cut copy paste | selectall remove',
          ...
          Pipeline.async(
            ...
            [
              ...
              UiFinder.sWaitForVisible(
                ...
                Body.body(),
                '.tox-editor-header button[title="More..."]'
              ),
    ...
    const sTestToolbarTop = Step.label(
      'Test toolbar top positioning',
      sWithEditor({}, (editor, tinyApis) => {
        const tinyActions = TinyActions(editor);
        const tinyUi = TinyUi(editor);
        const initialContainerTop = Cell(getUiContainerTop(editor));

        const getExpectedToolbarPos = () =>
          initialContainerTop.get() + toolbarHeight; // top of ui container + toolbar height
    ...
    const sTestToolbarBottom = Step.label(
      'Test toolbar bottom positioning',
      sWithEditor({ toolbar_location: 'bottom' }, (editor, tinyApis) => {
        const tinyActions = TinyActions(editor);
        const tinyUi = TinyUi(editor);
        const initialContainerTop = Cell(getUiContainerTop(editor));

        const getExpectedToolbarPos = () =>
          initialContainerTop.get() - toolbarHeight * 2; // top of ui container - two toolbar heights
        ...
        return [
          ...
          sOpenFloatingToolbarAndAssertPosition(tinyUi, getExpectedToolbarPos, [
            ...
            Waiter.sTryUntil(
              ...
              Step.sync(() => {
                Assert.eq(
                  ...
                  true,
                  getUiContainerTop(editor) >=
                    initialContainerTop.get() + lineHeight * 3
                ); // Wait for the toolbar to move three lines
              })
            ),
            Step.sync(() => {
            ...
            sAssertFloatingToolbarPosition(
              ...
              105,
              465
            ) // top of ui container - two toolbar heights
          ])
        ];
      })
    );
    ...
    Pipeline.async(
      ...
      [
        Log.stepsAsStep(
          'TINY-4725',
          'Inline Editor Floating Toolbar Drawer Position test',
          [sTestToolbarTop, sTestToolbarBottom]
        )
      ],
      () => success(),
      failure
