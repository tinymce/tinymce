...
UnitTest.asynctest('Inline Editor Floating Toolbar Drawer Position test', (success, failure) => {
  ...
  const sPressEnterNTimes = (tinyApis: TinyApis, tinyActions: TinyActions, times: number) =>
    GeneralSteps.sequence([tinyApis.sFocus(), ...GeneralSteps.repeat(times, tinyActions.sContentKeydown(Keys.enter()))]);
  ...
  Pipeline.async(
    {},
    [Log.stepsAsStep('TINY-4725', 'Inline Editor Floating Toolbar Drawer Position test', [sTestToolbarTop, sTestToolbarBottom])],
    () => success(),
    failure
  );
});
