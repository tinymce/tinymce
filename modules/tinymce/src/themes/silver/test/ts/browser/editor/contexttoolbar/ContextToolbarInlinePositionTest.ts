...
UnitTest.asynctest(
  'Inline editor ContextToolbar Position test',
  (success, failure) => {
    SilverTheme();
    ...
    TinyLoader.setup(
      (editor, onSuccess, onFailure) => {
        ...
        const sScrollTo = (x: number, y: number) =>
          Step.sync(() => {
            const editorPos = Location.absolute(
          ...
          Waiter.sTryUntil(
            ...
            Step.sync(() => {
              UiFinder.findIn(Body.body(), '.tox-pop').each((ele) => {
                ...
                const dirStyle = parseInt(
                  Css.getRaw(ele, direction).getOr('0').replace('px', ''),
                  10
                );
                Assertions.assertEq(
                  'Assert toolbar positioning',
                  position,
                  posStyle
                );
                Assertions.assertEq(
                  `Assert toolbar position - ${direction} ${dirStyle}px ~= ${value}px`,
                  true,
                  Math.abs(dirStyle - value) <= diff
                );
              });
            }),
            10,
        ...
        const sTestPositionWhileScrolling = (scenario: Scenario) =>
          Log.stepsAsStep('TBA', scenario.label, [
            tinyApis.sSetContent(
              `<p style="height: 100px"></p><p style="height: 25px;${
                scenario.contentStyles || ''
              }">${scenario.content}</p><p style="height: 100px"></p>`
            ),
            sScrollTo(0, -250),
            tinyApis.sSetCursor(
        ...
        Pipeline.async(
          ...
          [
            ...
            Log.stepsAsStep('TBA', 'Context toolbar position', [
              ...
              UiFinder.sWaitForVisible(
                'Waiting for toolbar to appear below the content area container',
                Body.body(),
                '.tox-pop.tox-pop--top.tox-pop--align-left'
              ),
