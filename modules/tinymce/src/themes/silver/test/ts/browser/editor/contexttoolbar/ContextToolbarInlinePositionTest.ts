...
UnitTest.asynctest('Inline editor ContextToolbar Position test', (success, failure) => {
  ...
  TinyLoader.setup(
    (editor, onSuccess, onFailure) => {
      ...
      const sAssertPosition = (position: string, direction: string, value: number, diff = 5) =>
        Waiter.sTryUntil(
      ...
      const sTestPositionWhileScrolling = (scenario: Scenario) =>
        Log.stepsAsStep('TBA', scenario.label, [
          tinyApis.sSetContent(
            `<p style="height: 100px"></p><p style="height: 25px;${scenario.contentStyles || ''}">${
              scenario.content
            }</p><p style="height: 100px"></p>`
          ),
          sScrollTo(0, -250),
