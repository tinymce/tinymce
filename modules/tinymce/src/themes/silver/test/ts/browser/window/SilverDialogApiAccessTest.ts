...
UnitTest.asynctest('WindowManager:simple-dialog access Test', (success, failure) => {
  ...
  TinyLoader.setupLight(
    (editor, onSuccess, onFailure) => {
      ...
      const testDialog = (label: string, params: { inline?: string }) =>
        Logger.t(
          `Testing ${label}`,
          GeneralSteps.sequence([
            store.sClear,
            Logger.t('Open a dialog, with a change button that accesses API asynchronously', sTestOpen(params)),

            Logger.t('Trigger the async functions by clicking on the button', Mouse.sClickOn(Body.body(), 'button:contains("Call")')),

            Logger.t('Click on the close button, so that dialog is shut down', DialogUtils.sClose),

            Waiter.sTryUntil(
              'Wait until getData and setData calls fire asynchronously',
              store.sAssertEq('Checking stuff', ['onCancel', 'onClose', 'currentData: Init Value', 'newData: Init Value'])
            )
          ])
        );

      Pipeline.async({}, [testDialog('Modal', {}), testDialog('Inline', { inline: 'toolbar' })], onSuccess, onFailure);
    },
    {
      theme: 'silver',
