...
UnitTest.asynctest('Editor ContextForm test', (success, failure) => {
  ...
  TinyLoader.setupLight(
    (editor, onSuccess, onFailure) => {
      ...
      const sFire = (event: string, object) =>
        Step.sync(() => {
          editor.fire(event, object);
        });

      const sHasDialog = (label: string) =>
        Chain.asStep(Body.body(), [
      ...
      Pipeline.async(
        ...
        [
          ...
          Log.step(
            'TBA',
            'Immediately launching a context form, and navigating and triggering enter and esc',
          ...
          skipInIE(
            Log.step(
              'TBA',
              'Launch a context form from a context toolbar',
              GeneralSteps.sequence([
