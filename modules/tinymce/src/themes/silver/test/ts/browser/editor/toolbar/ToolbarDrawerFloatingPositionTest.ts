...
UnitTest.asynctest(
  'Editor Floating Toolbar Drawer Position test',
  (success, failure) => {
    Theme();
    const sScrollTo = (x: number, y: number) =>
      Step.sync(() => Scroll.to(x, y));
              ...
              Pipeline.async(
                ...
                [
                  ...
                  sOpenFloatingToolbarAndAssertPosition(
                    tinyUi,
                    () => initialContainerPos.top() + 39
                  ), // top of ui container + toolbar height
                  Step.sync(() => {
                  ...
                  sOpenFloatingToolbarAndAssertPosition(
                    tinyUi,
                    () => initialContainerPos.top() + 39 + 2000
                  ) // top of ui container + toolbar height + scroll pos
                ],
                () => onSuccess(editor),
                onFailure
              );
