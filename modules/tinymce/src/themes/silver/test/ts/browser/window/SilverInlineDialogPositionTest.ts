...
UnitTest.asynctest(
  'WindowManager:inline-dialog Position Test',
  (success, failure) => {
    Theme();
    ...
    const cAssertPos = (pos: string, x: number, y: number) =>
      Chain.control(
        Chain.op((dialog: Element<HTMLElement>) => {
          ...
          Assertions.assertEq(
            `Dialog position (${position}) should be ${pos}`,
            pos,
            position
          );
          Assertions.assertEq(
            `Dialog top position (${top}px) should be ~${y}px`,
            true,
            Math.abs(top - y) < diff
          );
          Assertions.assertEq(
            `Dialog left position (${left}px) should be ~${x}px`,
            true,
            Math.abs(left - x) < diff
          );
        }),
        Guard.tryUntil('Wait for dialog position to update')
      );

    const cScrollTo = (x: number, y: number) =>
      Chain.control(
        Chain.op(() => Scroll.to(x, y)),
        Guard.addLogging(`Scrolling to (${x}, ${y})`)
      );

    const createTestOpenChain = (editor: Editor) =>
      Chain.fromChains([
        DialogUtils.cOpen(editor, dialogSpec, { inline: 'toolbar' }),
        Chain.injectThunked(() => {
          const dialog = UiFinder.findIn(
            Body.body(),
            '.tox-dialog-inline'
          ).getOrDie();
            ...
            Chain.asStep(Body.body(), [
              Chain.label(
                ...
                NamedChain.asChain([
                  ...
                  NamedChain.direct(
                    'body',
                    cResizeToPos(600, 400, 500, 300),
                    '_'
                  ),
                  NamedChain.direct(
                    'dialog',
                    cAssertPos('absolute', 5, -171),
                    '_'
                  ), // Toolbar wraps so y diff is 100 + toolbar height

                  // Enlarge the editor to 500px
                  NamedChain.direct('resizeHandle', Mouse.cMouseDown, '_'),
                  NamedChain.direct(
                    'body',
                    cResizeToPos(500, 300, 700, 500),
                    '_'
                  ),
                  NamedChain.direct(
                    'dialog',
                    cAssertPos('absolute', 205, -410),
                    '_'
                  ),
              ...
              Chain.label(
                'Test initial position when initially scrolled',
                NamedChain.asChain([
                  NamedChain.direct(
                    NamedChain.inputName(),
                    Chain.identity,
      ...
      Chain.async((editor: Editor, onSuccess, onFailure) => {
        ...
        Pipeline.async(
          ...
          [
            Chain.asStep(Body.body(), [
              Chain.label(
                'Position of dialog should be constant when toolbar bottom docks',
                NamedChain.asChain([
                  NamedChain.direct(
                    NamedChain.inputName(),
                    Chain.identity,
              ...
              Chain.label(
                ...
                NamedChain.asChain([
                  ...
                  NamedChain.direct(
                    'body',
                    cResizeToPos(600, 400, 600, 300),
                    '_'
                  ),
                  NamedChain.direct(
                    'dialog',
                    cAssertPos('absolute', 105, -1287),
                    '_'
                  ),
    ...
    Pipeline.async(
      {},
      [sToolbarTopStep, sToolbarBottomStep, sToolbarBottomInlineStep],
      success,
      failure
    );
  }
