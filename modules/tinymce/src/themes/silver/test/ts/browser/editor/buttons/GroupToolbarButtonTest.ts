...
UnitTest.asynctest('GroupToolbarButtonTest', (success, failure) => {
  ...
  const sTestToolbarGroup = (
    settings: Record<string, any>,
    buttonSelector: string,
    toolbarSelector: string,
    expectedStruct: StructAssert
  ) =>
    Chain.asStep({}, [
  ...
  Pipeline.async(
    ...
    [
      ...
      Log.step(
        'TINY-4229',
        'Register floating group toolbar button via editor API',
        sTestToolbarGroup(
          {
            toolbar: 'alignment',
            setup: (editor) => {
      ...
      Log.step(
        'TINY-4616',
        'Group toolbars are ignored when using wrap toolbar mode',
        Chain.asStep({}, [
          cTestWithEditor(
            {
              ...defaultToolbarGroupSettings,
