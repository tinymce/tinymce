...
UnitTest.asynctest(
  'ToolbarBottomTest - assert direction that menus open in when toolbar_location: "bottom"',
  (success, failure) => {
    Theme();
      ...
      return Log.chainsAsStep('', scenario.message, [
        NamedChain.asChain([
          ...
          NamedChain.direct(
            'body',
            Chain.fromChains([
              UiFinder.cFindIn(scenario.assertAbove),
              Chain.mapper(Boxes.box)
            ]),
            'upperBoxBounds'
          ),
          NamedChain.direct(
            'body',
            Chain.fromChains([
              UiFinder.cFindIn(scenario.assertBelow),
              Chain.mapper(Boxes.box)
            ]),
            'lowerBoxBounds'
          ),
      ...
      [
        ...
        sScenarioAsStep({
          ...
          settings: {
            ...
            toolbar_mode: 'floating',
            toolbar:
              Arr.range(10, Fun.constant('bold | italic ')).join('') + 'align'
          },
          initial: [
            {
              clickOn: 'button[title="More..."]',
        ...
        sScenarioAsStep({
          message:
            'Dropdown menu used in a dialog (i.e. not in the toolbar) should open downwards',
          settings: {
            setup: (editor) => {
              editor.on('init', () => {
