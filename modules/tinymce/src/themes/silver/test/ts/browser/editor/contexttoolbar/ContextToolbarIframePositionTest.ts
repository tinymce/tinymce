...
UnitTest.asynctest(
  'IFrame editor ContextToolbar Position test',
  (success, failure) => {
    FullscreenPlugin();
    SilverTheme();
    ...
    TinyLoader.setup(
      (editor, onSuccess, onFailure) => {
        ...
        const sScrollTo = (x: number, y: number) =>
          Step.sync(() => Scroll.to(x, y, Element.fromDom(editor.getDoc())));

        const sAssertPosition = (position: string, value: number, diff = 5) =>
          Waiter.sTryUntil(
            'Wait for toolbar to be positioned',
            Step.sync(() => {
              UiFinder.findIn(Body.body(), '.tox-pop').each((ele) => {
                const styles = parseInt(
                  Css.getRaw(ele, position).getOr('0').replace('px', ''),
                  10
                );
                Assertions.assertEq(
                  `Assert toolbar position - ${position} ${styles}px ~= ${value}px`,
                  true,
                  Math.abs(styles - value) <= diff
                );
              });
            }),
            10,
        ...
        const sAssertFullscreenPosition = (
          position: string,
          value: number,
          diff = 5
        ) =>
          Waiter.sTryUntil(
            ...
            Step.sync(() => {
              UiFinder.findIn(Body.body(), '.tox-pop').each((ele) => {
                ...
                Assertions.assertEq(
                  `Assert toolbar position - ${position} ${pos[position]}px ~= ${value}px`,
                  true,
                  Math.abs(pos[position] - value) <= diff
                );
              });
            }),
            10,
        ...
        const sWaitForToolbarHidden = UiFinder.sWaitForHidden(
          'Waiting for toolbar to be hidden',
          Body.body(),
          '.tox-pop'
        );

        const sTestPositionWhileScrolling = (scenario: Scenario) =>
          Log.stepsAsStep('TBA', scenario.label, [
            tinyApis.sSetContent(
              '<p style="height: 100px"></p>' +
                '<p style="height: 100px"></p>' +
                '<p style="height: 100px"></p>' +
                `<p style="height: 25px;${scenario.contentStyles || ''}">${
                  scenario.content
                }</p>` +
                '<p style="height: 100px"></p>' +
                '<p style="height: 100px"></p>' +
            ...
            sScrollTo(0, 200),
            tinyApis.sSetCursor(
              scenario.cursor.elementPath,
              scenario.cursor.offset
            ),
            UiFinder.sWaitForVisible(
              'Waiting for toolbar to appear above content',
              Body.body(),
              '.tox-pop.tox-pop--bottom' + scenario.classes
            ),
            ...
            sScrollTo(0, 300),
            UiFinder.sWaitForVisible(
              'Waiting for toolbar to appear below content',
              Body.body(),
              '.tox-pop.tox-pop--top' + scenario.classes
            ),
        ...
        Pipeline.async(
          ...
          [
            ...
            Log.stepsAsStep(
              'TBA',
              'Context toolbar selection position while scrolling',
              [
                // north/south
                sTestPositionWhileScrolling({
                  label: 'north to south to hidden',
                  content:
                    'Lorem ipsum dolor sit amet, consectetur adipiscing elit <a href="http://tiny.cloud">link</a>',
                  cursor: {
                    elementPath: [3, 1, 0],
                    offset: 1
                  },
                ...
                sTestPositionWhileScrolling({
                  ...
                  content:
                    '<a href="http://tiny.cloud">link</a> Lorem ipsum dolor sit amet, consectetur adipiscing elit',
                  cursor: {
                    elementPath: [3, 0, 0],
                    offset: 1
                  },
            ...
            Log.stepsAsStep(
              'TBA',
              'Context toolbar falls back to positioning inside the content',
              [
                tinyApis.sSetContent(
                  `<p><img src="${imageSrc}" style="height: 380px; width: 100px"></p>`
                ),
                tinyApis.sSelect('img', []),
                UiFinder.sWaitForVisible(
                  'Waiting for toolbar to appear to top inside content',
                  Body.body(),
                  '.tox-pop.tox-pop--top'
                ),
            ...
            Log.stepsAsStep(
              'TINY-4586',
              `Line context toolbar remains inside iframe container and doesn't overlap the header`,
              [
                tinyApis.sSetContent(
            ...
            Log.stepsAsStep(
              'TINY-4023',
              'Context toolbar is visible in fullscreen mode',
              [
                ...
                UiFinder.sWaitForVisible(
                  'Check toolbar is still visible',
                  Body.body(),
                  '.tox-pop.tox-pop--top'
                ),
                tinyUi.sClickOnToolbar(
                ...
                Waiter.sTryUntil(
                  'Wait for fullscreen to turn off',
                  UiFinder.sNotExists(Body.body(), fullscreenSelector)
                )
              ]
            ),
            ...
            Log.stepsAsStep(
              'TBA',
              'Context toolbar should hide when scrolled out of view',
              [
                Step.sync(() => {
                ...
                tinyApis.sSetContent(
                  '<p><a href="http://tiny.cloud">link</a></p>'
                ),
      ...
      },
      {
        ...
        setup: (ed: Editor) => {
          ...
          ed.ui.registry.addContextToolbar('test-node-toolbar', {
            predicate: (node) =>
              node.nodeName && node.nodeName.toLowerCase() === 'img',
            items: 'alpha',
            position: 'node'
          });
          ed.ui.registry.addContextToolbar('test-line-toolbar', {
            predicate: (node) =>
              node.nodeName && node.nodeName.toLowerCase() === 'div',
            items: 'alpha',
            position: 'line'
          });
        }
