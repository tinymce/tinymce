...
UnitTest.asynctest('IFrame editor ContextToolbar Position test', (success, failure) => {
  ...
  TinyLoader.setup(
    (editor, onSuccess, onFailure) => {
      ...
      const sAssertFullscreenPosition = (position: string, value: number, diff = 5) =>
        Waiter.sTryUntil(
          'Wait for toolbar to be positioned',
          Step.sync(() => {
            UiFinder.findIn(Body.body(), '.tox-pop').each((ele) => {
      ...
      Pipeline.async(
        ...
        [
          ...
          Log.stepsAsStep('TINY-4586', `Line context toolbar remains inside iframe container and doesn't overlap the header`, [
            tinyApis.sSetContent('<p style="height: 400px"></p>' + '<div style="height: 25px;"></div>' + '<p style="height: 400px"></p>'),
            sScrollTo(0, 225),
