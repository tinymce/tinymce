...
UnitTest.asynctest('Inline Editor Toolbar Position test', (success, failure) => {
  ...
  const sAssertAbsolutePos = (container: Element, contentArea: Element, position: 'above' | 'below') =>
    Waiter.sTryUntil(
      'Wait for toolbar to be absolute',
      Step.sync(() => {
        ...
        const assertTop = position === 'above' ? containerAreaBounds.y - container.dom().clientHeight : containerAreaBounds.bottom;
  ...
  const sScrollToElement = (contentAreaContainer: Element, selector: string, alignWindowBottom = false) =>
    Step.sync(() => {
      const elm = UiFinder.findIn(contentAreaContainer, selector).getOrDie();
      elm.dom().scrollIntoView(alignWindowBottom);
    });

  const sScrollToElementAndActivate = (tinyApis: TinyApis, element: Element, selector: string, alignWindowBottom = false) =>
    Step.label(
      'Activate editor',
  ...
  const cTest = (getSteps: (data: Data) => Step<any, any>[]) =>
    Chain.runStepsOnValue((editor: Editor) => {
      ...
      editor.setContent(
        '<p>START CONTENT</p>' +
          Arr.range(98, (i) => (i === 49 ? '<p>STOP AND CLICK HERE</p>' : '<p>Some content...</p>')).join('\n') +
          '<p>END CONTENT</p>'
      );
  ...
  Pipeline.async(
    ...
    [
      Log.chainsAsStep('', 'Test inline toolbar position with toolbar_location: "top"', [
        ...
        cTest(({ editor, tinyApis, header, container, contentAreaContainer }) => [
