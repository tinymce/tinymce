...
UnitTest.asynctest(
  'tinymce.themes.silver.test.browser.sidebar.SidebarTest',
      ...
      function (editor, onSuccess, onFailure) {
        ...
        const sClickAndAssertEvents = function (tooltip, expected: EventLog[]) {
          return GeneralSteps.sequence([
            ...
            Waiter.sTryUntil(
              'Checking sidebar callbacks',
              store.sAssertEq('Asserting sidebar callbacks', expected)
            )
          ]);
        };
        ...
        Pipeline.async(
          editor,
          Log.steps('TBA', 'Sidebar actions test', [
            ...
            Waiter.sTryUntil(
              'Checking initial events',
              store.sAssertEq('Asserting intial render and hide of sidebar', [
            ...
            sClickAndAssertEvents('My sidebar 1', [
              { name: 'mysidebar1:show', index: 0 }
            ]),
            sClickAndAssertEvents('My sidebar 2', [
              { name: 'mysidebar1:hide', index: 0 },
              { name: 'mysidebar2:show', index: 1 }
            ]),
            sClickAndAssertEvents('My sidebar 3', [
              { name: 'mysidebar2:hide', index: 1 },
              { name: 'mysidebar3:show', index: 2 }
            ]),
            sClickAndAssertEvents('My sidebar 3', [
              { name: 'mysidebar3:hide', index: 2 }
            ])
      ...
      },
      {
        ...
        setup(editor) {
          const logEvent = (name: string) => (
            api: Sidebar.SidebarInstanceApi
          ) => {
            const index = Traverse.findIndex(
              Element.fromDom(api.element())
            ).getOr(-1);
            const entry: EventLog = { name, index };
            store.adder(entry)();
          };
          const handleSetup = (eventName: string) => (
            api: Sidebar.SidebarInstanceApi
          ) => {
            api
              .element()
