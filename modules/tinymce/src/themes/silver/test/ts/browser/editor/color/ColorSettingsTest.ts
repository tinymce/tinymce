...
UnitTest.asynctest('ColorSettingsTest', (success, failure) => {
  ...
  const sAssertColors = function (input, expected) {
    ...
    const assertColor = (
      ...
      assert.eq(
        ...
        'Color value should match (within ' +
          delta +
          '). Expected: ' +
          expectedColor +
          ' Actual: ' +
    ...
    return Logger.t(
      ...
      Step.sync(function () {
        ...
        for (let i = 0; i < expected.length; i++) {
          Assert.eq(
            'Color type should match',
            expected[i].type,
            colors[i].type
          );
          Assert.eq(
            'Color text should match',
            expected[i].text,
            colors[i].text
          );
  ...
  const colorSettings = [
    ...
    'Black',
    'PeachPuff',
    'Some horrible pink/orange color',
    'rgba(255, 99, 71, 0.5)',
    'Pale tomato'
  ];
