  ...
  TinyLoader.setup(
    (editor, onSuccess, onFailure) => {
      ...
      const sAssertFocusOnItem = (itemText: string) =>
        FocusTools.sTryOnSelector(`Focus should be on ${itemText}`, doc, `.tox-collection__item:contains("${itemText}")`);

      const sAssertFocusOnToolbarButton = (buttonText: string) =>
        FocusTools.sTryOnSelector(`Focus should be on ${buttonText}`, doc, `.tox-toolbar__group button:contains("${buttonText}")`);

      const sAssertFocusOnAlignToolbarButton = () =>
        FocusTools.sTryOnSelector('Focus should be on Align', doc, '.tox-toolbar__group button[aria-label="Align"]');

      const sAssertItemTicks = (label: string, expectedTicks: boolean[]) =>
        Logger.t(
      ...
      const sCheckItemsAtLocationPlus = (
        beforeStep: Step<any, any>,
        afterStep: Step<any, any>,
        sOpen: (text: string) => Step<any, any>
      ) => (label: string, expectedTicks: boolean[], menuText: string, path: number[], offset: number) =>
        Logger.t(
          label,
      ...
      const sCheckSubItemsAtLocation = (expectedSubmenu: string) =>
        sCheckItemsAtLocationPlus(
          GeneralSteps.sequence([Keyboard.sKeydown(doc, Keys.right(), {}), sAssertFocusOnItem(expectedSubmenu)]),
      ...
      const sTestAlignment = Log.stepsAsStep('TBA', 'Checking alignment ticks and updating', [
        ...
        sCheckAlignItemsAtLocation('First paragraph after "centering"', [false, true, false, false], 'Center', [0, 0], 'Fi'.length),

        sCheckAlignItemsAtLocation('Second paragraph with no set alignment', [false, false, false, false], 'Align', [1, 0], 'Se'.length),
      ...
      const sTestFontSizeSelect = Log.stepsAsStep('TBA', 'Checking fontsize ticks and updating', [
        ...
        sCheckItemsAtLocation('First paragraph after "8pt', [true].concat(range(6, () => false)), '8pt', [0, 0, 0], 'Fi'.length),

        sCheckItemsAtLocation(
          'Second paragraph with no set font size',
          range(2, () => false)
            .concat([true])
            .concat(range(4, () => false)),
          '12pt',
          [1, 0],
          'Se'.length
        ),
      ...
      const sTestFormatSelect = Log.stepsAsStep('TBA', 'Checking format ticks and updating', [
        ...
        sCheckItemsAtLocation('First block after "h1', [false, true].concat(range(6, () => false)), 'Heading 1:first', [0, 0], 'Fi'.length),
        ...
        sAssertFocusOnItem('Paragraph'),
        sAssertItemTicks('Checking blocks in menu', [false, true].concat(range(6, () => false))),
        Keyboard.sKeydown(doc, Keys.escape(), {}),
        Keyboard.sKeydown(doc, Keys.escape(), {})
      ]);
      ...
      const sTestStyleSelect = Log.stepsAsStep('TBA', 'Checking style ticks and updating', [
        ...
        Keyboard.sKeydown(doc, Keys.right(), {}),
        sAssertFocusOnItem('Heading 1'),
        sAssertItemTicks('Checking headings in menu', [true].concat(range(5, () => false))),
        Keyboard.sKeydown(doc, Keys.escape(), {}),
        Keyboard.sKeydown(doc, Keys.escape(), {}),
        Keyboard.sKeydown(doc, Keys.escape(), {})
      ]);
      ...
      Pipeline.async(
        {},
        [sTestAlignment, sTestFontSelect, sTestFontSizeSelect, sTestFormatSelect, sTestStyleSelect, sTestToolbarKeyboardNav],
        onSuccess,
        onFailure
      );
    },
