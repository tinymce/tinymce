...
UnitTest.asynctest('NotificationManagerImpl test', (success, failure) => {
  ...
  const cAssertPosition = (prefix: string, x: number, y: number, diff: number = 5) =>
    Chain.op((notification: NotificationApi) => {
  ...
  const sNotificationTopStep = Chain.asStep({}, [
    ...
    Chain.async((editor: Editor, onSuccess, onFailure) => {
      const cAssertStructure = (label: string, type: string, message: string, progress?: number) =>
        Chain.op((notification: NotificationApi) => {
          Assertions.assertStructure(
            ...
            ApproxStructure.build((s, str, arr) =>
              s.element('div', {
                ...
                classes: [arr.has('tox-notification'), arr.has(`tox-notification--${type}`)],
                children: [
                  s.element('div', {
  ...
  Pipeline.async({}, [sNotificationTopStep, sNotificationBottomStep], success, failure);
});
