...
UnitTest.asynctest('WindowManager:configurations Test', (success, failure) => {
  ...
  const sSetupDialogWithoutDragging = (conf) =>
    Step.sync(() => {
      windowManager.open(conf, {}, Fun.noop);
    });

  const sSetupDialogWithDragging = (conf) =>
    Step.sync(() => {
  ...
  const sTestWrongBodyType = shouldFail(
    'The body type should return a useful error',
    {
    ...
    (err) => {
      ...
      Assertions.assertEq(
        'This should throw a configuration error: showing the exact failure',
        message[2],
        'The chosen schema: "foo" did not exist in branches: {'
      );
    }
  );

  const sTestMissingPanelItems = shouldFail(
    'body panel is missing items: []',
    {
    ...
    (err) => {
      ...
      Assertions.assertEq(
        'This should throw a configuration error: showing the exact failure',
        message[1],
        'Failed path: (dialog > body > branch: panel)'
      );
      Assertions.assertEq(
        'This should throw a configuration error: showing the exact failure',
        message[2],
        'Could not find valid *strict* value for "items" in {'
      );
    }
  );

  const sTestMinRequiredConfigWithDragging = createTest(
    'The smallest config to get draggable dialog working, it should have this DOM structure',
    {
    ...
    (rootElement: SugarElement) => {
      Assertions.assertStructure(
        'A basic dialog should have these components',
  ...
  const sTestMinRequiredConfigWithoutDragging = createTest(
    'The smallest config to get non-draggable dialog working, it should have this DOM structure',
    {
