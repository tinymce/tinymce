...
UnitTest.asynctest('WindowManager:redial Test', (success, failure) => {
  ...
  Pipeline.async(
    ...
    [
      ...
      Logger.t('Button should be disabled', UiFinder.sNotExists(Body.body(), 'button:contains("Destination: DialogB"):not([disabled])')),

      Mouse.sClickOn(Body.body(), 'button:contains("Disable other")'),
      Mouse.sClickOn(Body.body(), 'button:contains("Destination: DialogB")'),

      Mouse.sClickOn(Body.body(), 'button:contains("Enable other")'),
      Logger.t('Button should be enabled', UiFinder.sExists(Body.body(), 'button:contains("Destination: DialogB"):not([disabled])')),
      Mouse.sClickOn(Body.body(), 'button:contains("Destination: DialogB")'),
      ...
      Logger.t('Tab "Two" should be selected', UiFinder.sExists(Body.body(), '.tox-dialog__body-nav-item--active:contains("two")')),

      sTestClose,
      Waiter.sTryUntil('Waiting for all dialog events when closing', store.sAssertEq('Checking stuff', ['closeWindow', 'onCloseC']))
    ],
    () => {
      helpers.destroy();
      success();
