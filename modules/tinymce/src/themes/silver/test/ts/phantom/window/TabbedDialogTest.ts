...
UnitTest.asynctest('WindowManager:tabbed-dialog Test', (success, failure) => {
  ...
  const cAssertFormContents = (
    label: string,
    f: (s, str, arr) => StructAssert
  ) =>
    Chain.op((tabview: Element) => {
      ...
      [
        ...
        Chain.asStep({}, [
          NamedChain.asChain([
            ...
            NamedChain.direct(
              'dialog',
              UiFinder.cFindIn('[role="tab"]:contains("Basic")'),
              'basicTab'
            ),
            NamedChain.direct(
              'dialog',
              UiFinder.cFindIn('[role="tab"]:contains("Advanced")'),
              'advancedTab'
            ),
            NamedChain.direct(
              'dialog',
              UiFinder.cFindIn('[role="tabpanel"]'),
              'tabview'
            ),
            NamedChain.direct(
              'dialog',
              Mouse.cClickOn('button:contains("-> Basic")'),
              '_'
            ),
            NamedChain.direct(
              'tabview',
              cAssertFormContents('Clicking Basic button', (s, str, _arr) =>
                s.element('input', {
            ...
            NamedChain.direct(
              'dialog',
              Mouse.cClickOn('button:contains("-> Advanced")'),
              '_'
            ),
            NamedChain.direct(
              'tabview',
              cAssertFormContents(
                'Clicking Advanced button (not tab)',
                (s, str, _arr) =>
                  s.element('textarea', {
            ...
            NamedChain.direct(
              'tabview',
              cAssertFormContents(
                'Clicking Basic button again (not tab)',
                (s, str, _arr) =>
                  s.element('input', {
                    value: str.is('First tab value')
                  })
