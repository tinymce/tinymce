  ...
  Pipeline.async({ }, [
    ...
    Chain.asStep({ }, [
      NamedChain.asChain([
        ...
        NamedChain.direct('tabview', cAssertFormContents('Clicking Basic button', (s, str, arr) => s.element('input', {
          value: str.is('First tab value')
        })), '_'),

        NamedChain.direct('dialog', Mouse.cClickOn('button:contains("-> Advanced")'), '_'),
        NamedChain.direct('tabview', cAssertFormContents('Clicking Advanced button (not tab)', (s, str, arr) => s.element('textarea', {
          value: str.is('Textarea value')
        })), '_'),

        NamedChain.direct('dialog', Mouse.cClickOn('button:contains("-> Basic")'), '_'),
        NamedChain.direct('tabview', cAssertFormContents('Clicking Basic button again (not tab)', (s, str, arr) => s.element('input', {
          value: str.is('First tab value')
        })), '_'),
      ])
    ])
