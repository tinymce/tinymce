...
UnitTest.asynctest(
  ...
  (success, failure) => {
    ...
    realm.setToolbarGroups([
      {
        label: 'group1',
        items: [LinkButton.sketch(realm, tEditor.editor())]
      }
    ]);
    ...
    const sAssertNavigation = function (label, prevEnabled, nextEnabled) {
      return Logger.t(
        ...
        Step.sync(function () {
          ...
          const prev = Traverse.parent(active)
            .bind(Traverse.prevSibling)
            .getOrDie('Could not find button to left');
          const next = Traverse.parent(active)
            .bind(Traverse.nextSibling)
            .getOrDie('Could not find button to right');
            ...
            Assertions.assertStructure(
              ...
              ApproxStructure.build(function (s, str, arr) {
                return s.element('span', {
                  ...
                  classes: [
                    (expected ? arr.not : arr.has)(
                      'tinymce-mobile-toolbar-navigation-disabled'
                    )
                  ]
                });
              }),
    ...
    const sAssertUrlFocused = GeneralSteps.sequence([
      FocusTools.sTryOnSelector(
        'Focus should be on input with link URL',
        doc,
    ...
    const sAssertTextFocused = GeneralSteps.sequence([
      FocusTools.sTryOnSelector(
        'Focus should be on input with link text',
        doc,
    ...
    const sAssertTitleFocused = GeneralSteps.sequence([
      FocusTools.sTryOnSelector(
        'Focus should be on input with link title',
        doc,
    ...
    const sAssertTargetFocused = GeneralSteps.sequence([
      FocusTools.sTryOnSelector(
        'Focus should be on input with link target',
        doc,
    ...
    const sTestNavigation = GeneralSteps.sequence([
      ...
      Step.wait(100),
      Logger.t(
        'Checking pressing tab at the end should not move focus',
        sAssertTargetFocused
      ),
    ...
    Pipeline.async(
      ...
      detection.browser.isChrome()
        ? [
            ...
            sAssertNavigation(
              'Checking initial navigation on text node',
              false,
              true
            ),
            ...
            sTestScenario({
              label:
                'Testing hitting ENTER after filling in URL and title (not text)',
              fields: {
                url: 'http://fake-url',
                title: 'Title'
              },
            ...
            sTestScenario({
              label:
                'Testing hitting ENTER after filling in URL, text, and title',
              fields: {
                url: 'http://fake-url',
                text: 'LinkText',
                title: 'Title'
            ...
            sTestScenario({
              label:
                'Testing hitting ENTER after filling in URL, text, title, and target',
              fields: {
                url: 'http://fake-url',
                text: 'LinkText',
                title: 'Title',
            ...
            sTestScenario({
              label:
                'Testing hitting ENTER after filling in URL with initial text selection',
            ...
            sTestScenario({
              label:
                'Testing hitting ENTER after filling in nothing with an existing link with url',
              node: Element.fromHtml(
                '<a href="http://prepared-url">Prepared</a>'
              ),
              fields: {},
              expected: [],
              mutations(node) {
                return Assertions.sAssertStructure(
                  'Checking mutated structure',
            ...
            sTestScenario({
              label:
                'Testing hitting ENTER after filling in URL with an existing link with url (and text content did not match URL previously)',
              node: Element.fromHtml(
                '<a href="http://prepared-url">Prepared</a>'
              ),
              fields: {
            ...
            sTestScenario({
              label:
                'Testing hitting ENTER after filling in URL with an existing link with url (and text content matched URL previously)',
              node: Element.fromHtml(
                '<a href="http://prepared-url">http://prepared-url</a>'
              ),
              fields: {
                url: 'http://new-url'
              },
            ...
            sTestScenario({
              label:
                'Testing hitting ENTER after filling in URL and text with an existing link with url',
              node: Element.fromHtml(
                '<a href="http://prepared-url">any text</a>'
              ),
              fields: {
                url: 'http://new-url',
                text: 'new-text'
