...
UnitTest.asynctest(
  'browser.tinymce.themes.mobile.EditorRemoveTest',
  (success, failure) => {
    ...
    Pipeline.async(
      ...
      [
        Chain.asStep({}, [
          ...
          Chain.op((editor) => {
            const wrapperElm = Element.fromHtml(
              '<div class="tinymce-editor"></div>'
            );
            Selectors.one('#' + editor.id).each((textareaElm) => {
              Insert.wrap(textareaElm, wrapperElm);
            });
            Selectors.one('.tinymce-mobile-outer-container').each(
              (editorElm) => {
          ...
          NamedChain.asChain([
            ...
            NamedChain.direct(
              'body',
              UiFinder.cExists(`.${Styles.resolve('mask-tap-icon')}`),
              '_'
            ),
            NamedChain.direct(
              'body',
              UiChains.cClickOnUi(
                'Click the tap to edit button',
                `.${Styles.resolve('mask-tap-icon')}`
              ),
              '_'
            ),
            NamedChain.direct(
            ...
            NamedChain.direct(
              'body',
              UiChains.cWaitForUi(
                'Check for the action group',
                '[aria-label="the action group"]'
              ),
              '_'
            ),
            NamedChain.direct(
              'body',
              UiFinder.cNotExists('[aria-label="The read only mode group"]'),
              '_'
            ),
            NamedChain.direct(
              'body',
              UiFinder.cNotExists(`.${Styles.resolve('mask-edit-icon')}`),
              '_'
            ),
            NamedChain.direct(
              'body',
              UiChains.cClickOnUi(
                'Click back to Tap to Edit screen',
                `.${Styles.resolve('icon-back')}`
              ),
              '_'
            ),
            NamedChain.direct(
          ...
          Assertions.cAssertStructure(
            'Assert Thor overrides removed from body',
          ...
          Assertions.cAssertStructure(
            'Assert Thor overrides removed from editor div',
            ApproxStructure.build((s, str) =>
              s.element('div', {
                attrs: cleanedThorAttrsStruct(str),
                children: []
