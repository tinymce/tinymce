...
const forToolbar = (clazz: string, action: (c: AlloyComponent) => void, extraBehaviours, editor: Editor): SketchSpec =>
  Button.sketch({
    ...
    buttonBehaviours: Merger.deepMerge(Behaviour.derive([Unselecting.config({})]), extraBehaviours)
  });

export { forToolbar, forToolbarCommand, forToolbarStateAction, forToolbarStateCommand, getToolbarIconButton };
