...
UnitTest.asynctest('tinymce.plugins.paste.browser.ImagePasteTest', (success, failure) => {
  ...
  const base64ImgSrc2 = ['R0lGODlhAQABAPAAAP8REf///yH5BAAAAAAALAAAAAABAAEAAAICRAEAOw=='].join('');
  ...
  suite.asyncTest('TestCase-TBA: Paste: pasteImages should set unique id in blobcache', function (editor, done, die) {
    ...
    event = mockEvent('paste', [base64ToBlob(base64ImgSrc, 'image/gif'), base64ToBlob(base64ImgSrc2, 'image/gif')]);
    clipboard.pasteImageData(event, rng);

    waitForSelector(editor, 'img')
      .then(function () {
        waitFor(() => hasCachedItem('mceclip0') && hasCachedItem('mceclip1'))
          .then(() => {
