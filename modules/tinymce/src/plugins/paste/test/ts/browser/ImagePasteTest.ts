...
UnitTest.asynctest(
  ...
  (success, failure) => {
    ...
    suite.asyncTest(
      'TestCase-TBA: Paste: pasteImages should set unique id in blobcache',
      function (editor, done, die) {
        let rng, event;
        const clipboard = Clipboard(editor, Cell('html'));

        const hasCachedItem = (name) =>
          !!editor.editorUpload.blobCache.get(name);
        ...
        waitForSelector(editor, 'img')
          .then(function () {
            waitFor(
              () => hasCachedItem('mceclip0') && hasCachedItem('mceclip1')
            )
              .then(() => {
                const cachedBlob1 = editor.editorUpload.blobCache.get(
                  'mceclip0'
                );
                const cachedBlob2 = editor.editorUpload.blobCache.get(
                  'mceclip1'
                );
    ...
    suite.asyncTest('TestCase-TBA: Paste: dropImages', function (
      editor,
      done,
      die
    ) {
      ...
      event = mockEvent('drop', [base64ToBlob(base64ImgSrc, 'image/gif')]);
      clipboard.pasteImageData(event, rng);
      ...
      waitForSelector(editor, 'img')
        .then(function () {
          LegacyUnit.equal(
            editor.getContent(),
            '<p><img src="data:image/gif;base64,' + base64ImgSrc + '" />a</p>'
          );
          LegacyUnit.strictEqual(
            editor.dom.select('img')[0].src.indexOf('blob:'),
            0
          );
    ...
    suite.asyncTest('TestCase-TBA: Paste: pasteImages', function (
      editor,
      done,
      die
    ) {
      ...
      event = mockEvent('paste', [base64ToBlob(base64ImgSrc, 'image/gif')]);
      clipboard.pasteImageData(event, rng);
    ...
    suite.asyncTest(
      'TestCase-TBA: Paste: dropImages - images_dataimg_filter',
      function (editor, done, die) {
        let rng, event;
        const clipboard = Clipboard(editor, Cell('html'));
        ...
        editor.settings.images_dataimg_filter = function (img) {
          LegacyUnit.strictEqual(
            img.src,
            'data:image/gif;base64,' + base64ImgSrc
          );
          return false;
        };
        rng = setupContent(editor);
    ...
    suite.asyncTest(
      'TestCase-TBA: Paste: pasteImages - images_dataimg_filter',
      function (editor, done, die) {
        let rng, event;
        const clipboard = Clipboard(editor, Cell('html'));
    ...
    TinyLoader.setupLight(
      function (editor, onSuccess, onFailure) {
        Pipeline.async(
          ...
          appendTeardown(
            editor,
            Log.steps('TBA', 'Paste: Test image paste', suite.toSteps(editor))
          ),
          onSuccess,
          onFailure
        );
