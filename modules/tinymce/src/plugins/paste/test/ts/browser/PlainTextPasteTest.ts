...
UnitTest.asynctest('tinymce.plugins.paste.browser.PlainTextPaste', (success, failure) => {
  ...
  const cAssertClipboardPaste = function (expected, data) {
    ...
    Obj.each(data, function (data, label) {
      chains.push(
        ...
        Chain.control(cAssertEditorContent(label, expected), Guard.tryUntil('Wait for paste to succeed.')),
        cClearEditor()
      );
    });

    return Chain.control(Chain.fromChains(chains), Guard.addLogging(`Assert clipboard paste ${expected}`));
  };
  ...
  Pipeline.async(
    ...
    [
      Chain.asStep(
        {},
        Log.chains('TBA', 'Paste: Assert forced_root_block <p></p> is added to the pasted data', [
      ...
      Chain.asStep(
        {},
        Log.chains('TBA', 'Paste: Assert forced_root_block <p class="attr"></p> is added to the pasted data', [
          cCreateEditorFromSettings({
            plugins: 'paste',
            forced_root_block: 'p',
      ...
      Chain.asStep(
        {},
        Log.chains('TBA', 'Paste: Assert forced_root_block is not added to the pasted data', [
          cCreateEditorFromSettings({
            plugins: 'paste',
