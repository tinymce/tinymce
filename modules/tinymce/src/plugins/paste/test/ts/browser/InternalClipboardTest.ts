...
UnitTest.asynctest(
  ...
  (success, failure) => {
    ...
    const sResetProcessEvents = Logger.t(
    ...
    const sPasteDataTransferEvent = function (
      editor: Editor,
      data: Record<string, string>
    ) {
      return Logger.t(
        'Paste data transfer event',
        Step.sync(function () {
    ...
    const sAssertClipboardData = function (
      expectedHtml: string,
      expectedText: string
    ) {
      return Logger.t(
        `Assert clipboard data ${expectedHtml}, ${expectedText}`,
        Step.sync(function () {
          Assert.eq(
            'text/html data should match',
            expectedHtml,
            dataTransfer.getData('text/html')
          );
          Assert.eq(
            'text/plain data should match',
            expectedText,
    ...
    const sPaste = function (
      editor: Editor,
      tinyApis: TinyApis,
      startHtml: string,
      pasteData: Record<string, string>,
      spath: number[],
      soffset: number,
      fpath: number[],
      foffset: number
    ...
    const sTestCopy = function (editor: Editor, tinyApis: TinyApis) {
      return (
        ...
        Log.stepsAsStep(
          'TBA',
          'Paste: Copy partialy selected inline elements',
          [
            sCopy(
              editor,
              tinyApis,
        ...
        Log.stepsAsStep(
          'TBA',
          'Copy collapsed selection with table selection',
          [
            sCopy(
              editor,
              tinyApis,
    ...
    const sAssertLastPreProcessEvent = function (expectedData) {
      return Logger.t(
        'Assert last preprocess event',
        Step.sync(function () {
          Assert.eq(
            'Internal property should be equal',
            expectedData.internal,
            lastPreProcessEvent.internal
          );
          Assert.eq(
            'Content property should be equal',
            expectedData.content,
            lastPreProcessEvent.content
          );
        })
      );
    };
    ...
    const sAssertLastPostProcessEvent = function (expectedData) {
      return Logger.t(
        'Assert last postprocess event',
        Step.sync(function () {
          Assert.eq(
            'Internal property should be equal',
            expectedData.internal,
            lastPostProcessEvent.internal
          );
          Assert.eq(
            'Content property should be equal',
            expectedData.content,
            lastPostProcessEvent.node.innerHTML
          );
        })
      );
    };

    const sWaitForProcessEvents = Waiter.sTryUntil(
      'Did not get any events fired',
      Step.sync(function () {
        Assert.eq(
          'PastePreProcess event object',
          lastPreProcessEvent !== null,
          true
        );
        Assert.eq(
          'PastePostProcess event object',
          lastPostProcessEvent !== null,
          true
        );
      })
    ...
    const sTestPaste = function (editor: Editor, tinyApis: TinyApis) {
      return (
        ...
        Log.stepsAsStep(
          'TBA',
          'Paste: Paste external content treated as plain text',
          [
            sPaste(
              editor,
              tinyApis,
        ...
        Log.stepsAsStep('TBA', 'Paste: Paste internal content with mark', [
          sPaste(
            ...
            tinyApis,
            '<p>abc</p>',
            { 'text/plain': 'X', 'text/html': InternalHtml.mark('<p>X</p>') },
            [0, 0],
            0,
            [0, 0],
            3
