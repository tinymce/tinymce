...
import {
  getPasteBinParent,
  PasteBin
} from 'tinymce/plugins/paste/core/PasteBin';
      ...
      return Chain.control(
        Chain.op(function (editor: any) {
          ...
          Obj.each(cases, function (c, i) {
            getPasteBinParent(editor).appendChild(
              editor.dom.createFragment(c.content)
            );
            Assertions.assertEq(
              c.label || 'Asserting paste bin case ' + i,
              c.result,
              pasteBin.getHtml()
            );
            pasteBin.remove();
    ...
    Pipeline.async(
      ...
      [
        Chain.asStep(
          ...
          Log.chains(
            'TBA',
            'Paste: Create editor from settings and test nested and adjacent paste bins',
            [cCreateEditorFromSettings(), cAssertCases(cases), cRemoveEditor()]
          )
        ),
        ...
        Chain.asStep(
          ...
          Log.chains(
            'TBA',
            'Paste: Create editor from html and test nested and adjacent paste bins',
            [
