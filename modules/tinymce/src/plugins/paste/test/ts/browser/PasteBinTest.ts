...
UnitTest.asynctest('tinymce.plugins.paste.browser.PasteBin', (success, failure) => {
  ...
  const cCreateEditorFromHtml = function (html, settings) {
    return Chain.control(cCreateEditorFromSettings(settings, html), Guard.addLogging(`Create editor using ${html}`));
  };
  ...
  Pipeline.async(
    ...
    [
      Chain.asStep(
        {},
        Log.chains('TBA', 'Paste: Create editor from settings and test nested and adjacent paste bins', [
      ...
      Chain.asStep(
        {},
        Log.chains('TBA', 'Paste: Create editor from html and test nested and adjacent paste bins', [
