...
UnitTest.asynctest('tinymce.plugins.paste.webdriver.CutTest', (success, failure) => {
  ...
  TinyLoader.setup(
    function (editor, onSuccess, onFailure) {
      ...
      Pipeline.async(
        {},
        platform.browser.isIE() || platform.browser.isSafari() || platform.browser.isEdge()
          ? []
          : Log.steps('TBA', 'Paste: Set and select content, cut using edit menu and assert cut content', [
              ...
              Chain.asStep({}, [ui.cWaitForUi('Wait for menu item', '[role="menuitem"]:contains("Cut")'), RealMouse.cClick()]),
