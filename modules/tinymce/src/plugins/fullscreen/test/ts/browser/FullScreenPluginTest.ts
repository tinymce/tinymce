    ...
    Chain.fromChainsWith(Body.body(), [
      ...
      Chain.op((dialog) => {
        if (Attr.has(dialog, 'aria-labelledby')) {
          ...
          const dialogLabel = SelectorFind.descendant<HTMLLabelElement>(
            dialog,
            '#' + labelledby
          ).getOrDie('Could not find labelledby');
          Assertions.assertEq(
            'Checking label text',
            ariaLabel,
            Html.get(dialogLabel)
          );
        } else {
    ...
    TinyLoader.setupLight(
      function (editor, onSuccess, onFailure) {
        ...
        const cAssertEditorAndLastEvent = (label, state) =>
          Chain.control(
            Chain.fromChains([
              ...
              Chain.op(() =>
                Assertions.assertEq(
                  'FullscreenStateChanged event',
                  state,
                  lastEventArgs.get().state
                )
              )
            ]),
        ...
        const cAssertFullscreenClass = (label, shouldExist) => {
          const selector = shouldExist
            ? 'root:.tox-fullscreen'
            : 'root::not(.tox-fullscreen)';
          const label2 = `Body and Html should ${
            shouldExist ? '' : 'not '
          }have "tox-fullscreen" class`;
        ...
        const cCloseOnlyWindow = Chain.control(
          Chain.op(() => {
            const dialogs = () =>
              UiFinder.findAllIn(Body.body(), '[role="dialog"]');
        ...
        Chain.pipeline(
          Log.chains(
            'TBA',
            'FullScreen: Toggle fullscreen on, open link dialog, insert link, close dialog and toggle fullscreen off',
            [
