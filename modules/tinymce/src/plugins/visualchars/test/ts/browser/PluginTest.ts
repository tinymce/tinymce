    ...
    function (editor, onSuccess, onFailure) {
      ...
      Pipeline.async(
        ...
        [
          Log.stepsAsStep(
            'TBA',
            'VisualChars: Set content, click visual chars button and assert span char is present in whitespaces, click the button again and assert no span is present in the whitespace',
            [
          ...
          Log.stepsAsStep(
            'TINY-4507',
            'VisualChars: Set content with HTML like content, click visual chars button and assert span char is present in whitespaces, click the button again and assert no span is present in the whitespace',
            [
              tinyApis.sSetContent('<p>&lt;img src=&quot;image.png&quot;&gt;&nbsp;</p>'),
              tinyUi.sClickOnToolbar('click on visualchars button', 'button'),
              tinyApis.sAssertContentStructure(
                sAssertStruct(ApproxStructure.build((s, str) => [s.text(str.is('<img src="image.png">')), s.element('span', {})]))
              ),
              tinyUi.sClickOnToolbar('click on visualchars button', 'button'),
              tinyApis.sAssertContentStructure(
                sAssertStruct(ApproxStructure.build((s, str) => [s.text(str.is('<img src="image.png">')), s.text(str.is(Unicode.nbsp))]))
              )
            ]
          )
        ],
