...
UnitTest.asynctest(
  ...
  (success, failure) => {
    ...
    suite.test('TestCase-TBA: NonEditable: noneditable class', function (
      editor
    ) {
    ...
    suite.test('TestCase-TBA: NonEditable: noneditable regexp', function (
      editor
    ) {
    ...
    suite.test(
      'TestCase-TBA: NonEditable: noneditable regexp inside cE=false',
      function (editor) {
    ...
    TinyLoader.setupLight(
      function (editor, onSuccess, onFailure) {
        Pipeline.async(
          ...
          Log.steps(
            'TBA',
            'NonEditable: Test noneditable class and regexp',
