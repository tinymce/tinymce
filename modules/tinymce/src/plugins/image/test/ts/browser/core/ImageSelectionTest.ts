...
UnitTest.asynctest('browser.tinymce.plugins.image.core.ImageSelectionTest', (success, failure) => {
  ...
  const sWaitForDragHandles = (editor: any): Step<any, any> =>
    Waiter.sTryUntil('wait for draghandles', UiFinder.sExists(Element.fromDom(editor.getBody()), '#mceResizeHandlenw'), 10, 5000);
    ...
    function (editor, onSuccess, onFailure) {
      ...
      Pipeline.async(
        ...
        [
                ...
                ApproxStructure.build(function (s, str) {
                  return s.element('div', {
                    children: [
                      ...
                      s.element('div', {
                        attrs: { id: str.is('mceResizeHandlenw') }
                      }),
                      s.element('div', {
                        attrs: { id: str.is('mceResizeHandlene') }
                      }),
                      s.element('div', {
                        attrs: { id: str.is('mceResizeHandlese') }
                      }),
                      s.element('div', {
                        attrs: { id: str.is('mceResizeHandlesw') }
                      })
                    ]
                  });
                })
          ...
          Logger.t(
            'Update figure, new dimensions and src',
            GeneralSteps.sequence([
          ...
          Logger.t(
            'Update image, new dimensions and src',
            GeneralSteps.sequence([
              tinyApis.sSetContent('<p>' + '<img src="image.png" alt="" width="200" height="200">' + '</p>'),
