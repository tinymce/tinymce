...
UnitTest.asynctest(
  'browser.tinymce.plugins.image.core.ImageDataTest',
  (success, failure) => {
    ...
    const cReadFromImage = Chain.control(
      Chain.mapper(function (elm: Element) {
        const img =
          Node.name(elm) === 'img'
            ? elm
            : SelectorFind.descendant(elm, 'img').getOrDie(
                'failed to find image'
              );
        return {
          model: read(normalizeCss, img.dom()),
          image: img,
          parent: elm
        };
      }),
    ...
    const cAssertFigure = Chain.op(function (data: any) {
      Assert.eq(
        'Parent should be a figure',
        true,
        isFigure(data.image.dom().parentNode)
      );
    });
    ...
    Pipeline.async(
      ...
      [
        Log.step(
          'TBA',
          'Image: getStyleValue from image data',
          Step.sync(() => {
            Assert.eq(
              'Should not produce any styles',
              '',
              getStyleValue(normalizeCss, defaultData())
            );
            Assert.eq(
              'Should produce border width',
              'border-width: 1px;',
              getStyleValue(normalizeCss, { ...defaultData(), border: '1' })
            );
            Assert.eq(
              'Should produce style',
              'border-style: solid;',
              getStyleValue(normalizeCss, {
                ...defaultData(),
                borderStyle: 'solid'
              })
            );
            Assert.eq(
              'Should produce style & border',
        ...
        Log.chainsAsStep(
          'TBA',
          'Image: Create image with empty fields except src',
          [
            cCreate({
              src: 'some.gif',
              alt: '',
              ...
              return s.element('figure', {
                ...
                children: [
                  ...
                  s.element('figcaption', {
                    ...
                    children: [s.text(str.is('Caption'))]
                  })
                ]
              });
            })
        ...
        Chain.asStep(
          ...
          Log.chains(
            'TBA',
            'Image: Read/write model to simple image without change',
            [
        ...
        Chain.asStep(
          ...
          Log.chains(
            'TBA',
            'Image: Read/write model to complex image without change',
            [
              cSetHtml(
                '<img src="some.gif" class="class" width="100" height="200" style="margin: 1px 2px; border: 1px solid red" alt="alt" title="title">'
              ),
              cReadFromImage,
              cAssertModel({
                src: 'some.gif',
        ...
        Chain.asStep(
          ...
          Log.chains(
            'TBA',
            'Image: Read/write model to simple image with changes',
            [
              cSetHtml('<img src="some.gif">'),
              cReadFromImage,
              cUpdateModel({
        ...
        Chain.asStep(
          ...
          Log.chains(
            'TBA',
            'Image: Read/write model to complex image with changes',
            [
              cSetHtml(
                '<img src="some.gif" class="class" width="100" height="200" style="margin: 1px 2px; border: 1px solid red" alt="alt" title="title">'
              ),
        ...
        Chain.asStep(
          ...
          Log.chains(
            'TBA',
            'Image: write null as alt will remove the attribute',
            [
        ...
        Chain.asStep(
          Element.fromTag('div'),
          Log.chains('TBA', 'Image: Toggle caption on', [
            cSetHtml('<img src="some.gif">'),
            cReadFromImage,
            cUpdateModel({
              caption: true
        ...
        Chain.asStep(
          Element.fromTag('div'),
          Log.chains('TBA', 'Image: Toggle caption off', [
            cSetHtml(
              '<figure class="image" contenteditable="false"><img src="some.gif"><figcaption contenteditable="true">Caption</figcaption></figure>'
            ),
            cReadFromImage,
            cUpdateModel({
              caption: false
        ...
        Chain.asStep(
          Element.fromTag('div'),
          Log.chains('TBA', 'Image: Update figure image data', [
            cSetHtml(
              '<figure class="image" contenteditable="false"><img src="some.gif"><figcaption contenteditable="true">Caption</figcaption></figure>'
            ),
            cReadFromImage,
        ...
        Chain.asStep(
          ...
          Log.chains(
            'TBA',
            'Image: Read/write model to image with style size without change',
            [
              cSetHtml(
                '<img src="some.gif" style="width: 100px; height: 200px">'
              ),
              cReadFromImage,
              cAssertModel({
                src: 'some.gif',
        ...
        Chain.asStep(
          ...
          Log.chains(
            'TBA',
            'Image: Read/write model to image with style size with size change',
            [
              cSetHtml(
                '<img src="some.gif" style="width: 100px; height: 200px">'
              ),
