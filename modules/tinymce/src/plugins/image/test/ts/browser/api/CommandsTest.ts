...
UnitTest.asynctest(
  'browser.tinymce.plugins.image.api.CommandsTest',
  (success, failure) => {
    SilverTheme();
    Plugin();
    ...
    TinyLoader.setupLight(
      (editor, onSuccess, onFailure) => {
        ...
        const sUpdateImage = (data: Partial<ImageData>) =>
          api.sExecCommand('mceUpdateImage', data);
        ...
        Pipeline.async(
          ...
          [
            Log.stepsAsStep(
              'TBA',
              'Insert image with all data specified except caption and isDecorative',
              [
            ...
            Log.stepsAsStep(
              'TBA',
              'Update image with all data specified except caption and isDecorative',
              [
            ...
            Log.stepsAsStep(
              'TBA',
              'Update image with empty title, width, height should not produce empty attributes',
              [
                api.sSetContent(
                  '<p><img src="#1" title="title" width="100" height="200" /></p>'
                ),
                api.sSetSelection([0], 0, [0], 1),
                sUpdateImage({
                  title: '',
