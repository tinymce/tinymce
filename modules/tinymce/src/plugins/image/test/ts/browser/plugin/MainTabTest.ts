...
UnitTest.asynctest(
  'All image dialog ui options on empty editor' +
    (Env.ceFalse ? '' : ' (old IE)'),
  (success, failure) => {
    SilverTheme();
    ImagePlugin();
    Pipeline.async(
      ...
      [
        Log.chainsAsStep(
          'TBA',
          'Image: all image dialog ui options on empty editor' +
            (Env.ceFalse ? '' : ' (old IE)'),
          [
            ...
            cAssertCleanHtml(
              ...
              (() => {
                if (Env.ceFalse) {
                  ...
                  // old IE
                  return '<p><img class="class1" src="src" alt="alt" width="100" height="200" /></p>';
                }
              })()
            ),
            Editor.cRemove
