...
UnitTest.asynctest(
  'browser.tinymce.plugins.image.DescriptiveImageDialogTest',
  (success, failure) => {
    SilverTheme();
    Plugin();
      ...
      function (editor, onSuccess, onFailure) {
        ...
        Pipeline.async(
          ...
          [
            Log.stepsAsStep(
              'TBA',
              'Insert Image Dialog basic keyboard navigation cycle ',
              [
            ...
            Log.chainsAsStep(
              'TBA',
              'Image update with empty alt should remove the existing alt attribute',
              [
                Chain.inject(editor),
            ...
            Log.chainsAsStep(
              'TBA',
              'Image update with decorative toggled on should produce empty alt and role=presentation',
              [
                Chain.inject(editor),
                ApiChains.cSetContent('<p><img src="#1" alt="alt1" /></p>'),
                ApiChains.cSetSelection([0], 0, [0], 1),
                ...
                cAssertCleanHtml(
                  'Checking output',
                  '<p><img role="presentation" src="#1" alt="" /></p>'
                )
              ]
            ),
            Log.chainsAsStep(
              'TBA',
              'Image update with decorative toggled off should produce empty alt and role=presentation',
              [
                Chain.inject(editor),
