  ...
  (success, failure) => {
    ...
    TinyLoader.setupLight(
      (editor, onSuccess, onFailure) => {
          ...
          Logger.t(
            ...
            GeneralSteps.sequence([
              ...
              Chain.asStep({}, [
                ...
                Chain.op((container) => {
                  const expected = {};
                  expected['.tox-tab:contains("' + title + '")'] = isPresent
                    ? 1
                    : 0;
          ...
          Step.async((next, die) => {
            Conversions.uriToBlob(b64).then((blob) => {
              Pipeline.async(
                {},
                [
                  FileInput.sRunOnPatchedFileInput(
                    [Files.createFile('logo.png', 0, blob)],
                    Chain.asStep({}, [
        ...
        const uploadTabNotPresent = Log.stepsAsStep(
          'TBA',
          'Image: Upload tab should not be present without images_upload_url or images_upload_handler',
          [
        ...
        const uploadTabPresentOnUploadUrl = Log.stepsAsStep(
          'TBA',
          'Image: Upload tab should be present when images_upload_url is set to some truthy value',
          [
            api.sSetContent('<p><img src="' + src + '" /></p>'),
            api.sSelect('img', []),
        ...
        const uploadTabNotPresentOnUploadUrlWithUploadTabDisabled = Log.stepsAsStep(
          'TBA',
          'Image: Upload tab should be not be present when images_upload_url is set to some truthy value and image_uploadtab is set to false',
          [
            api.sSetContent('<p><img src="' + src + '" /></p>'),
            api.sSelect('img', []),
        ...
        const uploadTabPresentOnUploadHandler = Log.stepsAsStep(
          'TBA',
          'Image: Upload tab should be present when images_upload_handler is set to some truthy value',
          [
            api.sSetContent('<p><img src="' + src + '" /></p>'),
            api.sSelect('img', []),
            api.sSetSetting('image_advtab', false), // make sure that Advanced tab appears separately
        ...
        const sAssertSrcTextValue = (expectedValue: string) =>
          Waiter.sTryUntil(
            'Waited for input to change to expected value',
            Chain.asStep(Body.body(), [
              UiFinder.cFindIn(
                'label.tox-label:contains("Source") + div > div > input.tox-textfield'
              ),
              Chain.op((input) => {
                Assertions.assertEq(
                  'Assert field source value ',
                  expectedValue,
                  input.dom().value
                );
              })
        ...
        const sAssertSrcTextValueStartsWith = (expectedValue: string) =>
          Waiter.sTryUntil(
            'Waited for input to change to start with expected value',
            Chain.asStep(Body.body(), [
              UiFinder.cFindIn(
                'label.tox-label:contains("Source") + div > div > input.tox-textfield'
              ),
              Chain.op((input) => {
                Assertions.assertEq(
                  'Assert field source value ',
                  true,
                  Strings.startsWith(input.dom().value, expectedValue)
                );
              })
            ]),
            10,
        ...
        const uploadWithCustomRoute = Log.stepsAsStep(
          'TBA',
          'Image: Image uploader test with custom route',
          [
            api.sSetContent(''),
            ...
            ui.sClickOnUi(
              ...
              '.tox-tab:contains("Upload")'
            ),
            sTriggerUpload,
            ui.sWaitForUi(
              'Wait for General tab to activate',
              '.tox-tab:contains("General")'
            ),
        ...
        const uploadWithCustomHandler = Log.stepsAsStep(
          'TBA',
          'Image: Image uploader test with images_upload_handler',
          [
            api.sSetContent(''),
            api.sSetSetting('images_upload_handler', (blobInfo, success) =>
              success('file.jpg')
        ...
        const uploadCustomHandlerBase64String = Log.stepsAsStep(
          'TBA',
          'Image: Test that we get full base64 string in images_upload_handler',
          [
            api.sSetContent(''),
            api.sSetSetting('images_upload_handler', (blobInfo, success) =>
        ...
        const uploadWithAutomaticUploadsDisabled = Log.stepsAsStep(
          'TBA',
          'Image: Image uploader test with automatic uploads disabled',
          [
            api.sSetContent(''),
