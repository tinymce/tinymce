          ...
          Log.stepsAsStep('TBA', 'Image: image proportion constrains should work directly', [
            ...
            Chain.asStep({}, [
              Chain.fromParent(tinyUi.cWaitForPopup('Wait for dialog', 'div[role="dialog"]'), [
                Chain.fromChains([UiFinder.cFindIn('button.tox-browse-url'), Mouse.cClick]),
                Chain.control(cAssertInputValue(generalTabSelectors.width, '1'), Guard.tryUntil('did not find width input with value 1')),
                cSetInputValue(generalTabSelectors.height, '5'),
                Chain.control(cAssertInputValue(generalTabSelectors.width, '5'), Guard.tryUntil('did not find width input with value 5'))
              ]),
              tinyUi.cSubmitDialog(),
              Chain.inject(editor),
              cAssertCleanHtml(
                'Checking output',
                '<p><img src="data:image/gif;base64,R0lGODlhAQABAIAAAAAAAP///yH5BAEAAAAALAAAAAABAAEAAAIBRAA7" alt="" width="5" height="5" /></p>'
              )
            ])
          ])
        ],
