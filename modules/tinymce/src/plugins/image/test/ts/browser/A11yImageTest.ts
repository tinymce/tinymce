...
UnitTest.asynctest(
  'browser.tinymce.plugins.image.A11yImageTest',
  (success, failure) => {
    SilverTheme();
    Plugin();
    ...
    TinyLoader.setupLight(
      function (editor: Editor, onSuccess, onFailure) {
        ...
        const createTestOnContent = (
          name: string,
          data: Partial<ImageDialogData>,
          cursorPos: Record<string, number | Array<number>>,
          initialContent: string,
          expectedContent: string
        ) =>
          Log.stepsAsStep('TBA', 'Image: ' + name, [
            sInitAndOpenDialog(initialContent, cursorPos),
            Chain.asStep({}, [cFillActiveDialog(data, true)]),
            ui.sClickOnUi(
              'click save',
        ...
        const createTestOnEmptyEditor = (
          ...
          createTestOnContent(
            name,
            data,
            { elementPath: [0], startOffset: 0, endOffset: 0 },
            '',
            expectedContent
          );

        const testUiStateDisabled = Log.stepsAsStep(
          'FOAM-11',
          'Test image UI state',
          [
            api.sExecCommand('mceImage', true),
            ...
            UiFinder.sExists(
              Body.body(),
              generalTabSelectors.alt + ':disabled'
            ),
            ui.sClickOnUi(
              'click save',
              'div[role="dialog"] button:contains("Save")'
        ...
        const testUiStateEnabled = (alt: string) =>
          Log.stepsAsStep('FOAM-11', 'Test image UI state', [
            api.sExecCommand('mceImage', true),
            ui.sWaitForPopup('Wait for Image dialog', 'div[role="dialog"]'),
        ...
        const suiteArr = [
          Log.stepsAsStep(
            'TBA',
            'Check the decorative checkbox toggles the alt text input',
            [
              sInitAndOpenDialog('', { elementPath: [0], offset: 0 }),
              Chain.asStep({}, [
                Chain.inject(Body.body()),
                UiFinder.cWaitForState(
                  'Check alt text input is enabled',
                  generalTabSelectors.alt,
                  (e) => !Attr.has(e, 'disabled')
                )
              ]),
              ui.sClickOnUi(
                'Click on decorative checkbox',
                generalTabSelectors.decorative
              ),
              Chain.asStep({}, [
                Chain.inject(Body.body()),
                UiFinder.cWaitForState(
