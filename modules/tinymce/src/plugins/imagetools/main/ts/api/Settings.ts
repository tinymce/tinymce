...
const getCorsHosts = (editor: Editor) =>
  editor.getParam('imagetools_cors_hosts', [], 'string[]');

const getCredentialsHosts = (editor: Editor) =>
  editor.getParam('imagetools_credentials_hosts', [], 'string[]');

const getFetchImage = (editor: Editor) =>
  Option.from(editor.getParam('imagetools_fetch_image', null, 'function'));
...
const getUploadTimeout = (editor: Editor) =>
  editor.getParam('images_upload_timeout', 30000, 'number');

const shouldReuseFilename = (editor: Editor) =>
