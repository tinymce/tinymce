...
UnitTest.test('atomic.tinymce.plugins.imagetools.ErrorsTest', function () {
  ...
  const testServiceErrors = function () {
    Assert.eq(
      ...
      Errors.getServiceErrorMsg('key_missing')
    );
    Assert.eq(
      'key not found',
      'The provided api key could not be found.',
      Errors.getServiceErrorMsg('key_not_found')
    );
    Assert.eq(
      'key not found',
      'The api key is not valid for the request origins.',
      Errors.getServiceErrorMsg('domain_not_trusted')
    );
  };
