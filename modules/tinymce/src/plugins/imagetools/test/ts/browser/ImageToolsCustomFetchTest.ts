...
UnitTest.asynctest(
  'browser.tinymce.plugins.imagetools.ImageToolsCustomFetchTest',
  (success, failure) => {
    const uploadHandlerState = ImageUtils.createStateContainer();
    const srcUrl =
      '/project/tinymce/src/plugins/imagetools/demo/img/dogleft.jpg';
      ...
      function (editor, onSuccess, onFailure) {
        ...
        Pipeline.async(
          ...
          [
            Log.stepsAsStep(
              'TBA',
              'ImageTools: flip image with custom fetch image',
              [
                ...
                Step.sync(() => {
                  const actualSrc = fetchState
                    .get()
                    .getOrDie('Could not get fetch state');
                  const expectedSrc =
                  ...
                  Assert.eq(
                    'Should be the expected input image',
                    expectedSrc,
                    actualSrc
                  );
                })
            ...
            Log.stepsAsStep(
              'TBA',
              'ImageTools: flip image with custom fetch image that returns an error',
              [
                tinyApis.sSetSetting('imagetools_fetch_image', () =>
                  Promise.reject('Custom fail')
                ),
                ImageUtils.sLoadImage(editor, srcUrl),
                tinyApis.sSelect('img', []),
                ImageUtils.sExecCommand(editor, 'mceImageFlipHorizontal'),
                UiFinder.sWaitFor(
                  ...
                  Body.body(),
                  '.tox-notification__body:contains("Custom fail")'
                )
              ]
            )
          ],
