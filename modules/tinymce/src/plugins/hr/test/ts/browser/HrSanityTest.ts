...
UnitTest.asynctest('browser.tinymce.plugins.fullscreen.HrSanitytest', (success, failure) => {
  ...
  TinyLoader.setupLight(
    function (editor, onSuccess, onFailure) {
      ...
      Pipeline.async(
        {},
        Log.steps('TBA', 'HorizontalRule: Click on the horizontal rule toolbar button and assert hr is added to the editor', [
          tinyUi.sClickOnToolbar('click on hr button', 'button[aria-label="Horizontal line"]'),
          tinyApis.sAssertContentStructure(
            ApproxStructure.build(function (s, _str) {
              return s.element('body', {
                children: [s.element('hr', {}), s.anything()]
              });
            })
          )
        ]),
