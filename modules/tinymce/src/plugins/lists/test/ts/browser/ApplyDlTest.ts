...
UnitTest.asynctest('tinymce.lists.browser.ApplyTest', (success, failure) => {
  ...
  suite.test('TestCase-TBA: Lists: Apply DL list to multiple Ps', function (
    editor
  ) {
  ...
  suite.test(
    'TestCase-TBA: Lists: Apply DL to P and merge with adjacent lists',
    function (editor) {
      editor.getBody().innerHTML = LegacyUnit.trimBrs(
        '<dl>' +
          '<dt>a</dt>' +
  ...
  TinyLoader.setupLight(
    function (editor, onSuccess, onFailure) {
      Pipeline.async(
        {},
        Log.steps('TBA', 'Lists: Apply DL tests', suite.toSteps(editor)),
        onSuccess,
        onFailure
      );
    },
