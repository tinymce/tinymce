...
UnitTest.asynctest('tinymce.lists.browser.IndentTest', (success, failure) => {
  ...
  suite.test(
    'TestCase-TBA: Lists: Indent single LI in OL and retain OLs list style in the new OL',
    function (editor) {
      editor.getBody().innerHTML = LegacyUnit.trimBrs(
  ...
  suite.test(
    'TestCase-TBA: Lists: Indent in table cell in table inside of list should not do anything',
    function (editor) {
      editor.getBody().innerHTML = LegacyUnit.trimBrs(
        '<ol>' +
          '<li>' +
  ...
  suite.test(
    'TestCase-TBA: Lists: Indent last LI to same level as middle LI',
    function (editor) {
      editor.getBody().innerHTML = LegacyUnit.trimBrs(
        '<ol>' +
          '<li>a' +
  ...
  suite.test('TestCase-TBA: Lists: Indent first LI and nested LI OL', function (
    editor
  ) {
    editor.getBody().innerHTML = LegacyUnit.trimBrs(
      '<ol>' + '<li>a' + '<ol>' + '<li>b</li>' + '</ol>' + '</li>' + '</ol>'
  ...
  suite.test(
    'TestCase-TBA: Lists: Indent second LI to same level as nested LI',
    function (editor) {
      editor.getBody().innerHTML = LegacyUnit.trimBrs(
        '<ul>' +
          '<li>a</li>' +
  ...
  suite.test(
    'TestCase-TBA: Lists: Indent second LI to same level as nested LI 2',
    function (editor) {
      editor.getBody().innerHTML = LegacyUnit.trimBrs(
        '<ul>' +
          '<li>a' +
  ...
  suite.test('TestCase-TBA: Lists: Indent second and third LI', function (
    editor
  ) {
    editor.getBody().innerHTML = LegacyUnit.trimBrs(
  ...
  suite.test(
    'TestCase-TBA: Lists: Indent second second li with next sibling to nested li',
    function (editor) {
      editor.getBody().innerHTML = LegacyUnit.trimBrs(
        '<ul>' +
          '<li>a</li>' +
  ...
  suite.test(
    'TestCase-TBA: Lists: Indent on second li with inner block element',
    function (editor) {
      editor.getBody().innerHTML = LegacyUnit.trimBrs(
        '<ul>' +
          '<li><p>a</p></li>' +
  ...
  TinyLoader.setupLight(
    function (editor, onSuccess, onFailure) {
      Pipeline.async(
        {},
        Log.steps('TBA', 'Lists: List indent tests', suite.toSteps(editor)),
        onSuccess,
        onFailure
      );
    },
