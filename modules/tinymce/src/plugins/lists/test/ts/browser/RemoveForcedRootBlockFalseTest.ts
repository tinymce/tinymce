...
UnitTest.asynctest(
  'tinymce.lists.browser.RemoveForcedRootBlockFalseTest',
  (success, failure) => {
    ...
    suite.test(
      'TestCase-TBA: Lists: Remove UL with single LI in BR mode',
      function (editor) {
    ...
    suite.test(
      'TestCase-TBA: Lists: Remove UL with multiple LI in BR mode',
      function (editor) {
        editor.getBody().innerHTML = LegacyUnit.trimBrs(
        ...
        LegacyUnit.equal(editor.getContent(), 'a<br />' + 'b');
        LegacyUnit.equal(editor.selection.getStart().nodeName, 'BODY');
      }
    );

    suite.test(
      'TestCase-TBA: Lists: Remove empty UL between two textblocks in BR mode',
      function (editor) {
        editor.getBody().innerHTML = LegacyUnit.trimBrs(
