...
UnitTest.asynctest(
  ...
  (success, failure) => {
    ...
    suite.test('TestCase-TBA: AutoLink: Urls ended with space', function (
      editor
    ) {
      editor.focus();
      LegacyUnit.equal(
      ...
      LegacyUnit.equal(
        typeUrl(editor, 'user@domain.com'),
        '<p><a href="mailto:user@domain.com">user@domain.com</a>&nbsp;</p>'
      );
      LegacyUnit.equal(
        typeUrl(editor, 'mailto:user@domain.com'),
        '<p><a href="mailto:user@domain.com">mailto:user@domain.com</a>&nbsp;</p>'
      );
      LegacyUnit.equal(
        typeUrl(editor, 'first-last@domain.com'),
        '<p><a href="mailto:first-last@domain.com">first-last@domain.com</a>&nbsp;</p>'
      );
    });
    ...
    suite.test('TestCase-TBA: AutoLink: Urls ended with )', function (editor) {
      ...
      LegacyUnit.equal(
        typeAnEclipsedURL(editor, 'www.domain.com'),
        '<p>(<a href="http://www.domain.com">www.domain.com</a>)</p>'
      );
      LegacyUnit.equal(
        typeAnEclipsedURL(editor, 'www.domain.com.'),
    ...
    suite.test('TestCase-TBA: AutoLink: Urls ended with new line', function (
      editor
    ) {
      LegacyUnit.equal(
    ...
    suite.test(
      'TestCase-TBA: AutoLink: Url inside blank formatting wrapper',
      function (editor) {
        editor.focus();
    ...
    suite.test(`TestCase-TBA: AutoLink: default_link_target='_self'`, function (
      editor
    ) {
    ...
    suite.test('TestCase-TBA: AutoLink: link_default_protocol=https', function (
      editor
    ) {
      ...
      LegacyUnit.equal(
        typeUrl(editor, 'www.domain.com'),
        '<p><a href="https://www.domain.com">www.domain.com</a>&nbsp;</p>'
      );
      LegacyUnit.equal(
        typeUrl(editor, 'www.domain.com.'),
        '<p><a href="https://www.domain.com">www.domain.com</a>.&nbsp;</p>'
    ...
    suite.test('TestCase-TBA: AutoLink: link_default_protocol=http', function (
      editor
    ) {
    ...
    TinyLoader.setupLight(
      function (editor, onSuccess, onFailure) {
        const steps =
          Env.browser.isIE() || Env.browser.isEdge()
            ? []
            : suite.toSteps(editor);
        Pipeline.async(
          {},
          Log.steps('TBA', 'AutoLink: Test autolink url inputs', steps),
          onSuccess,
          onFailure
        );
      },
