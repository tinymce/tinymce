...
UnitTest.asynctest(
  'browser.tinymce.plugins.importcss.ImportCssTest',
  (success, failure) => {
    Plugin();
    Theme();
          ...
          (editor, onSuccess, onFailure) => {
            ...
            const sAssertMenu = (label: string, expected, hasIcons) =>
              Chain.asStep(Body.body(), [
                UiFinder.cWaitForVisible(
                  'Waiting for styles menu to appear',
                  '[role="menu"]'
                ),
            ...
            Pipeline.async(
              ...
              [
                ...
                sAssertMenu(
                  'Checking stuff',
                  assertions.menuContents,
                  assertions.menuHasIcons
                )
              ].concat(
                assertions.choice
                  .map((c) => [
                    ...
                    Mouse.sClickOn(
                      Body.body(),
                      `.tox-collection__item .tox-collection__item-label:contains(${c})`
                    ),
                    Assertions.sAssertPresence(
                      `${c} should be present`,
                      {
    ...
    Pipeline.async(
      ...
      [
        ...
        Log.step(
          ...
          sTestEditorWithSettings(
            {
              menuContents: [{ tag: 'h1', html: 'h1.red', submenu: false }],
              menuHasIcons: true,
              choice: Option.some('h1.red')
            },
            {
          ...
          sTestEditorWithSettings(
            ...
            },
            {
              ...
              importcss_selector_filter: (sel) =>
                sel.indexOf('p') > -1 || sel.indexOf('inline') > -1
            }
          )
        ),
