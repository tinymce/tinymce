...
UnitTest.asynctest(
  'browser.tinymce.plugins.spellchecker.AddToDictionaryTest',
  (success, failure) => {
    SpellcheckerPlugin();
    SilverTheme();
    ...
    TinyLoader.setupLight(
      function (editor, onSuccess, onFailure) {
        ...
        Pipeline.async(
          {},
          Log.steps('TBA', 'Spellchecker: Add a new word to dictionary', [
            api.sFocus(),
            ...
            Chain.asStep(Body.body(), [
              ...
              UiFinder.cFindIn(
                '.tox-collection__item-label:contains("Add to dictionary")'
              ),
              Mouse.cClick
            ]),
            Step.sync(() =>
              Assert.eq('dict should now have hello', ['hello'], dict)
            )
          ]),
          onSuccess,
          onFailure
