...
UnitTest.asynctest(
  'browser.tinymce.plugins.spellchecker.SpellcheckerSpanClassTest',
  (success, failure) => {
    SpellcheckerPlugin();
    SilverTheme();
      ...
      function (editor, onSuccess, onFailure) {
        ...
        Pipeline.async(
          ...
          [
            ...
            Log.stepsAsStep(
              'TBA',
              'Spellchecker: Spelling marks should not reuse existing span. Spelling marks will be nested inside existing spans',
              [
                api.sSetContent(
                  '<p>hello <span class="bold">bold</span> world</p>'
                ),
                api.sAssertContentPresence({
                  span: 1
                }),
                ui.sClickOnToolbar(
                  'click spellcheck button',
                  '[title="Spellcheck"] > .tox-tbtn'
                ),
                api.sAssertContentPresence({
                  'span': 4,
            ...
            Log.stepsAsStep(
              'TBA',
              'Spellchecker: Spelling marks should keep selection/content when wrapping',
              [
            ...
            Log.stepsAsStep(
              'TBA',
              'Spellchecker: Spelling marks should keep selection/content unwrap',
              [
                api.sSetContent('<p>hello <strong>bold</strong> world</p>'),
                ui.sClickOnToolbar(
                  'click spellcheck button',
      ...
      },
      {
        ...
        spellchecker_callback(method, text, success, _failure) {
          if (method === 'spellcheck') {
            success({
              dictionary: dict,
              words: { hello: ['word1'], world: ['word2'], bold: ['word3'] }
            });
