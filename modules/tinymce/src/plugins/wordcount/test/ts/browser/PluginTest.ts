...
UnitTest.asynctest('browser.tinymce.plugins.wordcount.PluginTest', (success, failure) => {
  ...
  const sReset = function (tinyApis: TinyApis) {
    return Logger.t('Reset content', GeneralSteps.sequence([tinyApis.sSetContent(''), sWaitForWordcount(0)]));
  };

  const sAssertWordcount = function (num: number) {
    return Logger.t(
      `Assert word count ${num}`,
      Step.sync(function () {
  ...
  const sAssertContent = (tinyApis: TinyApis, expected: string) =>
    GeneralSteps.sequence(Log.steps('TBA', 'asserting contents after undo', [tinyApis.sAssertContent(expected)]));
  ...
  const sTestSetContent = function (tinyApis: TinyApis) {
    return GeneralSteps.sequence(
      Log.steps('TBA', 'WordCount: Set test content and assert word count', [
        sReset(tinyApis),
  ...
  const sTestKeystroke = function (editor: Editor, tinyApis: TinyApis) {
    return GeneralSteps.sequence(
      Log.steps('TBA', 'WordCount: Test keystroke and assert word count', [
        sReset(tinyApis),
  ...
  const sTestUndoRedo = function (editor: Editor, tinyApis: TinyApis) {
    return GeneralSteps.sequence(
      Log.steps('TBA', 'WordCount: Test undo and redo', [
        sReset(tinyApis),
  ...
  TinyLoader.setup(
    function (editor, onSuccess, onFailure) {
      ...
      Pipeline.async(
        {},
        [sTestSetContent(tinyApis), sTestKeystroke(editor, tinyApis), sTestUndoRedo(editor, tinyApis)],
        onSuccess,
        onFailure
      );
    },
