...
UnitTest.asynctest(
  'browser.tinymce.plugins.table.InlineEditorInsideTableTest',
  (success, failure) => {
    Plugin();
    SilverTheme();
    ...
    const step = Step.raw((_, next, die, initLogs) => {
      NamedChain.pipeline(
        [
          ...
          NamedChain.read(
            'container',
            Chain.op((div) => Remove.remove(div))
          )
        ],
        next,
        die,
    ...
    Pipeline.async(
      ...
      [
        Log.step(
          'TBA',
          'Table: Table outside of inline editor should not become resizable',
          step
        )
      ],
      () => success(),
