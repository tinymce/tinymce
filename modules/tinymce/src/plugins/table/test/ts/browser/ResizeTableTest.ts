...
UnitTest.asynctest(
  ...
  (success, failure) => {
    ...
    const assertWithin = function (value, min, max) {
      Assertions.assertEq(
        'asserting if value falls within a certain range',
        true,
        value >= min && value <= max
      );
    };
    ...
    const cAssertWidths = Chain.op(function (input: any) {
      ...
      const expectedPercent =
        (input.widthAfter.px / input.widthBefore.px) * 100;
      ...
      Assertions.assertEq(
        'table width should be in percents',
        true,
        input.widthAfter.isPercent
      );
      assertWithin(
        input.widthAfter.raw,
        expectedPercent - 1,
        expectedPercent + 1
      );
    });
    ...
    const cInsertResizeMeasure = (cInsert: Chain<Editor, Element>) =>
      NamedChain.asChain([
    ...
    const cAssertUnitBeforeResize = (unit: string) =>
      Chain.op((widths: any) => {
        Assertions.assertEq(
          `table width before resizing is in ${unit}`,
          unit === '%',
          widths.widthBefore.isPercent
        );
      });

    const cAssertUnitAfterResize = (unit: string) =>
      Chain.op((widths: any) => {
        Assertions.assertEq(
          `table width after resizing is in ${unit}`,
          unit === '%',
          widths.widthAfter.isPercent
        );
      });

    const cAssertEventData = (state, expectedEventName) =>
      Chain.op((_) => {
        Assertions.assertEq(
          'Should be table element',
          'TABLE',
          state.get().target.nodeName
        );
        Assertions.assertEq(
          'Should be expected resize event',
          expectedEventName,
          state.get().type
        );
        Assertions.assertEq(
          'Should have width',
          'number',
          typeof state.get().width
        );
        Assertions.assertEq(
          'Should have height',
          'number',
          typeof state.get().height
        );
      });
    ...
    Pipeline.async(
      ...
      [
        Logger.t(
          'Test default config of [table_responsive_width=unset], resize should detect current unit',
          Chain.asStep({}, [
            NamedChain.write(
              'editor',
            ...
            NamedChain.direct(
              'editor',
              cInsertResizeMeasure(
                TableTestUtils.cInsertRaw(
                  '<table style="width: 100%;"><tbody><tr><td style="width: 50%;"></td><td style="width: 50%;"></td></tr></tbody></table>'
                )
              ),
              'widths'
            ),
            ...
            NamedChain.direct(
              ...
              cInsertResizeMeasure(
                TableTestUtils.cInsertRaw(
                  '<table style="width: 200px;"><tbody><tr><td></td><td></td></tr></tbody></table>'
                )
              ),
              'widths'
            ),
        ...
        Logger.t(
          'Test default config of [table_responsive_width=true], new tables should default to % and resize should force %',
          Chain.asStep({}, [
            NamedChain.write(
              'editor',
              McEditor.cFromSettings({
            ...
            NamedChain.direct(
              'editor',
              cInsertResizeMeasure(TableTestUtils.cInsertTable(5, 2)),
              'widths'
            ),
        ...
        Logger.t(
          'Test [table_responsive_width=false], new tables should default to px and resize should force px',
          Chain.asStep({}, [
            NamedChain.write(
              'editor',
              McEditor.cFromSettings({
