...
UnitTest.asynctest('browser.tinymce.plugins.table.DragResizeTest', (success, failure) => {
  ...
  const sDragDrop = (container: Element, selector: string, dx: number, dy: number) =>
    Logger.t(
      ...
      Chain.asStep(container, [UiFinder.cFindIn(selector), Mouse.cMouseDown, Mouse.cMouseMoveTo(dx, dy), Mouse.cMouseUpTo(dx, dy)])
    );
  ...
  const sMouseover = (container, selector) =>
    Logger.t('Place mouse point over element', Chain.asStep(container, [UiFinder.cFindIn(selector), Mouse.cMouseOver]));
  ...
  const sSetStateFrom = (editor, path) =>
    Logger.t(
      ...
      Step.sync(() => {
        const element = Hierarchy.follow(Element.fromDom(editor.getBody()), path).getOrDie('could not find element') as Element<
          HTMLElement
        >;
  ...
  const sWaitForSelection = (editor: Editor, tinyApis: TinyApis) =>
    Logger.t(
      ...
      GeneralSteps.sequence([
        ...
        Waiter.sTryUntil('wait for resize handles', UiFinder.sExists(Element.fromDom(editor.getBody()), '#mceResizeHandlese'))
      ])
    );
