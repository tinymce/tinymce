  ...
  (success, failure) => {
    ...
    suite.test('TestCase-TBA: Table: mceTableMergeCells', function (editor) {
      testCommand(editor, 'mceTableMergeCells', [
        ...
        {
          message:
            'Should merge cells in two cols/rows into one cell with colspan',
          before:
            '<table>' +
            '<tbody>' +
            '<tr><td data-mce-selected="1">a1</td><td data-mce-selected="1">b1</td></tr>' +
        ...
        {
          message:
            'Should reduce rowspans to 2 keep the colspan and remove one tr',
          before:
            '<table>' +
            '<tbody>' +
    ...
    TinyLoader.setupLight(
      function (editor, onSuccess, onFailure) {
        Pipeline.async(
          ...
          Log.steps(
            'TBA',
            'Table: Test merge cell commands',
