...
UnitTest.asynctest(
  'browser.tinymce.plugins.table.TableDialogGeneralTest',
  (success, failure) => {
    Plugin();
    SilverTheme();
    ...
    const generalSelectors = {
      ...
      cellspacing:
        'label.tox-label:contains(Cell spacing) + input.tox-textfield',
      cellpadding:
        'label.tox-label:contains(Cell padding) + input.tox-textfield',
    ...
    TinyLoader.setup(
      (editor, onSuccess, onFailure) => {
        ...
        const standardOkTest = () =>
          Log.stepsAsStep('TBA', 'Table: Table properties dialog standard ok', [
            sSetTable,
            sSetCursor,
            TableTestUtils.sOpenTableDialog(tinyUi),
            TableTestUtils.sAssertDialogValues(
              emptyStandardData,
              false,
              generalSelectors
            ),
            TableTestUtils.sClickDialogButton(
              'empty dialog with empty details',
              true
            ),
            TableTestUtils.sAssertElementStructure(
              editor,
              'table',
              htmlEmptyTable
            ),
        ...
        const standardFillOkTest = () => {
          const htmlFilledEmptyTable = ApproxStructure.build(function (
            s,
            str /* , arr*/
          ) {
          ...
          return Log.stepsAsStep(
            'TBA',
            'Table: Table properties dialog standard fill ok',
            [
              sSetTable,
              sSetCursor,
              TableTestUtils.sOpenTableDialog(tinyUi),
              ...
              TableTestUtils.sClickDialogButton(
                'filled dialog with full details',
                true
              ),
              TableTestUtils.sAssertApproxElementStructure(
                editor,
                'table',
                htmlFilledEmptyTable
              ),
              TableTestUtils.sOpenTableDialog(tinyUi),
              TableTestUtils.sAssertDialogValues(
                fullStandardData,
        ...
        const allOffOkTest = () => {
          const htmlFilledAllOffTable =
            '<table style="height: 500px; width: 500px; float: left;"><tbody><tr><td>X</td></tr></tbody></table>';
          ...
          return Log.stepsAsStep(
            'TBA',
            'Table: Table properties dialog all ui off fill ok',
            [
        ...
        const allOnOkTest = () => {
          const htmlFilledAllOnTable = ApproxStructure.build(function (
            s,
            str /* , arr*/
          ) {
            return s.element('table', {
          ...
          return Log.stepsAsStep(
            'TBA',
            'Table: Table properties dialog all ui on fill ok',
            [
        ...
        const execCommandTest = () => {
          const baseHtml =
            '<table style="height: 500px; width: 500px; border-width: 1px; border-spacing: 5px; background-color: rgb(0, 0, 255); border-color: rgb(255, 0, 0); border-style: dotted; float: left;"><caption><br></caption><tbody><tr><td>X</td></tr></tbody></table>';
        ...
        const okCancelTest = () => {
          ...
          const newHtml =
            '<table class="dog" style="width: 500px; height: 500px; float: left; background-color: #0000ff; border: 1px dotted #ff0000; border-spacing: 5px; border-collapse: collapse;" border="1"><caption>&nbsp;</caption><tbody><tr><td style="border-color: #ff0000; border-width: 1px; padding: 5px;">X</td></tr></tbody></table>';
          ...
          return Log.stepsAsStep(
            'TBA',
            'Table: Test cancel changes nothing and save does',
            [
              tinyApis.sSetSetting('table_class_list', [
                { title: 'None', value: '' },
                { title: 'Dog', value: 'dog' },
