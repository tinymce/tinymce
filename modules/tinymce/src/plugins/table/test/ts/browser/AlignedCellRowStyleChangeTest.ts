...
UnitTest.asynctest(
  'browser.tinymce.plugins.table.AlignedCellRowStyleChangeTest',
  (success, failure) => {
    SilverTheme();
    Plugin();
    ...
    TinyLoader.setupLight(
      (editor: Editor, onSuccess, onFailure) => {
        ...
        Pipeline.async(
          ...
          Log.steps(
            'TBA',
            'Table: Set background color on selected table row with text-align: center',
            [
