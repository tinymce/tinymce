...
UnitTest.asynctest('browser.tinymce.plugins.table.TableRowDialogTest', (success, failure) => {
  ...
  TinyLoader.setupLight(
    (editor, onSuccess, onFailure) => {
      ...
      const baseGetTest = () =>
        Log.stepsAsStep('TBA', 'Table: Table row properties dialog (get data from plain cell)', [
      ...
      const baseGetSetTest = () =>
        Log.stepsAsStep('TBA', 'Table: Table row properties dialog (update all)', [
          UiFinder.sWaitForVisible('waiting for editor', Element.fromDom(document.body), 'div.tox-tinymce'),
          tinyApis.sSetContent(baseHtml),
          tinyApis.sSelect('td', [0]),
          TableTestUtils.sOpenTableDialog(tinyUi),
      ...
      const captionTest = () =>
        Log.stepsAsStep('TBA', 'Table: Caption should always stay the firstChild of the table (see TINY-1167)', [
      ...
      const advGetTest = () =>
        Log.stepsAsStep('TBA', 'Table: Table row properties dialog (get data from complex row)', [
      ...
      const advGetSetTest = () =>
        Log.stepsAsStep('TBA', 'Table: Update advanced styles from row properties dialog', [
          UiFinder.sWaitForVisible('waiting for editor', Element.fromDom(document.body), 'div.tox-tinymce'),
      ...
      const advRemoveTest = () =>
        Log.stepsAsStep('TBA', 'Table: Remove all advanced styles through the style field', [
          tinyApis.sSetContent(advHtml),
