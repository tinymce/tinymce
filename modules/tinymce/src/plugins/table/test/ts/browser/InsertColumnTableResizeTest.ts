...
UnitTest.asynctest(
  'browser.tinymce.plugins.table.InsertColumnTableResizeTest',
  (success, failure) => {
    Plugin();
    SilverTheme();
    ...
    const cInsertColumnMeasureWidth = (label: string, data: TestData) =>
      Log.chain(
        'TBA',
        'Insert column before, insert column after, erase column and measure table widths',
        NamedChain.asChain([
          ...
          Chain.label(
            ...
            NamedChain.read('editor', TableTestUtils.cInsertColumnBefore)
          ),
          Chain.label(
            'Insert column after',
            NamedChain.read('editor', TableTestUtils.cInsertColumnAfter)
          ),
          Chain.label(
            'Delete column',
            NamedChain.read('editor', TableTestUtils.cDeleteColumn)
          ),
          Chain.label(
    ...
    const cAssertWidth = (label: string, data: TestData) =>
      Chain.label(
        ...
        NamedChain.asChain([
          ...
          NamedChain.direct(
            'editor',
            cInsertColumnMeasureWidth(label, data),
            'widths'
          ),
    ...
    NamedChain.pipeline(
      Log.chains(
        'TBA',
        'Table: Insert columns, erase column and assert the table width does not change',
        [
          NamedChain.write(
            'editor',
