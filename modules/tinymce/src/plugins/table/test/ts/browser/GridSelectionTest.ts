  ...
  (success, failure) => {
    ...
    TinyLoader.setupLight(
      function (editor, onSuccess, onFailure) {
        ...
        const sAssertTableSelection = function (
          tableHtml: string,
          selectCells: string[],
          cellContents: string[]
        ) {
          ...
          const getCells = (table): HTMLTableCellElement[] =>
            editor.$(table).find('td').toArray();

          const getSelectedCells = (table) =>
            editor.$(table).find('td[data-mce-selected]').toArray();
        ...
        const sAssertSelectionContent = function (
          editor,
          expectedHtml: string
        ) {
          return Step.sync(() => {
            Assertions.assertHtml(
              'Should be expected content',
              expectedHtml,
        ...
        Pipeline.async(
          ...
          assertions.concat(
            Log.steps(
              'TBA',
              'Table: Extracted selection contents should be without internal attributes',
              [
                tinyApis.sSetRawContent(
                  '<table><tr><td data-mce-selected="1">a</td><td>b</td></tr><tr><td data-mce-selected="1">c</td><td>d</td></tr></table>'
                ),
                sAssertSelectionContent(
                  editor,
