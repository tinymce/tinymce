...
UnitTest.asynctest('browser.tinymce.plugins.table.TableCellDialogTest', (success, failure) => {
  ...
  TinyLoader.setupLight(
    (editor, onSuccess, onFailure) => {
      ...
      const baseHtml =
        '<table>' + '<tbody>' + '<tr>' + '<td data-mce-selected="1">a</td>' + '<td>b</td>' + '</tr>' + '</tbody>' + '</table>';
      ...
      const baseGetTest = () =>
        Log.stepsAsStep('TBA', 'Table: Table cell properties dialog (get data from basic cell)', [
      ...
      const baseGetSetTest = () =>
        Log.stepsAsStep('TBA', 'Table: Table cell properties dialog (get/set data from/to basic cell)', [
          tinyApis.sSetSetting('table_cell_advtab', false),
          tinyApis.sSetContent(baseHtml),
          tinyApis.sSelect('td', [0]),
          TableTestUtils.sOpenTableDialog(tinyUi),
