...
UnitTest.asynctest(
  'browser.tinymce.plugins.table.ContextToolbarTest',
  (success, failure) => {
    Theme();
    Plugin();
    ...
    TinyLoader.setup(
      function (editor, onSuccess, onFailure) {
        ...
        const sAddTableAndOpenContextToolbar = (html: string) =>
          GeneralSteps.sequence([
            tinyApis.sSetContent(html),
            tinyUi.sWaitForUi(
              'Wait for table context toolbar',
              '.tox-toolbar button[aria-label="Table properties"]'
            )
          ]);

        // Use keyboard shortcut ctrl+F9 to navigate to the context toolbar
        const sPressKeyboardShortcutKey = Keyboard.sKeydown(
        ...
        const sAssertButtonDisabled = (label: string, selector: string) =>
          tinyUi.sWaitForUi(
            label,
            `.tox-pop__dialog ${selector}.tox-tbtn--disabled`
          );
          ...
          Chain.asStep({}, [
            Chain.fromParent(
              tinyUi.cWaitForPopup(
                'wait for context toolbar',
                '.tox-pop__dialog div'
              ),
              [Chain.fromChains([UiFinder.cFindIn(selector), Mouse.cClick])]
            )
          ]);

        const sAssertHtmlStructure = (label, expectedHtml) =>
          Chain.asStep({ editor }, [
            NamedChain.read(
              'editor',
              Chain.op((editor) => {
        ...
        Pipeline.async(
          ...
          [
            ...
            Log.stepsAsStep(
              'TBA',
              'Table: context toolbar keyboard navigation test',
              [
                sAddTableAndOpenContextToolbar(tableHtml),
                sPressKeyboardShortcutKey,
                sAssertFocusOnItem(
                  'Table properties button',
                  'button[aria-label="Table properties"]'
                ),
                sPressRightArrowKey,
                sAssertFocusOnItem(
                  'Delete table button',
                ...
                sAssertFocusOnItem(
                  ...
                  'button[aria-label="Insert row before"]'
                ),
                sPressRightArrowKey,
                sAssertFocusOnItem(
                  'Insert row below button',
                  'button[aria-label="Insert row after"]'
                ),
                sPressRightArrowKey,
                sAssertFocusOnItem(
                  'Delete row button',
                ...
                sAssertFocusOnItem(
                  ...
                  'button[aria-label="Insert column before"]'
                ),
                sPressRightArrowKey,
                sAssertFocusOnItem(
                  'Insert column after button',
                  'button[aria-label="Insert column after"]'
                ),
                sPressRightArrowKey,
                sAssertFocusOnItem(
            ...
            Log.stepsAsStep(
              'TBA',
              'Table: context toolbar functionality test',
              [
                sAddTableAndOpenContextToolbar(tableHtml),
                ...
                sAssertHtmlStructure(
                  'Assert insert row before',
                  '<table><tbody><tr><td><br></td></tr><tr><td><br></td></tr></tbody></table>'
                ),

                sClickOnToolbarButton('button[aria-label="Insert row after"]'),
                sAssertHtmlStructure(
                  'Assert insert row after',
                  '<table><tbody><tr><td><br></td></tr><tr><td><br></td></tr><tr><td><br></td></tr></tbody></table>'
                ),
                ...
                sClickOnToolbarButton(
                  'button[aria-label="Insert column before"]'
                ),
                sAssertHtmlStructure(
                  'Assert insert column before',
            ...
            Log.stepsAsStep(
              'TBA',
              'Table: context toolbar functionality test with focus in caption',
              [
                sAddTableAndOpenContextToolbar(
                  '<table style = "width: 5%;"><caption>abc</caption><tbody><tr><td>x</td></tr></tbody></table>'
                ),
                ...
                sAssertButtonDisabled(
                  'Assert insert row before disabled',
                  'button[aria-label="Insert row before"]'
                ),
                sAssertButtonDisabled(
                  'Assert insert row after disabled',
                  'button[aria-label="Insert row after"]'
                ),
                sAssertButtonDisabled(
                  'Assert delete row disabled',
                  'button[aria-label="Delete row"]'
                ),
                sAssertButtonDisabled(
                  'Assert insert column before disabled',
                  'button[aria-label="Insert column before"]'
                ),
                sAssertButtonDisabled(
                  'Assert insert column after disabled',
                  'button[aria-label="Insert column after"]'
                ),
                sAssertButtonDisabled(
                  'Assert delete column disabled',
                  'button[aria-label="Delete column"]'
                ),
