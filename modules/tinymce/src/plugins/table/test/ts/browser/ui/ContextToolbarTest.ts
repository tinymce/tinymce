...
UnitTest.asynctest('browser.tinymce.plugins.table.ContextToolbarTest', (success, failure) => {
  ...
  TinyLoader.setup(
    function (editor, onSuccess, onFailure) {
      ...
      const tableHtml = '<table style = "width: 5%;">' + '<tbody>' + '<tr>' + '<td></td>' + '</tr>' + '</tbody>' + '</table>';
      ...
      const sAssertButtonDisabled = (label: string, selector: string) =>
        tinyUi.sWaitForUi(label, `.tox-pop__dialog ${selector}.tox-tbtn--disabled`);
      ...
      const sOpenAndCloseDialog = GeneralSteps.sequence([
        Chain.asStep(editor, [tinyUi.cWaitForPopup('wait for dialog', 'div[role="dialog"]'), UiChains.cCloseDialog('div[role="dialog"]')]),
