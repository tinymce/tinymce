...
UnitTest.asynctest(
  'browser.tinymce.plugins.table.UnmergeCellTableResizeTest',
  (success, failure) => {
    Plugin();
    SilverTheme();
    ...
    const emptyTable = {
      ...
      select: (editor) => {
        Keyboard.keydown(
          Keys.down(),
          { shift: true },
          TinyDom.fromDom(editor.getBody())
        );
      }
    };
    ...
    const cUnmergeCellsMeasureTableWidth = (label, data) =>
      Log.chain(
        'TBA',
        'Merge and unmerge cells, measure table widths',
        NamedChain.asChain([
          ...
          Chain.label(
            'Store width after merge/unmerge',
    ...
    const cMergeResizeUnmergeMeasureWidth = (label, data) =>
      Log.chain(
        'TBA',
        'Merge cells and resize table, unmerge cells and measure table widths',
        NamedChain.asChain([
          NamedChain.direct(NamedChain.inputName(), Chain.identity, 'editor'),
          Chain.label(
            'Insert table',
      ...
      Chain.op((input: any) => {
        if (input.widthBefore.isPercent) {
          ...
          const actualDiff = Math.abs(
            input.widthBefore.px - input.widthAfter.px
          );
          Assert.eq(
            `table width should be approx (within 1px): ${input.widthBefore.raw}% (${input.widthBefore.px}px) ~= ${input.widthAfter.raw}% (${input.widthAfter.px}px)`,
            true,
            actualDiff <= 1
          );
        } else {
          Assertions.assertEq(
            'table width should not change',
            input.widthBefore,
            input.widthAfter
          );
        }
    ...
    const cAssertWidth = (label, data) =>
      Chain.label(
        ...
        NamedChain.asChain([
          ...
          NamedChain.direct(
            'editor',
            cUnmergeCellsMeasureTableWidth(label, data),
            'widths'
          ),
    ...
    const cMergeResizeSplitAssertWidth = (label, data) =>
      Chain.label(
        ...
        NamedChain.asChain([
          ...
          NamedChain.direct(
            'editor',
            cMergeResizeUnmergeMeasureWidth(label, data),
            'widths'
          ),
          NamedChain.read('widths', cAssertWidths)
        ])
    ...
    NamedChain.pipeline(
      Log.chains(
        'TBA',
        'Table: Resize table, merge and split cells, assert the table width does not change',
        [
          NamedChain.write(
            'editor',
          ...
          NamedChain.read(
            'editor',
            cAssertWidth('with contents in some cells', contentsInSomeCells)
          ),
          NamedChain.read(
            'editor',
            cAssertWidth('with contents in all cells', contentsInAllCells)
          ),
          NamedChain.read(
            'editor',
            cAssertWidth('with headings', tableWithHeadings)
          ),
          NamedChain.read(
            'editor',
            cAssertWidth('with caption', tableWithCaption)
          ),
          NamedChain.read(
            'editor',
            cAssertWidth('with nested tables', nestedTables)
          ),

          NamedChain.read(
            'editor',
            cAssertWidth('with an entire row merged and split', selectedRow)
          ),
          NamedChain.read(
            'editor',
            cAssertWidth('with one selected cell', singleCell)
          ),
          NamedChain.read(
            'editor',
            cAssertWidth('with whole table merged and split', mergeWholeTable)
          ),
          NamedChain.read(
            'editor',
            cMergeResizeSplitAssertWidth(
              'resized after merge and then split',
              mergeResizeSplit
            )
          ),
