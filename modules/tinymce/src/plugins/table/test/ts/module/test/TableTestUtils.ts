...
const sAssertTableStructure = (editor, structure) =>
  Logger.t(
...
const sOpenTableDialog = (ui: TinyUi) =>
  Logger.t(
    'Open table dialog',
...
const sAssertElementStructure = (editor, selector, expected) =>
  Logger.t(
    'Assert HTML structure of the element ' + expected,
    Step.sync(() => {
...
const sClickDialogButton = (label: string, isSave: boolean) =>
  Logger.t(
  ...
  Logger.t(
    ...
    Chain.asStep({}, [
      ...
      Chain.op(function (dialog) {
        Assertions.assertPresence(label, expected, dialog);
      })
    ])
  );

const sAssertSelectValue = (label, section, expected) =>
  Logger.t(
