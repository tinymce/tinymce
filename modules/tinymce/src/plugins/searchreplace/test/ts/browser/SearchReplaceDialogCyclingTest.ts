...
UnitTest.asynctest(
  'browser.tinymce.plugins.searchreplace.SearchReplaceDialogCyclingTest',
  (success, failure) => {
    Theme();
    SearchreplacePlugin();
    ...
    TinyLoader.setupLight(
      (editor, onSuccess, onFailure) => {
        ...
        const sSelectPreference = (name: string) =>
          Utils.sSelectPreference(tinyUi, name);

        const sAssertMatchFound = (index: number) =>
          Step.sync(() => {
            const matches = SelectorFilter.descendants(
              Element.fromDom(editor.getBody()),
              '.mce-match-marker'
            );
            const elem = matches[index];
            Assertions.assertEq(
              `Check match ${index} is marked as selected`,
              true,
              Class.has(elem, 'mce-match-marker-selected')
            );
          });
        ...
        const sTestCycling = (sCycle: Step<any, any>, dir: Direction) => [
          Log.stepsAsStep(
            'TINY-4506',
            'SearchReplace: Test cycling through results without any preferences',
            [
              tinyApis.sSetContent('<p>fish fish fish</p>'),
              tinyApis.sSetSelection([0, 0], 0, [0, 0], 4),
              Utils.sOpenDialog(tinyUi),
              Utils.sAssertFieldValue(
                tinyUi,
                'input.tox-textfield[placeholder="Find"]',
                'fish'
              ),
              sFind,
              sAssertMatchFound(0),
          ...
          Log.stepsAsStep(
            'TINY-4506',
            'SearchReplace: Test cycling through results with matchcase enabled',
            [
          ...
          Log.stepsAsStep(
            'TINY-4506',
            'SearchReplace: Test cycling through results with wholewords enabled',
            [
          ...
          Log.stepsAsStep(
            'TINY-4506',
            'SearchReplace: Test cycling through results with special characters',
            [
        ...
        Pipeline.async(
          ...
          [
            Log.stepsAsStep(
              'TINY-4506',
              'SearchReplace: Test cycling using find',
              sTestCycling(sFind, Direction.FORWARDS)
            ),
            Log.stepsAsStep(
              'TINY-4506',
              'SearchReplace: Test cycling using next',
              sTestCycling(sNext, Direction.FORWARDS)
            ),
            Log.stepsAsStep(
              'TINY-4506',
              'SearchReplace: Test cycling using previous',
              sTestCycling(sPrev, Direction.BACKWARDS)
            )
          ],
          onSuccess,
          onFailure
