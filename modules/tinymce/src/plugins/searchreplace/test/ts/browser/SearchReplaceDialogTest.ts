...
UnitTest.asynctest(
  'browser.tinymce.plugins.searchreplace.SearchReplaceDialogTest',
  (success, failure) => {
    Theme();
    SearchreplacePlugin();
    ...
    TinyLoader.setupLight(
      (editor, onSuccess, onFailure) => {
        ...
        const sAssertFound = (count: number) =>
          tinyApis.sAssertContentPresence({
            '.mce-match-marker': count
          });

        const sFindAndAssertFound = (count: number) =>
        ...
        Pipeline.async(
          ...
          [
            ...
            Log.stepsAsStep(
              'TBA',
              'SearchReplace: Test some content selected',
              [
                tinyApis.sSetContent('<p>fish fish fish</p>'),
            ...
            Log.stepsAsStep(
              'TBA',
              'SearchReplace: Test some content selected with matchcase enabled',
              [
                tinyApis.sSetContent('<p>fish Fish fish</p>'),
                tinyApis.sSetSelection([0, 0], 5, [0, 0], 9),
                Utils.sOpenDialog(tinyUi),
            ...
            Log.stepsAsStep(
              'TBA',
              'SearchReplace: Test some content selected with wholewords enabled',
              [
            ...
            Log.stepsAsStep(
              'TBA',
              'SearchReplace: Test some content selected while changing preferences',
              [
