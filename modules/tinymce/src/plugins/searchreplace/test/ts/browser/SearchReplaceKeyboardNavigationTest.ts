...
UnitTest.asynctest(
  'browser.tinymce.plugins.searchreplace.SearchReplaceKeyboardNavigationTest',
    ...
    const sOpenDialog = (ui: TinyUi) =>
      Logger.t(
        ...
        GeneralSteps.sequence([
          ui.sClickOnToolbar(
            'Click on find and replace button, there should be only 1 button in the toolbar',
    ...
    TinyLoader.setup(
      function (editor, onSuccess, onFailure) {
        ...
        const sAssertFocused = (name, selector) =>
          FocusTools.sTryOnSelector(name, doc, selector);
        ...
        Pipeline.async(
          ...
          [
            Log.stepsAsStep(
              'TINY-3914',
              'Find and replace: Reaching find and replace via the keyboard',
              [
                sFocusToolbar,
                ...
                sAssertFocused(
                  'Find and replace edit menu item',
                  '.tox-collection__item:contains("Find and replace")'
                ), // Menu item can be reached by keyboard
                sPressEsc,
                sPressTab,
            ...
            Log.stepsAsStep(
              'TINY-3914',
              'Find and replace: Dialog keyboard navigation',
              [
                sOpenDialog(tinyUi),
                sAssertFocused(
                  'Find input',
                ...
                sAssertFocused(
                  'Replace with input',
                  '.tox-textfield[placeholder="Replace with"]'
                ),
                sPressTab,
                sAssertFocused(
                  'Placeholder menu button',
                  '.tox-tbtn--select[title="Preferences"]'
                ),
                sPressDown,
                sAssertFocused(
                  'Match case menu item',
                  '.tox-collection__item:contains("Match case")'
                ), // Menu items can be reached by keyboard
                sPressEnter,
                sAssertFocused(
                  'Placeholder menu button',
                  '.tox-tbtn--select[title="Preferences"]'
            ...
            Log.stepsAsStep(
              'TINY-3961',
              'Find and replace: Dialog keyboard focus is returned to find input',
              [
            ...
            Log.stepsAsStep(
              'TINY-4014',
              'Find and replace: Dialog keyboard focus is returned to find input after displaying an alert',
              [
                tinyApis.sSetContent('<p>fish fish fish</p>'),
                sOpenDialog(tinyUi),
                sAssertFocused(
                ...
                sAssertFocused(
                  ...
                  '.tox-alert-dialog .tox-button[title="OK"]'
                ),
                sPressEnter,
                sAssertFocused(
                  'Find input',
