import { Chain, Log, Mouse, Pipeline, Step, UiControls, UiFinder, Logger } from '@ephox/agar';
        ...
        Log.steps('TBA', 'SearchReplace: replace one of three found, undo and redo and assert there is no matcher spans in editor', [
          ...
          Chain.asStep({}, [
            Chain.fromParent(tinyUi.cWaitForPopup('wait for dialog', 'div[role="dialog"]'), [
              Chain.fromChains([UiFinder.cFindIn('input.tox-textfield[placeholder="Find"]'), UiControls.cSetValue('cats')]),
              Chain.fromChains([UiFinder.cFindIn('input.tox-textfield[placeholder="Replace with"]'), UiControls.cSetValue('dogs')]),
              Chain.fromChains([UiFinder.cFindIn('button:contains("Find")'), Mouse.cClick]),
              Chain.fromChains([UiFinder.cWaitFor('wait for button to be enabled', 'button[disabled!="disabled"]:contains("Replace")')]),
              Chain.fromChains([UiFinder.cFindIn('button:contains("Replace")'), Mouse.cClick]),
              Chain.fromChains([UiFinder.cFindIn('button[aria-label="Close"]'), Mouse.cClick])
            ])
          ]),
          sUndo(editor),
