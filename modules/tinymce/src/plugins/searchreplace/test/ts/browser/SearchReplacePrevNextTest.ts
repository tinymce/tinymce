...
describe('browser.tinymce.plugins.searchreplace.SearchReplacePrevNextTest', () => {
  ...
  const pAssertButtonsEnabled = async () => {
    ...
    await UiFinder.pWaitFor('prev button enabled', body, 'button[data-mce-btn="prev"]:not([disabled])');
  ...
  const pAssertNextPrevButtonsDisabled = async () => {
    ...
    await UiFinder.pWaitFor('prev button disabled', body, 'button[data-mce-btn="prev"][disabled]');
  };
