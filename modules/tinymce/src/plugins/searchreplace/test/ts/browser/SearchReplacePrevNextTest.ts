...
UnitTest.asynctest(
  'browser.tinymce.plugins.searchreplace.SearchReplacePrevNextTest',
  (success, failure) => {
    Theme();
    SearchreplacePlugin();
      ...
      function (editor, onSuccess, onFailure) {
        ...
        const cAssertButtonsEnabled = NamedChain.asChain([
          ...
          NamedChain.direct(
            NamedChain.inputName(),
            UiFinder.cWaitFor(
              'wait for prev button to be enabled',
          ...
          NamedChain.direct(
            NamedChain.inputName(),
            UiFinder.cWaitFor(
              'wait for replace button to be enabled',
              'button[title="Replace"]:not([disabled])'
            ),
            '_'
          ),
        ...
        const cAssertNextPrevButtonsDisabled = NamedChain.asChain([
          ...
          NamedChain.direct(
            NamedChain.inputName(),
            UiFinder.cWaitFor(
              'wait for prev button to be disabled',
              'button[title="Previous"][disabled]'
            ),
            '_'
          ),
          NamedChain.outputInput
        ...
        const cClickButton = (name: string) =>
          Chain.fromChains([
        ...
        Pipeline.async(
          ...
          Log.steps(
            'TBA',
            'SearchReplace: Test Prev and Next buttons become enabled and disabled at right places when multiple matches exist',
            [
              ...
              Chain.asStep({}, [
                Chain.fromParent(
                  ...
                  [
                    ...
                    Chain.fromChains([
                      UiFinder.cFindIn(
                        'input.tox-textfield[placeholder="Replace with"]'
                      ),
