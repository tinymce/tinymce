...
const sClickFind = (ui) =>
  ui.sClickOnUi('Click find', '[role=dialog] button:contains("Find")');
const sClickNext = (ui) =>
  ui.sClickOnUi('Click next', '[role=dialog] button[title="Next"]');
const sClickPrev = (ui) =>
  ui.sClickOnUi('Click previous', '[role=dialog] button[title="Previous"]');
...
const sSelectPreference = (ui, name: string) =>
  GeneralSteps.sequence([
    ...
    ui.sClickOnUi(
      ...
      '.tox-selected-menu[role=menu] div[title="' + name + '"]'
    )
  ]);
