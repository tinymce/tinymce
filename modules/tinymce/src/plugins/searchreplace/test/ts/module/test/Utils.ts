...
const cFindInDialog = (ui: TinyUi, selector: string) =>
  Chain.control(
    Chain.fromChains([ui.cWaitForPopup('Wait for dialog', 'div[role="dialog"]'), UiFinder.cFindIn(selector)]),
...
const sAssertFieldValue = (ui: TinyUi, selector: string, value: string) =>
  Waiter.sTryUntil(
    ...
    Chain.asStep({}, [cFindInDialog(ui, selector), UiControls.cGetValue, Assertions.cAssertEq(`Assert ${value} value`, value)]),
    20,
    3000
  );
...
const sSelectPreference = (ui, name: string) =>
  GeneralSteps.sequence([
...
export { cFindInDialog, sAssertFieldValue, sOpenDialog, sCloseDialog, sClickFind, sClickNext, sClickPrev, sSelectPreference };
