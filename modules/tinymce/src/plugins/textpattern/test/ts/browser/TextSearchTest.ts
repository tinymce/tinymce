        ...
        const steps = Utils.withTeardown(
          [
            Log.stepsAsStep('TBA', 'TextSearch: text before from element', [
              ...
              Step.sync(() => {
                const rng = editor.selection.getRng();
                const content = TextSearch.textBefore(
                  rng.startContainer,
                  rng.startOffset,
                  editorBody
                );
                assertSpot(
                  'Text before from end of paragraph',
                  content,
                  'bc',
                  2
                );
            ...
            Log.stepsAsStep('TBA', 'TextSearch: text before from text node', [
              ...
              Step.sync(() => {
                const rng = editor.selection.getRng();
                const contentEnd = TextSearch.textBefore(
                  rng.startContainer,
                  2,
                  editorBody
                );
                assertSpot('Text before within text node', contentEnd, 'bc', 2);
                const contentStart = TextSearch.textBefore(
                  rng.startContainer,
                  0,
                  editorBody
                );
                assertSpot(
                  'Text before within text node',
                  contentStart,
                  'bc',
                  0
                );
            ...
            Log.stepsAsStep(
              'TBA',
              'TextSearch: scan right over fragmented text',
              [
                tinyApis.sSetContent('<p>*<a href="#">a</a>bc</p>'),
                tinyApis.sSetCursor([0, 0], 0),
                Step.sync(() => {
                  const startNode = editor.selection.getRng()
                  ...
                  const content = TextSearch.scanRight(
                    startNode,
                    4,
                    editorBody
                  );
                  const outOfRange = TextSearch.scanRight(
                    startNode,
                    10,
                    editorBody
                  );
                  ...
                  assertSpot(
                    'Scan right over anchor element',
                    content,
                    'bc',
                    2
                  );
                  Assertions.assertEq(
                    'Scan right with out of range offset is none',
                    true,
                    outOfRange.isNone()
                  );
                })
            ...
            Log.stepsAsStep(
              'TBA',
              'TextSearch: scan left over fragmented text',
              [
                tinyApis.sSetContent('<p>*<a href="#">a</a>bc</p>'),
                tinyApis.sSetCursor([0, 2], 2),
                Step.sync(() => {
                  ...
                  const content = TextSearch.scanLeft(
                    startNode,
                    -2,
                    editorBody
                  );
                  const outOfRange = TextSearch.scanLeft(
                    startNode,
                    -10,
                    editorBody
                  );
                  ...
                  Assertions.assertEq(
                    'Scan left with out of range offset is none',
                    true,
                    outOfRange.isNone()
                  );
                })
            ...
            Log.stepsAsStep(
              'TBA',
              'TextSearch: repeat left over fragmented text',
              [
                ...
                Step.sync(() => {
                  ...
                  Assertions.assertDomEq(
                    'Repeat left until asterisk found',
                    Element.fromDom(asteriskNode),
                    Element.fromDom(asterisk)
                  );
                  const anchor = repeatLeftUntil(editor, 'a');
                  Assertions.assertDomEq(
                    'Repeat left until anchor found',
                    Element.fromDom(anchorNode),
                    Element.fromDom(anchor)
                  );
                  const boundary = repeatLeftUntil(editor, 'def');
                  Assertions.assertEq(
                    'Repeat left until block boundary found',
                    null,
                    boundary
                  );
                })
            ...
            Log.stepsAsStep(
              'TBA',
              'TextSearch: repeat right over fragmented text',
              [
                ...
                Step.sync(() => {
                  ...
                  Assertions.assertDomEq(
                    'Repeat right until bc found',
                    Element.fromDom(contentNode),
                    Element.fromDom(asterisk)
                  );
                  const anchor = repeatRightUntil(editor, 'a');
                  Assertions.assertDomEq(
                    'Repeat right until anchor found',
                    Element.fromDom(anchorNode),
                    Element.fromDom(anchor)
                  );
                  const boundary = repeatRightUntil(editor, 'def');
                  Assertions.assertEq(
                    'Repeat right until block boundary found',
                    null,
                    boundary
                  );
                })
