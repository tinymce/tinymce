  ...
  TinyLoader.setupLight(
    (editor, onSuccess, onFailure) => {
      ...
      const sSetContentAndCursor = (content: string, elementPath: number[], offset: number) =>
        GeneralSteps.sequence([tinyApis.sSetRawContent(`<div>${content}</div>`), tinyApis.sSetCursor([0].concat(elementPath), offset)]);
      ...
      Pipeline.async(
        ...
        [
          Log.stepsAsStep('TBA', 'Run on text without pattern returns no matching patterns', [
            ...
            Chain.asStep(editor, [cGetInlinePattern(inlinePatterns), cAssertPatterns([])])
          ]),
          ...
          Log.stepsAsStep('TBA', 'inline * with uncollapsed range returns no match', [
            ...
            Chain.asStep(editor, [cGetInlinePattern(inlinePatterns, true), cAssertPatterns([])])
          ]),
