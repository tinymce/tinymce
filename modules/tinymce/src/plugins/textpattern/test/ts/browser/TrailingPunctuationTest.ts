...
UnitTest.asynctest('browser.tinymce.plugins.textpattern.TrailingPunctuationTest', (success, failure) => {
  Theme();
  TextpatternPlugin();
                ...
                return s.element('body', {
                  children: [
                    s.element('p', {
                      children: [
                        s.element(tag, {
                          children: [s.text(str.is(rawText))]
                        }),
  ...
  TinyLoader.setupLight(
    function (editor, onSuccess, onFailure) {
      ...
      Pipeline.async(
        {},
        Log.steps('TBA', 'TextPattern: TrailingPunctuationTest', [
          tnt('em with ,', '*a*', ',', 'em', 'a'),
