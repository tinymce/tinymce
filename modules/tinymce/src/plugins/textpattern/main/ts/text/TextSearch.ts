...
const alwaysNext = (startNode: Node) => (node: Node) =>
  startNode === node ? -1 : 0;
...
const isBoundary = (dom: DOMUtils) => (node: Node) =>
  dom.isBlock(node) ||
  Arr.contains(['BR', 'IMG', 'HR', 'INPUT'], node.nodeName) ||
...
const textBefore = (
  ...
  if (isText(node) && offset >= 0) {
    ...
    ).map((prev) => Spot.point(prev.container, prev.container.data.length));
  }
};
...
const textAfter = (
  ...
  if (isText(node) && offset >= node.length) {
    ...
    ).map((prev) => Spot.point(prev.container, 0));
  }
};
...
const scanRight = (
  ...
  if (offset <= text.length) {
    ...
    return Option.from(
      textSeeker.forwards(node, offset, alwaysNext(node), rootNode)
    ).bind((next) => scanRight(next.container, offset - text.length, rootNode));
  }
};
...
export { repeatLeft, repeatRight, scanLeft, scanRight, textBefore, textAfter };
