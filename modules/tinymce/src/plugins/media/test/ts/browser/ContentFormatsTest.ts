...
UnitTest.asynctest('browser.tinymce.plugins.media.ContentFormatsTest', function (success, failure) {
  ...
  suite.test('TestCase-TBA: Media: Embed retain as is', function (editor) {
    editor.setContent('<embed src="320x240.ogg" width="100" height="200">text<a href="#">link</a></embed>');

    LegacyUnit.equal(editor.getContent(), '<p><embed src="320x240.ogg" width="100" height="200"></embed>text<a href="#">link</a></p>');
  });

  suite.test('TestCase-TBA: Media: Video retain as is', function (editor) {
    editor.setContent('<video src="320x240.ogg" autoplay loop controls>text<a href="#">link</a></video>');
  ...
  suite.test('TestCase-TBA: Media: Iframe retain as is', function (editor) {
    editor.setContent('<iframe src="320x240.ogg" allowfullscreen>text<a href="#">link</a></iframe>');
  ...
  suite.test('TestCase-TBA: Media: Media script elements', function (editor) {
    editor.setContent(
      '<script src="http://media1.tinymce.com/123456"></sc' + 'ript>' + '<script src="http://media2.tinymce.com/123456"></sc' + 'ript>'
    );
  ...
  suite.test('TestCase-TBA: Media: XSS content', function (editor) {
    ...
    testXss('<p><audio><noscript><svg onload="javascript:alert(1)"></svg></noscript></audio>', '<p><audio></audio></p>');
    testXss('<p><audio><script><svg onload="javascript:alert(1)"></svg></s' + 'cript></audio>', '<p><audio></audio></p>');
  });
