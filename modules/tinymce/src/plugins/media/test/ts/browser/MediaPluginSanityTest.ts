...
UnitTest.asynctest(
  'browser.tinymce.plugins.media.MediaPluginSanityTest',
      ...
      function (editor, onSuccess, onFailure) {
        ...
        Pipeline.async(
          ...
          [
            Log.stepsAsStep(
              'TBA',
              'Media: Embed content, open dialog, set size and assert constrained and unconstrained size recalculation',
              [
                apis.sSetContent(''),
            ...
            Log.stepsAsStep(
              'TBA',
              `Media: Test changing source, width and height doesn't delete other values`,
              [
                apis.sSetContent(''),
