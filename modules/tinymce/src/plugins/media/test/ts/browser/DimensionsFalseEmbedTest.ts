    ...
    function (editor, onSuccess, onFailure) {
      ...
      Pipeline.async(
        ...
        Log.steps('TINY-950', 'Media: Open dialog, set text area content, close dialog and assert content structure', [
          ...
          Utils.sPasteTextareaValue(tinyUi, '<iframe width="200" height="100" src="a" ' + ' frameborder="0" allowfullscreen></iframe>'),
          Utils.sSubmitDialog(tinyUi),
          Waiter.sTryUntil('content was not expected structure', tinyApis.sAssertContentStructure(struct))
        ]),
        onSuccess,
        onFailure
      );
