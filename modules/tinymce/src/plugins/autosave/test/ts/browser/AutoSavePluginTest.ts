...
UnitTest.asynctest(
  ...
  (success, failure) => {
    ...
    suite.test('TestCase-TBA: AutoSave: isEmpty true', function (editor) {
      ...
      LegacyUnit.equal(editor.plugins.autosave.isEmpty('<p><br /></p>'), true);
      LegacyUnit.equal(
        editor.plugins.autosave.isEmpty('<p><br data-mce-bogus="true" /></p>'),
        true
      );

      LegacyUnit.equal(
        editor.plugins.autosave.isEmpty('<p><br><br></p>'),
        true
      );
      LegacyUnit.equal(
        editor.plugins.autosave.isEmpty('<p><br /><br /></p>'),
      ...
      LegacyUnit.equal(
        editor.plugins.autosave.isEmpty(
          '<p><br data-mce-bogus="true" /><br data-mce-bogus="true" /></p>'
        ),
        true
      );
    });
    ...
    suite.test('TestCase-TBA: AutoSave: isEmpty false', function (editor) {
      ...
      LegacyUnit.equal(
        editor.plugins.autosave.isEmpty('<p><br />X</p>'),
        false
      );
      LegacyUnit.equal(
        editor.plugins.autosave.isEmpty('<p><br data-mce-bogus="true" />X</p>'),
        false
      );

      LegacyUnit.equal(
        editor.plugins.autosave.isEmpty('<p><br><br>X</p>'),
        false
      );
      LegacyUnit.equal(
        editor.plugins.autosave.isEmpty('<p><br /><br />X</p>'),
      ...
      LegacyUnit.equal(
        editor.plugins.autosave.isEmpty(
          '<p><br data-mce-bogus="true" /><br data-mce-bogus="true" />X</p>'
        ),
        false
      );
      ...
      LegacyUnit.equal(
        editor.plugins.autosave.isEmpty('<img src="x" />'),
        false
      );
    });

    suite.test(
      'TestCase-TBA: AutoSave: hasDraft/storeDraft/restoreDraft',
      function (editor) {
    ...
    suite.test(
      'TestCase-TBA: AutoSave: recognises location hash change',
      function (editor) {
        LegacyUnit.equal(editor.plugins.autosave.hasDraft(), false);
    ...
    TinyLoader.setupLight(
      function (editor, onSuccess, onFailure) {
        Pipeline.async(
          ...
          Log.steps(
            'TBA',
            'AutoSave: Test autosave isEmpty true/false, drafts and location hash change',
