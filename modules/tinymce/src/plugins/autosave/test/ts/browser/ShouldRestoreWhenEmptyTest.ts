...
UnitTest.asynctest(
  'browser.tinymce.plugins.autosave.ShouldRestoreWhenEmptyTest',
  (success, failure) => {
    Theme();
    Plugin();

    const cAssertHasDraft = (expected: boolean) =>
      Chain.op((editor: Editor) => {
        Assert.eq(
          `should${!expected ? "n't" : ''} have draft`,
          expected,
    ...
    Pipeline.async(
      ...
      [
        Logger.t(
          'should restore draft when empty with setting',
          Chain.asStep({}, [
        ...
        Logger.t(
          `shouldn't restore draft when empty without setting`,
          Chain.asStep({}, [
            McEditor.cFromSettings({
              base_url: '/project/tinymce/js/tinymce',
              plugins: 'autosave',
