...
UnitTest.asynctest(
  'browser.tinymce.plugins.quickbars.SelectionToolbarTest',
  (success, failure) => {
    const imgSrc =
        ...
        Chain.control(
          Chain.fromChains([
            ...
            Assertions.cAssertStructure(
              `Check ${name} button is ${state ? 'active' : 'inactive'}`,
              ApproxStructure.build((s, str, arr) =>
                s.element('button', {
                  classes: [
                    state
                      ? arr.has('tox-tbtn--enabled')
                      : arr.not('tox-tbtn--enabled')
                  ]
                })
              )
            )
    ...
    const sWaitForTextToolbarAndAssertState = (
      tinyUi: TinyUi,
      bold: boolean,
      italic: boolean,
      heading2: boolean,
      ...
      GeneralSteps.sequence([
        tinyUi.sWaitForUi(
          'wait for text selection toolbar to show',
          '.tox-toolbar'
        ),
    ...
    const sSetImageAndAssertToolbarState = (
      tinyApis: TinyApis,
      tinyUi: TinyUi,
      useFigure: boolean,
      alignment?: Alignment
      ...
      return GeneralSteps.sequence([
        tinyApis.sSetContent(
          '<p>Some <strong>bold</strong> and <em>italic</em> content.</p>' +
            imageHtml
        ),
        tinyApis.sSelect(useFigure ? 'figure' : 'img', []),
        tinyUi.sWaitForUi(
          'wait for image selection toolbar to show',
          '.tox-toolbar'
        ),
        sAssertButtonToggledState('Align left', alignment === Alignment.Left),
        sAssertButtonToggledState(
          'Align center',
        ...
        Pipeline.async(
          ...
          [
            ...
            Log.stepsAsStep('TBA', 'Text selection toolbar', [
              tinyApis.sSetContent(
                '<p>Some <strong>bold</strong> and <em>italic</em> content.</p><blockquote><p>Some quoted content</p></blockquote>'
              ),
              tinyApis.sSetSelection([0, 0], 0, [0, 0], 4),
              sWaitForTextToolbarAndAssertState(
                tinyUi,
                false,
                false,
                false,
            ...
            Log.stepsAsStep('TBA', 'Image selection toolbar', [
              ...
              sSetImageAndAssertToolbarState(
                tinyApis,
                tinyUi,
                false,
                Alignment.Left
