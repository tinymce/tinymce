  ...
  (success, failure) => {
    ...
    const sWaitForVisualBlocks = function (editor) {
      return Waiter.sTryUntil(
        'Wait for background css to be applied to first element',
        Step.sync(function () {
          const p = Element.fromDom(editor.getBody().firstChild);
          const background = Css.get(p, 'background-image');
          Assertions.assertEq(
            'Paragraph should have a url background',
            true,
    ...
    TinyLoader.setupLight(
      function (editor, onSuccess, onFailure) {
        ...
        Pipeline.async(
          ...
          Log.steps(
            'TBA',
            'VisualBlocks: Toggle on/off visualblocks and compute previews',
            [
              tinyApis.sExecCommand('mceVisualBlocks'),
              sWaitForVisualBlocks(editor),
              Step.sync(function () {
                Assertions.assertEq(
                  'Visual blocks class should exist',
                  true,
                  Class.has(
                ...
                Assertions.assertEq(
                  'Should not have a border property',
                  true,
                  editor.formatter
                    .getCssText('h1')
                    .indexOf('border:1px dashed') === -1
                );
              }),
              tinyApis.sExecCommand('mceVisualBlocks'),
              Step.sync(function () {
                Assertions.assertEq(
                  'Visual blocks class should not exist',
                  false,
                  Class.has(
                    Element.fromDom(editor.getBody()),
                    'mce-visualblocks'
                ...
                Assertions.assertEq(
                  'Visual blocks class should still not exist',
                  false,
                  Class.has(
                    Element.fromDom(editor.getBody()),
                    'mce-visualblocks'
