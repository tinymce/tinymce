...
UnitTest.asynctest(
  'browser.tinymce.plugins.preview.PreviewContentCssTest',
  (success, failure) => {
    PreviewPlugin();
    SilverTheme();
    ...
    const sAssertIframeHtmlContains = (editor: Editor, text: string) =>
      Logger.t(
        'Assert Iframe Html contains ' + text,
        Step.sync(() => {
    ...
    TinyLoader.setupLight(
      (editor, onSuccess, onFailure) => {
        ...
        const contentCssUrl = editor.documentBaseURI.toAbsolute(
          '/project/tinymce/js/tinymce/skins/content/default/content.css'
        );
        ...
        Pipeline.async(
          ...
          Log.steps(
            'TBA',
            'Preview: Set content, set content_css_cors and assert link elements. Delete setting and assert crossOrigin attr is removed',
            [
              tinyApis.sSetContent('<p>hello world</p>'),
              tinyApis.sSetSetting('content_css_cors', true),
              sAssertIframeHtmlContains(
                editor,
                `<link type="text/css" rel="stylesheet" href="${contentCssUrl}" crossorigin="anonymous">`
              ),
              tinyApis.sSetSetting('content_css_cors', false),
              sAssertIframeHtmlContains(
                editor,
                `<link type="text/css" rel="stylesheet" href="${contentCssUrl}">`
              )
            ]
          ),
          onSuccess,
