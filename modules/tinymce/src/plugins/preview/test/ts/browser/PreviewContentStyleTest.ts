...
UnitTest.asynctest(
  ...
  (success, failure) => {
    ...
    const sAssertIframeContains = (editor, text, expected) =>
      Step.sync(() => {
    ...
    const sAssertIframeHtmlContains = function (editor, text) {
      return Logger.t(
        ...
        sAssertIframeContains(editor, text, true)
      );
    };

    const sAssertIframeHtmlNotContains = function (editor, text) {
      return Logger.t(
        'Assert Iframe Html does not contain ' + text,
        sAssertIframeContains(editor, text, false)
      );
    };
    ...
    TinyLoader.setupLight(
      function (editor, onSuccess, onFailure) {
        ...
        Pipeline.async(
          ...
          Log.steps(
            'TBA',
            'Preview: Set content, set style setting and assert content and style. Delete style and assert style is removed',
            [
