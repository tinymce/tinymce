...
UnitTest.asynctest('Custom Help Tabs test', (success, failure) => {
  ...
  Pipeline.async(
    ...
    [
      ...
      Logger.t(
        'Test help_tabs with pre-registered and new tabs',
        makeStep(
          {
            plugins: 'help',
            toolbar: 'help',
      ...
      Logger.t(
        'Test things do not break if a tab name does not have a spec',
        makeStep(
          {
            plugins: 'help',
            toolbar: 'help',
            theme: 'silver',
            base_url: '/project/tinymce/js/tinymce',
            help_tabs: ['shortcuts', 'plugins', 'versions', 'unknown']
          },
