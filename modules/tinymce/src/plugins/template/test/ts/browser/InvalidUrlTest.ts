...
UnitTest.asynctest(
  'browser.tinymce.plugins.template.InvalidUrlTest',
  (success, failure) => {
    TemplatePlugin();
    SilverTheme();
      ...
      (editor, onSuccess, onFailure) => {
        ...
        Pipeline.async(
          ...
          [
            Log.stepsAsStep(
              'TBA',
              'Template: Test loading in snippet from file that does not exist',
              [
                tinyApis.sSetContent(''),
                tinyApis.sSetSetting('templates', [
                  { title: 'invalid', description: 'b', url: '/custom/404' },
                  {
                    title: 'a',
                    description: 'a',
                ...
                tinyUi.sWaitForPopup(
                  'Waited for alert dialog to be visible',
                  alertDialogSelector
                ),
                tinyUi.sClickOnUi(
                  'Click on OK button',
                ...
                tinyUi.sClickOnUi(
                  ...
                  'button.tox-button:contains(Save)'
                ),
                Waiter.sTryUntil(
