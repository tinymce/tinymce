        ...
        Waiter.sTryUntil(
          ...
          Step.sync(function () {
            Assertions.assertEq(
              ...
              'uppercase',
              editor.dom.getStyle(editor.getBody().firstChild, 'text-transform', true)
            );
            Assertions.assertEq(
              'Styles not added to actual element',
              '',
              editor.dom.getStyle(editor.getBody().firstChild, 'text-transform', false)
            );
          }),
          10,
          3000
    ...
    function (editor, onSuccess, onFailure) {
      Pipeline.async(
        ...
        Log.steps(
          ...
          [sParseStyles(editor), sProtectConditionalCommentsInHeadFoot(editor)].concat(suite.toSteps(editor))
        ),
        onSuccess,
        onFailure
      );
