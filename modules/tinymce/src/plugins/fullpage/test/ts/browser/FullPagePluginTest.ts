...
UnitTest.asynctest(
  ...
  function (success, failure) {
    ...
    suite.test('TestCase-TBA: FullPage: Keep header/footer intact', function (
      editor
    ) {
      ...
      LegacyUnit.equal(
        normalizeHTML(editor.getContent()),
        '<html><body><p>Test</p>',
      ...
      LegacyUnit.equal(
        ...
        'Header/footer is intact.'
      );
    });

    suite.test('TestCase-TBA: FullPage: Default header/footer', function (
      editor
    ) {
    ...
    suite.test('TestCase-TBA: FullPage: Parse body attributes', function (
      editor
    ) {
      editor.setContent('<html><body><p>Test</p></body></html>');
      ...
      LegacyUnit.equal(
        editor.dom.getStyle(editor.getBody().firstChild, 'display', true),
        'block',
        'No styles added to iframe document'
      );

      editor.setContent(
        '<html><body style="color:#FF0000"><p>Test</p></body></html>'
      );
      LegacyUnit.equal(
        editor.getBody().style.color.length > 0,
        true,
        'Color added to body'
      );
    ...
    suite.test(
      'TestCase-TBA: FullPage: fullpage_hide_in_source_view: false',
      function (editor) {
    ...
    suite.test('TestCase-TBA: FullPage: link elements', function (editor) {
      editor.setContent(
        '<html><head><link rel="stylesheet" href="a.css"><link rel="something"></head><body><p>c</p></body></html>'
      );
      LegacyUnit.equal(
        editor.getContent(),
    ...
    suite.test('TestCase-TBA: FullPage: add/remove stylesheets', function (
      editor
    ) {
      ...
      editor.setContent(
        '<html><head><link rel="stylesheet" href="a.css"></head><body><p>c</p></body></html>'
      );
    ...
    const sParseStyles = function (editor) {
      return Logger.t(
        ...
        GeneralSteps.sequence([
          Step.sync(function () {
            editor.setContent(
              '<html><head><style>p {text-transform: uppercase}</style></head><body dir="rtl"><p>Test</p></body></html>'
            );
          }),
          Waiter.sTryUntil(
            ...
            Step.sync(function () {
              Assertions.assertEq(
                'Styles added to iframe document',
                'uppercase',
                editor.dom.getStyle(
              ...
              Assertions.assertEq(
                'Styles not added to actual element',
                '',
                editor.dom.getStyle(
                  editor.getBody().firstChild,
                  'text-transform',
    ...
    const sProtectConditionalCommentsInHeadFoot = function (editor) {
      return Logger.t(
        'Set and assert styles were added to iframe document',
        GeneralSteps.sequence([
          Step.sync(function () {
            editor.setContent(
              [
    ...
    TinyLoader.setupLight(
      function (editor, onSuccess, onFailure) {
        Pipeline.async(
          ...
          Log.steps(
            'TBA',
            'FullPage: Test full page header, footer, body attributes, hide in source view and adding and removing stylesheets',
            [
              sParseStyles(editor),
