...
UnitTest.asynctest(
  'browser.tinymce.plugins.charmap.CharmapUserDefinedTest',
  (success, failure) => {
    CharmapPlugin();
    SilverTheme();
        ...
        Pipeline.async(
          ...
          Log.steps('TBA', 'Charmap: User defined charmap', [
            ...
            FocusTools.sTryOnSelector(
              'Focus should have moved to input',
              doc,
              'input'
            ),
            ...
            tinyUi.sWaitForUi(
              'wait for character A',
              '.tox-collection .tox-collection__item-icon:contains(A)'
            )
          ]),
          onSuccess,
          onFailure
