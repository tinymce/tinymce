...
UnitTest.asynctest(
  ...
  (success, failure) => {
    ...
    suite.test('TestCase-TBA: Charmap: Replace characters by array', function (
      editor
    ) {
    ...
    suite.test(
      'TestCase-TBA: Charmap: Replace characters by function',
      function (editor) {
    ...
    suite.test('TestCase-TBA: Charmap: Append characters by array', function (
      editor
    ) {
      editor.settings.charmap = [[67, 'Latin C']];
    ...
    suite.test(
      'TestCase-TBA: Charmap: Append characters by function',
      function (editor) {
        editor.settings.charmap = [[67, 'Latin C']];
    ...
    TinyLoader.setupLight(
      function (editor, onSuccess, onFailure) {
        Pipeline.async(
          ...
          Log.steps(
            'TBA',
            'Charmap: Test replacing, appending and inserting characters',
