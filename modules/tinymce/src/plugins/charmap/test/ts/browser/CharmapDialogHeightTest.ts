...
UnitTest.asynctest('browser.tinymce.plugins.charmap.DialogHeightTest', (success, failure) => {
  ...
  const cTabPanelHeight = Chain.binder<Element, string, string>((tabpanel) =>
    Css.getRaw(tabpanel, 'height').fold(() => Result.error('tabpanel has no height'), Result.value)
  );
        ...
        Log.steps('TBA', 'Charmap: Search for items, dialog height should not change when fewer items returned', [
          ...
          Chain.asStep(Body.body(), [
            NamedChain.asChain([
              ...
              NamedChain.direct(
                'body',
                UiFinder.cWaitForState('wait until ', '[role="dialog"] .tox-collection__group', (e) => Traverse.childNodesCount(e) === 0),
                '_'
              ),
