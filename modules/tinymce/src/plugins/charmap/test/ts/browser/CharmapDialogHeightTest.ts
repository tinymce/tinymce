...
UnitTest.asynctest(
  'browser.tinymce.plugins.charmap.DialogHeightTest',
  (success, failure) => {
    CharmapPlugin();
    SilverTheme();
    ...
    const cTabPanelHeight = Chain.binder<Element, string, string>((tabpanel) =>
      Css.getRaw(tabpanel, 'height').fold(
        () => Result.error('tabpanel has no height'),
        Result.value
      )
    );
    ...
    TinyLoader.setupLight(
      function (editor, onSuccess, onFailure) {
        ...
        Pipeline.async(
          ...
          Log.steps(
            'TBA',
            'Charmap: Search for items, dialog height should not change when fewer items returned',
            [
              tinyApis.sFocus(),
              tinyUi.sClickOnToolbar(
                'click charmap',
                'button[aria-label="Special character"]'
              ),
              Chain.asStep({}, [
              ...
              Chain.asStep(Body.body(), [
                NamedChain.asChain([
                  ...
                  NamedChain.direct(
                    'body',
                    UiFinder.cFindIn('[role="dialog"] [role="tabpanel"]'),
                    'tabpanel'
                  ),
                  ...
                  NamedChain.direct(
                    ...
                    UiFinder.cWaitForState(
                      'wait until ',
                      '[role="dialog"] .tox-collection__group',
                      (e) => Traverse.childNodesCount(e) === 0
                    ),
                    '_'
                  ),
                  ...
                  NamedChain.bundle((bindings) =>
                    ...
                    parseInt(bindings.oldheight, 10) !==
                    parseInt(bindings.newheight, 10)
                      ? Result.error(
                          `Old height and new height differ. Old height: '${bindings.oldheight}' new height '${bindings.newheight}'`
                        )
                      : Result.value({})
                  )
                ])
