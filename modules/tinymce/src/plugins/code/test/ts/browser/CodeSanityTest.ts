...
UnitTest.asynctest(
  ...
  (success, failure) => {
    ...
    const toolbarButtonSelector =
      '[role="toolbar"] button[aria-label="Source code"]';
    ...
    TinyLoader.setupLight(
      (editor: Editor, onSuccess, onFailure) => {
        ...
        const sAssertTextareaContent = (expected) =>
          Logger.t(
            'Asserting textarea content is ' + expected,
            Step.sync(() => {
              const textarea: any = document.querySelector(
                'div[role="dialog"] textarea'
              );
        ...
        const sSubmitDialog = (docBody) =>
          GeneralSteps.sequence(
            Logger.ts('Clicking on the Save button to close dialog', [
        ...
        Pipeline.async(
          ...
          [
            Log.stepsAsStep(
              'TBA',
              'Code: Set content in empty editor and assert values',
              [
                Mouse.sClickOn(
            ...
            Log.stepsAsStep(
              'TBA',
              'Code: Reopen dialog and check textarea content is correct',
              [
                Mouse.sClickOn(
                  Element.fromDom(editor.getContainer()),
                  toolbarButtonSelector
            ...
            Log.stepsAsStep(
              'TBA',
              'Code: Change source code and assert editor content changes',
              [
