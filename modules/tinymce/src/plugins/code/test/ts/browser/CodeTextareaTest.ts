...
UnitTest.asynctest(
  'browser.tinymce.plugins.code.CodeTextareaTest',
  (success, failure) => {
    CodePlugin();
    SilverTheme();
      ...
      (editor: Editor, onSuccess, onFailure) => {
        ...
        const cAssertWhiteSpace = () =>
          NamedChain.asChain([
            ...
            NamedChain.read(
              ...
              Chain.op((whitespace) => {
                Assertions.assertEq(
                  'Textarea should have "white-space: pre-wrap"',
                  'pre-wrap',
                  whitespace
                );
              })
        ...
        Pipeline.async(
          ...
          [
            Log.stepsAsStep(
              'TBA',
              'Code: Verify if "white-space: pre-wrap" style is set on the textarea',
              [sAssertStyleExits]
            )
          ],
          onSuccess,
