...
UnitTest.asynctest('browser.tinymce.plugins.code.CodeTextareaTest', (success, failure) => {
  ...
  TinyLoader.setupLight(
    (editor: Editor, onSuccess, onFailure) => {
      ...
      const sAssertStyleExits = Chain.asStep({ editor }, [cAssertWhiteSpace(), UiChains.cCloseDialog('div[role="dialog"]')]);

      // Test verifies that new lines and whitespaces are allowed within the textarea in Firefox and IE
      Pipeline.async(
        {},
        [Log.stepsAsStep('TBA', 'Code: Verify if "white-space: pre-wrap" style is set on the textarea', [sAssertStyleExits])],
        onSuccess,
        onFailure
      );
    },
