...
const sOpenLinkDialog = (ui: TinyUi) =>
  Logger.t(
    'Open link dialog',
...
const sClickOnDialog = (label: string, selector: string) =>
  Logger.t(
    'Click on dialog',
    GeneralSteps.sequence([
      UiFinder.sWaitForVisible(
        'Waiting for item to appear',
        TinyDom.fromDom(document.body),
...
const sClickOnConfirmDialog = (label: string, state: boolean) =>
  Logger.t(
...
const sWaitForConfirmClose = Logger.t(
...
const sSetHtmlSelectValue = (group: string, newValue) =>
  Logger.t(
