import { Pipeline, UiFinder, FocusTools, Log, GeneralSteps } from '@ephox/agar';
  ...
  TinyLoader.setupLight(
    (editor, onSuccess, onFailure) => {
      ...
      Pipeline.async(
        {},
        [
          Log.stepsAsStep('TBA', 'Test regex for non relative ftp link', [testNoProtocolConfirm('ftp://testftp.com')]),
          ...
          Log.stepsAsStep('TBA', 'Test regex for non relative link with no protocol', [testProtocolConfirm('www.http.com', 'http://')]),
          Log.stepsAsStep('TBA', 'Test regex for relative link', [testNoProtocolConfirm('test.jpg')]),
          Log.stepsAsStep('TBA', 'Test regex for anchor link', [testNoProtocolConfirm('#test')]),
          Log.stepsAsStep('TBA', 'Test regex for email link with mailto:', [testNoProtocolConfirm('mailto:no-reply@example.com')]),
          Log.stepsAsStep('TBA', 'Test regex for email link', [testProtocolConfirm('no-reply@example.com', 'mailto:')])
        ],
        onSuccess,
        onFailure
      );
