    ...
    TinyLoader.setupLight(
      function (editor, onSuccess, onFailure) {
        ...
        const sLinkTheSelection = function () {
          return Logger.t(
            'Link the selection',
            TestLinkUi.sInsertLink(ui, 'http://google.com')
          );
        };
        ...
        Pipeline.async(
          ...
          Log.steps(
            'TBA',
            'Link: Set content, select and link the selection, assert link is present. Then select and unlink the selection, assert link is not present',
            [
              ...
              sLinkTheSelection(),
              sAssertPresence({
                'figure.image > a[href="http://google.com"] > img': 1
              }),
