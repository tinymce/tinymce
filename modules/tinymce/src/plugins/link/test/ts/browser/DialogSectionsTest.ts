...
UnitTest.asynctest(
  'browser.tinymce.plugins.link.DialogSectionsTest',
  (success, failure) => {
    LinkPlugin();
    SilverTheme();
    ...
    TinyLoader.setupLight(
      function (editor, onSuccess, onFailure) {
          ...
          Log.stepsAsStep('TBA', 'Link: Settings: ' + getStr(sections), [
            GeneralSteps.sequence(
              Arr.map(sections, ({ setting }) =>
                setting.value
                  .map((v) => tinyApis.sSetSetting(setting.key, v))
                  .getOrThunk(() => tinyApis.sDeleteSetting(setting.key))
              )
            ),
        ...
        const sCheckRelSection = (
          exists: boolean,
          value: Option<Array<{ value: string; title: string }>>
        ) =>
          Log.step(
            'TBA',
