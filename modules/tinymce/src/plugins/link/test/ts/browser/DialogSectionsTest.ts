  ...
  TinyLoader.setupLight(
    function (editor, onSuccess, onFailure) {
      ...
      const sCheckSections = (sections: TestSection[]) =>
        Log.stepsAsStep('TBA', 'Link: Settings: ' + getStr(sections), [
          GeneralSteps.sequence(
            Arr.map(sections, ({ setting }) =>
              setting.value.map((v) => tinyApis.sSetSetting(setting.key, v)).getOrThunk(() => tinyApis.sDeleteSetting(setting.key))
            )
          ),
          TestLinkUi.sOpenLinkDialog(tinyUi),
          GeneralSteps.sequence(
            Arr.map(sections, ({ selector, exists }) => {
      ...
      const sCheckTargetSection = (exists: boolean, value: Option<boolean>) =>
        Log.step(
          'TBA',
          'Link: sCheckTargetSection',
          sCheckSections([
            {
      ...
      const sCheckTitleSection = (exists: boolean, value: Option<boolean>) =>
        Log.step(
          'TBA',
          'Link: sCheckTitleSection',
          sCheckSections([
            {
      ...
      const sCheckRelSection = (exists: boolean, value: Option<Array<{ value: string; title: string }>>) =>
        Log.step(
          'TBA',
          'Link: sCheckRelSection',
          sCheckSections([
            {
      ...
      const sCheckClassSection = (exists: boolean, value: Option<Array<{ value: string; title: string }>>) =>
        Log.step(
          'TBA',
          'Link: sCheckClassSection',
          sCheckSections([
            {
      ...
      const sCheckLinkListSection = (exists: boolean, value: Option<Array<{ value: string; title: string }>>) =>
        Log.step(
          'TBA',
          'Link: sCheckLinkListSection',
          sCheckSections([
            {
