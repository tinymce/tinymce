...
UnitTest.asynctest(
  'browser.tinymce.plugins.link.DefaultLinkTargetTest',
  (success, failure) => {
    LinkPlugin();
    SilverTheme();
      ...
      function (editor, onSuccess, onFailure) {
        ...
        Pipeline.async(
          ...
          [
            ...
            Log.stepsAsStep(
              'TBA',
              'Link: does not add target if no default is set',
              [
            ...
            Log.stepsAsStep(
              'TBA',
              'Link: adds target if default is set and target_list is enabled',
              [
                tinyApis.sSetSetting('default_link_target', '_blank'),
            ...
            Log.stepsAsStep(
              'TBA',
              'Link: adds target if default is set and target_list is disabled',
              [
                tinyApis.sSetSetting('default_link_target', '_blank'),
            ...
            Log.stepsAsStep(
              'TBA',
              `Link: changing to current window doesn't apply the default`,
              [
                tinyApis.sSetSetting('default_link_target', '_blank'),
                TestLinkUi.sInsertLink(tinyUi, 'http://www.google.com'),
                TestLinkUi.sAssertContentPresence(tinyApis, {
                ...
                TestLinkUi.sSetHtmlSelectValue(
                  'Open link in...',
                  'Current Window'
                ),
                TestLinkUi.sClickSave,
                TestLinkUi.sAssertContentPresence(tinyApis, {
                  'a:not([target="_blank"])': 1,
                  'a': 1
                }),
                tinyApis.sSetContent('')
              ]
            ...
            Log.stepsAsStep(
              'TBA',
              `Link: default isn't applied to an existing link`,
              [
                tinyApis.sSetSetting('default_link_target', '_blank'),
                tinyApis.sSetContent(
                  '<a href="http://www.google.com">https://www.google.com/</a>'
                ),
                TestLinkUi.sAssertContentPresence(tinyApis, {
                  'a:not([target="_blank"])': 1,
                  'a': 1
