...
UnitTest.asynctest(
  'browser.tinymce.plugins.link.QuickLinkTest',
  (success, failure) => {
    SilverTheme();
    LinkPlugin();
      ...
      function (editor, onSuccess, onFailure) {
        ...
        const sOpenQuickLink = GeneralSteps.sequence([
          ...
          FocusTools.sTryOnSelector(
            'Selector should be in contextform input',
            doc,
            '.tox-toolbar input'
          )
        ]);
        ...
        Pipeline.async(
          ...
          [
            ...
            Log.stepsAsStep(
              'TBA',
              'Checking that QuickLink can insert a link',
              [
                sOpenQuickLink,
            ...
            Log.stepsAsStep(
              'TBA',
              'Checking that QuickLink can add a link to selected text and keep the current text',
              [
            ...
            Log.stepsAsStep(
              'TBA',
              'Checking that QuickLink can add a link to a selected image and keep the current image',
              [
            ...
            Log.stepsAsStep(
              'TBA',
              'Checking that QuickLink can edit an existing link',
              [
                tinyApis.sSetContent(
                  '<p><a href="http://tiny.cloud/3">Word</a></p>'
                ),
                tinyApis.sSetSelection(
                  [0, 0, 0],
                  'W'.length,
            ...
            Log.stepsAsStep(
              'TBA',
              'Checking that QuickLink can remove an existing link',
              [
                tinyApis.sSetContent(
                  '<p><a href="http://tiny.cloud/4">Word</a></p>'
                ),
                tinyApis.sSetSelection(
                  [0, 0, 0],
                  'Wor'.length,
