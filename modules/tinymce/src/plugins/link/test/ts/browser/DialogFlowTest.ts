...
UnitTest.asynctest(
  'browser.tinymce.plugins.link.DialogFlowTest',
  (success, failure) => {
    LinkPlugin();
    SilverTheme();
    ...
    TinyLoader.setupLight(
      function (editor, onSuccess, onFailure) {
        ...
        const sAssertInputValue = (expected: string, group: string) =>
          Logger.t(
            'Assert input value',
            Chain.asStep({}, [
              TestLinkUi.cFindInDialog(
                'label:contains("' + group + '") + input'
              ),
        ...
        // FIX: Dupe
        const sAssertUrlStructure = (expected: (s, str, arr) => any) =>
          Logger.t(
            'Assert url structure',
            Chain.asStep({}, [
              TestLinkUi.cFindInDialog(
                'label:contains("URL") + .tox-form__controls-h-stack input'
              ),
              Chain.op((urlinput) => {
        ...
        const testChangingAnchorValue = Log.stepsAsStep(
          'TBA',
          'Link: Switching anchor changes the href and text',
          [
            tinyApis.sSetContent(
              '<p><a name="anchor1"></a>Our Anchor1</p><p><a name="anchor2"></a>Our Anchor2</p>'
            ),
        ...
        const testChangingUrlValueWith = (sChooseItem: Step<any, any>) =>
          Log.stepsAsStep(
            'TBA',
            'Link: Choosing something in the urlinput changes text and value',
            [
              tinyApis.sSetContent(
                '<h1>Header One</h1><h2 id="existing-id">Header2</h2>'
              ),
              TestLinkUi.sOpenLinkDialog(tinyUi),
              ...
              Chain.asStep(TinyDom.fromDom(editor.getBody()), [
                NamedChain.asChain([
                  ...
                  NamedChain.direct(
                    'h1',
                    Chain.mapper((h1) => Attr.get(h1, 'id')),
                    'h1-id'
                  ),
                  NamedChain.bundle((obj) =>
                    UiFinder.findIn(
                      obj[NamedChain.inputName()],
                      `a[href="#${obj['h1-id']}"]:contains("Header One")`
                    )
                  )
                ])
              ])
        ...
        const testChangingUrlValueWithKeyboard = Log.step(
          'TBA',
          'Link: With Keyboard',
          testChangingUrlValueWith(
            GeneralSteps.sequence([Keyboard.sKeydown(doc, Keys.enter(), {})])
          )
        );

        const testChangingUrlValueWithMouse = Log.step(
          'TBA',
          'Link: With Mouse',
          testChangingUrlValueWith(
            Mouse.sClickOn(
        ...
        const testChangingUrlValueManually = Log.stepsAsStep(
          'TBA',
          'Link: Change urlinput value manually',
          [
