  ...
  TinyLoader.setupLight(
    function (editor, onSuccess, onFailure) {
      ...
      const sAssertUrlStructure = (expected: (s, str, arr) => any) =>
        Logger.t(
          ...
          Chain.asStep({}, [
            ...
            Chain.op((urlinput) => {
              Assertions.assertStructure('Checking content of url input', ApproxStructure.build(expected), urlinput);
            })
          ])
        );
      ...
      const testChangingUrlValueWith = (sChooseItem: Step<any, any>) =>
        Log.stepsAsStep('TBA', 'Link: Choosing something in the urlinput changes text and value', [
      ...
      const testChangingUrlValueWithKeyboard = Log.step(
        'TBA',
        'Link: With Keyboard',
        testChangingUrlValueWith(GeneralSteps.sequence([Keyboard.sKeydown(doc, Keys.enter(), {})]))
      );
