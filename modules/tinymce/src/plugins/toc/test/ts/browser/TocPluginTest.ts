...
UnitTest.asynctest(
  ...
  (success, failure) => {
    ...
    suite.test('TestCase-TBA: TableOfContents: mceInsertToc', function (
      editor
    ) {
      ...
      LegacyUnit.equal(
        $toc.find('ul ul ul').length,
        0,
        'no levels beyond 2 are included'
      );

      stripAttribs($toc, ['data-mce-href', 'data-mce-selected']);

      LegacyUnit.equal(
        trimBr(HtmlUtils.normalizeHtml($toc[0].outerHTML)),
    ...
    suite.test(
      'TestCase-TBA: TableOfContents: mceInsertToc - flat structure',
      function (editor) {
        editor.getBody().innerHTML =
          '<h1 id="h1">H1</h1>' +
          '<p>This is some text.</p><br />' +
        ...
        LegacyUnit.equal(
          trimBr(HtmlUtils.normalizeHtml($toc[0].innerHTML)),
          '<h3 contenteditable="true">Table of Contents</h3>' +
            '<ul>' +
            '<li>' +
            '<a href="#h1">H1</a>' +
    ...
    suite.test('TestCase-TBA: TableOfContents: mceUpdateToc', function (
      editor
    ) {
      editor.getBody().innerHTML =
        '<h1 id="h1">H1</h1>' +
      ...
      LegacyUnit.equal(
        editor.$('.tst-toc > ul a[href="#h5"]').length,
        1,
    ...
    suite.test('TestCase-TBA: TableOfContents: Misc.', function (editor) {
      ...
      LegacyUnit.equal(
        /contenteditable/i.test(contents),
        false,
        'cE stripped for getContent()'
      );
      ...
      LegacyUnit.deepEqual(
        $toc.attr('contentEditable'),
        'false',
        'cE added back after setContent()'
      );
      LegacyUnit.deepEqual(
        $toc.find(':first-child').attr('contentEditable'),
        'true',
    ...
    TinyLoader.setupLight(
      function (editor, onSuccess, onFailure) {
        Pipeline.async(
          ...
          Log.steps(
            'TBA',
            'TableOfContents: Api insert, update and test table of contents',
