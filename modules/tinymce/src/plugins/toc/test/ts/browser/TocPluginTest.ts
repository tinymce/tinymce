...
UnitTest.asynctest('browser.tinymce.plugins.toc.TocPluginTest', (success, failure) => {
  ...
  suite.test('TestCase-TBA: TableOfContents: mceUpdateToc', function (editor) {
    ...
    LegacyUnit.equal(editor.$('.tst-toc > ul a[href="#h5"]').length, 1, 'ToC has been successfully updated');
  });
  ...
  suite.test('TestCase-TBA: TableOfContents: Misc.', function (editor) {
    ...
    LegacyUnit.deepEqual($toc.find(':first-child').attr('contentEditable'), 'true', 'cE added back to title after setContent()');
  ...
  TinyLoader.setupLight(
    function (editor, onSuccess, onFailure) {
      Pipeline.async(
        {},
        Log.steps('TBA', 'TableOfContents: Api insert, update and test table of contents', suite.toSteps(editor)),
        onSuccess,
        onFailure
      );
    },
