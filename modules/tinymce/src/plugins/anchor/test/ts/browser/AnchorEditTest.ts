...
UnitTest.asynctest(
  'browser.tinymce.plugins.anchor.AnchorEditTest',
  (success, failure) => {
    AnchorPlugin();
    Theme();
    ...
    TinyLoader.setupLight(
      (editor, onSuccess, onFailure) => {
        ...
        Pipeline.async(
          ...
          Log.steps(
            'TBA',
            'Anchor: Add anchor, change anchor, undo anchor change then the anchor should be there as first entered',
            [
              tinyApis.sFocus(),
              ...
              tinyUi.sWaitForUi(
                'Anchor toolbar button is highlighted',
                'button[aria-label="Anchor"][aria-pressed="true"]'
              ),
              tinyApis.sExecCommand('mceAnchor'),
              tinyUi.sWaitForPopup(
                'wait for window',
              ...
              tinyApis.sAssertContentPresence({ 'a.mce-item-anchor#abc': 1 }),
              tinyUi.sWaitForUi(
                'Anchor toolbar button is not highlighted',
                'button[aria-label="Anchor"][aria-pressed="false"]'
              )
            ]
          ),
          onSuccess,
