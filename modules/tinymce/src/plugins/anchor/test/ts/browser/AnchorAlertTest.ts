...
UnitTest.asynctest(
  'browser.tinymce.plugins.anchor.AnchorAlertTest',
  (success, failure) => {
    AnchorPlugin();
    Theme();
    ...
    TinyLoader.setupLight(
      (editor, onSuccess, onFailure) => {
        ...
        const alertDialogSelector =
          'div[role="dialog"].tox-dialog.tox-alert-dialog';

        Pipeline.async(
          {},
          Log.steps(
            'TINY-2788',
            'Anchor: Add anchor with invalid id, check alert appears',
            [
              ...
              tinyUi.sClickOnUi(
                'Click on OK button',
                'button.tox-button:contains(OK)'
              ),
              Waiter.sTryUntil(
                'Alert dialog should close',
                UiFinder.sNotExists(docBody, alertDialogSelector)
              ),
              Waiter.sTryUntil(
                'Anchor Dialog should not close',
                UiFinder.sExists(docBody, dialogSelector)
              ),
              tinyUi.sClickOnUi(
                'Click on Cancel button',
                'button.tox-button:contains(Cancel)'
              ),
              Waiter.sTryUntil(
                'Anchor Dialog should close',
