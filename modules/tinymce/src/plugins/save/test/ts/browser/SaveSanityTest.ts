    ...
    TinyLoader.setupLight(
      function (editor, onSuccess, onFailure) {
        ...
        Pipeline.async(
          ...
          Log.steps(
            'TBA',
            'Save: Assert Save button is disabled when editor is opened. Add content and assert Save button is enabled',
            [
              tinyUi.sWaitForUi(
                'check button',
                'button.tox-tbtn--disabled[aria-label="Save"]'
              ),
              ...
              tinyUi.sWaitForUi(
                'check button',
                'button[aria-label="Save"]:not(.tox-tbtn--disabled)'
              )
            ]
          ),
          onSuccess,
