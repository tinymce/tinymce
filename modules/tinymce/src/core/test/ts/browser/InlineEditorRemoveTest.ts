...
UnitTest.asynctest(
  'browser.tinymce.core.InlineEditorRemoveTest',
  (success, failure) => {
    Theme();
    ...
    Pipeline.async(
      ...
      [
        Logger.t(
          'Removing inline editor should remove all data-mce-bogus tags',
          Chain.asStep({}, [
            McEditor.cFromHtml('<div></div>', settings),
            ApiChains.cSetRawContent(
              '<p data-mce-bogus="all">b</p><p data-mce-bogus="1">b</p>'
            ),
            cRemoveEditor,
