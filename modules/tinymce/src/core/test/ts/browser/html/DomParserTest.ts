...
UnitTest.asynctest('browser.tinymce.core.html.DomParserTest', function (success, failure) {
  ...
  suite.test('Forced root blocks', function () {
    ...
    root = parser.parse('<!-- a -->' + 'b' + '<b>c</b>' + '<p>d</p>' + '<p>e</p>' + 'f' + '<b>g</b>' + 'h');
    LegacyUnit.equal(
      serializer.serialize(root),
  ...
  suite.test('Forced root blocks attrs', function () {
    ...
    LegacyUnit.equal(
      serializer.serialize(root),
      '<!-- a -->' + '<p class="class1">b<strong>c</strong></p>' + '<p>d</p>' + '<p>e</p>' + '<p class="class1">f<strong>g</strong>h</p>',
      'Mixed text nodes, inline elements and blocks.'
    );
  });
  ...
  suite.test('Bug #7543 removes whitespace between bogus elements before a block', function () {
    ...
    LegacyUnit.equal(
      serializer.serialize(DomParser().parse('<div><b data-mce-bogus="1">a</b> <b data-mce-bogus="1">b</b><p>c</p></div>')),
      '<div>a b<p>c</p></div>'
    );
  });
  ...
  suite.test('Bug #7582 removes whitespace between bogus elements before a block', function () {
    ...
    LegacyUnit.equal(
      serializer.serialize(DomParser().parse('<div>1 <span data-mce-bogus="1">2</span><div>3</div></div>')),
  ...
  suite.test('extract base64 uris to blobcache if blob cache is provided', () => {
    ...
    Assertions.assertEq('Should be html with blob uri', `<p><img src="${blobUri}" /></p>`, serializedHtml);
  ...
  suite.test('do not extract base64 uris for transparent images used by for example the page break plugin', () => {
    ...
    Assertions.assertEq('Should be the unchanged transparent image source', Env.transparentSrc, root.getAll('img')[0].attr('src'));
  ...
  suite.test('do not extract base64 uris if blob cache is not provided', () => {
    ...
    Assertions.assertEq('Should be html with base64 uri retained', html, serializedHtml);
  });
