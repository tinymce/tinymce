...
UnitTest.asynctest('browser.tinymce.core.html.SerializerTest', function (success, failure) {
  ...
  suite.test('Basic serialization', function () {
    ...
    LegacyUnit.equal(serializer.serialize(DomParser().parse('<B>text</B><IMG src="1.gif">')), '<strong>text</strong><img src="1.gif" />');
  ...
  suite.test('Sorting of attributes', function () {
    ...
    LegacyUnit.equal(serializer.serialize(DomParser().parse('<b class="class" id="id">x</b>')), '<strong id="id" class="class">x</strong>');
  });
