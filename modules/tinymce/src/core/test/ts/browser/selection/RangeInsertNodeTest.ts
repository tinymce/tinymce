...
UnitTest.asynctest(
  'browser.tinymce.core.selection.RangeInsertNode',
  (success, failure) => {
    Theme();
    ...
    const fragmentFromHtml = (
      ...
      Fragment.fromElements(Elements.fromHtml(html, scope), scope).dom();
      ...
      (editor: Editor, onSuccess, onFailure) => {
        ...
        Pipeline.async(
          ...
          [
                ...
                tinyApis.sAssertContentStructure(
                  ApproxStructure.build((s, str, _arr) =>
                    s.element('body', {
                      children: [
                        s.element('p', {
                          children: [s.text(str.is('X')), s.text(str.is('a'))]
                        })
                      ]
                    })
                  )
                ...
                tinyApis.sAssertContentStructure(
                  ApproxStructure.build((s, str, _arr) =>
                    s.element('body', {
                      children: [
                        s.element('p', {
                          children: [s.text(str.is('a')), s.text(str.is('X'))]
                        })
                      ]
                    })
                  )
            ...
            Logger.t(
              'Insert document fragment at start of text',
              GeneralSteps.sequence([
                tinyApis.sFocus(),
                tinyApis.sSetRawContent('<p>a</p>'),
                tinyApis.sSetCursor([0, 0], 0),
