...
UnitTest.asynctest(
  'browser.tinymce.core.selection.MultiClickSelectionTest',
  (success, failure) => {
    Theme();
    ...
    TinyLoader.setupLight(
      function (editor, onSuccess, onFailure) {
        ...
        const testXClicksNormalisation = (clickCount) =>
          GeneralSteps.sequence([
            Logger.t(
              'Normalize selection from index text node to text node offsets with ' +
                clickCount +
                ' clicks',
              GeneralSteps.sequence([
            ...
            Logger.t(
              'Normalize selection start in text node end after paragraph with ' +
                clickCount +
                ' clicks',
              GeneralSteps.sequence([
                tinyApis.sSetContent('<p>abc</p>'),
