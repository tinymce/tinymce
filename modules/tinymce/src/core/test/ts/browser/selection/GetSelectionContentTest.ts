...
UnitTest.asynctest(
  ...
  (success, failure) => {
    ...
    const sAssertGetContentOverrideBeforeGetContent = (
      label: string,
      editor: Editor,
      expectedContents: string,
      args: any = {}
    ...
    TinyLoader.setupLight(
      (editor: Editor, onSuccess, onFailure) => {
        ...
        Pipeline.async(
          ...
          [
            Logger.t(
              'Should be empty contents on a caret selection',
              GeneralSteps.sequence([
                tinyApis.sSetContent('<p>a</p>'),
                tinyApis.sSetSelection([0, 0], 0, [0, 0], 0),
                sAssertGetContent(
                  'Should be empty selection on caret',
                  editor,
                  ''
                )
              ])
            ...
            Logger.t(
              'Should be text contents on a range selection',
              GeneralSteps.sequence([
                tinyApis.sSetContent('<p>a</p>'),
            ...
            Logger.t(
              'Should be text contents provided by override handler',
              GeneralSteps.sequence([
                tinyApis.sSetContent('<p>a</p>'),
                tinyApis.sSetSelection([0, 0], 0, [0, 0], 1),
                sAssertGetContentOverrideBeforeGetContent(
                  'Should be overridden content',
                  editor,
                  'X'
                )
              ])
            ...
            Logger.t(
              `Should be text contents when editor isn't focused and format is text`,
              GeneralSteps.sequence([
                sAddTestDiv,
