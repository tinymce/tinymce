  ...
  function (success, failure) {
    ...
    const cSetSelection = function (startPath, soffset, finishPath, foffset) {
      return Chain.op(function () {
        const sc = Hierarchy.follow(
          ...
          startPath
        ).getOrDie('invalid startPath');
        const fc = Hierarchy.follow(
          Element.fromDom(viewBlock.get()),
          finishPath
        ).getOrDie('invalid finishPath');
    ...
    const cAssertSelection = function (
      ...
      return Chain.op(function () {
        ...
        const sel = WindowSelection.getExact(win.dom()).getOrDie(
          'no selection'
        );
    ...
    const cManipulateBookmarkOffsets = function (startPad, finishPad) {
      return Chain.mapper(function (bookmark: Option<any>) {
        return bookmark.map(function (bm) {
          return Selection.range(
            bm.start(),
            bm.soffset() + startPad,
            bm.finish(),
            bm.foffset() + finishPad
          );
        });
      });
    };
    ...
    const cDeleteElement = function (path) {
      return Chain.op(function () {
        Hierarchy.follow(Element.fromDom(viewBlock.get()), path).each(
          Remove.remove
        );
      });
    };

    const cAssertBookmark = function (
      startPath,
      startOffset,
      finishPath,
      finishOffset
    ...
    const cSetSelectionFromBookmark = Chain.op(function (
      bookmark: Option<any>
    ) {
      bookmark.each(function (b) {
        ...
        SelectionBookmark.validate(root, b).each(function (rng) {
    ...
    Pipeline.async(
      ...
      [
        ...
        Logger.t(
          'two p tags, delete one and should be none',
          Chain.asStep(viewBlock, [
            cSetHtml('<p>abc</p><p>123</p>'),
            cSetSelection([0, 0], 0, [1, 0], 1),
            cGetBookmark([]),
        ...
        Logger.t(
          'three p tags, delete middle and should be none',
          Chain.asStep(viewBlock, [
        ...
        Logger.t(
          'backwards selection should set a non-backwards bookmark, one p tag',
          Chain.asStep(viewBlock, [
            cSetHtml('<p>hello</p>'),
        ...
        Logger.t(
          'backwards selection should set a non-backwards bookmark, two p tags',
          Chain.asStep(viewBlock, [
        ...
        Logger.t(
          'readRange with with win without getSelection should return Option.none',
          Chain.asStep({}, [
