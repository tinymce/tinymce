...
UnitTest.asynctest('browser.tinymce.core.delete.CefDeleteTest', function (success, failure) {
  ...
  const sTestDeletePadd = function (editor: Editor, tinyApis: TinyApis, tinyActions: TinyActions) {
    return GeneralSteps.sequence([
      ...
      Logger.t(
        'Should padd editor with paragraph and br if the editor is empty after delete of a cef element',
        GeneralSteps.sequence([
      ...
      Logger.t(
        'Should padd editor with empty paragraph if we delete last element',
        GeneralSteps.sequence([
          tinyApis.sSetContent('<div contenteditable="false">a</div>'),
      ...
      Logger.t(
        'Should remove fake caret if we delete block cef',
        GeneralSteps.sequence([
