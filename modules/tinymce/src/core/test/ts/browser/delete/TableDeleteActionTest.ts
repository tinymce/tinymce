...
UnitTest.asynctest(
  ...
  function (success, failure) {
    ...
    const cExtractActionCells = Chain.binder(function (actionOpt: Option<any>) {
      return actionOpt.fold(fail('unexpected nothing'), function (action) {
    ...
    const cExtractTableFromDeleteAction = Chain.binder(function (
      actionOpt: Option<any>
    ) {
      return actionOpt.fold(fail('unexpected nothing'), function (action) {
    ...
    Pipeline.async(
      ...
      [
        Logger.t(
          'collapsed range should return none',
          Chain.asStep({}, [
            cFromHtml(
        ...
        Logger.t(
          'select two out of three cells returns the emptycells action',
          Chain.asStep({}, [
            cFromHtml(
              '<table><tbody><tr><td>a</td><td>b</td><td>c</td></tr></tbody></table>',
              [0, 0, 0, 0],
        ...
        Logger.t(
          'select three out of three cells returns the removeTable action',
          Chain.asStep({}, [
            cFromHtml(
              '<table><tbody><tr><td>a</td><td>b</td><td>c</td></tr></tbody></table>',
              [0, 0, 0, 0],
        ...
        Logger.t(
          'select between rows, not all cells',
          Chain.asStep({}, [
            cFromHtml(
            ...
            Assertions.cAssertEq(
              'should be cells',
              '<th>b</th><th>c</th><td>d</td>'
            )
          ])
        ),

        Logger.t(
          'select between rows, all cells',
          Chain.asStep({}, [
            cFromHtml(
              '<table><tbody><tr><th>a</th><th>b</th><th>c</th></tr><tr><td>d</td><td>e</td><td>f</td></tr></tbody></table>',
              [0, 0, 0, 0],
        ...
        Logger.t(
          ...
          Chain.asStep({}, [
            ...
            cExtractTableFromDeleteAction,
            Assertions.cAssertEq(
              'should be cell from first table only',
        ...
        Logger.t(
          ...
          Chain.asStep({}, [
            ...
            cExtractTableFromDeleteAction,
            Assertions.cAssertEq(
              'should cells from partially selected table',
              '<table><tbody><tr><td>a</td></tr></tbody></table>'
            )
          ])
        ),
