...
UnitTest.asynctest(
  'browser.tinymce.core.caret.LineReader',
  (success, failure) => {
    const viewBlock = ViewBlock();
    const browser = PlatformDetection.detect().browser;
    const isSafari13OrLower =
      browser.isSafari() &&
      (browser.version.major < 13 ||
        (browser.version.major === 13 && browser.version.minor < 1));
    ...
    const cGetPositionsUntilPreviousLine = (path: number[], offset: number) =>
      Chain.mapper(function (scope: any) {
        const container = Hierarchy.follow(
          Element.fromDom(scope.get()),
          path
        ).getOrDie();
    ...
    const cFindClosestHorizontalPosition = (path: number[], offset: number) =>
      Chain.mapper(function (positions: CaretPosition[]) {
        const container = Hierarchy.follow(
      ...
      Chain.op(function (actualPositions: CaretPosition[]) {
        ...
        Arr.each(expectedPositions, (p: Path, i) => {
          ...
          Assertions.assertDomEq(
            ...
            container,
            Element.fromDom(actualPositions[i].container())
          );
          Assertions.assertEq(
            'Should be the expected offset',
            p.offset,
            actualPositions[i].offset()
          );
        });
      });
    ...
    const cAssertNone = Chain.op(function (a: Option<any>) {
      Assertions.assertEq(
        'Option return value should be none',
        true,
        a.isNone()
      );
    });
    ...
    const cAssertLineInfoCaretPositions = (expectedPositions: Path[]) =>
      Chain.op(function (lineInfo: LineInfo) {
    ...
    const cAssertBreakPositionNone = Chain.op(function (linebreak: LineInfo) {
      Assertions.assertEq(
        'Should not be a line break position',
        true,
        linebreak.breakAt.isNone()
      );
    });

    const cAssertBreakPosition = (path: number[], offset: number) =>
      Chain.op(function (linebreak: LineInfo) {
        const container = Hierarchy.follow(
          Element.fromDom(viewBlock.get()),
          path
        ).getOrDie();
        ...
        Assertions.assertDomEq(
          'Should be the expected container',
          container,
          Element.fromDom(breakPos.container())
        );
        Assertions.assertEq(
          'Should be the expected offset',
          offset,
    ...
    const cAssertBreakType = (expectedBreakType: BreakType) =>
      Chain.op(function (linebreak: LineInfo) {
        ...
        Assertions.assertEq(
          'Should be the expected break type',
          expectedBreakType,
          actualBreakType
        );
      });
      ...
      [
        Logger.t(
          ...
          GeneralSteps.sequence([
            Logger.t(
              'Should be an empty array of positions and no linebreak',
            ...
            Logger.t(
              'Should be an array with the first position and second position and no linebreak',
              Chain.asStep(viewBlock, [
                cSetHtml('<p>ab</p>'),
            ...
            Logger.t(
              'Should be an array with one position from the second line and a break on the first line 1 <br>',
              Chain.asStep(viewBlock, [
                cSetHtml('<p>a<br>b</p>'),
                cGetPositionsUntilPreviousLine([0, 2], 1),
                cAssertLineInfoCaretPositions([{ path: [0, 2], offset: 0 }]),
                cAssertBreakType(BreakType.Br),
                cAssertBreakPosition([0], 1)
              ])
            ),
            Logger.t(
              'Should be an array with one position from the second line and a break on the first line 2 <br>',
              Chain.asStep(viewBlock, [
            ...
            Logger.t(
              'Should be an array with one position from the second line and a break on the first line <p>',
              Chain.asStep(viewBlock, [
                cSetHtml('<p>a</p><p>b</p>'),
                cGetPositionsUntilPreviousLine([1, 0], 1),
                cAssertLineInfoCaretPositions([{ path: [1, 0], offset: 0 }]),
                cAssertBreakType(BreakType.Block),
                cAssertBreakPosition([0, 0], 1)
              ])
            ),
            Logger.t(
              'Should be an array with one position from the second line and a break on the first line (wrap)',
              Chain.asStep(
                viewBlock,
                Arr.flatten([
                  [
            ...
            Logger.t(
              'Should be an array with zero positions from the second line and a break on the first line',
              Chain.asStep(
                viewBlock,
                Arr.flatten([
                  [
        ...
        Logger.t(
          ...
          GeneralSteps.sequence([
            ...
            Logger.t(
              'Should be an array with one position from the first line and a break on the first line <br>',
              Chain.asStep(viewBlock, [
                cSetHtml('<p>a<br>b</p>'),
                cGetPositionsUntilNextLine([0, 0], 0),
                cAssertLineInfoCaretPositions([
            ...
            Logger.t(
              ...
              Chain.asStep(viewBlock, [
                ...
                cAssertLineInfoCaretPositions([{ path: [0], offset: 1 }]),
                cAssertBreakType(BreakType.Br),
                cAssertBreakPosition([0], 1)
              ])
            ),
            Logger.t(
              'Should be an array with one position from the first line and a break on the first line <p>',
              Chain.asStep(viewBlock, [
                cSetHtml('<p>a</p><p>b</p>'),
                cGetPositionsUntilNextLine([0, 0], 0),
                cAssertLineInfoCaretPositions([{ path: [0, 0], offset: 1 }]),
                cAssertBreakType(BreakType.Block),
                cAssertBreakPosition([1, 0], 0)
              ])
            ),
            Logger.t(
              'Should be an array with one position from the second line and a break on the last line',
              Chain.asStep(viewBlock, [
                cSetHtml('<div style="width: 10px">abc def ghi</div>'),
                cGetPositionsUntilNextLine([0, 0], 6),
                cAssertLineInfoCaretPositions([{ path: [0, 0], offset: 7 }]),
                cAssertBreakType(BreakType.Wrap),
                cAssertBreakPosition([0, 0], 8)
              ])
            ),
            Logger.t(
              'Should be an array with zero positions from the second line and a break on the last line',
              Chain.asStep(viewBlock, [
                cSetHtml('<div style="width: 10px">abc def ghi</div>'),
        ...
        Logger.t(
          ...
          GeneralSteps.sequence([
            Logger.t(
              'Should return true at first visual position in paragraph',
              Chain.asStep(viewBlock, [
                cSetHtml('<p>a</p>'),
                cIsAtFirstLine([0, 0], 0),
                Assertions.cAssertEq(
                  'Should be true on first position in paragraph',
                  true
                )
              ])
            ),
            Logger.t(
              'Should return true at second visual position in paragraph',
              Chain.asStep(viewBlock, [
                cSetHtml('<p>a</p>'),
                cIsAtFirstLine([0, 0], 1),
                Assertions.cAssertEq(
                  'Should be true on second position in paragraph',
                  true
                )
              ])
            ),
            Logger.t(
              'Should return false at second br line in paragraph',
              Chain.asStep(viewBlock, [
                cSetHtml('<p>a<br>b</p>'),
                cIsAtFirstLine([0, 2], 0),
                Assertions.cAssertEq(
                  'Should be false on second line in paragraph',
                  false
                )
              ])
            ),
            Logger.t(
              'Should return false at second pos after br line in paragraph',
              Chain.asStep(viewBlock, [
                cSetHtml('<p>a<br>b</p>'),
            ...
            Logger.t(
              'Should return false at second paragraph',
              Chain.asStep(viewBlock, [
                cSetHtml('<p>a</p><p>b</p>'),
            ...
            Logger.t(
              'Should return false at second line in a wrapped element',
              Chain.asStep(viewBlock, [
                cSetHtml('<div style="width: 10px">abc def ghi</div>'),
            ...
            Logger.t(
              'Should return true at paragraph in td',
              Chain.asStep(viewBlock, [
                cSetHtml(
                ...
                Assertions.cAssertEq(
                  'Should be true since it is the first line in td',
                  true
                )
              ])
            ),
            Logger.t(
              'Should return false at second paragraph in td',
              Chain.asStep(viewBlock, [
                cSetHtml(
                ...
                Assertions.cAssertEq(
                  'Should be false since it is the second line in td',
                  false
                )
              ])
            )
        ...
        Logger.t(
          ...
          GeneralSteps.sequence([
            ...
            Logger.t(
              'Should return false at before first br line in paragraph',
              Chain.asStep(viewBlock, [
                cSetHtml('<p>a<br>b</p>'),
                cIsAtLastLine([0, 0], 0),
                Assertions.cAssertEq(
                  'Should be false on first line in paragraph',
                  false
                )
              ])
            ),
            Logger.t(
              'Should return false at first line at second pos before br line in paragraph',
              Chain.asStep(viewBlock, [
                cSetHtml('<p>a<br>b</p>'),
                cIsAtLastLine([0, 0], 1),
                Assertions.cAssertEq(
            ...
            Logger.t(
              'Should return false at first paragraph',
              Chain.asStep(viewBlock, [
                cSetHtml('<p>a</p><p>b</p>'),
                cIsAtLastLine([0, 0], 0),
                Assertions.cAssertEq(
                  'Should be false on first paragraph line',
                  false
                )
              ])
            ),
            ...
            Logger.t(
              'Should return false at first paragraph in td',
              Chain.asStep(viewBlock, [
                cSetHtml(
                  '<table><tbody><tr><td><p>a</p><p>b</p></td></tr></tbody></table>'
                ),
                ...
                Assertions.cAssertEq(
                  'Should be false since it is the first line in td',
                  false
                )
              ])
            ),
            Logger.t(
              'Should return true at second paragraph in td',
              Chain.asStep(viewBlock, [
                cSetHtml(
                  '<table><tbody><tr><td><p>a</p><p>b</p></td></tr></tbody></table>'
                ),
                ...
                Assertions.cAssertEq(
                  'Should be true since it is the second line in td',
                  true
                )
              ])
            )
        ...
        Logger.t(
          ...
          GeneralSteps.sequence([
            Logger.t(
              'Should return zero positions since there is no line above',
              Chain.asStep(viewBlock, [
                cSetHtml('<p>a</p>'),
            ...
            Logger.t(
              'Should return three positions for the line above',
              Chain.asStep(viewBlock, [
            ...
            Logger.t(
              'Should return four positions for the line above2',
              Chain.asStep(viewBlock, [
        ...
        Logger.t(
          ...
          GeneralSteps.sequence([
            Logger.t(
              'Should return zero positions since there is no line below',
              Chain.asStep(viewBlock, [
                cSetHtml('<p>a</p>'),
            ...
            Logger.t(
              'Should return three positions for the line below',
              Chain.asStep(viewBlock, [
            ...
            Logger.t(
              'Should return five positions for the line below',
              Chain.asStep(viewBlock, [
        ...
        Logger.t(
          'findClosestHoriontalPosition (above)',
          GeneralSteps.sequence([
            Logger.t(
              'Should not return a position since there is no above positions',
              Chain.asStep(viewBlock, [
                cSetHtml('<p>ab</p>'),
            ...
            Logger.t(
              'Should return first caret position on the line above',
              Chain.asStep(viewBlock, [
            ...
            Logger.t(
              'Should return last caret position on the line above',
              Chain.asStep(viewBlock, [
                cSetHtml('<p>ab</p><p>cd</p>'),
                cGetAbovePositions([1, 0], 0),
            ...
            Logger.t(
              'Should return first indexed caret position on the line above',
              Chain.asStep(viewBlock, [
            ...
            Logger.t(
              'Should return last text node position at the line above',
              Chain.asStep(viewBlock, [
        ...
        Logger.t(
          'findClosestHoriontalPosition (below)',
          GeneralSteps.sequence([
            Logger.t(
              'Should not return a position since there is no below positions',
              Chain.asStep(viewBlock, [
                cSetHtml('<p>ab</p>'),
                cGetBelowPositions([0, 0], 0),
                cFindClosestHorizontalPosition([0, 0], 0),
            ...
            Logger.t(
              'Should return first caret position on the line below',
              Chain.asStep(viewBlock, [
                cSetHtml('<p>ab</p><p>cd</p>'),
                cGetBelowPositions([0, 0], 0),
                cFindClosestHorizontalPosition([0, 0], 0),
            ...
            Logger.t(
              'Should return last caret position on the line below',
              Chain.asStep(viewBlock, [
                cSetHtml('<p>ab</p><p>cd</p>'),
                cGetBelowPositions([0, 0], 0),
            ...
            Logger.t(
              'Should return first indexed caret position on the line below',
              Chain.asStep(viewBlock, [
                cSetHtml('<p><input></p><p><input></p>'),
            ...
            Logger.t(
              'Should return first text node position at the line below',
              Chain.asStep(viewBlock, [
                cSetHtml('<p>a<input>b</p><p>a<input>b</p>'),
                cGetBelowPositions([0, 0], 0),
                cFindClosestHorizontalPosition([0, 0], 0),
            ...
            Logger.t(
              'Should return last text node position at the line below',
              Chain.asStep(viewBlock, [
                cSetHtml('<p>a<input>b</p><p>a<input>b</p>'),
