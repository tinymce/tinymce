...
UnitTest.asynctest('browser.tinymce.core.CaretContainerRemoveTest', function (
  ...
  const sTestRemove = Logger.t(
    ...
    Step.sync(function () {
      ...
      Assertions.assertEq(
        'Should not be inline container',
        false,
        CaretContainer.isCaretContainerInline(getRoot().firstChild)
      );
    })
  ...
  const sTestRemoveAndRepositionBeforeOffset = Logger.t(
    ...
    Step.sync(function () {
      setupHtml(
        '<span contentEditable="false">1</span><span contentEditable="false">2</span>'
      );
