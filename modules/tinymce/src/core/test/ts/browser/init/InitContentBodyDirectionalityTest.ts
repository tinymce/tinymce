...
UnitTest.asynctest(
  'browser.tinymce.core.init.InitContentBodyDirectionalityTest',
  (success, failure) => {
    Theme();
    ...
    const cSetContent = (content: string) =>
      Chain.mapper(function (editor: Editor) {
        return editor.editorCommands.execCommand(
          'mceSetContent',
          false,
          content
        );
    ...
    Pipeline.async(
      ...
      [
        Log.step(
          'TBA',
          'Test default directionality of the editor when set to use a rtl language',
          makeStep(
            {
        ...
        Log.step(
          'TBA',
          'Test directionality of the editor when set using the config option while localized using a rtl language',
          makeStep(
            {
              base_url: '/project/tinymce/js/tinymce',
              language: 'ar',
        ...
        Log.step(
          'TBA',
          'Test directionality of editor when using a language with no directionality set',
          makeStep(
            {
              base_url: '/project/tinymce/js/tinymce',
              language: 'en'
