...
UnitTest.asynctest(
  ...
  function (success, failure) {
    ...
    suite.asyncTest('target (initialise on element without id)', function (
      _,
      done
    ) {
    ...
    suite.asyncTest(
      'target (selector option takes precedence over target option)',
      function (_, done) {
    ...
    suite.asyncTest('selector on non existing targets', function (_, done) {
      EditorManager.init({
        ...
        Assertions.assertEq(
          'Should be an result that is zero length',
          0,
          result.length
        );
        teardown(done);
    ...
    suite.asyncTest(
      'target (each editor should have a different target)',
      function (_, done) {
    ...
    suite.asyncTest('Test base_url and suffix options', function (_, done) {
      ...
      EditorManager.init({
        ...
        init_instance_callback(ed) {
          Assertions.assertEq(
            'Should have set suffix on EditorManager',
            '.min',
            EditorManager.suffix
          );
          Assertions.assertEq(
            'Should have set suffix on editor',
            '.min',
            ed.suffix
          );

          Assertions.assertEq(
            'Should have set baseURL on EditorManager',
            EditorManager.documentBaseURL + 'fake/url',
            EditorManager.baseURL
          );

          Assertions.assertEq(
            'Should have set baseURI on EditorManager',
            EditorManager.documentBaseURL + 'fake/url',
            EditorManager.baseURI.source
          );
          Assertions.assertEq(
            'Should have set baseURI on editor',
            EditorManager.documentBaseURL + 'fake/url',
            ed.baseURI.source
          );
    ...
    const getSkinCssFilenames = function () {
      return Arr.bind(
        SelectorFilter.descendants(Element.fromDom(document), 'link'),
        function (link) {
    ...
    const mCreateInlineModeMultipleInstances = Step.label(
      'mCreateInlineModeMultipleInstances',
      Step.stateful(function (_value, next, die) {
        viewBlock.update(
          '<div class="tinymce-editor"><p>a</p></div><div class="tinymce-editor"><p>b</p></div>'
        );
    ...
    const mAssertEditors = Step.label(
      'mAssertEditors',
      Step.stateful(function (editors: any[], next, _die) {
        Assertions.assertHtml(
          'Editor contents should be the first div content',
          '<p>a</p>',
          editors[0].getContent()
        );
        Assertions.assertHtml(
          'Editor contents should be the second div content',
          '<p>b</p>',
          editors[1].getContent()
        );
        ...
        Assertions.assertStructure(
          'Editor container should match expected structure',
          containerApproxStructure,
          Element.fromDom(editors[0].editorContainer)
        );
        Assertions.assertStructure(
          'Editor container should match expected structure',
          containerApproxStructure,
          Element.fromDom(editors[1].editorContainer)
        );
        ...
        Assertions.assertEq(
          'Targets should be two since there are two editors',
          2,
          targets.length
        );
    ...
    const sRemoveAllEditors = Step.label(
      'sRemoveAllEditors',
      Step.sync(function () {
        EditorManager.remove();
      })
    ...
    const mAssertTargets = Step.label(
      'mAssertTargets',
      Step.stateful(function (targets: any[], next, _die) {
        Assertions.assertEq(
          'Targets should be two since there are two editors',
          2,
          targets.length
        ...
        Arr.each(targets, function (target) {
          Assertions.assertEq(
            'Target parent should not be null',
            true,
    ...
    const sInitAndAssertContent = (
      html: string,
      selector: string,
      expectedEditorContent: string
    ) =>
      Step.async((done) => {
    ...
    Pipeline.async(
      ...
      [
        ...suite.toSteps({}),
        Logger.t(
          'Initialize multiple inline editors and remove them',
        ...
        Logger.t(
          'Initialize on textarea with initial content',
          GeneralSteps.sequence([
            sInitAndAssertContent(
        ...
        Logger.t(
          'Initialize on input with initial content',
          GeneralSteps.sequence([
            sInitAndAssertContent(
        ...
        Logger.t(
          'Initialize on list item with initial content',
          GeneralSteps.sequence([
            sInitAndAssertContent(
              '<ul><li>Initial Content</li></ul>',
              'li',
              'Initial Content'
            )
          ])
