...
UnitTest.asynctest(
  'browser.tinymce.core.init.InitEditorIconTest',
  (success, failure) => {
    Theme();
    ...
    TinyLoader.setup(
      (editor, onSuccess, onFailure) => {
        ...
        Pipeline.async(
          ...
          [
            Log.stepsAsStep(
              'TBA',
              'Should have been able to load custom icon pack',
              [
                UiFinder.sNotExists(Body.body(), '.tox-notification'),
                Assertions.sAssertEq(
                  'IconManager should have custom icon pack',
                  customIconPack,
                  IconManager.get('custom')
                )
              ]
            ),
            Log.stepsAsStep('TBA', 'Icon overrides', [
              ...
              Assertions.sAssertEq(
                'Icon pack icon',
                customIconPack.icons.bold,
                icons.bold
              ),
              Assertions.sAssertEq(
                'Default pack icon',
                getDefaultIcons().italic,
                icons.italic
              )
            ])
          ],
      ...
      },
      {
        icons: 'custom',
        icons_url:
          '/project/tinymce/src/core/test/assets/icons/custom/icons.js',
