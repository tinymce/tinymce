...
UnitTest.test('browser.tinymce.core.init.ContentCssTest', () => {
  ...
  testContentCss(
    'Expected array with two absolute urls from an array of relative strings',
    [`${baseUrl}/content/a.css`, `${baseUrl}/content/b.css`],
    ['/content/a.css', '/content/b.css']
  );
  testContentCss(
    'Expected array with absolute url from absolute string',
  ...
  testContentCss('Expected empty array on empty input', [], []);
  testContentCss(
    'Expected array with absolute url from a comma separated list of css files',
