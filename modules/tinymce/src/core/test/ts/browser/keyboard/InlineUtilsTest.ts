...
UnitTest.asynctest('browser.tinymce.core.keyboard.InlineUtilsTest', function (
  ...
  const cAssertPosition = function (path, expectedOffset) {
    return Chain.mapper(function (elmPos: any) {
      ...
      Assertions.assertDomEq(
        ...
        Element.fromDom(elmPos.pos.container()),
        expectedContainer
      );
      Assertions.assertEq(
  ...
  const cSplitAt = function (path, offset) {
    return Chain.mapper(function (elm: any) {
      const textNode = Hierarchy.follow(elm, path)
        .filter(Node.isText)
        .getOrDie();
      ...
      Logger.t(
        ...
        Step.sync(function () {
          ...
          Assertions.assertEq(
            ...
            InlineUtils.isInlineTarget(
              createFakeEditor({}),
              Element.fromHtml('<span class="mce-annotation"></span>').dom()
            )
          );
          Assertions.assertEq(
