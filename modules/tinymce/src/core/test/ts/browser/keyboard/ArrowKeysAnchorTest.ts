...
UnitTest.asynctest(
  ...
  function (success, failure) {
        ...
        return s.element('p', {
          children: [
            ...
            s.element('a', {
              ...
              children: [s.text(str.is(expectedText))]
            }),
            s.text(str.is('c'))
          ]
        });
    ...
    const anchorSurroundedWithZwspInside = function (start: boolean) {
      return anchorSurroundedWithText(
        start ? Zwsp.ZWSP + 'b' : 'b' + Zwsp.ZWSP
      );
    };
    ...
    const anchorsZwspOutside = function (
      texts: string[],
      before: boolean,
      index: number
    ) {
    ...
    const anchorsZwspInside = function (
      texts: string[],
      start: boolean,
      index: number
    ) {
      return ApproxStructure.build(function (s, str /* , arr*/) {
        const children = Arr.map(texts, function (text, i) {
          ...
          return s.element('a', {
            attrs: {
              'data-mce-selected':
                i === index ? str.is('inline-boundary') : str.none('1'),
              'data-mce-href': str.is('#'),
              'href': str.is('#')
            },
            children: [s.text(str.is(i === index ? zwspText : text))]
          });
        });
    ...
    const sAssertContentStructure = function (
      editor: Editor,
      expected: StructAssert
    ) {
    ...
    const sAssertCursor = function (
      tinyApis: TinyApis,
      elementPath: number[],
      offset: number
    ) {
      return tinyApis.sAssertSelection(
        elementPath,
        offset,
        elementPath,
        offset
    ...
    const sTestArrowsSingleAnchor = function (
      tinyApis: TinyApis,
      tinyActions: TinyActions,
      editor: Editor
    ) {
      return Logger.t(
        'sTestArrowsSingleAnchor',
    ...
    const sTestArrowsAnchorSurroundedByText = function (
      tinyApis: TinyApis,
      tinyActions: TinyActions,
      editor: Editor
    ) {
      return Logger.t(
        'sTestArrowsAnchorSurroundedByText',
        GeneralSteps.sequence([
          ...
          sAssertContentStructure(
            editor,
            anchorSurroundedWithZwspInside(START)
          ),
          ...
          sAssertContentStructure(
            editor,
            anchorSurroundedWithZwspOutside(BEFORE)
          ),
          ...
          sAssertContentStructure(
            editor,
            anchorSurroundedWithZwspOutside(AFTER)
          )
        ])
      );
    };

    const sTestArrowsMultipleAnchors = function (
      tinyApis: TinyApis,
      tinyActions: TinyActions,
      editor: Editor
    ) {
      return Logger.t(
        'sTestArrowsMultipleAnchors',
        GeneralSteps.sequence([
          ...
          sAssertContentStructure(
            editor,
            anchorsZwspOutside(['a', 'b'], BEFORE, 0)
          ),
          ...
          sAssertContentStructure(
            editor,
            anchorsZwspOutside(['a', 'b'], BEFORE, 1)
          ),
          ...
          sAssertContentStructure(
            editor,
            anchorsZwspOutside(['a', 'b'], AFTER, 1)
          ),
