...
UnitTest.asynctest('browser.tinymce.core.keyboard.EnterKeyCeFalseTest', function (success, failure) {
  ...
  suite.test('Enter before cE=false div', function (editor) {
    ...
    pressEnter(editor);
    LegacyUnit.equal(HtmlUtils.cleanHtml(editor.getBody().innerHTML), '<p><br data-mce-bogus="1"></p><div contenteditable="false">x</div>');
  ...
  suite.test('Enter after cE=false div', function (editor) {
    ...
    pressEnter(editor);
    LegacyUnit.equal(HtmlUtils.cleanHtml(editor.getBody().innerHTML), '<div contenteditable="false">x</div><p><br data-mce-bogus="1"></p>');
    LegacyUnit.equal(editor.selection.getNode().nodeName, 'P');
  });
