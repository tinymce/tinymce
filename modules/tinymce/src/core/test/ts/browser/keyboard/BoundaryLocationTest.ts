...
UnitTest.asynctest('browser.tinymce.core.keyboard.BoundaryLocationTest', function (success, failure) {
  ...
  const locationName = function (location) {
    return location.fold(Fun.constant('before'), Fun.constant('start'), Fun.constant('end'), Fun.constant('after'));
  };

  const locationElement = function (location) {
    return Element.fromDom(location.fold(Fun.identity, Fun.identity, Fun.identity, Fun.identity));
  };
  ...
  const sTestPrevLocationsBetweenInlines = Logger.t(
    ...
    GeneralSteps.sequence([sTestPrevLocation('<p><a href="a">a</a><a href="b">b</a></p>', [0, 1, 0], 0, 'before', 'a:nth-child(2)')])
  );
  ...
  const sTestNextLocationsBetweenInlines = Logger.t(
    ...
    GeneralSteps.sequence([sTestNextLocation('<p><a href="a">a</a><a href="a">b</a></p>', [0, 0, 0], 1, 'after', 'a:nth-child(1)')])
  );
