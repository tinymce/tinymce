...
UnitTest.asynctest('browser.tinymce.focus.FocusControllerTest', function (
  ...
  suite.test('isEditorUIElement on valid element', function () {
    ...
    LegacyUnit.equal(
      FocusController.isEditorUIElement(uiElm),
      true,
      'Should be true since mce- is a ui prefix'
    );
  });
  ...
  suite.test('isEditorUIElement on invalid element', function () {
    ...
    LegacyUnit.equal(
      FocusController.isEditorUIElement(noUiElm),
      false,
      'Should be true since mcex- is not a ui prefix'
    );
  });

  suite.test(
    'isEditorUIElement when api predicate is overwritten',
    function () {
      ...
      LegacyUnit.equal(
        FocusController.isEditorUIElement(customUiElm),
        true,
        'Should be true it is a valid ui element now'
      );
      LegacyUnit.equal(
        FocusController.isEditorUIElement(customNoUiElm),
        false,
        'Should be false since it not matching predicate'
      );
      ...
      LegacyUnit.equal(
        FocusController.isEditorUIElement(customUiElm),
        false,
        'Should be false since the predicate is restored'
      );
    }
  );
  ...
  suite.test('isUIElement on valid element', function (editor) {
    ...
    LegacyUnit.equal(
      FocusController.isUIElement(editor, uiElm1),
      true,
      'Should be true since mce- is a ui prefix'
    );
    LegacyUnit.equal(
      FocusController.isUIElement(editor, uiElm2),
      true,
      'Should be true since mcex- is a ui prefix'
    );
    LegacyUnit.equal(
      FocusController.isUIElement(editor, uiElm3),
      true,
      'Should be true since tox- is a ui prefix'
    );
    LegacyUnit.equal(
      FocusController.isUIElement(editor, noUiElm),
      false,
      'Should be false since mcey- is not a ui prefix'
    );
    delete editor.settings.custom_ui_selector;
  });

  suite.test('isEditorContentAreaElement on valid element', function () {
    const contentAreaElm1 = DOMUtils.DOM.create(
      'div',
      { class: 'mce-content-body' },
      null
    );
    const contentAreaElm2 = DOMUtils.DOM.create(
      'div',
      { class: 'tox-edit-area__iframe' },
      null
    );
    LegacyUnit.equal(
      FocusController.isEditorContentAreaElement(contentAreaElm1),
      true,
      'Should be true since mce-content-body is a content area container element'
    );
    LegacyUnit.equal(
      FocusController.isEditorContentAreaElement(contentAreaElm2),
      true,
      'Should be true since tox-edit-area__iframe is content area container element'
    );
  });
