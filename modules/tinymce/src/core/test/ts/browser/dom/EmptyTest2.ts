...
import * as Empty2 from 'tinymce/core/dom/Empty2';
...
describe('browser.tinymce.core.dom.EmptyTest', () => {
  ...
  const testEmpty = (html: string, expected: boolean, options?: Empty2.AdjustOptions) => {
    const schema = hook.editor().schema;
    const elm = SugarElement.fromHtml(html);
    const expectedLabel = expected ? 'empty' : 'not empty';
    assert.equal(Empty2.isEmpty(schema, elm, options), expected, html + ' should be treated as ' + expectedLabel);
  };
  ...
  it('TINY-10010: table cell with empty CET should not be treated as empty', () => {
    ...
    assert.isFalse(Empty2.isEmpty(schema, td));
  });
});
