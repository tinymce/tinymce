  ...
  (success, failure) => {
    const sCheckElement = function (name, predicate, expectedValue) {
      return Step.sync(function () {
        Assertions.assertEq(
          'Should be the expected value for specified element',
          expectedValue,
          predicate(Element.fromTag(name))
        );
      });
    };
    ...
    const sCheckText = function (predicate) {
      return Step.sync(function () {
        Assertions.assertEq(
          'Should be false for non element',
          false,
          predicate(Element.fromText('text'))
        );
      });
    };
    ...
    Pipeline.async(
      ...
      [
        ...
        Logger.t(
          'Check whitespace preserve elements',
          GeneralSteps.sequence([
