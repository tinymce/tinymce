...
UnitTest.asynctest('browser.tinymce.core.dom.DomUtilsTest', function (
  ...
  suite.test('parseStyle', function () {
    ...
    dom.setHTML(
      'test',
      '<span id="test2" style="   margin-left: 1px;    margin-top: 1px;   margin-right: 1px;   margin-bottom: 1px   "></span>'
    );
  ...
  suite.test('loadCSS contentCssCors enabled', function () {
    ...
    const CustomDOM = DOMUtils(iframeDoc, {
      keep_values: true,
      schema: Schema(),
      contentCssCors: true
    });
  ...
  suite.test('split', function () {
    ...
    LegacyUnit.equal(
      DOM.get('test').innerHTML.toLowerCase().replace(/\s+/g, ''),
