...
UnitTest.asynctest(
  'browser.tinymce.core.dom.ReferrerPolicyTest',
  (success, failure) => {
    const platform = PlatformDetection.detect();
    Theme();

    const cAssertReferrerLinkPresence = (
      ...
      Chain.op((editor: Editor) => {
        const links = editor
          .getDoc()
          .querySelectorAll(`link[referrerpolicy="${referrerPolicy}"]`);
        Assert.eq(
          `should have link with referrerpolicy="${referrerPolicy}"`,
          expected,
          links.length > 0
        );
      });

    const cLoadScript = (url) =>
    ...
    Pipeline.async(
      {},
      platform.browser.isEdge() || platform.browser.isIE()
        ? []
        : [
            Logger.t(
              'assert referrerpolicy presence with no setting set',
              Chain.asStep({}, [
            ...
            Logger.t(
              'assert referrerpolicy presence with setting set',
              Chain.asStep({}, [
                McEditor.cFromSettings({
