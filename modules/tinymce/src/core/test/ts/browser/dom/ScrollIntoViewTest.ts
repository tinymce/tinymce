  ...
  (success, failure) => {
    ...
    const sSetContent = function (
      ...
      return GeneralSteps.sequence([
        ...
        Waiter.sTryUntil(
          'Wait for scrollHeight to be updated',
          Step.sync(function () {
            Assertions.assertEq(
              'Scroll body should be more than 100',
              true,
              editor.getBody().scrollHeight > 100
            );
          })
        )
      ]);
    ...
    const sScrollIntoView = function (
      editor: Editor,
      selector: string,
      alignToTop?: boolean
    ) {
      return Step.sync(function () {
        editor.selection.scrollIntoView(
          editor.dom.select(selector)[0],
          alignToTop
        );
      });
    };

    const sScrollElementIntoView = function (
      editor: Editor,
      selector: string,
      alignToTop?: boolean
    ) {
    ...
    const sAssertScrollPosition = function (
      editor: Editor,
      x: number,
      y: number
    ) {
      return Step.sync(function () {
        ...
        Assertions.assertEq(
          `Scroll position X should be expected value: ${x} got ${actualX}`,
          x,
          actualX
        );
        Assertions.assertEq(
          `Scroll position Y should be expected value: ${y} got ${actualY}`,
          y,
          actualY
        );
      });
    ...
    const sAssertApproxScrollPosition = function (
      editor: Editor,
      x: number,
      y: number
    ) {
      return Step.sync(function () {
        ...
        Assertions.assertEq(
          `Scroll position X should be expected value: ${x} got ${actualX}`,
          true,
          Math.abs(x - actualX) < 5
        );
        Assertions.assertEq(
          `Scroll position Y should be expected value: ${y} got ${actualY}`,
          true,
          Math.abs(y - actualY) < 5
        );
      });
    };
    ...
    const mAssertScrollIntoViewEventInfo = function (
      editor: Editor,
      expectedElementSelector: string,
      expectedAlignToTop: boolean
    ) {
      return Step.stateful(function (value: any, next, _die) {
        const expectedTarget = Element.fromDom(
          editor.dom.select(expectedElementSelector)[0]
        );
        const actualTarget = Element.fromDom(value.state.get().elm);
        Assertions.assertDomEq(
          'Target should be expected element',
          expectedTarget,
          actualTarget
        );
        Assertions.assertEq(
          'Align to top should be expected value',
          expectedAlignToTop,
          value.state.get().alignToTop
        );
      ...
      return [
        ...
        Logger.t(
          ...
          GeneralSteps.sequence([
            Logger.t(
              'Scroll to element align to bottom',
              GeneralSteps.sequence([
                sScrollReset(editor),
                sSetContent(
                  editor,
                  tinyApis,
                  '<div style="height: 1000px">a</div><div style="height: 50px">b</div><div style="height: 1000px">a</div>'
                ),
            ...
            Logger.t(
              'Scroll to element align to top',
              GeneralSteps.sequence([
                sScrollReset(editor),
                sSetContent(
                  editor,
            ...
            Logger.t(
              `Scroll to element already in view shouldn't do anything`,
              GeneralSteps.sequence([
                sScrollReset(editor),
                sSetContent(
                  editor,
                  tinyApis,
                  '<div style="height: 1000px">a</div><div style="height: 50px">b</div><div style="height: 600px">a</div>'
                ),
                Step.sync(() => {
            ...
            Logger.t(
              'Scroll to element with height larger than viewport should align to top',
              GeneralSteps.sequence([
                sScrollReset(editor),
                sSetContent(
                  editor,
          ...
          GeneralSteps.sequence([
            Logger.t(
              ...
              GeneralSteps.sequence([
                ...
                sScrollIntoView(editor, 'div:nth-child(2)', false),
                mAssertScrollIntoViewEventInfo(
                  editor,
                  'div:nth-child(2)',
                  false
                ),
            ...
            Logger.t(
              'Scroll to element align to bottom (private api)',
              GeneralSteps.sequence([
                sScrollReset(editor),
                sSetContent(
                  editor,
            ...
            Logger.t(
              'Scroll to element align to top (private api)',
              GeneralSteps.sequence([
                sScrollReset(editor),
                sSetContent(
                  editor,
    ...
    TinyLoader.setup(
      function (editor, onSuccess, onFailure) {
        ...
        Pipeline.async(
          {},
          isPhantomJs() ? [] : steps(editor, tinyApis),
          onSuccess,
          onFailure
        );
      },
