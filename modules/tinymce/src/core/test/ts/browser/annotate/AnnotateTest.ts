...
import {
  sAnnotate,
  sAssertHtmlContent
} from '../../module/test/AnnotationAsserts';
import { Element } from '@ephox/sugar';

UnitTest.asynctest(
  'browser.tinymce.core.annotate.AnnotateTest',
  (success, failure) => {
    Theme();
    ...
    TinyLoader.setupLight(
      function (editor: Editor, onSuccess, onFailure) {
        ...
        const sTestWordGrabIfCollapsed = <T>(): Step<T, T> =>
          Logger.t(
            ...
            StepSequence.sequenceSame([
              ...
              tinyApis.sSetContent(
                '<p>This is the first paragraph here</p><p>This is the second.</p>'
              ),
              tinyApis.sSetSelection(
                [0, 0],
                'This is the first p'.length,
                [0, 0],
                'This is the first p'.length
              ),
              sAnnotate(editor, 'test-annotation', 'test-uid', {
        ...
        const sTestCanAnnotateDirectParentOfRoot = <T>(): Step<T, T> =>
          Logger.t(
        ...
        const sTestCanAnnotateBeforeTwoNonBreakingSpaces = <T>(): Step<T, T> =>
          Logger.t(
            'Should annotate when the cursor is collapsed before two nbsps',
            StepSequence.sequenceSame([
              tinyApis.sSetContent(
                '<p>Annotation here &nbsp;&nbsp;, please</p>'
              ),
              tinyApis.sSetCursor([0, 0], 'Annotation here '.length),
              sAnnotate(editor, 'test-annotation', 'test-uid', {
                anything: 'nbsp-paragraph'
              }),
        ...
        const sTestCanAnnotateWithinTwoNonBreakingSpaces = <T>(): Step<T, T> =>
          Logger.t(
        ...
        const sTestCanAnnotateAfterTwoNonBreakingSpaces = <T>(): Step<T, T> =>
          Logger.t(
        ...
        const sTestDoesNotWordGrabIfNotCollapsed = <T>(): Step<T, T> =>
          Logger.t(
        ...
        const sTestInOneParagraph = <T>(): Step<T, T> =>
          Logger.t(
        ...
        const sTestInTwoParagraphs = <T>(): Step<T, T> =>
          Logger.t(
        ...
        const sTestInThreeParagraphs = <T>(): Step<T, T> =>
          Logger.t(
