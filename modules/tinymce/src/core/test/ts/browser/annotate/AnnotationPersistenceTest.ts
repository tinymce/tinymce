...
UnitTest.asynctest(
  'browser.tinymce.core.annotate.AnnotationPersistenceTest',
  (success, failure) => {
    Theme();

    const sUndoLevel = (editor: Editor) =>
      Step.sync(() => {
        editor.undoManager.add();
      });

    const sRunTinyWithSettings = (
      annotation: AnnotatorSettings,
    ...
    const sSetupSingleAnnotation = (tinyApis: TinyApis, editor: Editor) =>
      GeneralSteps.sequence([
        ...
        tinyApis.sSetSelection(
          [0, 0],
          'This is the only p'.length,
          [0, 0],
          'This is the only par'.length
        ),
    ...
    const sContentContains = (
      ...
      Chain.asStep({}, [
        ...
        Chain.op((content) => {
          Assertions.assertEq(
            'editor.getContent() should contain: ' +
              pattern +
              ' = ' +
              isContained,
            true,
      ...
      [
        Logger.t(
          ...
          sRunTinyWithSettings(
            settingsWithPersistence,
            (tinyApis: TinyApis, ed: Editor) => [
        ...
        Logger.t(
          ...
          sRunTinyWithSettings(
            settingsWithDefaultPersistence,
            (tinyApis: TinyApis, ed: Editor) => [
              sSetupSingleAnnotation(tinyApis, ed),
              sContentContains(tinyApis, ed, 'mce-annotation', true)
            ]
