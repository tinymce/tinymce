    ...
    function (editor: Editor, onSuccess, onFailure) {
      ...
      const sTestAnnotationEvents = <T>(
        ...
        StepSequence.sequenceSame<T>([
          ...
          Waiter.sTryUntil(label, sAssertChanges('sTestAnnotationEvents.sAssertChanges', expected))
        ]);
      ...
      const sTestChanges = <T>() =>
        StepSequence.sequenceSame<T>([
          ...
          Waiter.sTryUntil('Waiting for no changes', sAssertChanges('Should be no changes', [])),
      ...
      setup: (ed: Editor) => {
        ed.on('init', () => {
          ...
          const listener = function (state, name, obj) {
            ...
            changes.set(changes.get().concat([{ uid: state ? obj.uid : null, name, state }]));
          };
