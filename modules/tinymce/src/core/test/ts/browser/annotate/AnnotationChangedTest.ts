...
UnitTest.asynctest(
  'browser.tinymce.core.annotate.AnnotationChangedTest',
  (success, failure) => {
    Theme();

    const changes: Cell<Array<{
      state: boolean;
      name: string;
      uid: string;
    }>> = Cell([]);

    const sAssertChanges = <T>(
      message: string,
      expected: Array<{ uid: string; state: boolean; name: string }>
    ): Step<T, T> =>
      Logger.t(
        message,
    ...
    TinyLoader.setupLight(
      function (editor: Editor, onSuccess, onFailure) {
        ...
        const sTestAnnotationEvents = <T>(
          label: string,
          start: number[],
          soffset: number,
          expected: Array<{ uid: string; name: string; state: boolean }>
        ): Step<T, T> =>
          StepSequence.sequenceSame<T>([
        ...
        const sTestChanges = <T>() =>
          StepSequence.sequenceSame<T>([
            ...
            tinyApis.sSetSelection(
              [2, 0],
              'This is the th'.length,
              [2, 0],
              'This is the thir'.length
            ),
            sAnnotate(editor, 'beta', 'id-three', {
              something: 'comment-three'
            }),
            ...
            sAnnotate(editor, 'gamma', 'id-four', {
              something: 'comment-four'
            }),
            ...
            sAnnotate(editor, 'delta', 'id-five', {
              something: 'comment-five'
            }),
        ...
        setup: (ed: Editor) => {
          ed.on('init', () => {
            ...
            const listener = function (state, name, obj) {
              ...
              if (state === false) {
                Assertions.assertEq(
                  'Argument count must be "2" (state, name) if state is false',
                  2,
                  arguments.length
                );
              } else {
                ...
                if (name === 'gamma') {
                  Assertions.assertEq(
                    'Gamma annotations must have 2 nodes',
                    2,
                    nodes.length
                  );
                }
              ...
              changes.set(
                changes
                  .get()
                  .concat([{ uid: state ? obj.uid : null, name, state }])
              );
            };
