...
const cScriptLoad = (id: string, url: string) =>
  Chain.async<any, Result<string, string>>((_input, next, _die) => {
    tinymce.Resource.load(id, url).then(
      (value) => {
...
const cAssertLoadSuccess = (expectedData: string) =>
...
const cAssertLoadFailure = (expectedErr: string) =>
  Chain.op<Result<string, string>>((actual) => {
    actual.fold(
      (err) => {
        Assertions.assertEq(
    ...
    [
      ...
      Step.label(
        ...
        Chain.asStep({}, [
          ...
          cAssertLoadFailure(
            `Script at URL "data:text/javascript,tinymce.Resource.add('invalid-id', 'value.4')" did not call \`tinymce.Resource.add('script.4', data)\` within 1 second`
          )
        ])
      )
    ],
