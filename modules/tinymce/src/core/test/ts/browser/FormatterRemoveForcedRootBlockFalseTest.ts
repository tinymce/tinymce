...
UnitTest.asynctest(
  'browser.tinymce.core.FormatterRemoveForcedRootBlockFalseTest',
  (success, failure) => {
    ...
    const getContent = (editor: Editor) =>
      editor.getContent().toLowerCase().replace(/[\r]+/g, '');

    suite.test(
      'Remove block format from first block with forced_root_block: false',
      (editor) => {
        ...
        LegacyUnit.equal(
          getContent(editor),
          'a<br />b',
          'Lines should be separated with br'
        );
      }
    );
