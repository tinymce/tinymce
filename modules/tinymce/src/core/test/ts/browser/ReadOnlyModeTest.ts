...
UnitTest.asynctest('browser.tinymce.core.ReadOnlyModeTest', (success, failure) => {
  ...
  TinyLoader.setup(
    function (editor: Editor, onSuccess, onFailure) {
      ...
      const sMouseOverTable = Chain.asStep(Element.fromDom(editor.getBody()), [UiFinder.cFindIn('table'), Mouse.cMouseOver]);
      ...
      Pipeline.async(
        ...
        [
          ...
          Log.stepsAsStep('TBA', 'Menus should close when switching to readonly mode', [
            sSetMode('design'),
            Chain.asStep(Body.body(), [UiFinder.cFindIn('.tox-mbtn:contains("File")'), Mouse.cClick]),
