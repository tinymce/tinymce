...
UnitTest.asynctest(
  'browser.tinymce.core.EditorManagerSetupTest',
  (success, failure) => {
    ...
    Pipeline.async(
      ...
      [
        Logger.t(
          'script baseURL and suffix with script in svg',
          Step.sync(() => {
            viewBlock.update(
              '<svg><script>!function(){}();</script></svg><script src="http://localhost/nonexistant/tinymce.min.js" type="application/javascript"></script>'
            );
            EditorManager.setup();
            Assertions.assertEq(
              'BaseURL is interpreted from the script src',
              EditorManager.baseURL,
              'http://localhost/nonexistant'
            );
            Assertions.assertEq(
              'Suffix is interpreted from the script src',
