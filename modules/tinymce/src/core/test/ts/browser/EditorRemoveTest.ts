...
UnitTest.asynctest(
  'browser.tinymce.core.EditorRemoveTest',
  (success, failure) => {
    Theme();
    ...
    const cAssertTextareaDisplayStyle = (expected) =>
      Chain.op((editor: any) => {
        const textareaElement = editor.getElement();

        Assert.eq(
          'element does not have the expected style',
          expected,
    ...
    Pipeline.async(
      ...
      [
        Logger.t(
          'remove editor without initializing it',
          Chain.asStep({}, [cCreateEditor, cRemoveEditor])
        ),

        Logger.t(
          'remove editor where the body has been removed',
          Chain.asStep({}, [
        ...
        Logger.t(
          'init editor with no display style',
          Chain.asStep({}, [
        ...
        Logger.t(
          'init editor with display: block',
          Chain.asStep({}, [
            McEditor.cFromHtml(
              '<textarea id="tinymce" style="display: block;"></textarea>',
              settings
            ),
            cAssertTextareaDisplayStyle('none'),
            cRemoveEditor,
