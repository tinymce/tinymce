...
UnitTest.asynctest(
  ...
  (success, failure) => {
    ...
    suite.asyncTest('get', function (_, done) {
      ...
      EditorManager.init({
        ...
        init_instance_callback(editor1) {
          ...
          LegacyUnit.equal(
            EditorManager.get(0) === EditorManager.activeEditor,
            true
          );
          ...
          LegacyUnit.equal(
            EditorManager.get(EditorManager.activeEditor.id) ===
    ...
    suite.asyncTest(
      'Do not reload language pack if it was already loaded or registered manually.',
      function (_, done) {
            ...
            EditorManager.init({
              ...
              init_instance_callback(editor2) {
                ...
                LegacyUnit.equal(
                  editor1.destroyed,
                  true,
                  'First editor instance should be destroyed'
                );
    ...
    suite.test('overrideDefaults', function () {
      ...
      LegacyUnit.strictEqual(
        ...
        'http://www.EditorManager.com/base'
      );
      LegacyUnit.strictEqual(EditorManager.suffix, 'x');
      LegacyUnit.strictEqual(
        new Editor('ed1', {}, EditorManager).settings.test,
        42
      );
      LegacyUnit.strictEqual(
        PluginManager.urls.testplugin,
        'http://custom.ephox.com/dir/testplugin'
      );
    ...
    suite.test('Init inline editor on invalid targets', function () {
      ...
      Tools.each(invalidNames.split(' '), function (invalidName) {
        ...
        LegacyUnit.strictEqual(
          EditorManager.get().length,
          0,
          'Should not have created an editor'
        );
