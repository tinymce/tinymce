...
UnitTest.asynctest(
  'browser.tinymce.core.content.InsertContentCommandTest',
  (success, failure) => {
    ...
    suite.test(
      'mceInsertContent HR at end of H1 with inline elements with P sibling',
      function (editor) {
        ...
        LegacyUnit.equal(
          getContent(editor),
          '<h1><strong>abc</strong></h1><hr /><p>def</p>'
        );
      }
    );
    ...
    suite.test('mceInsertContent empty block', function (editor) {
      ...
      LegacyUnit.equalDom(
        editor.selection.getNode(),
        editor.getBody().childNodes[1]
      );
      LegacyUnit.equal(editor.selection.getNode().nodeName, 'P');
      LegacyUnit.equal(
        getContent(editor),
        '<h1>a</h1><p>\u00a0</p><h1>bc</h1>'
      );
    });

    suite.test('mceInsertContent table at end of H1 with P sibling', function (
      editor
    ) {
      editor.setContent('<h1>abc</h1><p>def</p>');
      LegacyUnit.setSelection(editor, 'h1', 3);
      ...
      LegacyUnit.equal(
        getContent(editor),
        '<h1>abc</h1><table><tbody><tr><td>\u00a0</td></tr></tbody></table><p>def</p>'
      );
    });
    ...
    suite.test('mceInsertContent - pre in text of pre', function (editor) {
      ...
      LegacyUnit.equal(
        getContent(editor),
        '<pre>1</pre><pre>abc</pre><pre>4</pre>'
      );
      rng = normalizeRng(editor.selection.getRng());
      ok(rng.collapsed);
    ...
    suite.test('mceInsertContent - li inside li', function (editor) {
      ...
      LegacyUnit.equal(
        getContent(editor),
        '<ul><li>1</li><li>abc</li><li>4</li></ul>'
      );
      rng = normalizeRng(editor.selection.getRng());
      ok(rng.collapsed);
    ...
    suite.test(
      'mceInsertContent - text inside empty p with br caret node',
      function (editor) {
        let rng;
        ...
        editor.execCommand('mceInsertContent', false, 'abc');
        LegacyUnit.equal(
          editor.getBody().innerHTML.toLowerCase(),
          '<p>abc</p>'
        );
        rng = normalizeRng(editor.selection.getRng());
        ok(rng.collapsed);
    ...
    suite.test('mceInsertContent - legacy content', function (editor) {
      ...
      editor.execCommand(
        'mceInsertContent',
        false,
        '<strike>strike</strike><font size="7">font</font>'
      );
      LegacyUnit.equal(
        editor.getContent(),
    ...
    suite.test('mceInsertContent - forced root block', function (editor) {
      ...
      LegacyUnit.equal(
        editor.getContent().replace(/<p>\u00a0<\/p>/g, ''),
        '<p>test<strong>123</strong></p><!-- a -->'
      );
    });

    suite.test('mceInsertContent - mixed inline content inside td', function (
      editor
    ) {
      // Forced root block
      ...
      LegacyUnit.equal(
        editor.getContent(),
        '<table><tbody><tr><td>test<strong>123</strong><!-- a -->X</td></tr></tbody></table>'
      );
    });

    suite.test(
      'mceInsertContent - invalid insertion with spans on page',
      function (editor) {
        const startingContent =
            '<p>123 testing <em>span later in document</em></p>',
    ...
    suite.test(
      'mceInsertContent - text with space before at start of block',
      function (editor) {
    ...
    suite.test(
      'mceInsertContent - text with space after at end of block',
      function (editor) {
        editor.getBody().innerHTML = '<p>a</p>';
    ...
    suite.test(
      'mceInsertContent - text with space before/after at middle of block',
      function (editor) {
    ...
    suite.test(
      'mceInsertContent - inline element with space before/after at middle of block',
      function (editor) {
        editor.getBody().innerHTML = '<p>ac</p>';
        LegacyUnit.setSelection(editor, 'p', 1);
    ...
    suite.test(
      'mceInsertContent - block element with space before/after at middle of block',
      function (editor) {
        editor.getBody().innerHTML = '<p>ac</p>';
        LegacyUnit.setSelection(editor, 'p', 1);
    ...
    suite.test('mceInsertContent - span in span same style color', function (
      editor
    ) {
      ...
      LegacyUnit.equal(
        editor.getContent(),
        '<p><span style="color: #ff0000;">abc</span></p>'
      );
    });

    suite.test(
      'mceInsertContent - span in span different style color',
      function (editor) {
        editor.getBody().innerHTML = '<span style="color:#ff0000">ac</strong>';
        LegacyUnit.setSelection(editor, 'span', 1);
        editor.execCommand('mceInsertContent', false, {
        ...
        LegacyUnit.equal(
          editor.getContent(),
          '<p><span style="color: #ff0000;">a<span style="color: #00ff00;">b</span>c</span></p>'
        );
      }
    );

    suite.test('mceInsertContent - select with option element', function (
      editor
    ) {
      ...
      editor.execCommand(
        'mceInsertContent',
        false,
        '2<select><option selected="selected">3</option></select>'
      );
      LegacyUnit.equal(
        editor.getContent(),
        '<p>12<select><option selected="selected">3</option></select></p>'
      );
    });

    suite.test(
      'mceInsertContent - insert P in span style element #7090',
      function (editor) {
        ...
        LegacyUnit.equal(
          editor.getContent(),
          '<p><span style="color: red;">1</span></p><p>2</p><p>3</p>'
        );
      }
    );

    suite.test(
      'mceInsertContent - insert char at char surrounded by spaces',
      function (editor) {
    ...
    suite.test('mceInsertRawHTML - insert link inside p', function (editor) {
      ...
      LegacyUnit.equal(
        editor.getContent(),
        '<p>abc <a href="#">Hello world!</a></p>'
      );
    });

    suite.test(
      'mceInsertRawHTML - insert char at char surrounded by spaces',
      function (editor) {
        editor.setContent('<p>a b c</p>');
        LegacyUnit.setSelection(editor, 'p', 2, 'p', 3);
        editor.execCommand('mceInsertRawHTML', false, '<strong>X</strong>');
        LegacyUnit.equal(
          JSON.serialize(editor.getContent()),
          '"<p>a <strong>X</strong> c</p>"'
        );
      }
    );
