...
UnitTest.asynctest(
  'browser.tinymce.core.content.EditorContentTest',
  (success, failure) => {
    const getFontTree = () => {
          ...
          [
            ...
            Logger.t(
              ...
              GeneralSteps.sequence([
                ...
                Step.sync(() => {
                  const tree = EditorContent.getContent(editor, {
                    format: 'tree'
                  }) as Node;
                  Assertions.assertHtml(
                    'Should be expected tree html',
                    '<p>tree</p>',
                    toHtml(tree)
                  );
                })
            ...
            Logger.t(
              'getContent tree filtered',
              GeneralSteps.sequence([
                Step.sync(() => {
                  EditorContent.setContent(
                    editor,
                    '<p><font size="7">x</font></p>',
                    { format: 'raw' }
                  );
                  const tree = EditorContent.getContent(editor, {
                    format: 'tree'
                  ...
                  Assertions.assertHtml(
                    'Should be expected tree filtered html',
                    '<p><span style="font-size: 300%;">x</span></p>',
                    toHtml(tree)
                  );
                })
              ])
            ...
            Logger.t(
              'getContent tree using public api',
              GeneralSteps.sequence([
                tinyApis.sSetContent('<p>html</p>'),
                Step.sync(() => {
                  const tree = editor.getContent({ format: 'tree' }) as Node;
                  Assertions.assertHtml(
                    'Should be expected filtered html',
                    '<p>html</p>',
                    toHtml(tree)
                  );
                })
              ...
              GeneralSteps.sequence([
                ...
                Step.sync(() => {
                  ...
                  Assertions.assertHtml(
                    'Should be expected html',
                    '<p>new html</p>',
                    <string>EditorContent.getContent(editor)
                  );
            ...
            Logger.t(
              ...
              GeneralSteps.sequence([
                ...
                Step.sync(() => {
                  ...
                  Assertions.assertHtml(
                    'Should be expected filtered html',
                    '<span style="font-size: 300%;">x</span>',
                    <string>EditorContent.getContent(editor)
                  );
                })
              ])
            ...
            Logger.t(
              'setContent tree using public api',
              GeneralSteps.sequence([
                tinyApis.sSetContent('<p>tree</p>'),
                Step.sync(() => {
