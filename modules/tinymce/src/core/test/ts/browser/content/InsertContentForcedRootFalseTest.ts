...
UnitTest.asynctest(
  'browser.tinymce.core.content.InsertContentForcedRootBlockFalseTest',
  (success, failure) => {
    ...
    suite.test('insertAtCaret - selected image with bogus div', function (
      ...
      editor.getBody().innerHTML =
        '<img src="about:blank" /><div data-mce-bogus="all">x</div>';
      editor.focus();
      // editor.selection.setCursorLocation(editor.getBody(), 0);
      editor.selection.select(editor.dom.select('img')[0]);
      InsertContent.insertAtCaret(editor, 'a');
      LegacyUnit.equal(
        trimBrs(editor.getBody().innerHTML),
        'a<div data-mce-bogus="all">x</div>'
      );
    });
    ...
    suite.test('insertAtCaret - selected text with bogus div', function (
      ...
      LegacyUnit.equal(
        trimBrs(editor.getBody().innerHTML),
        'b<div data-mce-bogus="all">x</div>'
      );
    });
