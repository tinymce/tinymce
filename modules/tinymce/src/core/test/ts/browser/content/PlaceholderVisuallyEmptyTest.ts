...
UnitTest.asynctest('browser.tinymce.core.content.PlaceholderVisuallyEmptyTest', (success, failure) => {
  ...
  TinyLoader.setupLight(
    (editor, onSuccess, onFailure) => {
      ...
      const sAssertEmpty = (label: string, expected: boolean, forcedRootBlockFalse: boolean) =>
        Step.sync(() => {
          const body = editor.getBody();
          Assertions.assertEq(label, expected, isVisuallyEmpty(editor.dom, body, forcedRootBlockFalse ? '' : 'p'));
        });

      const sTestEmpty = (content: string, forcedRootBlockFalse: boolean = false) =>
        GeneralSteps.sequence([tinyApi.sSetRawContent(content), sAssertEmpty(`Check "${content}" is empty`, true, forcedRootBlockFalse)]);

      const sTestNotEmpty = (content: string, forcedRootBlockFalse: boolean = false) =>
      ...
      Pipeline.async(
        {},
        [
          Log.stepsAsStep('TINY-3917', 'Check initial content is empty', [sAssertEmpty('Check base empty content', true, true)]),
