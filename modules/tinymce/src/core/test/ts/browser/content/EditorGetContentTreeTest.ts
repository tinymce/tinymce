...
UnitTest.asynctest(
  'browser.tinymce.core.content.EditorGetContentTreeTest',
  (success, failure) => {
    Theme();
      ...
      function (editor, onSuccess, onFailure) {
        ...
        Pipeline.async(
          ...
          [
            ...
            Logger.t(
              'Get selection as tree',
              GeneralSteps.sequence([
                tinyApis.sSetContent('<p>ab<em>c</em></p>'),
                tinyApis.sSetSelection([0, 0], 1, [0, 1, 0], 1),
                Step.sync(function () {
                  const html = toHtml(
                    editor.selection.getContent({ format: 'tree' })
                  );
                  Assertions.assertHtml(
                    'Should be expected selection html',
                    'b<em>c</em>',
                    html
                  );
                })
            ...
            Logger.t(
              'Get selection as tree with whitespace',
              GeneralSteps.sequence([
