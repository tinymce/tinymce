...
UnitTest.asynctest(
  'browser.tinymce.core.content.EditorResetContentTest',
  (success, failure) => {
    Theme();
    ...
    TinyLoader.setupLight(
      (editor: Editor, onSuccess, onFailure) => {
        ...
        const sAssertEditorState = (content: string) =>
          Step.sync(() => {
            ...
            Assertions.assertHtml(
              'Editor content should be expected html',
              content,
              html
            );
            Assertions.assertEq(
              'Editor should not be dirty',
              false,
              editor.isDirty()
            );
            Assertions.assertEq(
              'UndoManager should not have any undo levels',
              false,
              editor.undoManager.hasUndo()
            );
            Assertions.assertEq(
              'UndoManager should not have any redo levels',
              false,
              editor.undoManager.hasRedo()
            );
            Assertions.assertEq(
              'Editor start content should match the original content',
        ...
        Pipeline.async(
          ...
          [
            Logger.t(
              'Reset editor content/state with initial content',
            ...
            Logger.t(
              'Reset editor content/state with custom content',
              GeneralSteps.sequence([
                tinyApis.sSetContent('<p>some</p><p>content</p>'),
            ...
            Logger.t(
              'Reset editor content/state with multiple undo levels',
              GeneralSteps.sequence([
                tinyApis.sSetContent('<p>some</p><p>content</p>'),
                ...
                Step.sync(() => {
                  Assertions.assertEq(
                    'Editor should be dirty',
                    true,
                    editor.isDirty()
                  );
                  Assertions.assertEq(
                    'UndoManager should have some undo levels',
                    true,
                    editor.undoManager.hasUndo()
                  );
                }),
