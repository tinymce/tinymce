...
UnitTest.asynctest('browser.tinymce.core.api.dom.RangeUtilsTest', function (
  ...
  const sTestDontNormalizeAtAnchors = Logger.t(
    `Don't normalize at anchors`,
    Step.sync(function () {
      viewBlock.update('a<a href="#">b</a>c');

      const rng1 = createRange(
      ...
      const rng2 = createRange(
        viewBlock.get().lastChild,
        0,
        viewBlock.get().lastChild,
        0
      );
  ...
  const sTestNormalize = GeneralSteps.sequence([sTestDontNormalizeAtAnchors]);
