  ...
  (success, failure) => {
    ...
    const assertResult = function (
      ...
      LegacyUnit.strictEqual(
        firstResult.element.src.indexOf(uploadedBlobInfo.id() + '.png') !== -1,
        true
      );
      LegacyUnit.strictEqual(firstResult.uploadUri, uploadUri);
      LegacyUnit.strictEqual(firstResult.blobInfo.id(), uploadedBlobInfo.id());
      LegacyUnit.strictEqual(
        firstResult.blobInfo.name(),
        uploadedBlobInfo.name()
      );
      LegacyUnit.strictEqual(
        firstResult.blobInfo.filename(),
        uploadedBlobInfo.filename()
      );
      LegacyUnit.strictEqual(
        firstResult.blobInfo.blob(),
        uploadedBlobInfo.blob()
      );
      LegacyUnit.strictEqual(
        firstResult.blobInfo.base64(),
        uploadedBlobInfo.base64()
      );
      LegacyUnit.strictEqual(
        firstResult.blobInfo.blobUri(),
        uploadedBlobInfo.blobUri()
      );
      LegacyUnit.strictEqual(
        firstResult.blobInfo.uri(),
        uploadedBlobInfo.uri()
      );
      LegacyUnit.equal(
        '<p><img src="' + uploadedBlobInfo.filename() + '" /></p>',
        editor.getContent()
      );
    ...
    suite.asyncTest('_scanForImages', function (editor, done, fail) {
      ...
      editor
        ...
        .then(function (result) {
          ...
          LegacyUnit.strictEqual(
            editor.editorUpload.blobCache.get(blobInfo.id()),
            blobInfo
          );
        })
        .then(done)
    ...
    suite.asyncTest('uploadImages retain blob urls after upload', function (
      editor,
      done,
      die
    ) {
      ...
      const assertResult = function (result) {
        ...
        LegacyUnit.strictEqual(
          hasBlobAsSource(result[0].element),
          true,
          'Not a blob url'
        );
        LegacyUnit.equal(
    ...
    suite.asyncTest('uploadImages reuse filename', (editor, done, _die) => {
      ...
      const assertResult = function (
        editor: Editor,
        _uploadedBlobInfo: BlobInfo,
        result: UploadResult[]
      ) {
        LegacyUnit.strictEqual(result.length, 1);
        LegacyUnit.strictEqual(result[0].status, true);
        LegacyUnit.equal(
          '<p><img src="custom.png?size=small" /></p>',
          editor.getContent()
        );
      ...
      editor.uploadImages((result) => {
        ...
        editor.uploadImages((_result) => {
          ...
          LegacyUnit.strictEqual(
            img.src.indexOf('custom.png?size=small&') !== -1,
            true,
            'Check the cache invalidation string was added'
          );
          LegacyUnit.strictEqual(
            editor.getContent(),
            '<p><img src="custom.png?size=small" /></p>'
    ...
    suite.asyncTest('uploadConcurrentImages (fail)', function (editor, done) {
      ...
      editor.settings.images_upload_handler = function (
        _data,
        _success,
        failure
      ) {
    ...
    suite.asyncTest(`Don't upload bogus image`, function (editor, done) {
      ...
      editor.getBody().innerHTML =
        '<img src="' + testBlobDataUri + '" data-mce-bogus="1">';
    ...
    suite.test('Retain blobs not in blob cache', function (editor) {
      editor.getBody().innerHTML =
        '<img src="blob:http%3A//host/f8d1e462-8646-485f-87c5-f9bcee5873c6">';
      LegacyUnit.equal(
        '<p><img src="blob:http%3A//host/f8d1e462-8646-485f-87c5-f9bcee5873c6" /></p>',
        editor.getContent()
      );
    });
