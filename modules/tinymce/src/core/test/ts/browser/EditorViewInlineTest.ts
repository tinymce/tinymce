...
UnitTest.asynctest('browser.tinymce.core.EditorViewInlineTest', function (
  ...
  const sTestIsXYInContentArea = function (editor, deltaX, deltaY) {
    ...
    const dy2 = -5 - deltaY;
    return Step.label(
      'Check points relative to deltaX=' + deltaX + ' deltaY=' + deltaY,
      Chain.asStep({}, [
        Chain.fromParent(
          Chain.label(
          ...
          [
            ...
            Chain.label(
              'Check 〈bottom right〉 + (' +
                ...
                dy2 +
                ') is ' +
                (hiddenScrollbar ? 'inside' : 'outside') +
                ' editor',
              Chain.op((rect) =>
                Assertions.assertEq(
                  hiddenScrollbar
    ...
    function (editor, onSuccess, onFailure) {
      ...
      const sSetContentToBigDiv = Step.label(
        ...
        tinyApis.sSetContent(
          '<div style="width: 5000px; height: 5000px">X</div>'
        )
      );
      ...
      Pipeline.async(
        ...
        [
          Logger.t(
            'isXYInContentArea without borders, margin',
          ...
          Logger.t(
            'isXYInContentArea with borders, margin',
            GeneralSteps.sequence([
