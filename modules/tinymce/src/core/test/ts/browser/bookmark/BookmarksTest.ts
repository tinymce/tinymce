...
UnitTest.asynctest('browser.tinymce.core.bookmark.BookmarksTest', (success, failure) => {
  ...
  const cGetBookmark = (type: number, normalized: boolean) =>
    NamedChain.direct(
      'editor',
      Chain.mapper((editor) => GetBookmark.getBookmark(editor.selection, type, normalized)),
      'bookmark'
    );
  ...
  const cGetFilledPersistentBookmark = (_type: number, _normalized: boolean) =>
    NamedChain.direct(
      'editor',
      Chain.mapper((editor) => GetBookmark.getPersistentBookmark(editor.selection, true)),
      'bookmark'
    );
  ...
  const cSetupEditor = (content, startPath, startOffset, endPath, endOffset) =>
    NamedChain.read(
      'editor',
      Chain.fromChains([ApiChains.cSetContent(content), ApiChains.cSetSelection(startPath, startOffset, endPath, endOffset)])
    );
  ...
  const cAssertSelection = (spath, soffset, fpath, foffset) =>
    NamedChain.read('editor', ApiChains.cAssertSelection(spath, soffset, fpath, foffset));

  const sBookmarkTest = (namedChains) =>
    Chain.asStep({}, [NamedChain.asChain(Arr.flatten([[cCreateNamedEditor], namedChains, [cRemoveEditor]]))]);
