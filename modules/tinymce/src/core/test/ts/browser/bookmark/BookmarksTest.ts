...
UnitTest.asynctest(
  'browser.tinymce.core.bookmark.BookmarksTest',
  (success, failure) => {
    Theme();
    ...
    const cGetFilledPersistentBookmark = (
      _type: number,
      _normalized: boolean
    ) =>
      NamedChain.direct(
    ...
    const cCreateNamedEditor = NamedChain.write(
      'editor',
    ...
    const cSetCursor = (path, offset) =>
      NamedChain.read('editor', ApiChains.cSetCursor(path, offset));

    const cResolveBookmark = cBundleOp((input) => {
      const rng = ResolveBookmark.resolve(
        input.editor.selection,
        input.bookmark
      ).getOrDie('Should be resolved');
      input.editor.selection.setRng(rng);
    });

    const cAssertSelection = (spath, soffset, fpath, foffset) =>
      NamedChain.read(
        'editor',
        ApiChains.cAssertSelection(spath, soffset, fpath, foffset)
      );

    const sBookmarkTest = (namedChains) =>
      Chain.asStep({}, [
        NamedChain.asChain(
          Arr.flatten([[cCreateNamedEditor], namedChains, [cRemoveEditor]])
        )
      ]);

    const cAssertRangeBookmark = (spath, soffset, fpath, foffset) =>
      cBundleOp((input) => {
        Assert.eq(
          'Should be a range bookmark',
          true,
          isRangeBookmark(input.bookmark)
        );
        assertRawRange(
          Element.fromDom(input.editor.getBody()),
          input.bookmark.rng,
          spath,
          soffset,
          fpath,
    ...
    const cAssertPathBookmark = (expectedStart, expectedEnd) =>
      cBundleOp((input) => {
        Assert.eq(
          'Should be a path bookmark',
          true,
          isPathBookmark(input.bookmark)
        );
        Assert.eq(
          'Should be expected start path',
          expectedStart,
          input.bookmark.start
        );
        Assert.eq(
          'Should be expected end path',
          expectedEnd,
          input.bookmark.end
        );
      });

    const cAssertIndexBookmark = (expectedName, expectedIndex) =>
      cBundleOp((input) => {
        Assert.eq(
          'Should be an index bookmark',
          true,
          isIndexBookmark(input.bookmark)
        );
        Assert.eq('Should be expected name', expectedName, input.bookmark.name);
        Assert.eq(
          'Should be expected index',
          expectedIndex,
          input.bookmark.index
        );
      });

    const cAssertStringPathBookmark = (expectedStart, expectedEnd) =>
      cBundleOp((input) => {
        Assert.eq(
          'Should be a string bookmark',
          true,
          isStringPathBookmark(input.bookmark)
        );
        Assert.eq(
          'Should be expected start',
          expectedStart,
          input.bookmark.start
        );
    ...
    Pipeline.async(
      ...
      [
        ...
        Logger.t(
          'Get persistent bookmark on element indexes',
          sBookmarkTest([
            cSetupEditor('<p><img src="about:blank"></p>', [0], 0, [0], 1),
        ...
        Logger.t(
          'Get persistent bookmark marker spans on text offsets',
          sBookmarkTest([
            cSetupEditor('<p>abc</p>', [0, 0], 1, [0, 0], 2),
            cGetBookmark(0, false),
            cAssertApproxRawContent(
              '<p>a<span data-mce-type="bookmark" id="mce_1_start"></span>b<span id="mce_1_end"></span>c</p>'
            ),
        ...
        Logger.t(
          'Get persistent bookmark marker spans on element indexes',
          sBookmarkTest([
            cSetupEditor('<p><input><input></p>', [0], 0, [0], 2),
            cGetBookmark(0, false),
            cAssertApproxRawContent(
              '<p><span data-mce-type="bookmark" id="mce_1_start"></span><input><input><span id="mce_1_end"></span></p>'
            ),
        ...
        Logger.t(
          'Get persistent bookmark filled with marker spans on text offsets',
          sBookmarkTest([
            cSetupEditor('<p>abc</p>', [0, 0], 1, [0, 0], 2),
