...
UnitTest.asynctest('browser.tinymce.core.bookmark.BookmarksTest', (success, failure) => {
  ...
  const cGetFilledPersistentBookmark = (type: number, normalized: boolean) => NamedChain.direct('editor', Chain.mapper((editor) => GetBookmark.getPersistentBookmark(editor.selection, true)), 'bookmark');
