...
UnitTest.asynctest('browser.tinymce.core.EditorTest', function (success, failure) {
  ...
  suite.test('Block script execution', function (editor) {
    ...
    LegacyUnit.equal(
      ...
      '<script type="mce-no/type"></script>' + '<script type="mce-x"></script>' + '<script type="mce-x"></script>' + '<p>x</p>'
    );
    LegacyUnit.equal(editor.getContent(), '<script></script>' + '<script type="x"></script>' + '<script type="x"></script>' + '<p>x</p>');
  });
