...
UnitTest.asynctest('webdriver.tinymce.core.content.PlaceholderTest', (success, failure) => {
  ...
  TinyLoader.setup(
    (editor: Editor, onSuccess, onFailure) => {
      ...
      const sSetContent = (content: string) => GeneralSteps.sequence([tinyApi.sSetContent(content), sClearCount]);
      ...
      Pipeline.async(
        ...
        [
          tinyApi.sFocus(),
          Log.stepsAsStep('TINY-3917', 'Check placeholder shown with no content', [sSetContent('<p></p>'), sAssertPlaceholderExists]),
