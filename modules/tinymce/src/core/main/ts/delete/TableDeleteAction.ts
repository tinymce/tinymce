...
const deleteAction = Adt.generate([{ removeTable: ['element'] }, { emptyCells: ['cells'] }]);

const isRootFromElement = (root: Element<any>): ((cur: Element<any>) => boolean) => (cur: Element<any>): boolean => Compare.eq(root, cur);
...
const isExpandedCellRng = (cellRng: TableCellRng): boolean => !Compare.eq(cellRng.start, cellRng.end);
...
const getTableFromCellRng = (cellRng: TableCellRng, isRoot: (e: Element<any>) => boolean): Option<Element<DomElement>> =>
  getClosestTable(cellRng.start, isRoot).bind((startParentTable) =>
    getClosestTable(cellRng.end, isRoot).bind((endParentTable) =>
      Options.someIf(Compare.eq(startParentTable, endParentTable), startParentTable)
    )
  );
...
const isWithinSameTable = (isRoot: (e: Element<any>) => boolean, cellRng: TableCellRng) => getTableFromCellRng(cellRng, isRoot).isSome();
