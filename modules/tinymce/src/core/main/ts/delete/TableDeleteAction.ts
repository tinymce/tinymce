...
const isRootFromElement = (
  root: Element<any>
): ((cur: Element<any>) => boolean) => (cur: Element<any>): boolean =>
  Compare.eq(root, cur);

const getClosestCell = (
  container: DomNode,
  isRoot: (e: Element<any>) => boolean
) => SelectorFind.closest(Element.fromDom(container), 'td,th', isRoot);

const getClosestTable = (cell, isRoot) =>
  SelectorFind.ancestor(cell, 'table', isRoot);

const isExpandedCellRng = (cellRng: TableCellRng): boolean =>
  !Compare.eq(cellRng.start, cellRng.end);

const getTableFromCellRng = (
  cellRng: TableCellRng,
  isRoot: (e: Element<any>) => boolean
  ...
  getClosestTable(cellRng.start, isRoot).bind((startParentTable) =>
    getClosestTable(cellRng.end, isRoot).bind((endParentTable) =>
      Options.someIf(
        Compare.eq(startParentTable, endParentTable),
        startParentTable
      )
    )
  );
...
const getCellRangeFromStartTable = (
  cellRng: TableCellRng,
  isRoot
): Option<TableCellRng> =>
  getClosestTable(cellRng.start, isRoot).bind((table) =>
    Arr.last(getTableCells(table)).map((endCell) =>
      tableCellRng(cellRng.start, endCell)
    )
  );
...
const partialSelection = (
  ...
  return rng.collapsed
    ? Option.none()
    : Options.lift2(startCell, endCell, tableCellRng).fold(
        () =>
          startCell.fold(
            () =>
              endCell.bind((endCell) =>
                getClosestTable(endCell, isRoot).bind((table) =>
                  Arr.head(getTableCells(table)).map((startCell) =>
        ...
        (cellRng: any) =>
          isWithinSameTable(isRoot, cellRng)
            ? Option.none()
            : getCellRangeFromStartTable(cellRng, isRoot)
      );
};

const isWithinSameTable = (
  isRoot: (e: Element<any>) => boolean,
  cellRng: TableCellRng
...
const getTableSelectionFromCellRng = (
  cellRng: TableCellRng,
  isRoot: (e: Element<any>) => boolean
) =>
...
const getTableSelectionFromRng = (root, rng: Range) => {
  ...
  return getCellRng(rng, isRoot).bind((cellRng) =>
    getTableSelectionFromCellRng(cellRng, isRoot)
  );
};

const getCellIndex = <T>(
  cells: Element<T>[],
  cell: Element<T>
): Option<number> => Arr.findIndex(cells, (x) => Compare.eq(x, cell));
...
const getAction = (tableSelection: TableSelection) =>
  getSelectedCells(tableSelection).map((selected) => {
...
export const getActionFromRange = (root, rng: Range) =>
  getTableSelectionFromRng(root, rng).bind(getAction);
