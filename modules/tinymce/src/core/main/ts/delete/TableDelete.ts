...
const collapseAndRestoreCellSelection = (editor: Editor) => {
  ...
  if (NodeType.isTableCell(selectedNode.dom) && Empty2.isEmpty(editor.schema, selectedNode)) {
...
const emptySingleTableCells = (editor: Editor, cells: SugarElement<HTMLTableCellElement>[], outsideDetails: Optional<OutsideTableDetails>): Optional<() => void> =>
  Optional.some(() => {
    ...
    const cellsToClean = outsideDetails.bind(({ rng, isStartInTable }) => {
      ...
      rng.deleteContents();
      handleEmptyBlock(editor, isStartInTable, outsideBlock.filter(Fun.curry(Empty2.isEmpty, editor.schema)));
      ...
      if (!Empty2.isEmpty(editor.schema, endPointCell)) {
...
const emptyMultiTableCells = (
  ...
  Optional.some(() => {
    ...
    const startTableCellsToClean = Empty2.isEmpty(editor.schema, startCell) ? startTableCells : startTableCells.slice(1);
    const endTableCellsToClean = Empty2.isEmpty(editor.schema, endCell) ? endTableCells : endTableCells.slice(0, -1);
...
const deleteCaretCells = (editor: Editor, forward: boolean, rootElm: SugarElement<Node>, startElm: SugarElement<Node>): Optional<() => void> => {
  ...
  return getParentCell(rootElm, startElm).bind(
    (fromCell) => Empty2.isEmpty(editor.schema, fromCell) ?
...
const deleteCaretCaption = (
  ...
  const from = CaretPosition.fromRangeStart(editor.selection.getRng());
  return Empty2.isEmpty(editor.schema, fromCaption) ?
