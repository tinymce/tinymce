...
const deleteCellRange = (editor: Editor, rootElm, rng: Range) =>
  TableDeleteAction.getActionFromRange(rootElm, rng).map((action) =>
    action.fold(Fun.curry(deleteTableElement, editor), Fun.curry(emptyCells, editor))
  );
...
const deleteTableRange = (editor: Editor, rootElm, rng: Range, startElm) =>
  getParentCaption(rootElm, startElm)
    .fold(
...
const deleteBetweenCells = (editor: Editor, rootElm, forward: boolean, fromCell, from: CaretPosition) =>
  CaretFinder.navigate(forward, editor.getBody(), from).bind((to) =>
    getParentCell(rootElm, Element.fromDom(to.getNode())).map((toCell) => Compare.eq(toCell, fromCell) === false)
  );
...
const isDeleteOfLastCharPos = (fromCaption, forward: boolean, from: CaretPosition, to: CaretPosition) =>
  CaretFinder.firstPositionIn(fromCaption.dom())
    .bind((first) =>
      CaretFinder.lastPositionIn(fromCaption.dom()).map((last) =>
        forward ? from.isEqual(first) && to.isEqual(last) : from.isEqual(last) && to.isEqual(first)
      )
    )
    .getOr(true);
...
const validateCaretCaption = (rootElm, fromCaption, to: CaretPosition) =>
  getParentCaption(rootElm, Element.fromDom(to.getNode())).map((toCaption) => Compare.eq(toCaption, fromCaption) === false);
...
const deleteCaretCells = (editor: Editor, forward: boolean, rootElm, startElm) => {
  ...
  return getParentCell(rootElm, startElm)
    .bind((fromCell) =>
      Empty.isEmpty(fromCell) ? emptyElement(editor, fromCell) : deleteBetweenCells(editor, rootElm, forward, fromCell, from)
    )
    .getOr(false);
};
