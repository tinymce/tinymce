...
const deleteCaptionRange = (editor: Editor, caption) =>
...
const deleteBetweenCells = (
  ...
  CaretFinder.navigate(forward, editor.getBody(), from).bind((to) =>
    getParentCell(rootElm, Element.fromDom(to.getNode())).map(
      (toCell) => Compare.eq(toCell, fromCell) === false
    )
  );
...
const validateCaretCaption = (rootElm, fromCaption, to: CaretPosition) =>
  getParentCaption(rootElm, Element.fromDom(to.getNode())).map(
    (toCaption) => Compare.eq(toCaption, fromCaption) === false
  );
...
const deleteCaret = (editor: Editor, forward: boolean, startElm: Element) => {
  ...
  return getParentCaption(rootElm, startElm).fold(
    ...
    (fromCaption) =>
      deleteCaretCaption(editor, forward, rootElm, fromCaption).getOr(false)
  );
};
