...
const hasSameParent = function (blockBoundary: BlockBoundary): boolean {
  return Traverse.parent(blockBoundary.from.block)
...
const isEditable = function (blockBoundary: BlockBoundary): boolean {
  return (
    NodeType.isContentEditableFalse(blockBoundary.from.block.dom()) === false &&
    NodeType.isContentEditableFalse(blockBoundary.to.block.dom()) === false
  );
};
