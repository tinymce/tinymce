...
const isCompoundElement = (node: Node) =>
  ElementType.isTableCell(Element.fromDom(node)) ||
  ElementType.isListItem(Element.fromDom(node));
...
const isAtContentEditableBlockCaret = (
  forward: boolean,
  from: CaretPosition
) => {
...
const isDeleteFromCefDifferentBlocks = (
  root: Node,
  forward: boolean,
  from: CaretPosition,
  to: CaretPosition
...
const deleteEmptyBlockOrMoveToCef = (
  root: Node,
  forward: boolean,
  from: CaretPosition,
  to: CaretPosition
...
const findCefPosition = (root: Node, forward: boolean, from: CaretPosition) =>
  CaretFinder.fromPosition(forward, root, from).bind(function (to) {
...
const skipMoveToActionFromInlineCefToContent = (
  root: Node,
  from: CaretPosition,
  deleteAction
) =>
...
const getContentEditableAction = (
  root: Node,
  forward: boolean,
  from: CaretPosition
) => {
...
const read = (root: Node, forward: boolean, rng: Range): Option<any> => {
  const normalizedRange = CaretUtils.normalizeRange(
    forward ? 1 : -1,
    root,
    rng
  );
  ...
  if (forward === false && isAfterContentEditableFalse(from)) {
    ...
    return findPreviousBr(rootElement, from).map((br) =>
    ...
    return findNextBr(rootElement, from).map((br) =>
      DeleteAction.remove(br.getNode())
    );
  } else {
