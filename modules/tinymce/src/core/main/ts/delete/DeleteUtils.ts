...
const getParentBlock = (
  ...
  Compare.contains(rootNode, elm)
    ? PredicateFind.closest(
        elm,
        (element) => isTextBlock(element) || isListItem(element),
...
const willDeleteLastPositionInElement = function (
  forward: boolean,
  fromPos: CaretPosition,
  elm
) {
...
export { getParentBlock, paddEmptyBody, willDeleteLastPositionInElement };
