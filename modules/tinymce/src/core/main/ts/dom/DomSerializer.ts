...
interface DomSerializer {
  ...
  serialize(
    node: DomElement,
    parserArgs: { format: 'tree' } & SerializerArgs
  ): Node;
...
const getHtmlFromNode = function (dom: DOMUtils, node: DomElement, args) {
  const html = Zwsp.trim(
    args.getInner ? node.innerHTML : dom.getOuterHTML(node)
  );
  return args.selection || isWsPreserveElement(Element.fromDom(node))
    ? html
    : Tools.trim(html);
};
...
export { DomSerializer, DomSerializerSettings };
