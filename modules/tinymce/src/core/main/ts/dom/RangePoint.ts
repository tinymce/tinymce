...
const isXYWithinRange = function (
  clientX: number,
  clientY: number,
  range: Range
): boolean {
  ...
  if (
    ...
    if (NodeType.isElement(elm)) {
      return Arr.exists(elm.getClientRects(), (rect) =>
        ClientRect.containsXY(rect, clientX, clientY)
      );
    }
  }

  return Arr.exists(range.getClientRects(), (rect) =>
    ClientRect.containsXY(rect, clientX, clientY)
  );
};

export { isXYWithinRange };
