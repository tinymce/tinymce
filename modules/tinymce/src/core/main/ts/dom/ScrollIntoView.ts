...
const excludeFromDescend = (element: SugarElement) =>
  Node.name(element) === 'textarea';

const fireScrollIntoViewEvent = (
  editor: Editor,
  data: ScrollIntoViewEvent
): boolean => {
  const scrollEvent = editor.fire('ScrollIntoView', data);
  return scrollEvent.isDefaultPrevented();
};

const fireAfterScrollIntoViewEvent = (
  editor: Editor,
  data: ScrollIntoViewEvent
): void => {
...
const descend = (
  ...
  offset: number
): { element: SugarElement; offset: number } => {
...
const createMarker = (element: SugarElement, offset: number): MarkerInfo => {
  ...
  const span = SugarElement.fromHtml(
    '<span data-mce-bogus="all">' + Zwsp.ZWSP + '</span>'
  );
...
const elementMarker = (element: HTMLElement): MarkerInfo =>
  markerInfo(SugarElement.fromDom(element), Fun.noop);
...
const preserveWith = (
  editor: Editor,
  f: (startElement: SugarElement, endElement: SugarElement) => void,
  rng: Range
) => {
...
const rangeIntoWindow = (editor: Editor, rng: Range, alignToTop?: boolean) =>
  withMarker(editor, intoWindow, rng, alignToTop);
const elementIntoWindow = (
  editor: Editor,
  element: HTMLElement,
  alignToTop?: boolean
) => withElement(editor, element, intoWindow, alignToTop);

const rangeIntoFrame = (editor: Editor, rng: Range, alignToTop?: boolean) =>
  withMarker(editor, intoFrame, rng, alignToTop);
const elementIntoFrame = (
  editor: Editor,
  element: HTMLElement,
  alignToTop?: boolean
...
export { scrollElementIntoView, scrollRangeIntoView };
