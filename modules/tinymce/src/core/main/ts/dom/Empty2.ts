...
const isContent = (node: Node, rootNode: Node, schema: Schema, options: AdjustOptions): boolean =>
  Type.isFunction(options.isContent) && options.isContent(node)
...
const isEmptyNode = (schema: Schema, targetNode: Node, options?: AdjustOptions): boolean => {
  ...
  if (options.check_root_as_content) {
    if (isContent(targetNode, targetNode, schema, options)) {
      return false;
    }
  }
  ...
  do {
    ...
    if (isContent(node, targetNode, schema, options)) {
      return false;
    }
