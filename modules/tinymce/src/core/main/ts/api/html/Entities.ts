...
const encodeAllRaw = (text: string): string =>
  ('' + text).replace(rawCharsRegExp, function (chr) {
    return baseEntities[chr] || chr;
  });
...
const decode = (text: string): string =>
  text.replace(entityRegExp, function (all, numeric) {
    if (numeric) {
      ...
      if (numeric > 0xffff) {
        ...
        return String.fromCharCode(
          0xd800 + (numeric >> 10), // eslint-disable-line no-bitwise
          0xdc00 + (numeric & 0x3ff) // eslint-disable-line no-bitwise
        );
      }
