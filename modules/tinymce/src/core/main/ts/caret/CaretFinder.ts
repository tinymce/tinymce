...
const navigateIgnore = (forward: boolean, root: Element, from: CaretPosition, ignoreFilter: (pos: CaretPosition) => boolean) =>
  navigate(forward, root, from).bind((pos) => (ignoreFilter(pos) ? navigateIgnore(forward, root, pos, ignoreFilter) : Option.some(pos)));
...
export { fromPosition, nextPosition, prevPosition, navigate, navigateIgnore, positionIn, firstPositionIn, lastPositionIn };
