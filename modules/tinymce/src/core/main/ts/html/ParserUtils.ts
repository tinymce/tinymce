...
const isEmpty = (
  schema: Schema,
  nonEmptyElements: SchemaMap,
  whitespaceElements: SchemaMap,
  node: Node
) =>
...
const isLineBreakNode = (node: Node, blockElements: SchemaMap) =>
  node && (blockElements[node.name] || node.name === 'br');
