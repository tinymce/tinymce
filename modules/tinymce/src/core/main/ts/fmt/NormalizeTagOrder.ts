...
const collectFontElements = (formatter: Formatter, wrappers: SugarElement<Element>[]) =>
...
export const normalizeFontSizeElementsAfterApply = (editor: Editor, appliedFormat: string, wrappers: SugarElement<Element>[]): void => {
  if (isNormalizingFormat(appliedFormat)) {
    const fontSizeElements = isFontSizeAlteringFormat(appliedFormat) ? wrappers : collectFontElements(editor.formatter, wrappers);
