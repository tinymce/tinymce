...
export default (): void => {
  ...
  DomEvent.bind(toggleModeBtn, 'click', () => {
    ...
    TextContent.set(statusDiv, `Mode: ${tinymce.activeEditor?.mode.get()}, Enabled: ${tinymce.activeEditor?.options.get('enabled')}`);
  });
  ...
  DomEvent.bind(toggleDisabledBtn, 'click', () => {
    tinymce.activeEditor?.options.set('enabled', !tinymce.activeEditor.options.get('enabled'));
    TextContent.set(statusDiv, `Mode: ${tinymce.activeEditor?.mode.get()}, Enabled: ${tinymce.activeEditor?.options.get('enabled')}`);
  });
  ...
  tinymce.init({
    ...
    setup: (ed) => {
      ed.on('init', () => {
        TextContent.set(statusDiv, `Mode: ${ed.mode.get()}, Enabled: ${ed.options.get('enabled')}`);
      });
    }
  });
};
