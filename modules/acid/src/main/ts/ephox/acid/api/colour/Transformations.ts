...
const anyToHex = (color: string): Hex => {
  const hexColorOpt = HexColour.fromString(color.replace('#', '').toUpperCase());

  const colorOpt = hexColorOpt.orThunk(() => {
    return RgbaColour.fromString(color).map((color) => HexColour.fromRgba(color));
  });

  return colorOpt.getOrThunk(() => {
