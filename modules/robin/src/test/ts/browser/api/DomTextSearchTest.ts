...
UnitTest.test('DomTextSearchTest', function () {
  ...
  const elemResult = DomTextSearch.expandRight(element, 0, {
    regex: wordbreaker,
    attempt: stopAtGap
  });
  checkAbort(elemResult);

  const elemResultB = DomTextSearch.expandRight(element, 8, {
    regex: wordbreaker,
    attempt: stopAtGap
  });
  checkAbort(elemResultB);

  const textResult1 = DomTextSearch.expandRight(text, 0, {
    regex: wordbreaker,
    attempt: stopAtGap
  });
  checkInfo(textResult1, text, 0);

  const textResult2 = DomTextSearch.expandRight(text, 1, {
    regex: wordbreaker,
    attempt: stopAtGap
  });
  checkInfo(textResult2, text, 9);

  const textResult = DomTextSearch.expandRight(text, 8, {
    regex: wordbreaker,
    attempt: stopAtGap
  });
  checkInfo(textResult, text, 9);

  const textResult3 = DomTextSearch.expandRight(text, 9, {
    regex: wordbreaker,
    attempt: stopAtGap
  });
  ...
  const textBResult = DomTextSearch.expandRight(textB, 0, {
    regex: wordbreaker,
    attempt: stopAtGap
  });
  checkInfo(textBResult, textB, 0);

  const textBResult1 = DomTextSearch.expandRight(textB, 1, {
    regex: wordbreaker,
    attempt: stopAtGap
  });
  ...
  checkAbort(
    DomTextSearch.expandLeft(element, 0, {
      regex: wordbreaker,
      attempt: stopAtGap
    })
  );
  checkAbort(
    DomTextSearch.expandLeft(element, 1, {
      regex: wordbreaker,
      attempt: stopAtGap
    })
  );
  checkAbort(
    DomTextSearch.expandLeft(element, 2, {
      regex: wordbreaker,
      attempt: stopAtGap
    })
  );
  checkEdge(
    DomTextSearch.expandLeft(text, 0, {
      regex: wordbreaker,
      attempt: stopAtGap
    }),
    text
  ...
  checkInfo(
    DomTextSearch.expandLeft(text, 1, {
      regex: wordbreaker,
      attempt: stopAtGap
    }),
    text,
  ...
  checkInfo(
    DomTextSearch.expandLeft(text, 3, {
      regex: wordbreaker,
      attempt: stopAtGap
    }),
    text,
  ...
  checkInfo(
    DomTextSearch.expandLeft(text, '@maurizio@'.length, {
      regex: wordbreaker,
      attempt: stopAtGap
    }),
    text,
  ...
  checkInfo(
    DomTextSearch.expandRight(textR, 0, {
      regex: wordfinder,
      attempt: stopAtGap
    }),
    textR,
    3
  ); // 3 is the location after the last space, starting from the left
  checkInfo(
    DomTextSearch.expandLeft(textR, 8, {
      regex: wordbreaker,
      attempt: stopAtGap
    }),
    textR,
    2
  ); // 2 is the location after the last character, starting from the right
  const textL = Element.fromText('words   ');
  //                     Pos:  0    45  8
  Assert.eq('eq', 8, textL.dom().length);
  checkInfo(
    DomTextSearch.expandRight(textL, 0, {
      regex: wordbreaker,
      attempt: stopAtGap
    }),
    textL,
    5
  ); // 5 is the location after the last character, starting from the left
  checkInfo(
    DomTextSearch.expandLeft(textL, 8, {
      regex: wordfinder,
      attempt: stopAtGap
    }),
    textL,
    4
  ); // 4 is the location after the last space, starting from the right
  ...
  const r1 = DomTextSearch.expandRight(w1, 3, {
    regex: wordbreaker,
    attempt: stopAtGap
  });
  checkInfo(r0, w1, 2);
  checkInfo(r1, w1, 7);
  const r2 = DomTextSearch.expandRight(w1, 7, {
    regex: wordfinder,
    attempt: stopAtGap
  });
  const r3 = DomTextSearch.expandRight(w2, 2, {
    regex: wordbreaker,
    attempt: stopAtGap
  });
  checkInfo(r2, w2, 2);
  checkInfo(r3, w2, 7);
  const r4 = DomTextSearch.expandRight(w2, 7, {
    regex: wordfinder,
    attempt: stopAtGap
  });
  const r5 = DomTextSearch.expandRight(w3, 2, {
    regex: wordbreaker,
    attempt: stopAtGap
  });
  checkInfo(r4, w3, 2);
  checkInfo(r5, w3, 7);
  const r6 = DomTextSearch.expandRight(w3, 7, {
    regex: wordfinder,
    attempt: stopAtGap
  });
  const r7 = DomTextSearch.expandRight(w3, 9, {
    regex: wordbreaker,
    attempt: stopAtGap
  });
  ...
  //                                               ^
  const l1 = DomTextSearch.expandLeft(w3, 9, {
    regex: wordfinder,
    attempt: stopAtGap
  });
  // '<div>  wordy  <span>  words  </span>  wordd  </div>'
  //                                            ^
  checkInfo(l1, w3, 6);
  const l2 = DomTextSearch.expandLeft(w3, 6, {
    regex: wordbreaker,
    attempt: stopAtGap
  });
  // '<div>  wordy  <span>  words  </span>  wordd  </div>'
  //                                       ^
  checkInfo(l2, w3, 1);
  const l3 = DomTextSearch.expandLeft(w3, 1, {
    regex: wordfinder,
    attempt: stopAtGap
  });
  // '<div>  wordy  <span>  words  </span>  wordd  </div>'
  //                            ^
  checkInfo(l3, w2, 6);
  const l4 = DomTextSearch.expandLeft(w2, 6, {
    regex: wordbreaker,
    attempt: stopAtGap
  });
  // '<div>  wordy  <span>  words  </span>  wordd  </div>'
  //                       ^
  checkInfo(l4, w2, 1);
  const l5 = DomTextSearch.expandLeft(w2, 1, {
    regex: wordfinder,
    attempt: stopAtGap
  });
  // '<div>  wordy  <span>  words  </span>  wordd  </div>'
  //             ^
  checkInfo(l5, w1, 6);
  const l6 = DomTextSearch.expandLeft(w1, 6, {
    regex: wordbreaker,
    attempt: stopAtGap
  });
  // '<div>  wordy  <span>  words  </span>  wordd  </div>'
  //        ^
  checkInfo(l6, w1, 1);
  const l7 = DomTextSearch.expandLeft(w1, 1, {
    regex: wordfinder,
    attempt: stopAtGap
  });
  // '<div>  wordy  <span>  words  </span>  wordd  </div>'
