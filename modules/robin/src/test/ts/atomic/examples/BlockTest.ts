...
UnitTest.test('BlockTest', function () {
  const doc = TestUniverse(
    Gene('root', 'root', [
      Gene('d1', 'div', [
        TextGene('d1_t1', 'List: '),
        Gene('ol1', 'ol', [
          Gene('li1', 'li', [TextGene('li1_text', 'Apples')]),
          Gene('li2', 'li', [TextGene('li2_text', 'Beans')]),
          Gene('li3', 'li', [TextGene('li3_text', 'Carrots')]),
          Gene('li4', 'li', [TextGene('li4_text', 'Diced Tomatoes')])
        ]),
        Gene('ol2', 'ol', [
          Gene('li5', 'li', [TextGene('li5_text', 'Elephants')])
        ])
      ])
    ])
  );
  ...
  const check = function (
    ...
    Assert.eq(
      'Checking parent :: Option',
      expected.getOr('none'),
      actual.getOr(Gene('none', 'none')).id
    );
  };
  ...
  check(
    Option.some('ol1'),
    ['li2', 'li4'],
    Look.predicate(doc, (item: Gene) => Structure.isBlock(doc, item))
  );
  check(
    Option.some('d1'),
    ['li1_text', 'li5_text'],
    Look.exact(doc, Gene('d1', 'div'))
  );
});
