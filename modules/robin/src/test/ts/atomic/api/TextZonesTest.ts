...
UnitTest.test('TextZonesTest', function () {
  ...
  const checkSingle = function (info: ArbIds) {
    ...
    assertProps(
      'Testing zones for single(' + info.startId + ')',
      doc1,
      actual.zones
    );
    return true;
  ...
  const checkRange = function (info: ArbRangeIds) {
    ...
    assertProps(
      'Testing zones for range(' + info.startId + '->' + info.finishId + ')',
      doc1,
      actual.zones
    );
    return true;
  ...
  PropertyAssertions.check(
    'Check that empty tags produce no zones',
