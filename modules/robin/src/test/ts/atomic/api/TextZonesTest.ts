...
UnitTest.test('TextZonesTest', function () {
        ...
        [
          ...
          Gene(
            ...
            [
              ...
              TextGene('en-g', 'er'),
              TextGene('en-h', ' '),
              Gene('isolated', 'span', [TextGene('en-i', 'isolated')]),
              TextGene('en-j', ' '),
  ...
  PropertyAssertions.check('Check text single', [arbIds(doc1, doc1.property().isText)], checkSingle);

  PropertyAssertions.check('Check text range', [arbRangeIds(doc1, doc1.property().isText)], checkRange);

  PropertyAssertions.check('Check that empty tags produce no zones', [arbIds(doc1, doc1.property().isEmptyTag)], function (info: ArbIds) {
    const item = doc1.find(doc1.get(), info.startId).getOrDie();
  ...
  PropertyAssertions.check('Check empty range', [arbRangeIds(doc1, doc1.property().isEmptyTag)], checkRange);

  PropertyAssertions.check('Check boundary single', [arbIds(doc1, doc1.property().isBoundary)], checkSingle);

  PropertyAssertions.check('Check boundary range', [arbRangeIds(doc1, doc1.property().isBoundary)], checkRange);
  ...
  PropertyAssertions.check('Check any tag range', [arbRangeIds(doc1, Fun.constant(true))], checkRange);
});
