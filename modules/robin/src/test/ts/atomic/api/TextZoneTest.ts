...
UnitTest.test('TextZoneTest', function () {
    ...
    expected.fold(
      function () {
        actual.fold(
          ...
          function (act) {
            assert.fail(label + '\nShould not have created zone: ' + JSON.stringify(JSON.stringify(rawOne(doc1, act))));
          }
        );
      },
      function (exp) {
  ...
  PropertyAssertions.check('Check text single', [arbIds(doc1, doc1.property().isText)], checkSingleProp);

  PropertyAssertions.check('Check text range', [arbRangeIds(doc1, doc1.property().isText)], checkRangeProp);

  PropertyAssertions.check('Check that empty tags produce no zone', [arbIds(doc1, doc1.property().isEmptyTag)], function (info: ArbIds) {
    const item = doc1.find(doc1.get(), info.startId).getOrDie();
  ...
  PropertyAssertions.check('Check empty range', [arbRangeIds(doc1, doc1.property().isEmptyTag)], checkRangeProp);

  PropertyAssertions.check('Check boundary single', [arbIds(doc1, doc1.property().isBoundary)], checkSingleProp);

  PropertyAssertions.check('Check boundary range', [arbRangeIds(doc1, doc1.property().isBoundary)], checkRangeProp);
  ...
  PropertyAssertions.check('Check any tag range', [arbRangeIds(doc1, Fun.constant(true))], checkRangeProp);
});
