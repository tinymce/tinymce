...
import {
  ...
  ArbRangeIds,
  arbRangeIds
} from 'ephox/robin/test/Arbitraries';
...
UnitTest.test('TextZoneTest', function () {
  ...
  const checkZone = function (
    label: string,
    expected: Option<RawZone>,
    actual: Option<Zone<Gene>>
  ) {
    expected.fold(
      function () {
        actual.fold(
          ...
          function (act) {
            assert.fail(
              label +
                '\nShould not have created zone: ' +
                JSON.stringify(JSON.stringify(rawOne(doc1, act)))
            );
          }
        );
      },
      function (exp) {
        actual.fold(
          function () {
            assert.fail(
              label +
                '\nDid not find a zone. Expected to find: ' +
  ...
  const checkSingleProp = function (info: ArbIds) {
    ...
    return actual.forall(function (zone) {
      assertProps('Testing zone for single(' + info.startId + ')', doc1, [
        zone
      ]);
      return true;
    });
  ...
  const checkRangeProp = function (info: ArbRangeIds) {
    ...
    return actual.forall(function (zone) {
      assertProps(
        'Testing zone for range(' + info.startId + '->' + info.finishId + ')',
        doc1,
        [zone]
      );
      return true;
  ...
  PropertyAssertions.check(
    'Check that empty tags produce no zone',
  ...
  PropertyAssertions.check(
    ...
    [
      arbRangeIds(doc1, function (item: Gene) {
        return !(
          doc1.property().isBoundary(item) ||
          doc1.property().isEmptyTag(item) ||
          doc1.property().isText(item)
        );
      })
    ],
    checkSingleProp
