...
const fromBoundedWith = function <E, D>(
  ...
  const groups: ZoneDetails<E>[] = Parent.subset(universe, left, right)
...
const transformEdges = function <E>(leftEdge: WordDecisionItem<E>, rightEdge: WordDecisionItem<E>) {
  return function <D>(universe: Universe<E, D>, element: E) {
    return universe.eq(element, leftEdge.item)
      ? leftEdge
      : universe.eq(element, rightEdge.item)
      ? rightEdge
      : WordDecision.detail(universe, element);
  };
};
