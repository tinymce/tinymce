...
const fromBoundedWith = function <E, D>(
  ...
  const output = TextZones.fromBoundedWith(
    universe,
    left,
    right,
    envLang,
...
const fromInline = function <E, D>(
  ...
  return edges.isEmpty
    ? scour(universe, element, envLang, onlyLang)
    : fromBoundedWith(
        universe,
        edges.left.item,
        edges.right.item,
...
const scour = function <E, D>(
  universe: Universe<E, D>,
  element: E,
  envLang: string,
  onlyLang: string
) {
  const lastOffset = universe.property().isText(element)
    ? universe.property().getText(element).length
    : universe.property().children(element).length;
...
export { fromRange, fromBounded, fromInline, empty };
