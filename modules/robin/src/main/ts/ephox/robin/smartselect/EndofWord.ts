...
const toEnd = function <E>(
  cluster: WordDecisionItem<E>[],
  start: E,
  soffset: number
) {
  if (cluster.length === 0) {
...
const all = function <E>(cluster: WordDecisionItem<E>[]) {
  ...
  const last = cluster[cluster.length - 1];
  return Option.some(
    WordRange(first.item, first.start, last.item, last.finish)
  );
};
...
const scan = function <E, D>(
  ...
  const postLength = Arr.filter(text.substring(offset, text.length), function (
    c
  ) {
  ...
  const rightEdge =
    offset + postLength === text.length && cluster.right.length === 0;
...
const before = function <E, D>(
  ...
  return info.rightEdge
    ? Option.none<WordRange<E>>()
    : toEnd(info.all, item, bindex);
};
...
const after = function <E, D>(
  ...
  const info = scan(universe, item, offset);
  return info.leftEdge
    ? Option.none<WordRange<E>>()
    : fromStart(info.all, item, aindex);
};
...
const both = function <E, D>(
  ...
  return bindex === aindex
    ? Option.none<WordRange<E>>()
    : Option.some(WordRange(item, bindex, item, aindex));
};
...
const neither = function <E, D>(
  universe: Universe<E, D>,
  item: E,
  offset: number
) {
  const info = scan(universe, item, offset);
  return info.leftEdge || info.rightEdge
    ? Option.none<WordRange<E>>()
    : all(info.all);
};

export { before, after, both, neither };
