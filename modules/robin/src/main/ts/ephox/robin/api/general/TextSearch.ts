...
// block boundary.
const expandLeft = function <E, D>(
  universe: Universe<E, D>,
  item: E,
  offset: number,
  rawSeeker: TextSearchSeeker
) {
  const seeker = seekerSig(rawSeeker);

  const process: TextSeekerPhaseProcessor<E, D> = function (
    uni,
    phase,
    pItem,
    pText,
    ...
    return TextSearchBase.rfind(
      pText.substring(0, lastOffset),
      seeker.regex()
    ).fold(
      function () {
...
// 'edge' returns the text element where the search stopped due to being adjacent to a
// block boundary.
const expandRight = function <E, D>(
  universe: Universe<E, D>,
  item: E,
  offset: number,
  rawSeeker: TextSearchSeeker
  ...
  const process: TextSeekerPhaseProcessor<E, D> = function (
    ...
    const optPos = TextSearchBase.lfind(
      pText.substring(firstOffset),
      seeker.regex()
    );
    return optPos.fold(
      function () {
