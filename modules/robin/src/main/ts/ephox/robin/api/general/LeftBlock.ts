...
const goLeft = function <E, D>(
  ...
  return next
    .map(function (n) {
      ...
      const isBlock =
        universe.property().isEmptyTag(n.item()) ||
        universe.property().isBoundary(n.item());
      if (isBlock) {
        return [];
      }
      // Only include the current item if the strategy permits it. Top won't add parents.
      const inclusions = strategy.inclusion(universe, n, item)
        ? [n.item()]
        : [];
...
const top = <E, D>(universe: Universe<E, D>, item: E) =>
  run(Walks.top, universe, item);
const all = <E, D>(universe: Universe<E, D>, item: E) =>
  run(Walks.all, universe, item);
