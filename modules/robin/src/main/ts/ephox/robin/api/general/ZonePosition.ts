...
export interface ZonePosition<E> {
  fold: <T>(aboveView: (item: E) => T, inView: (item: E) => T, belowView: (item: E) => T) => T;
  match: <T>(branches: { aboveView: (item: E) => T; inView: (item: E) => T; belowView: (item: E) => T }) => T;
...
} = Adt.generate([{ aboveView: ['item'] }, { inView: ['item'] }, { belowView: ['item'] }]);
