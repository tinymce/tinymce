...
const slice = function <E, D>(
  ...
  return firstIndex > -1 && lastIndex > -1
    ? Option.some(children.slice(firstIndex, lastIndex + 1))
    : Option.none<E[]>();
};

const breakPath = function <E, D>(
  universe: Universe<E, D>,
  element: E,
  common: E,
  breaker: (
  ...
  const isTop = function (elem: E) {
    return universe
      .property()
      .parent(elem)
      .fold(Fun.constant(true), Fun.curry(universe.eq, common));
  };

  return Parent.breakPath(universe, element, isTop, breaker);
};

const breakLeft = function <E, D>(
  universe: Universe<E, D>,
  element: E,
  common: E
) {
...
const same = function <E, D>(
  ...
  return universe.eq(common, element)
    ? Option.some([element])
    : breakToCommon(universe, common, element, element);
};

const breakToCommon = function <E, D>(
  universe: Universe<E, D>,
  common: E,
  start: E,
  finish: E
    ...
    .orThunk(function () {
      ...
      return Parent.sharedOne(
        ...
        function (_, elem) {
          return isRoot(elem)
            ? Option.some(elem)
            : universe.up().predicate(elem, isRoot);
        },
        [start, finish]
      );
    })
...
const diff = function <E, D>(
  ...
  return shared(universe, isRoot, start, finish, ceiling).bind(function (
    common
  ) {
...
const fracture = function <E, D>(
  ...
  start: E,
  finish: E,
  ceiling: (e: E) => E = Fun.identity
) {
  return universe.eq(start, finish)
    ? same(universe, isRoot, start, ceiling)
    : diff(universe, isRoot, start, finish, ceiling);
};
