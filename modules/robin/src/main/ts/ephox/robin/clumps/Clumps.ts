      ...
      const resumption = isParent(universe, element, boundary)
        ...
        : (function () {
            ...
            return !universe.eq(leaf.element(), boundary)
              ...
              : Gather.walk(universe, boundary, Gather.advance, Gather.walkers().right()).map(function (g) {
                  return g.item();
                });
          })();
...
const single = function <E, D>(universe: Universe<E, D>, isRoot: (e: E) => boolean, item: E, soffset: number, foffset: number): Clump<E>[] {
  ...
  return changed
    ? collect(universe, isRoot, start.element(), start.offset(), finish.element(), finish.offset())
