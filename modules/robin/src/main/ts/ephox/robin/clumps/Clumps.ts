...
const descendBlock = function <E, D>(
  ...
  if (!skip(universe, leaf.element())) {
    ...
    return skipToRight(universe, isRoot, leaf.element())
      .map(function (next) {
...
const isBlock = function <E, D>(universe: Universe<E, D>, item: E) {
  return (
    Structure.isFrame(universe, item) ||
    Structure.isBlock(universe, item) ||
    Arr.contains(['li'], universe.property().name(item))
  );
};

const skipToRight = function <E, D>(
  universe: Universe<E, D>,
  isRoot: (e: E) => boolean,
  item: E
) {
  return Gather.seekRight(
    universe,
    item,
    function (i) {
...
const skip = function <E, D>(universe: Universe<E, D>, item: E) {
  ...
  return universe
    ...
    .exists(function (p) {
      ...
      return Arr.contains(
        ['table', 'tbody', 'thead', 'tfoot', 'tr', 'ul', 'ol'],
        universe.property().name(p)
      );
    });
};

const walk = function <E, D>(
  universe: Universe<E, D>,
  isRoot: (e: E) => boolean,
  mode: Transition,
  element: E,
...
const scan = function <E, D>(
  ...
  return step.fold(
    ...
    function (boundary, last, _mode) {
      ...
      const resumption = isParent(universe, element, boundary)
        ? resume(universe, isRoot, boundary, target)
        : (function () {
            const leaf = descendBlock(universe, isRoot, boundary);
            return !universe.eq(leaf.element(), boundary)
              ? Option.some(leaf.element())
              : Gather.walk(
                  universe,
                  boundary,
                  Gather.advance,
                  Gather.walkers().right()
                ).map(function (g) {
                  return g.item();
                });
          })();
      ...
      return resumption.fold(
        ...
        function (n) {
          ...
          return [current].concat(
            scan(universe, isRoot, Gather.advance, n, n, target)
          );
        }
      );
    },
...
const getEnd = function <E, D>(universe: Universe<E, D>, item: E) {
  return universe.property().isText(item)
    ? universe.property().getText(item).length
    : universe.property().children(item).length;
};

const drop = function <E, D>(
  universe: Universe<E, D>,
  item: E,
  offset: number
): E {
...
const skipInBlock = function <E, D>(
  universe: Universe<E, D>,
  isRoot: (e: E) => boolean,
  item: E,
  offset: number
...
const doCollect = function <E, D>(
  ...
  return Arr.map(raw, function (r) {
    ...
    const foff = universe.eq(r.finish, finish)
      ? foffset
      : getEnd(universe, r.finish);
...
const single = function <E, D>(
  ...
  const changed =
    !universe.eq(start.element(), item) || !universe.eq(finish.element(), item);
  return changed
    ? collect(
        universe,
        isRoot,
