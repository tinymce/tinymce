...
const has = function (element: Element<DomNode>, key: string) {
  ...
  return dom && (dom as DomElement).hasAttribute
    ? (dom as DomElement).hasAttribute(key)
    : false;
};
...
const transferOne = function (
  source: Element<DomElement>,
  destination: Element<DomElement>,
  attr: string
) {
