...
const ancestor = function <T extends DomElement = DomElement>(
  scope: Element<DomNode>,
  selector: string,
  isRoot?: (e: Element<DomNode>) => boolean
) {
  return PredicateFind.ancestor(
    scope,
    function (e): e is Element<T> {
...
const closest = function <T extends DomElement = DomElement>(
  scope: Element<DomNode>,
  selector: string,
  isRoot?: (e: Element<DomNode>) => boolean
) {
