...
const inAncestor = <A, D, E>(
  ancestor: Element<A>,
  descendants: Element<D>[],
  element: Element<E>,
  index: number
): AddressInAncestor<A, D, E> => ({
...
const inParent = <P, C, E>(
  parent: Element<P>,
  children: Element<C>[],
  element: Element<E>,
  index: number
): AddressInParent<P, C, E> => ({
  ...
  return Traverse.parent(element).bind(function (parent) {
    ...
    return indexOf(children, element).map(function (index) {
      return inParent(
        parent,
        children,
        element as Element<E & DomNode & ChildNode>,
        index
      );
    });
  });
...
const selectorsInParent = function <
  E extends DomNode,
  S extends DomElement = DomElement
>(element: Element<E>, selector: string) {
  return Traverse.parent(element).bind(function (parent) {
...
const descendantsInAncestor = function <
  E extends DomNode,
  A extends DomElement = DomElement,
  D extends DomElement = DomElement
>(element: Element<E>, ancestorSelector: string, descendantSelector: string) {
  return SelectorFind.closest<A>(element, ancestorSelector).bind(function (
    ancestor
  ) {
    const descendants = SelectorFilter.descendants<D>(
      ancestor,
      descendantSelector
    );
