...
const ancestor = function <A>(scope: Element, transform: (e: Element) => Option<A>, isRoot?): Option<A> {
  ...
  while (element.parentNode) {
    ...
    if (transformed.isSome()) { return transformed; } else if (stop(el)) { break; }
  }
  return Option.none<A>();
};

const closest = function <A>(scope: Element, transform: (e: Element) => Option<A>, isRoot?): Option<A> {
