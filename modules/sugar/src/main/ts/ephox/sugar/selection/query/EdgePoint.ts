import { ClientRect, Document, DOMRect, Node as DomNode, Element as DomElement } from '@ephox/dom-globals';
...
const createCollapsedNode = function (doc: Element<Document>, target: Element<DomNode>, collapseDirection: boolean) {
...
const locateInElement = function (doc: Element<Document>, node: Element<DomElement>, x: number) {
...
const locateInEmpty = function (doc: Element<Document>, node: Element<DomElement>, x: number) {
  const rect = node.dom().getBoundingClientRect();
  const collapseDirection = getCollapseDirection(rect, x);
  return Option.some(createCollapsedNode(doc, node, collapseDirection));
};

const search = function (doc: Element<Document>, node: Element<DomElement>, x: number) {
