...
const createCollapsedNode = function (
  doc: Element<Document>,
  target: Element<DomNode>,
...
const locateInElement = function (
  doc: Element<Document>,
...
const search = function (
  ...
  const f =
    Traverse.children(node).length === 0 ? locateInEmpty : locateInElement;
  return f(doc, node, x);
};
