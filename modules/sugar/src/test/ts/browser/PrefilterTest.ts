...
UnitTest.test('Browser Test: PrefilterTest', function () {
  const root = Element.fromHtml('<div>' + '<span>dog</span>' + '<br>' + '<img>' + '<span>cat</span>' + '</div>');
  ...
  const check = function (label, expected, elementPath, offset) {
    const element = Hierarchy.follow(root, elementPath).getOrDie(
      'Test: ' + label + '. Could not find the element path within root: ' + elementPath
    );
