...
UnitTest.test('AriaRegisterTest', function () {
  const checkTextButton = function (expectedElement, expectedContent, element, contentElement) {
  ...
  const checkLabelledField = function (expectedDom, field, name, labelText) {
    const element = AriaRegister.labelledField(field, name, labelText);
  ...
  const pair = function (store: object, key: string, baseAssertion, buildAssertion) {
    const strAssert = function (label, actual) {
    ...
    const arrAssert = function (label, array) {
