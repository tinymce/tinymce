...
UnitTest.test('AriaRegisterTest', function () {
  ...
  const matchToString = function (subject) {
  ...
  const checkValues = function (label, type, expected, getActual) {
    Obj.each(expected, function (exp, k) {
      exp.fold(function () { // no actual & no expected -> OK
        ...
        getActual(k).fold(function () { // no actual, but expected
          ...
          const ok = StringMatch.matches(expMatch, actual);
  ...
  const oStarts = Fun.compose(Option.some, function (s) { return StringMatch.starts(s, StringMatch.caseInsensitive); });
  const oExact = Fun.compose(Option.some, function (s) { return StringMatch.exact(s, StringMatch.caseInsensitive); });

  // This test is used to ensure that the tokens created by the constious aria voice pathways are
  // correct. Note, that nothing will actually be adding to the page, this is all about checking
  // the values themselves.

  const check = function (label, rootAttributes, attributes, styles, root, token) {
    checkValues(label, 'Parent Attribute', rootAttributes, function (k) {
      const a = Attr.get(root, k);
      return Option.from(a);
    });

    checkValues(label, 'Attribute', attributes, function (k) {
      const a = Attr.get(token, k);
      return Option.from(a);
    });
  ...
  const checkDynamic = function (label, rootAttributes, attributes, styles, voice, text) {
    const root = Element.fromTag('div');
    voice(root, text);
    const token = Traverse.firstChild(root).getOrDie();
    check(label, rootAttributes, attributes, styles, root, token);
    assert.eq(text, Html.get(token));
  };

  const checkStatic = function (label, rootAttributes, attributes, styles, voice, text) {
    const root = Element.fromTag('div');
    const token = AriaVoice.describe(root, text);
    check(label, rootAttributes, attributes, styles, root, token);
    assert.eq(text, Html.get(token));
  };
