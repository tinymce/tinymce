...
): SimpleResult<any, Record<string, any>> =>
  values.length > 0
    ? SimpleResult.svalue(
        Merger.deepMerge(base, Merger.merge.apply(undefined, values))
      )
...
const consolidateObj = <E>(
  objects: Array<SimpleResult<E[], any>>,
  base: Record<string, any>
): SimpleResult<E[], any> => {
