...
const moveBy = function (
  warehouse: Warehouse,
  cell: Element,
  row: number,
  column: number
) {
  return Warehouse.findItem(warehouse, cell, Compare.eq).bind(function (
    detail
  ) {
    const startRow =
...
const intercepts = function (
  ...
  return CellGroup.getAnyBox(warehouse, start, finish).map(function (bounds) {
    const inside = Warehouse.filterItems(
      warehouse,
      Fun.curry(CellBounds.inSelection, bounds)
    );
...
export { moveBy, intercepts, parentCell };
