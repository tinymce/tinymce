  ...
  return Arr.map(grid, function (row) {
    return GridRow.mapCells(row, function (cell) {
      return isTarget(cell)
        ? Structs.elementnew(substitution(cell.element(), comparator), true)
        : cell;
    });
  });
};

const notStartRow = function (
  grid: Structs.RowCells[],
  rowIndex: number,
  colIndex: number,
  comparator: CompElm
...
const notStartColumn = function (
  row: Structs.RowCells,
  index: number,
  comparator: CompElm
) {
  return (
...
export { replaceColumn, replaceRow };
