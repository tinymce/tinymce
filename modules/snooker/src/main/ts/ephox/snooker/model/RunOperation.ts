...
const findInWarehouse = (warehouse: Warehouse, element: Element): Option<DetailExt> =>
  Arr.findMap(warehouse.all, (r) => Arr.find(r.cells(), (e) => Compare.eq(element, e.element())));
...
const onMergable = (_warehouse: Warehouse, target: TargetMergable): Option<ExtractMergable> => target.mergable();

const onUnmergable = (_warehouse: Warehouse, target: TargetUnmergable): Option<Element[]> => target.unmergable();
...
export { run, toDetailList, onCell, onCells, onPaste, onPasteRows, onMergable, onUnmergable };
