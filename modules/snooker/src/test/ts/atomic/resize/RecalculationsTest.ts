...
UnitTest.test('RecalculationsTest', function () {
  ...
  const expectedParts: (
    widths: { element: string; width: number }[],
    heights: { element: string; height: number }[]
  ) => Parts = Struct.immutable('widths', 'heights');
  ...
  check(
    [expectedParts([{ element: 'a', width: 10 }], [{ element: 'a', height: 10 }])],
    [r('r0', [d('a', 1, 1)], 'tbody')],
    dimensions([10], [10])
  );
  ...
  check(
    [
      expectedParts(
        [
          { element: 'a00', width: 20 },
          { element: 'a01', width: 20 },
          { element: 'a10', width: 20 },
          { element: 'a11', width: 20 }
        ],
        [
          { element: 'a00', height: 15 },
          { element: 'a01', height: 15 },
          { element: 'a10', height: 9 },
          { element: 'a11', height: 9 }
        ]
      )
    ],
    [r('r0', [d('a00', 1, 1), d('a01', 1, 1)], 'tbody'), r('r1', [d('a10', 1, 1), d('a11', 1, 1)], 'tbody')],
    dimensions([20, 20], [15, 9])
  );

  // 2x2 grid merged into a single cell with total dimensions double the width and height
  check(
    [expectedParts([{ element: 'a', width: 40 }], [{ element: 'a', height: 60 }])],
    [
      r('r0', [d('a', 2, 2)], 'tbody'),
      r('r1', [], 'tbody') // optional
    ],
  ...
  check(
    ...
    [r('r0', [d('a', 2, 2), d('b', 1, 1)], 'tbody'), r('r1', [d('c', 1, 1)], 'tbody')],
