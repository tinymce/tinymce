...
UnitTest.test('CellBounds.isWithin Test', function () {
  ...
  const f = (fakeEle: any, cells: Structs.Detail[], section: 'tbody') =>
    Structs.rowdata(fakeEle as Element, cells, section);

  const testTableA = [
    f(
      'r1',
      [s('a', 1, 1), s('b', 1, 1), s('c', 1, 1), s('d', 1, 1), s('e', 1, 1)],
      'tbody'
    ),
    ...
    f(
      'r5',
      [s('s', 1, 1), s('t', 1, 1), s('u', 1, 1), s('v', 1, 1), s('z', 1, 1)],
      'tbody'
    )
  ];
  ...
  const checkInSelection = function (
    expected: boolean,
    warehouse: Warehouse,
    bounds: Structs.Bounds,
    row: number,
  ...
  const testTableC = [
    ...
    f(
      'r5',
      [s('l', 1, 1), s('m', 1, 1), s('n', 1, 1), s('o', 1, 1), s('p', 1, 1)],
      'tbody'
    )
  ];
