...
import { DtEvent, getEventType, setEventType } from 'ephox/dragster/datatransfer/EventType';
import { getMode, DtMode, setMode } from 'ephox/dragster/datatransfer/Mode';
...
describe('atomic.dragster.datatransfer.DataTransferCloneTest', () => {
  ...
  const getTestDataTransfer = (mode: DtMode): DataTransfer => {
    const dataTransfer = createDataTransfer();

    setMode(dataTransfer, DtMode.ReadWrite);

    // set some test data
    setEventType(dataTransfer, DtEvent.Dragstart);
  ...
  const assertTestDataTransfer = (dataTransfer: DataTransfer, isClone: boolean) => {
    ...
    KAssert.eqSome(normalizeLabel('should have expected event'), DtEvent.Dragstart, getEventType(dataTransfer));
  ...
  const testCloneDataTransfer = (mode: DtMode) => {
  ...
  it('TINY-9601: Can create clone of dataTransfer in read-write mode', () => testCloneDataTransfer(DtMode.ReadWrite));

  it('TINY-9601: Can create clone of dataTransfer in read-only mode', () => testCloneDataTransfer(DtMode.ReadOnly));

  it('TINY-9601: Can create clone of dataTransfer in protected mode', () => testCloneDataTransfer(DtMode.Protected));
});
