...
export const baseToolbarToggleButtonFields = [FieldSchema.defaultedBoolean('active', false)].concat(baseToolbarButtonFields);

export const toggleButtonSchema = ValueSchema.objOf(
  baseToolbarToggleButtonFields.concat([FieldSchema.strictString('type'), FieldSchema.strictFunction('onAction')])
);
