...
export const groupToolbarButtonSchema = ValueSchema.objOf(
  [
    ...
    FieldSchema.strictOf(
      'items',
      ValueSchema.oneOf([
        ValueSchema.arrOfObj([FieldSchema.strictString('name'), FieldSchema.strictArrayOf('items', ValueSchema.string)]),
        ValueSchema.string
      ])
    )
...
export const createGroupToolbarButton = (spec: any): Result<GroupToolbarButton, ValueSchema.SchemaError<any>> =>
