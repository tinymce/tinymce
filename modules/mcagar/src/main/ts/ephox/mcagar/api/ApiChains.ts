...
export interface ApiChains {
  ...
  cSetSelectionFrom: <T extends Editor>(
    spec: Cursors.CursorSpec | Cursors.RangeSpec
  ) => Chain<T, T>;
  cSetSelection: <T extends Editor>(
    startPath: number[],
    soffset: number,
    finishPath: number[],
    foffset: number
  ) => Chain<T, T>;
  cSetCursor: <T extends Editor>(
    elementPath: number[],
    offset: number
  ) => Chain<T, T>;
...
const cNodeChanged = <T extends Editor>(): Chain<T, T> =>
  Chain.op(function (editor: T) {
    editor.nodeChanged();
  });
...
const cSetSelectionFrom = function <T extends Editor>(
  spec: Cursors.CursorSpec | Cursors.RangeSpec
): Chain<T, T> {
...
const cSetCursor = function <T extends Editor>(
  elementPath: number[],
  offset: number
): Chain<T, T> {
...
const cSetSelection = function <T extends Editor>(
  startPath: number[],
  soffset: number,
  finishPath: number[],
  foffset: number
  ...
  return Chain.op(function (editor: T) {
    const range = TinySelections.createDomSelection(
      lazyBody(editor),
      startPath,
      soffset,
      finishPath,
...
const cSetSetting = function <T extends Editor>(
  key: string,
  value: any
): Chain<T, T> {
  return Chain.op(function (editor: T) {
...
const cSelect = function <T extends Editor>(
  selector: string,
  path: number[]
): Chain<T, T> {
  return Chain.op(function (editor: T) {
...
const cGetContent: Chain<Editor, string> = Chain.mapper(function (
  editor: Editor
) {
...
const cAssertContentStructure = function <T extends Editor>(
  expected: StructAssert
) {
  return Chain.op(function (editor: T) {
...
const cAssertSelection = function <T extends Editor>(
  startPath: number[],
  soffset: number,
  finishPath: number[],
  foffset: number
