...
export interface TinyScenarios {
  genScenario: (
    genContent: ContentGenerator,
    selectionExclusions: SelectionExclusions
  ) => Scenario;
  arbScenario: (
    genContent: ContentGenerator,
    options: ArbScenarioOptions
  ) => Scenario;

  sAsyncProperty: <T, X, Y>(
    label: string,
    generator: ContentGenerator,
    step: Step<X, Y>,
    options: AsyncPropertyOptions
  ) => Step<T, T>;
}
...
export const TinyScenarios = function (editor: Editor): TinyScenarios {
  ...
  const genScenario = function (
    ...
    return genContent.flatMap(function (structure: Element) {
      ...
      return Generators.selection(
        ...
        selectionExclusions
      ).map(function (selection: SimRange) {
  ...
  const sAsyncProperty = function <T, X, Y>(
    label: string,
    generator: ContentGenerator,
    step: Step<X, Y>,
    options: AsyncPropertyOptions
