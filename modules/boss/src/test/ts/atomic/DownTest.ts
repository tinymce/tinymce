...
UnitTest.test('DownTest', function () {
  const family = Tracks.track(
    Gene('1', 'root', [
      Gene('1.1', 'duck', [
        ...
        Gene('1.1.2', 'goose', [
          Gene('1.1.2.1', 'duck', []),
          Gene('1.1.2.2', 'duck', [Gene('1.1.2.2.1', 'goose', [])])
        ]),
        Gene('1.1.3', 'duck', []),
        Gene('1.1.4', 'duck', [Gene('1.1.4.1', 'duck', [])])
      ])
    ]),
    Option.none()
  );
  ...
  const checkPredicate = function (
    expected: string[],
    id: string,
    predicate: (e: Gene) => boolean
  ) {
    const start = Locator.byId(family, id).getOrDie(
      'Did not find start: ' + id
    );
  ...
  checkSelector(
    ['1.1', '1.1.2.1', '1.1.2.2', '1.1.3', '1.1.4', '1.1.4.1'],
    'duck'
  );
  checkSelector(
    [
