...
const comparePosition = function (item: Gene, other: Gene) {
  ...
  const itemIndex = Arr.findIndex(all, function (x) {
    return item.id === x;
  });
  const otherIndex = Arr.findIndex(all, function (x) {
    return other.id === x;
  });
  return itemIndex
    .bind(function (iIndex) {
...
const prevSibling = function (item: Gene): Option<Gene> {
  ...
  const itemIndex = Arr.findIndex(kin, function (x) {
    return item.id === x.id;
  });
