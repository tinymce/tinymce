  ...
  fc.assert(
    fc.asyncProperty(
      ...
      (i, f) =>
        new Promise((resolve, reject) => {
          Future.pure(i)
            .bind(Fun.compose(Future.pure, f))
            .get((ii) => {
              eqAsync('get', f(i), ii, reject, tString);
              resolve();
            });
...
promiseTest('Future: parallel spec', () =>
  fc.assert(
    fc.asyncProperty(
      fc.array(fc.tuple(fc.integer(1, 10), fc.integer())),
      (tuples) =>
        new Promise((resolve, reject) => {
          Futures.par(
            Arr.map(tuples, ([timeout, value]) =>
              Future.nu((cb) => {
                setTimeout(() => {
                  cb(value);
                }, timeout);
...
promiseTest('Future: mapM spec', () =>
  fc.assert(
    fc.asyncProperty(
      ...
      (tuples) =>
        new Promise((resolve, reject) => {
          Futures.mapM(tuples, ([timeout, value]) =>
            Future.nu((cb) => {
              setTimeout(() => {
                cb(value);
              }, timeout);
