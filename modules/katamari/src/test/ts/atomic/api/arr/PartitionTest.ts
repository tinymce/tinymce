...
UnitTest.test('Arr.partition: unit tests', () => {
  const check = (input: unknown[], expected) => {
    ...
    Assert.eq(
      'partition frozen',
      expected,
      Arr.partition(Object.freeze(input.slice()), f)
    );
  };
  ...
  check(
    ...
    {
      pass: ['yes 1', 'yes 2', 'yes 3', 'yes 4', 'yes 5'],
      fail: ['no 1', 'no 2', 'no 3', 'no 4', 'no 5']
    }
  );
});
  ...
  () => {
    fc.assert(
      fc.property(fc.array(fc.integer()), (arr) => {
        ...
        return (
          Arr.forall(output.fail, (x) => !predicate(x)) &&
          Arr.forall(output.pass, predicate)
        );
      })
    );
  }
