...
UnitTest.test(
  'Arr.findIndex: If predicate is always false, then index is always none',
  () => {
    fc.assert(
      fc.property(fc.array(fc.integer()), (arr) => {
        Assert.eq(
  ...
  fc.assert(
    fc.property(fc.array(fc.integer()), (arr) => {
      ...
      Assert.eq(
        'findIndex vs find',
        Arr.find(arr, pred),
        Arr.findIndex(arr, pred).map((x) => arr[x]),
        tOption(tNumber)
      );
    })
  );
