...
UnitTest.test('Zip: unit tests', () => {
  const check1 = (
    expectedZipToObject: Option<Record<string, string>>,
    expectedZipToTuples: Option<Array<{ k: string; v: string }>>,
    keys: string[],
    values: string[]
  ) => {
...
UnitTest.test('zipToObject has matching keys and values', () => {
  fc.assert(
    fc.property(fc.array(fc.asciiString(1, 30)), (rawValues: string[]) => {
