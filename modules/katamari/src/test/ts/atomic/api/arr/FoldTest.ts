...
UnitTest.test('fold: unit tests', () => {
  const checkl = (expected, input: any[], f, acc) => {
    ...
    Assert.eq(
      'foldl',
      expected,
      Arr.foldl(Object.freeze(input.slice()), f, acc)
    );
  };
  ...
  const checkr = (expected, input, f, acc) => {
    ...
    Assert.eq(
      'foldr',
      expected,
      Arr.foldr(Object.freeze(input.slice()), f, acc)
    );
  };
...
UnitTest.test('foldl concat [ ] xs === reverse(xs)', () => {
  fc.assert(
    fc.property(fc.array(fc.integer()), (arr: number[]) => {
      const output: number[] = Arr.foldl(
        arr,
        (b: number[], a: number) => [a].concat(b),
        []
      );
