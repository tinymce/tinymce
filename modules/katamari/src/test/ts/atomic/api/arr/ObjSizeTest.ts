...
if (navigator.userAgent.indexOf('PhantomJS') <= -1) {
  ...
  UnitTest.test('Obj.size: inductive case', () => {
    fc.assert(
      fc.property(fc.dictionary(fc.asciiString(1, 30), fc.integer()), fc.asciiString(1, 30), fc.integer(), (obj, k, v) => {
        const objWithoutK = Obj.filter(obj, (x, i) => i !== k);
        Assert.eq('Adding key adds 1 to size', Obj.size(objWithoutK) + 1, Obj.size({ k: v, ...objWithoutK }));
      })
    );
  });
}
