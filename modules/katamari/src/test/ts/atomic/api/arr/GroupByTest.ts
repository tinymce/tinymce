...
UnitTest.test('Arr.groupBy: unit tests', () => {
  const check = (input: unknown[], expected) => {
    ...
    Assert.eq(
      'groupBy frozen',
      expected,
      Arr.groupBy(Object.freeze(input.slice()), f)
    );
  };
...
UnitTest.test(
  'Arr.groupBy: Adjacent groups have different hashes, and everything in a group has the same hash',
  () => {
    fc.assert(
      fc.property(
        fc.array(fc.integer()),
        ...
        (xs, f) => {
          ...
          const hasSameGroup = Arr.exists(values, (v, i) =>
            i > 0 ? values[i - 1] === values[i] : false
          );

          if (hasSameGroup) {
            Assert.fail(
              'A group is next to another group with the same g(..) value'
            );
          }
          return true;
        }
...
UnitTest.test('Flattening groups equals the original array', () => {
  fc.assert(
    fc.property(fc.array(fc.integer()), fc.func(fc.string()), (xs, f) => {
      const groups = Arr.groupBy(xs, (x) => f(x));
