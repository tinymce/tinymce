...
UnitTest.test('ObjFindTest', function () {
  ...
  checkNone({}, function (v, _k) {
    return v > 0;
  });
  checkObj(3, { test: 3 }, function (_v, k) {
    return k === 'test';
  });
  checkNone({ test: 0 }, function (v, _k) {
    return v > 0;
  });
  checkObj(4, { blah: 4, test: 3 }, function (v, _k) {
    return v > 0;
  });
  checkNone({ blah: 4, test: 3 }, function (v, _k) {
    return v === 12;
  });

  const obj = { blah: 4, test: 3 };
  checkObj(4, obj, function (v, k, o) {
    return o === obj;
  });
});
...
UnitTest.test('If object is empty, find is always none', () => {
  fc.assert(
    fc.property(fc.func(fc.boolean()), function (pred) {
