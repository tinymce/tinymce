...
UnitTest.test(
  ...
  () => {
    fc.assert(
      fc.property(fc.dictionary(fc.asciiString(), fc.string(1, 40)), (obj) => {
...
UnitTest.test(
  ...
  () => {
    fc.assert(
      fc.property(fc.dictionary(fc.asciiString(1, 30), fc.integer()), (obj) => {
        ...
        Assert.eq(
          'Something in "f" passed predicate',
          false,
          Arr.exists(falseKeys, matches)
        );
        Assert.eq(
          'Something in "t" failed predicate',
          true,
          Arr.forall(trueKeys, matches)
        );
      })
    );
  }
