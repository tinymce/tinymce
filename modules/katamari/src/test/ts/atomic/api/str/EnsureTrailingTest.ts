...
UnitTest.test(
  'ensureTrailing is identity if string already ends with suffix',
  () => {
    fc.assert(
      fc.property(fc.string(), fc.string(), function (prefix, suffix) {
...
UnitTest.test('ensureTrailing endsWith', () => {
  fc.assert(
    fc.property(fc.string(), fc.string(), (s, suffix) =>
      Strings.endsWith(Strings.ensureTrailing(s, suffix), suffix)
    )
  );
});
