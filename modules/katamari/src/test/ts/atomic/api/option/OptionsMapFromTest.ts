...
UnitTest.test('Options.mapFrom', () => {
  ...
  Assert.eq(
    'eq',
    Option.none(),
    Options.mapFrom<number, number>(null, Fun.die('boom')),
    tOption()
  );
  Assert.eq(
    'eq',
    Option.none(),
    Options.mapFrom<number, number>(undefined, Fun.die('boom')),
    tOption()
  );
});
...
UnitTest.test('Options.mapFrom === Options.map().from()', () => {
  ...
  const check = (input: number | null | undefined) => {
    Assert.eq(
      'eq',
      true,
      Options.mapFrom(input, f).equals(Option.from(input).map(f))
    );
  };
