...
UnitTest.test('OptionInstances.testable<number>', () => {
  ...
  Assert.eq(
    'some(3) == some(3) - default component eq',
    Option.some(3),
    Option.some(3),
    tOption()
  );
...
UnitTest.test('OptionInstances.testable<string>', () => {
  ...
  Assert.eq(
    'some("a") == some("a") - default component eq',
    Option.some('a'),
    Option.some('a'),
    tOption()
  );
...
UnitTest.test('OptionInstances pprint', () => {
  Assert.eq(
    'pprint none',
    'Option.none()',
    Pprint.render(Option.none(), tOption(tString))
  );
  Assert.eq(
    'pprint some string',
    'Option.some(\n  "cat"\n)',
  ...
  Assert.eq(
    'pprint none - default component pprint',
    'Option.none()',
    Pprint.render(Option.none(), tOption())
  );
  Assert.eq(
    'pprint some string - default component pprint',
    'Option.some(\n  "cat"\n)',
