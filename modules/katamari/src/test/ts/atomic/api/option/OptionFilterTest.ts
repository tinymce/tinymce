...
import { arbOptionSome } from 'ephox/katamari/test/arb/ArbDataTypes';
...
UnitTest.test('Option.filter', () => {
  Assert.eq(
    'filter #1',
    none(),
    none<number>().filter(constant(true)),
    tOption()
  );
  Assert.eq(
    'filter #2',
    none(),
    none<number>().filter(constant(false)),
    tOption()
  );
  ...
  Assert.eq(
    'filter #9',
    some(6),
    some(6).filter(Fun.constant(true)),
    tOption(tNumber)
  );
  Assert.eq(
    'filter',
    some(5),
    some(5).filter(Fun.constant(true)),
    tOption(tNumber)
  );
  Assert.eq(
    'filter',
    none(),
    some(5).filter(Fun.constant(false)),
    tOption(tNumber)
  );
});
  ...
  fc.assert(
    fc.property(arbOptionSome(fc.integer()), (opt) => {
      Assert.eq(
        'eq',
        true,
        tOption().eq(none(), opt.filter(Fun.constant(false)))
      );
    })
  );
});
  ...
  fc.assert(
    fc.property(fc.integer(), (x) => {
      Assert.eq(
        'eq',
        true,
        tOption().eq(some(x), some(x).filter(Fun.constant(true)))
      );
    })
  );
});
...
UnitTest.test('Checking some(x).filter(f) === some(x) iff. f(x)', () => {
  fc.assert(
    fc.property(fc.integer(), fc.func(fc.boolean()), (i, f) => {
      const opt = some(i);
      if (f(i)) {
        Assert.eq(
          'filter true',
