...
UnitTest.test('OptionsTest', () => {
  ...
  Assert.eq(
    'each returns undefined 1',
    undefined,
    Option.some(1).each(Fun.identity)
  );
  Assert.eq(
    'each returns undefined 2',
    undefined,
    Option.none().each(Fun.identity)
  );
});

UnitTest.test('Options.cat of only nones should be an empty array', () => {
  fc.assert(
    fc.property(fc.array(ArbDataTypes.arbOptionNone()), (options) => {
...
UnitTest.test('Options.cat of Arr.map(xs, Option.some) should be xs', () => {
  fc.assert(
    fc.property(fc.array(fc.json()), (arr) => {
...
UnitTest.test(
  ...
  () => {
    fc.assert(
      fc.property(fc.array(ArbDataTypes.arbOption(fc.integer())), (arr) => {
    ...
    fc.assert(
      fc.property(
        ...
        (before, on, after) => {
          ...
          const output = Options.cat(
            beforeNones.concat(onSomes).concat(afterNones)
          );
