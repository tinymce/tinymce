...
UnitTest.test('OptionSomeTest', () => {
  ...
  Assert.eq(
    'fold 3',
    ['z'],
    Option.some('z').fold(Fun.die('boom'), function () {
  ...
  Assert.eq(
    'fold 4',
    'az',
    Option.some('a').fold(Fun.die('boom'), (x) => x + 'z')
  );
});
...
UnitTest.test('Checking some(x).getOrThunk(_ -> v) === x', () => {
  fc.assert(
    fc.property(fc.integer(), fc.func(fc.integer()), (a, thunk) => {
...
UnitTest.test('Checking some.getOrDie() never throws', () => {
  fc.assert(
    fc.property(fc.integer(), fc.string(1, 40), (i, s) => {
...
UnitTest.test('Checking some(x).or(oSomeValue) === some(x)', () => {
  fc.assert(
    fc.property(fc.integer(), arbOptionSome(fc.integer()), (json, other) => {
...
UnitTest.test('Checking some(x).orThunk(_ -> oSomeValue) === some(x)', () => {
  fc.assert(
    fc.property(fc.integer(), arbOptionSome(fc.integer()), (i, other) => {
