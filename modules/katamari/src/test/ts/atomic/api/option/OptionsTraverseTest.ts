...
UnitTest.test('Options.traverse - unit tests', () => {
  ...
  Assert.eq(
    ...
    Option.some(['3cat']),
    Options.traverse<number, string>([3], (x: number): Option<string> => Option.some(x + 'cat')),
    tOption(tArray(tString))
  );
  ...
  Assert.eq(
    ...
    Option.none(),
    Options.traverse<number, string>([3], (_x: number): Option<string> => Option.none()),
    tOption(tArray(tString))
  );
  ...
  Assert.eq(
    ...
    Option.none(),
    Options.traverse<number, number>([3, 4], (x: number): Option<number> => Options.someIf(x === 3, x)),
