...
UnitTest.test('Options.sequence: unit tests', () => {
  ...
  Assert.eq(
    'eq',
    Option.some([3]),
    Options.sequence<number>([Option.some(3)]),
    tOption()
  );
  Assert.eq(
    'eq',
    Option.some([1, 2]),
    Options.sequence<number>([Option.some(1), Option.some(2)]),
    tOption()
  );
  ...
  Assert.eq(
    'eq',
    Option.none(),
    Options.sequence<number>([Option.some(1), Option.none()]),
    tOption()
  );
  Assert.eq(
    'eq',
    Option.none(),
    Options.sequence<number>([Option.none(), Option.some(343)]),
    tOption()
  );
});
  ...
  fc.assert(
    fc.property(fc.integer(), fc.integer(), (n, m) => {
      Assert.eq(
        'eq',
        Option.some([n, m]),
        Options.sequence<number>([Option.some(n), Option.some(m)]),
        tOption()
      );
    })
  );
  ...
  fc.assert(
    fc.property(fc.array(fc.integer()), (n) => {
      ...
      Assert.eq(
        'eq',
        Option.none(),
        Options.sequence<number>([...someNumbers, Option.none<number>()]),
        tOption()
      );
    })
  );
  ...
  fc.assert(
    fc.property(fc.array(fc.integer()), (n) => {
      ...
      Assert.eq(
        'eq',
        Option.none(),
        Options.sequence<number>([Option.none<number>(), ...someNumbers]),
        tOption()
      );
    })
  );
