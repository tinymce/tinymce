...
UnitTest.test('ADT Test', () => {
  ...
  const checkInvalidGenerate = (cases, message) => {
    checkInvalid(
      'generate() did not throw an error. Input: "' + message + '"',
      () => {
        Adt.generate(cases);
      }
    );
...
const arbUnknown = fc
  .array(fc.string())
  .map((guesses) => newAdt.unknown(guesses));

const arbExact = fc
  .tuple(fc.integer(), fc.integer())
  .map((arr) => newAdt.exact(arr[0], arr[1]));
...
UnitTest.test('Error is thrown if not all arguments are supplied', () => {
  fc.assert(
    fc.property(arbAdt, fc.array(arbKeys, 1, 40), (subject, exclusions) => {
...
UnitTest.test('adt.nothing.match should pass [ ]', () => {
  fc.assert(
    fc.property(arbNothing, (subject) => {
...
UnitTest.test('adt.nothing.match should be same as fold', () => {
  fc.assert(
    fc.property(arbNothing, (subject) => {
      ...
      const folded = subject.fold(
        record,
        Fun.die('should not be unknown'),
        Fun.die('should not be exact')
      );
...
UnitTest.test('adt.unknown.match should pass 1 parameter: [ guesses ]', () => {
  fc.assert(
    fc.property(arbUnknown, (subject) => {
      const contents = subject.match({
...
UnitTest.test(
  'adt.exact.match should pass 2 parameters [ value, precision ]',
  () => {
    fc.assert(
      fc.property(arbExact, (subject) => {
        const contents = subject.match({
          nothing: Fun.die('should not be nothing'),
          unknown: Fun.die('should not be unknown'),
          exact: record
...
UnitTest.test(
  'adt.match must have the right arguments, not just the right number',
  () => {
    fc.assert(
      fc.property(arbAdt, (subject) => {
        try {
          subject.match({
            not: Fun.identity,
            the: Fun.identity,
