 ...
 */
export const zipToObject = function <V> (keys: string[] | number[], values: V[]): Record<string, V> {
...
export const zipToTuples = function <K, V> (keys: K[], values: V[]): Array<{ k: K; v: V }> {
  if (keys.length !== values.length) {
    throw new Error(`Assertion failed: keys.length !== values.length (${keys.length} !== ${values.length})`);
  }
  const r: Array<{ k: K; v: V }> = [];
  for (let i = 0; i < keys.length; i++) {
