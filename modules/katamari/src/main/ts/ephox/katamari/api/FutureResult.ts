...
const wrap = function <A = any, E = any>(
  delegate: Future<Result<A, E>>
): FutureResult<A, E> {
  ...
  const withTimeout = function (timeout: number, errorThunk: () => E) {
    return wrap(
      Future.nu(function (callback: (value: Result<A, E>) => void) {
        let timedOut = false;
...
const nu = function <A = any, E = any>(
  worker: (completer: (result: Result<A, E>) => void) => void
) {
