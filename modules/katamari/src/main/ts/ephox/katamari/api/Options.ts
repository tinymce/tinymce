 ...
 */
export const traverse = <A, B>(arr: ArrayLike<A>, f: (a: A) => Option<B>): Option<Array<B>> => sequence(Arr.map(arr, f));
...
export const lift4 = <A, B, C, D, E>(
  ...
  oa.isSome() && ob.isSome() && oc.isSome() && od.isSome()
    ? Option.some(f(oa.getOrDie(), ob.getOrDie(), oc.getOrDie(), od.getOrDie()))
    : Option.none<E>();

export const lift5 = <A, B, C, D, E, F>(
  oa: Option<A>,
  ob: Option<B>,
  oc: Option<C>,
  od: Option<D>,
  ...
  oa.isSome() && ob.isSome() && oc.isSome() && od.isSome() && oe.isSome()
    ? Option.some(f(oa.getOrDie(), ob.getOrDie(), oc.getOrDie(), od.getOrDie(), oe.getOrDie()))
    : Option.none<F>();
...
export const someIf = <A>(b: boolean, a: A): Option<A> => (b ? Option.some(a) : Option.none<A>());
