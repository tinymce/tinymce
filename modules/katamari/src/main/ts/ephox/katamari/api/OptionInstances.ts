...
export const pprintOption = <A>(pprintA: Pprint<A> = Pprint.pprintAny): Pprint<Option<A>> =>
  Pprint.pprint((oa: Option<A>) =>
    oa.fold(
...
export const eqOption = <A>(eqA: Eq<A> = Eq.eqAny): Eq<Option<A>> => Eq.eq((oa, ob) => oa.equals_(ob, eqA.eq));
