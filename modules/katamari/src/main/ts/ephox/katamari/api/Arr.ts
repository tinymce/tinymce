...
const rawIndexOf = <T>(ts: ArrayLike<T>, t: T): number => nativeIndexOf.call(ts, t);
...
export const bind = <T = any, U = any>(xs: ArrayLike<T>, f: ArrayMorphism<T, U[]>): U[] => flatten(map(xs, f));
...
export const equal = <T>(a1: ArrayLike<T>, a2: ArrayLike<T>, eq: Eq.Eq<T> = Eq.eqAny) => Eq.eqArray(eq).eq(a1, a2);
