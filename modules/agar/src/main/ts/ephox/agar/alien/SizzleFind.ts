...
const toArrayEl = (elements: (Node | Window)[]): Element<Node | Window>[] => Arr.map(elements, Element.fromDom);

/* Petrie makes extensive use of :visible, :has() and :contains() which are sizzle extensions */
const descendants = (sugarElement: Element<any>, selector: string): Element<any>[] => toArrayEl(Sizzle(selector, sugarElement.dom()));

const matches = (sugarElement: Element<any>, selector: string): boolean => Sizzle.matchesSelector(sugarElement.dom(), selector);
