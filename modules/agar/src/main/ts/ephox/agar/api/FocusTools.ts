...
const cGetFocused: Chain<Element<any>, Element<any>> = Chain.binder((doc) =>
  Focus.active(doc).fold(() => Result.error('Could not find active element'), Result.value)
);

const cSetFocused: Chain<Element<any>, Element<any>> = Chain.op(Focus.focus);

const cGetOwnerDoc: Chain<Element<Node>, Element<Document>> = Chain.mapper(Traverse.owner);
...
const sIsOn = <T>(label: string, element: Element<any>): Step<T, T> =>
  Chain.asStep<T, Element>(element, [
    ...
    cGetFocused,
    Chain.binder(
      (active: Element<any>): Result<Element, string> =>
        Compare.eq(element, active)
          ? Result.value(active)
...
const sIsOnSelector = <T>(label: string, doc: Element<any>, selector: string): Step<T, T> =>
  Logger.t(
    ...
    Chain.asStep<T, Element>(doc, [
      ...
      Chain.binder(
        (active: Element<any>): Result<Element, string> =>
          SizzleFind.matches(active, selector)
            ? Result.value(active)
...
const sTryOnSelector = <T>(label: string, doc: Element<any>, selector: string): Step<T, T> =>
  Logger.t<T, T>(
    ...
    Waiter.sTryUntil('Waiting for focus', sIsOnSelector(label, doc, selector), 10, 4000)
  );

const cSetFocus = (label: string, selector: string): Chain<Element, Element> =>
  // Input: container
  Chain.fromChains([Chain.control<Element, Element, Element>(UiFinder.cFindIn(selector), Guard.addLogging(label)), cSetFocused]);

const cSetActiveValue = (newValue: string): Chain<Element, Element> =>
  // Input: container
  Chain.fromChains([cGetOwnerDoc, cGetFocused, UiControls.cSetValue(newValue)]);

// Input: container
const cGetActiveValue: Chain<Element, string> = Chain.fromChains([cGetOwnerDoc, cGetFocused, UiControls.cGetValue]);
...
const sSetActiveValue = <T>(doc: Element<any>, newValue: string): Step<T, T> =>
  Chain.asStep<T, Element>(doc, [cGetFocused, UiControls.cSetValue(newValue)]);
