...
const toStep = <U extends any[]>(method: (...args: U) => void) => <T>(...args: U) =>
  Step.sync<T>(() => {
    method.apply(undefined, args);
  });

const toChain = <B, C>(method: (label: TestLabel, expected: B, actual: C) => void) => (label: string, expected: B): Chain<C, C> =>
  Chain.op<C>((actual: C) => {
