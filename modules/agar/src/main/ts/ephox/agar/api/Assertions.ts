...
const toStep = <U extends any[]>(method: (...args: U) => void) => <T>(
  ...args: U
) =>
  Step.sync<T>(() => {
...
const toChain = <B, C>(
  method: (label: TestLabel, expected: B, actual: C) => void
...
const assertHtmlStructure = (
  ...
  assertStructure(
    ...
    Element.fromHtml(actual)
  );
};

const assertHtmlStructure2 = (
  label: TestLabel,
  expected: string,
  actual: Element<any>
): void => {
...
const cAssertEq: <T>(label: string, expected: T) => Chain<T, T> = toChain(
  Assert.eq
);
