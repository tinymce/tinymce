...
const findIn = (container: Element<any>, selector: string): Result<Element<any>, TestLabel> => UiSearcher.findIn(container, selector);

const findAllIn = (container: Element<any>, selector: string): Element<any>[] => UiSearcher.findAllIn(container, selector);
...
const cFindWithState = (selector: string, predicate: (element: Element<any>) => boolean): Chain<Element<any>, Element<any>> =>
  Chain.fromChains([cFindIn(selector), cHasState(predicate)]);

// Wait for a selector to have state. Max wait time: 10 seconds.
const cWaitForState = (
  message: string,
  selector: string,
  predicate: (element: Element<any>) => boolean
): Chain<Element<any>, Element<any>> => Chain.control(cFindWithState(selector, predicate), Guard.tryUntil(message, 10, 10000));
...
const cFindIn = (selector: string): Chain<Element<any>, Element<any>> =>
  Chain.binder((container: Element<any>) => findIn(container, selector));

const cFindAllIn = (selector: string): Chain<Element<any>, Element<any>[]> =>
  Chain.mapper((container: Element<any>) => findAllIn(container, selector));
