...
const elementOfArb = (arb) => arb.smap(createTag, Node.name, Truncate.getHtml);

const elementOf = (tag) => createTag(tag);

const textOfArb = (arb) => arb.smap(Element.fromText, Text.get, Text.get);

const textOf = (s) => Element.fromText(s);

export { elementOfArb, elementOf, comment, textOf, textOfArb };
