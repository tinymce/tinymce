...
const elementQueue = (
  ...
  const current = () =>
    i >= 0 && i < items.length
      ? Option.some(items[i])
      : Option.none<Element<any>>();

  const peek = () =>
    i + 1 < items.length
      ? Option.some(items[i + 1])
      : Option.none<Element<any>>();
...
const element = (tag: string, fields: ElementFields): StructAssert => {
  const doAssert = (actual: Element<any>): void => {
    Assert.eq(
      () => 'Incorrect node name for: ' + Truncate.getHtml(actual),
      tag,
      Node.name(actual)
    );
    ...
    const html =
      fields.html !== undefined
        ? Option.some(fields.html)
        : Option.none<StringAssert>();
    const value =
      fields.value !== undefined
        ? Option.some(fields.value)
        : Option.none<StringAssert>();
    const children =
      fields.children !== undefined
        ? Option.some(fields.children)
        : Option.none<StructAssert[]>();
...
const text = (s: StringAssert, combineSiblings = false): StructAssert => {
  const doAssert = (queue: ElementQueue): void => {
    queue.take().fold(
      () => {
        assert.fail(
          'No more nodes, so cannot check if its text is: ' +
            s.show() +
            ' for ' +
            queue.context()
        );
      ...
      (actual) => {
        Text.getOption(actual).fold(
          () => {
            assert.fail(
              'Node is not a text node, so cannot check if its text is: ' +
                s.show() +
                ' for ' +
                queue.context()
            );
          ...
          (t: string) => {
            ...
            if (s.strAssert === undefined) {
              throw new Error(
                JSON.stringify(s) + ' is not a *string assertion*'
              );
            }
...
const applyAssert = (structAssert: StructAssert, queue: ElementQueue) => {
  if (structAssert.type === 'advanced') {
    ...
    queue.take().fold(
      () => {
        assert.fail(
          'Expected more children to satisfy assertion for ' + queue.context()
        );
      },
      (item) => {
...
const repeat = (min: number, max: number | true = min) => (
  structAssert: StructAssert
): StructAssert => {
  const doAssert = (queue: ElementQueue) => {
    let i = 0;
    for (; i < min; i++) {
...
const assertAttrs = (
  expectedAttrs: Record<string, StringAssert>,
  actual: Element<any>
) => {
  Obj.each(expectedAttrs, (v, k) => {
    if (v.strAssert === undefined) {
      throw new Error(
        JSON.stringify(v) +
          ' is not a *string assertion*.\nSpecified in *expected* attributes of ' +
          Truncate.getHtml(actual)
      );
    }
    const actualValue = Attr.has(actual, k)
      ? Attr.get(actual, k)
      : ApproxComparisons.missing();
    v.strAssert(
      () =>
...
const assertClasses = (
  ...
  Arr.each(expectedClasses, (eCls) => {
    if (eCls.arrAssert === undefined) {
      throw new Error(
        JSON.stringify(eCls) +
          ' is not an *array assertion*.\nSpecified in *expected* classes of ' +
          Truncate.getHtml(actual)
      );
    }
    eCls.arrAssert(
      () => 'Checking classes in ' + Truncate.getHtml(actual) + '\n',
      actualClasses
    );
  });
};
...
const assertStyles = (
  expectedStyles: Record<string, StringAssert>,
  actual: Element<any>
) => {
  Obj.each(expectedStyles, (v, k) => {
    const actualValue = Css.getRaw(actual, k).getOrThunk(
      ApproxComparisons.missing
    );
    if (v.strAssert === undefined) {
      throw new Error(
        JSON.stringify(v) +
          ' is not a *string assertion*.\nSpecified in *expected* styles of ' +
          Truncate.getHtml(actual)
      );
    }
    v.strAssert(
...
const assertHtml = (
  expectedHtml: Option<StringAssert>,
  actual: Element<any>
) => {
    ...
    if (expected.strAssert === undefined) {
      throw new Error(
        JSON.stringify(expected) +
          ' is not a *string assertion*.\nSpecified in *expected* innerHTML of ' +
          Truncate.getHtml(actual)
      );
    }
    expected.strAssert(
      () => 'Checking HTML of ' + Truncate.getHtml(actual),
      actualHtml
    );
  });
};
...
const assertValue = (
  expectedValue: Option<StringAssert>,
  actual: Element<any>
) => {
  expectedValue.each((v) => {
    if (v.strAssert === undefined) {
      throw new Error(
        JSON.stringify(v) +
          ' is not a *string assertion*.\nSpecified in *expected* value of ' +
          Truncate.getHtml(actual)
      );
    }
    v.strAssert(
...
const assertChildren = (expectedChildren: Option<StructAssert[]>, actual) => {
  expectedChildren.each((expected) => {
    const children = elementQueue(
      Traverse.children(actual),
      Option.some(actual)
    );
    Arr.each(expected, (structExpectation, i) => {
      if (structExpectation.doAssert === undefined) {
        throw new Error(
          JSON.stringify(structExpectation) +
            ' is not a *structure assertion*.\n' +
            'Specified in *expected* children of ' +
            Truncate.getHtml(actual)
        );
      }
      if (structExpectation.type === 'advanced') {
        ...
        children.take().fold(
          () => {
            assert.fail(
              'Expected more children to satisfy assertion ' +
                i +
                ' for ' +
                children.context()
            );
