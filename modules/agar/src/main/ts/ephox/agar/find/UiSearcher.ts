...
} = Adt.generate([{ self: ['element', 'selector'] }, { children: ['element', 'selector'] }, { descendants: ['element', 'selector'] }]);
...
const select = (element: Element<any>, selector: string): Option<Element<any>> =>
  derive(element, selector).fold(matchesSelf, SizzleFind.child, SizzleFind.descendant);
...
const findIn = (container: Element<any>, selector: string): Result<Element<any>, TestLabel> =>
  toResult(() => 'Could not find selector: ' + selector + ' in ' + Truncate.getHtml(container), select(container, selector));

const findAllIn = (container: Element<any>, selector: string): Array<Element<any>> => selectAll(container, selector);
