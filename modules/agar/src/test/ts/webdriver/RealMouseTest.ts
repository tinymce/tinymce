...
UnitTest.asynctest('RealMouseTest', (success, failure) => {
  ...
  style.innerHTML =
    'button[data-test]:hover { background-color: blue; color: white; } button.other { background-color: blue; color: white; } button';
    ...
    [
      ...
      Chain.asStep(container, [
        ...
        Chain.op((button) => {
          ...
          Assertions.assertEq(
            `button doesn't have ${RealMouse.BedrockIdAttribute} attribute`,
            false,
            Attr.has(button, RealMouse.BedrockIdAttribute)
          );
        })
      ])
    ],
