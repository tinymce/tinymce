  ...
  Pipeline.async(
    ...
    [
      ...
      Step.control(
        ...
        Guard.tryUntilNot('Focus should not be on something that is not in the DOM')
      ),
      ...
      Step.raw((value, next, die, logs) => {
        Chain.asStep(value.container, [FocusTools.cSetFocus('Setting focus via chains on the input', 'input')]).runStep(
          value,
          next,
          die,
          logs
      ...
      Step.raw((value, next, die, logs) => {
        Chain.asStep(value.container, [FocusTools.cSetActiveValue('chained.value')]).runStep(value, next, die, logs);
      }),
