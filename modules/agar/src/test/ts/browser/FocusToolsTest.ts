    ...
    [
      ...
      Chain.asStep(doc, [
        ...
        Chain.control(
          ...
          Guard.addLogging(
            'Asserting the value of the input field after it has been set.'
          )
        )
      ]),
      Step.raw((state, next, die, logs) => {
        FocusTools.sIsOn('checking that sIsOn works', state.input).runStep(
          state,
          next,
          die,
          logs
      ...
      Step.control(
        FocusTools.sIsOnSelector(
          'tryUntilNotCheck (sIsOnSelector)',
          doc,
          '.not-really-there'
        ),
        Guard.tryUntilNot('Focus should not be on something that is not there')
      ),

      Step.control(
        FocusTools.sTryOnSelector(
          'tryUntilNotCheck (sTryOnSelector)',
          doc,
          '.not-really-there'
        ),
        Guard.tryUntilNot('Focus should not be on something that is not there')
      ...
      Step.raw((value, next, die, logs) => {
        Chain.asStep(value.container, [
          ...
          Assertions.cAssertEq(
            'Checking the value of input after set by chaining APIs',
            'chained.value'
          )
        ]).runStep(value, next, die, logs);
      }),
      ...
      Step.raw((value, next, die, logs) => {
        Chain.asStep(doc, [
          FocusTools.cGetFocused,
          Assertions.cAssertDomEq(
            'Checking that focused element is the input',
            value.input
          )
        ]).runStep(value, next, die, logs);
      }),
