...
UnitTest.asynctest('DragnDropTest', (success, failure) => {
  ...
  const draggable = Element.fromHtml(
  ...
  const sAssertStoreItems = (expectedStoreItems: string[]) =>
    Step.sync(() => {
  ...
  Pipeline.async(
    ...
    [
      Logger.t(
        'Drag/drop element with data using selectors',
      ...
      Logger.t(
        'Drag/drop element with data using elements',
        GeneralSteps.sequence([
          sClearStore,
          Step.sync(() => {
            const from = SelectorFind.descendant(
              Body.body(),
              '.draggable'
            ).getOrDie('Could not find from element.');
            const to = SelectorFind.descendant(
              Body.body(),
              '.dropzone'
            ).getOrDie('Could not find to element.');
