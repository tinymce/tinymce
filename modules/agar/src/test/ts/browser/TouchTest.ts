  ...
  Pipeline.async(
    ...
    [
      ...
      runStep(
        ...
        Step.sync(() => Touch.point('touchstart', container, 0, 0))
      ),
      ...
      runStep(
        ...
        Chain.asStep(container, [UiFinder.cFindIn('input'), Touch.cTouchStart])
      ),

      runStep(
        'cTouchEnd input',
        ['input.touchend', 'container.touchend'],
        Chain.asStep(container, [UiFinder.cFindIn('input'), Touch.cTouchEnd])
      ),

      runStep(
        'cTouchMove input',
        ['input.touchmove', 'container.touchmove'],
        Chain.asStep(container, [UiFinder.cFindIn('input'), Touch.cTouchMove])
      )
    ],
    () => {
      Arr.each(handlers, (h) => {
