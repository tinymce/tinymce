...
import { Attr, Body, Compare, Css, Insert, Node, PredicateFilter, Remove, SelectorFilter, Text, Traverse, Truncate } from '@ephox/sugar';
...
UnitTest.test('Arbitraries Test', () => {
  ...
  checkProperty(
    ...
    (data) =>
      assertProperty(
        'style and attr api',
        data,
        (elem) => Node.name(elem) === 'span' && (Attr.get(elem, 'data-a') !== 'b' || Css.getRaw(elem, 'color').getOr('') !== 'red')
      )
  );
  ...
  checkProperty('Inline elements should have display: inline', Arbitraries.content('inline'), (element) =>
    ...
    assertProperty(
      '(display === inline)',
      element,
      (elem) => Css.get(elem, 'display') !== 'inline' || Arr.contains(['span-underline', 'span-strikethrough'], Node.name(elem))
    )
  );

  checkProperty('Container elements', Arbitraries.content('container'), (element) =>
  ...
  checkProperty('Formatting elements should only contain (display === inline)', Arbitraries.content('formatting'), (section) =>
    assertProperty(
