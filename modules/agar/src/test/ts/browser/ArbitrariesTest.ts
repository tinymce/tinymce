...
UnitTest.test('Arbitraries Test', () => {
  const assertProperty = (label, element, assertion) => {
    ...
    if (failing.length > 0) {
      return (
        'These elements did not satisfy property ' +
        label +
        ': \n' +
  ...
  checkProperty(
    'Text nodes should have node type 3',
    Arbitraries.content('netext'),
    (textnode) => {
  ...
  checkProperty(
    'Zerowidth text nodes should have node type 3 and be uFEFF',
    Arbitraries.content('zerowidth'),
    (textnode) => {
      Assertions.assertEq('Node type of "zerowidth"', 3, Node.type(textnode));
      Assertions.assertEq(
        'Text value of zerowidth',
        '\uFEFF',
        Text.get(textnode)
      );
      return true;
  ...
  checkProperty(
    'Zerowidths text nodes should have node type 3 and be uFEFF or u200B',
    Arbitraries.content('zerowidths'),
    (textnode) => {
      Assertions.assertEq('Node type of "zerowidths"', 3, Node.type(textnode));
      Assertions.assertEq(
        'Zerowidths cursor value: ' + Text.get(textnode),
        true,
        Arr.contains(['\uFEFF', '\u200B'], Text.get(textnode))
      );
      return true;
    }
  );
  ...
  checkProperty(
    'Spans should have attributes and styles',
    Arbitraries.content('test-data', {
      'test-data': {
        recursionDepth: 1,
        type: 'composite',
        tags: {
    ...
    (data) =>
      assertProperty(
        ...
        (elem) =>
          Node.name(elem) === 'span' &&
          (Attr.get(elem, 'data-a') !== 'b' ||
            Css.getRaw(elem, 'color').getOr('') !== 'red')
      )
  );

  checkProperty(
    'Testing out attribute and style decorators',
    Arbitraries.content('test-data', {
      'test-data': {
        type: 'leaf',
        tag: 'span',
        attributes: Generators.chooseOne([
          {
            ...
            value: Jsc.constant('hi').generator
          },
          {
            weight: 2.0,
            property: 'contenteditable',
            value: Jsc.constant('true').generator
          }
        ]),
        styles: Generators.chooseOne([
          { weight: 1.0, property: 'color', value: Generators.hexColor },
          {
            weight: 0.5,
            property: 'visibility',
            value: Jsc.elements(['hidden', 'visible']).generator
          }
        ]),
        components: {}
      }
    ...
    (leaf) => {
      ...
      return (
        !(hasDataCustom && hasContentEditable) &&
        (hasDataCustom || hasContentEditable) &&
        !(hasColor && hasVisibility) &&
        (hasColor || hasVisibility) &&
  ...
  checkProperty(
    'Testing out attribute and style decorators (enforce)',
    Arbitraries.content('test-data', {
      'test-data': {
        type: 'leaf',
        tag: 'span',
    ...
    (leaf) => {
      Assertions.assertEq(
        'data-custom should be "hi"',
        'enforced-hi',
        Attr.get(leaf, 'data-custom')
      );
      Assertions.assertEq(
        'contenteditable should be "false"',
        'false',
        Attr.get(leaf, 'contenteditable')
      );
      Assertions.assertEq(
        'should have color: blue',
        'blue',
        Css.getRaw(leaf, 'color').getOrDie('Must have color')
      );
      Assertions.assertEq(
        'should have visibility: hidden',
        'hidden',
        Css.getRaw(leaf, 'visibility').getOrDie('Must have visibility')
      );
      return true;
    }
  );
  ...
  checkProperty(
    'Comment nodes should have node type 8',
    Arbitraries.content('comment'),
    (comment) => {
  ...
  checkProperty(
    ...
    Arbitraries.content('whitespace'),
    (element) => {
      if (Node.isText(element)) {
        Assertions.assertEq(
          'Text content of "whitespace"',
          '',
          Text.get(element).trim()
        );
        return true;
      } else if (Node.isElement(element)) {
        Assertions.assertEq(
          'Node name of "whitespace"',
          'br',
          Node.name(element)
        );
        return true;
  ...
  checkProperty(
    'Inline elements should have display: inline',
    Arbitraries.content('inline'),
    (element) =>
  ...
  checkProperty(
    'Container elements',
    Arbitraries.content('container'),
    (element) =>
      assertProperty(
        'if display === inline, no descendants have display block',
        element,
        (elem) => {
          if (Css.get(elem, 'display') === 'inline') {
            const descendants = PredicateFilter.descendants(
              elem,
              (kin) =>
                Node.isElement(kin) && Css.get(kin, 'display') !== 'inline'
            );
  ...
  checkProperty(
    'Formatting elements should only contain (display === inline)',
  ...
  checkProperty(
    'Table cell elements',
    Arbitraries.content('tablecell'),
    (element) => {
      Assertions.assertEq(
        'Cells should be th|td',
        true,
        ['td', 'th'].indexOf(Node.name(element)) > -1
      );
      return true;
    }
  );
  ...
  checkProperty(
    'Table body elements',
    Arbitraries.content('tbody'),
    (element) => {
      Assertions.assertEq(
        'Table body must be <tbody>',
        'tbody',
        Node.name(element)
      );
      return true;
  ...
  checkProperty(
    'Table foot elements',
    Arbitraries.content('tfoot'),
    (element) => {
      Assertions.assertEq(
        'Table foot must be <tfoot>',
        'tfoot',
        Node.name(element)
      );
      return true;
  ...
  checkProperty(
    'Table head elements',
    Arbitraries.content('thead'),
    (element) => {
      Assertions.assertEq(
        'Table head must be <thead>',
        'thead',
        Node.name(element)
      );
      return true;
  ...
  checkProperty(
    ...
    (element) => {
      ...
      Assertions.assertEq(
        'There should 1 caption element (chance 100%)',
        true,
        captions.length === 1
      );
      return true;
  ...
  checkProperty(
    'ol and ul elements',
    Arbitraries.content('list'),
    (element) => {
      Assertions.assertEq(
        'Lists should be ol|ul',
        true,
        ['ol', 'ul'].indexOf(Node.name(element)) > -1
      );
      return true;
    }
  );
