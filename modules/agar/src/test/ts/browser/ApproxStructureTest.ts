...
UnitTest.asynctest('ApproxStructureTest', (success, _failure) => {
  const html =
    '<div data-key="test-1" selected="double" class="test1 root" style="display: block;">' +
    '<div selected="true">' +
  ...
  check(
    ApproxStructure.build((s, str, arr) =>
      s.element('div', {
        ...
        classes: [arr.has('test1'), arr.not('dog'), arr.hasPrefix('tes')],
        styles: {
          ...
          s.element('div', {
            ...
            children: [
              s.element('span', {
                ...
                classes: [arr.not('enabled'), arr.has('disabled')],
                html: str.is('span')
              })
            ]
          }),
  ...
  const struct1 = ApproxStructure.build((s, str, arr) =>
    s.either([
      s.element('span', {
        classes: [arr.has('hello'), arr.not('fizzbuzz')]
      }),
      s.element('div', {
        classes: [arr.has('fizzbuzz'), arr.not('hello')]
      })
    ])
  );
  ...
  Assertions.assertStructure(
    ...
    ApproxStructure.build((s, str, _arr) =>
      s.element('div', {
        children: [s.text(str.is('hello world'), true)]
      })
    ),
    container
  );
