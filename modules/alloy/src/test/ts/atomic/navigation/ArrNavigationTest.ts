...
UnitTest.test('ArrNavigationTest', () => {
  ...
  const genIndexInArray = (array: number[]) =>
    Jsc.integer(0, array.length - 1).generator;

  const genTestCase = Jsc.nearray(Jsc.integer).generator.flatMap(
    (values1: number[]) =>
  ...
  const genUniqueNumTestCase = genUniqueArray(2, 10).flatMap(
    (values: number[]) =>
  ...
  Jsc.property(
    ...
    (testCase: { values: number[]; index: number }) => {
      ArrNavigation.cycleNext(
        testCase.values,
        testCase.index,
        Fun.constant(true)
      ).getOrDie('Should always be able to cycle next on a >= 2 length array');
      return true;
    }
  );
  ...
  Jsc.property(
    ...
    (testCase: { values: number[]; index: number }) => {
      ...
      const before = ArrNavigation.cyclePrev<number>(
        testCase.values,
        initial,
        Fun.constant(true)
      ).getOrDie('Should always be able to cycle prev on a >= 2 length array');
      // Note, the index is the same as the value, so we can do this.
      const after = ArrNavigation.cycleNext(
        testCase.values,
        before,
        Fun.constant(true)
      ).getOrDie('Should always be able to cycle next on a >= 2 length array');
  ...
  Jsc.property(
    ...
    (testCase: { values: number[]; index: number }) => {
      const initial = testCase.index;
      const after = ArrNavigation.cycleNext<number>(
        testCase.values,
        initial,
        Fun.constant(true)
      ).getOrDie('Should always be able to cycle next on a >= 2 length array');
      // Note, the index is the same as the value, so we can do this.
      const before = ArrNavigation.cyclePrev(
        testCase.values,
        after,
        Fun.constant(true)
      ).getOrDie('Should always be able to cycle prev on a >= 2 length array');
  ...
  Jsc.property(
    ...
    (testCase: { values: number[]; index: number }) => {
      ...
      return (
        Jsc.eq(before, testCase.values.length - 1) ||
        Jsc.eq(testCase.index - 1, before)
      );
    }
  );
