...
UnitTest.test('ArrNavigationTest', () => {
  const genUniqueArray = (min: number, max: number) =>
    Jsc.integer(min, max).generator.map((num: number) => {
      const r = [];
      for (let i = 0; i < num; i++) {
        r[i] = i;
      }
  ...
  const genTestCase = Jsc.nearray(Jsc.integer).generator.flatMap((values1: number[]) =>
    Jsc.nearray(Jsc.integer).generator.flatMap((values2: number[]) => {
  ...
  const genUniqueNumTestCase = genUniqueArray(2, 10).flatMap((values: number[]) =>
    genIndexInArray(values).map((index: number) => ({
      values,
      index
    }))
  );
  ...
  Jsc.property('Cycling should always be possible in a >= 2 length array', arbTestCase, (testCase: { values: number[]; index: number }) => {
  ...
  Jsc.property('Cycling next makes an index of 0, or one higher', arbUniqueNumTestCase, (testCase: { values: number[]; index: number }) => {
  ...
  Jsc.property('Unique: Try next should be some(+1) or none', arbUniqueNumTestCase, (testCase: { values: number[]; index: number }) =>
    ArrNavigation.tryNext(testCase.values, testCase.index, Fun.constant(true)).fold(
      () =>
  ...
  Jsc.property('Unique: Try prev should be some(-1) or none', arbUniqueNumTestCase, (testCase: { values: number[]; index: number }) =>
    ArrNavigation.tryPrev(testCase.values, testCase.index, Fun.constant(true)).fold(
      () =>
