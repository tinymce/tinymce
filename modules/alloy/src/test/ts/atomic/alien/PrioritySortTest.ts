...
UnitTest.test('PrioritySortTest', () => {
  const checkErr = <T extends Record<string, () => any>>(
    expected: string,
    input: T[],
    order: string[]
  ) => {
    ...
    actual.fold(
      (err) => {
        ...
        Assert.eq(
          'Checking the error of priority sort',
          errMessage.indexOf(expected) > -1,
          true
        );
      },
      (val) => {
        Assert.fail(
          'Priority sort should have thrown error: ' +
            expected +
            '\nWas: ' +
  ...
  const checkVal = <T extends Record<string, () => any>>(
    expected: string[],
    input: T[],
    order: string[]
  ) => {
    ...
    actual.fold(
      ...
      (val) => {
        Assert.eq(
          'Checking the value of priority sort',
          expected,
          Arr.map(val, (v) => v.letter())
        );
      }
    );
  };
  ...
  Logger.sync('Checking [ a, d, f ] ordering', () =>
    checkErr(
  ...
  Logger.sync('Checking [ a, d, f ] ordering when all specified', () =>
    checkVal(['a', 'd', 'f'], letters(['a', 'd', 'f']), [
      'a',
      'b',
      'c',
      'd',
