...
UnitTest.test('Atomic Test: ui.slider.SliderModelTest', () => {
  ...
  const arbRanged = Jsc.bless({
    generator: Jsc.nat.generator.flatMap((min: number) =>
      arb1Up.generator.flatMap((width: number) => {
        const max = min + width;
  ...
  const arbBounds = Jsc.bless({
    generator: Jsc.nat.generator.flatMap((min: number) =>
      arb1Up.generator.map((width: number) => ({
        left: min,
        width,
        right: min + width
      }))
  ...
  Jsc.syncProperty(
    ...
    [arbData],
    (data: TestData) => {
      const newValue = SliderModel.reduceBy(
        data.value,
        data.min,
        data.max,
        data.stepSize
      ...
      Assert.eq(
        'Checking value',
        true,
        newValue <= data.value && newValue >= data.min - 1
      );
      return true;
    },
    {}
  ...
  Jsc.syncProperty(
    ...
    [arbData],
    (data: TestData) => {
      const newValue = SliderModel.increaseBy(
        data.value,
        data.min,
        data.max,
        data.stepSize
      ...
      Assert.eq(
        'Checking value',
        true,
        newValue >= data.value && newValue <= data.max + 1
      );
      return true;
    },
    {}
  ...
  Jsc.syncProperty(
    ...
    [arbData, arbBounds, Jsc.nat, Jsc.nat],
    (
      data: TestData,
      bounds: TestBounds,
      xValue: number,
      ...
      Assert.eq(
        'Checking factors correctly: ' + newValue,
        true,
        Math.floor(f) === f ||
          newValue === data.min ||
          newValue === data.max ||
  ...
  Jsc.syncProperty(
    ...
    (
      ...
      const args = {
        ...
        step: data.stepSize,
        snap: data.snapToGrid,
        snapStart: Option.some(
          snapOffset + data.min <= data.max ? snapOffset + data.min : data.max
        ),
        rounded: data.rounded,
        hasMinEdge: data.hasLedge,
        hasMaxEdge: data.hasRedge,
