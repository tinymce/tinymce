...
export default (doc: Element<HTMLDocument>, gui: GuiSystem, typeahead: AlloyComponent) => {
  const sWaitForMenu = (label: string) =>
    Logger.t(label, Waiter.sTryUntil('Waiting for menu to appear', UiFinder.sExists(gui.element(), '.selected-menu'), 100, 4000));
  ...
  const sWaitForNoMenu = (label: string) =>
    Logger.t(label, Waiter.sTryUntil('Waiting for menu to go away', UiFinder.sNotExists(gui.element(), '.selected-menu'), 100, 1000));

  const sAssertFocusOnTypeahead = (label: string) =>
    Logger.t(label, FocusTools.sTryOnSelector('Focus should be on typeahead', doc, 'input'));

  const sAssertValue = (label: string, expected: string) =>
    Logger.t(
      label + ' sAssertValue',
      Chain.asStep(typeahead.element(), [Chain.mapper(Value.get), Assertions.cAssertEq('Checking value of typeahead', expected)])
    );
