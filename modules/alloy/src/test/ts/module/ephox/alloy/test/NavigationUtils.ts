...
const sequence = (
  ...
  key: number,
  modifiers: {},
  identifiers: Array<{ label: string; selector: string }>
) => {
  const array = range(identifiers.length, (_, i) => [
    Keyboard.sKeydown(doc, key, modifiers),
    FocusTools.sTryOnSelector(
      'Focus should move from ' +
        (i > 0 ? identifiers[i - 1].label : '(start)') +
        ' to ' +
        identifiers[i].label,
      doc,
