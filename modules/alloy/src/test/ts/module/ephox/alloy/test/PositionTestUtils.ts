...
const cAddPopupToSinkWithin = (sinkName: string, elem: Element) =>
  NamedChain.bundle((data: any) => {
    ...
    const positioner = () =>
      Positioning.positionWithin(sink, data.anchor, data.popup, boxElement);
    return cAddPopupToSinkCommon(data, sink, positioner);
  });
  ...
  Chain.control(
    NamedChain.bundle((data: any) => {
      ...
      return inside
        ...
        : Result.error(
            new Error(
              'The popup does not appear within the ' +
                label +
                ' sink container'
            )
          );
  ...
  Chain.control(
    NamedChain.bundle((data: any) => {
      ...
      return inside
        ...
        : Result.error(
            new Error(
              'The popup does not appear within window viewport for the ' +
                sinkName +
                ' sink'
            )
          );
    ...
    Guard.tryUntil(
      'Ensuring that the popup is inside window viewport for the ' +
        sinkName +
        ' sink'
    )
  );
