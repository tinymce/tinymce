...
const cTestPopupInViewport = (sinkName: string) =>
  Chain.control(
    NamedChain.bundle((data: any) => {
      ...
      return inside
        ? Result.value(data)
        : Result.error(new Error('The popup does not appear within window viewport for the ' + sinkName + ' sink'));
    }),
...
const cAddTopBottomMargin = (amount: string) =>
  Chain.mapper((component: AlloyComponent) => {
    Css.set(component.element(), 'margin-top', amount);
    Css.set(component.element(), 'margin-bottom', amount);
    return component;
  });

const cTestSinkPopupPosition = (sinkName: string) =>
  Chain.fromChains([cTestPopupInSink(sinkName, sinkName), cTestPopupInViewport(sinkName)]);

const cTestSink = (label: string, sinkName: string) =>
  ChainUtils.cLogging(label, [cAddPopupToSink(sinkName), cTestSinkPopupPosition(sinkName)]);

const cTestSinkWithin = (label: string, sinkName: string, elem: Element) =>
  ChainUtils.cLogging(label, [cAddPopupToSinkWithin(sinkName, elem), cTestSinkPopupPosition(sinkName)]);

const cTestSinkWithinBounds = (label: string, sinkName: string, bounds: Bounds) =>
  ChainUtils.cLogging(label, [cAddPopupToSinkWithinBounds(sinkName, bounds), cTestSinkPopupPosition(sinkName)]);

const cScrollDown = (componentName: string, amount: string) =>
  ChainUtils.cLogging('Adding margin to ' + componentName + ' and scrolling to it', [
    NamedChain.direct(componentName, cAddTopBottomMargin(amount), '_'),
    NamedChain.direct(componentName, cScrollTo, '_')
  ]);

export { cTestSink, cTestSinkWithin, cTestSinkWithinBounds, cScrollDown };
