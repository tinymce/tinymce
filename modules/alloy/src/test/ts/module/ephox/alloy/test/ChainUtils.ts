...
const cLogging = <T, U>(label: string, chains: Array<Chain<T, U>>) => {
  const logChains = Arr.map(chains, (c) =>
    Chain.control(c, Guard.addLogging(label))
  );
...
const cFindUid = (uid: string) =>
  Chain.binder((context: any) => context.getByUid(uid));
...
const cFindUids = (
  ...
  const others = Arr.map(keys, (k) =>
    NamedChain.direct('context', cFindUid(lookups[k]), k)
  );

  return NamedChain.asChain(
    [NamedChain.writeValue('context', gui)].concat(others)
  );
};
...
export { cLogging, cFindUids, cToElement, eToComponent };
