...
const sAssertFailIs = (label: string, expected: string, f: () => void) =>
  Step.control(
    Step.sync(() => {
      let passed = false;
      try {
...
const sAssertFailContains = (label: string, expected: string, f: () => void) =>
  Step.control(
    Step.sync(() => {
      let passed = false;
      try {
...
export { sAssertFailIs, sAssertFailContains };
