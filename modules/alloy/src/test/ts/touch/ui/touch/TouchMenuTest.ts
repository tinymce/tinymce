                ...
                Option.from(
                  Arr.map(
                    [
                      {
                        type: 'item',
                        data: { value: 'dog', meta: { text: 'Dog' } }
                      },
                      {
                        type: 'item',
                        data: { value: 'elephant', meta: { text: 'Elephant' } }
                      }
                    ],
                    munge
                  )
      ...
      (doc, _body, gui, component, store) => {
        ...
        return [
          ...
          Step.sync(() => {
            ...
            Assertions.assertEq(
              'Checking selected class should be off initially',
              false,
              Class.has(component.element(), 'touch-menu-open')
            );
            const rect = component.element().dom().getBoundingClientRect();
            fireTouchstart(component.element(), rect.x, rect.y);
            Assertions.assertEq(
              'Checking selected class should be on',
              true,
              Class.has(component.element(), 'touch-menu-open')
            );
            store.assertEq(
              'Checking no hovering messages until menu appears',
              []
            );
            fireTouchend(component.element());
            Assertions.assertEq(
              'Checking selected class should be off again',
              false,
              Class.has(component.element(), 'touch-menu-open')
            );
            store.clear();
          ...
          Step.sync(() => {
            ...
            Assertions.assertEq(
              'Checking selected class should now be on',
              true,
              Class.has(component.element(), 'touch-menu-open')
            );
          }),
          ...
          store.sAssertEq(
            'Hover on should be fired immediately after longpress menu appears',
            ['onHoverOn']
          ),

          sFireTouchmoveOn(
            component.element(),
            '[role="menu"] [data-value="dog"]'
          ),
          sAssertMenuStructure(
            'Checking menu structure with hover over first item',
          ...
          store.sAssertEq('Hover off should be fire when an item gets focus', [
            'onHoverOn',
            'onHoverOff'
          ]),
          sFireTouchmoveOn(
            component.element(),
            '[role="menu"] [data-value="elephant"]'
          ),
          sAssertMenuStructure(
            'Checking menu structure with hover over first item',
            ApproxStructure.build((s, _str, arr) =>
          ...
          store.sAssertEq(
            'Hover off should not fire again until hover on has fired',
            ['onHoverOn', 'onHoverOff']
          ),
          sFireTouchmoveOn(component.element(), '.touch-menu-button'),
          sAssertMenuStructure(
            'Checking menu structure with hover over the touch button (so nothing selected)',
            ApproxStructure.build((s, _str, arr) =>
              s.element('div', {
                children: [
                  s.element('div', {
          ...
          store.sAssertEq(
            'Hover on should fire again now the button is under the "touch"',
          ...
          sAssertMenuStructure(
            'Checking menu structure with hover over nothing (so nothing selected)',
            ApproxStructure.build((s, _str, arr) =>
              s.element('div', {
                children: [
                  s.element('div', {
