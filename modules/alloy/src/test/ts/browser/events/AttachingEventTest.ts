  ...
  (success, failure) => {
          ...
          events: AlloyEvents.derive([
            AlloyEvents.runOnAttached((comp, simulatedEvent) => {
              ...
              const parent = Traverse.parent(comp.element())
                ...
                .getOrDie('At attachedToDom, a DOM parent must exist');
            ...
            AlloyEvents.runOnDetached((comp, simulatedEvent) => {
              ...
              const parent = Traverse.parent(comp.element())
                .filter(Node.isElement)
                .getOrDie('At detachedFromDom, a DOM parent must exist');
    ...
    Pipeline.async(
      ...
      [
        ...
        Step.sync(() => {
          gui.add(wrapper);
          store.assertEq('After adding to system, init should have fired', [
            'init'
          ]);
        }),
        ...
        store.sAssertEq(
          'After adding to system and waiting, still only init should have fired',
          ['init']
        ),
        store.sClear,
        ...
        store.sAssertEq('After adding to the DOM, should have fired attached', [
          'attached-to:main-container'
        ]),
        store.sClear,
        ...
        store.sAssertEq(
          'After detaching from the DOM, should have fired detached',
          ['detached-from:main-container']
        )
      ],
      () => {
        success();
