...
UnitTest.asynctest('Browser Test: events.AttachingEventTest', (success, failure) => {
      ...
      Container.sketch({
        events: AlloyEvents.derive([
          AlloyEvents.runOnAttached((comp, simulatedEvent) => {
            simulatedEvent.stop();
            const parent = Traverse.parent(comp.element()).filter(Node.isElement).getOrDie('At attachedToDom, a DOM parent must exist');
            store.adder('attached-to:' + Attr.get(parent, 'class'))();
          }),

          AlloyEvents.runOnDetached((comp, simulatedEvent) => {
            simulatedEvent.stop();
            const parent = Traverse.parent(comp.element()).filter(Node.isElement).getOrDie('At detachedFromDom, a DOM parent must exist');
  ...
  Pipeline.async(
    ...
    [
      Step.sync(() => {
        Assert.eq('Checking that the component has no size', 0, wrapper.element().dom().getBoundingClientRect().width);
      }),
      ...
      Step.sync(() => {
        Assert.eq('Now added to the DOM, so should have size 100', 100, wrapper.element().dom().getBoundingClientRect().width);
      }),
