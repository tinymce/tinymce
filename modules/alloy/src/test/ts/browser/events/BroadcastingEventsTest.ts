...
UnitTest.asynctest('Browser Test: events.BroadcastingEventsTest', (success, failure) => {
  ...
  const bodyMargin = ['body { margin-top: 2000px; }'];
  ...
  GuiSetup.setup(
    ...
    (doc, _body, gui, _component, store) => {
      ...
      return [
        ...
        Waiter.sTryUntil('Checking for scrolling message', store.sAssertEq('Should have scrolled', ['scroll'])),
        store.sClear,
        Step.sync(() => {
          window.scrollTo(0, 0);
        }),
        Waiter.sTryUntil('Checking for scrolling message (scrolling back to 0)', store.sAssertEq('Should have scrolled', ['scroll'])),
        store.sClear,
        Step.sync(() => {
        ...
        Waiter.sTryUntil('Checking for resize message', store.sAssertEq('Should have resized', ['resize'])),
        store.sClear,
