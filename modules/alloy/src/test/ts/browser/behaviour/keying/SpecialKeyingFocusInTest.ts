...
UnitTest.asynctest('SpecialKeyingFocusInTest', (success, failure) => {
  GuiSetup.setup(
    ...
    (doc, body, _gui, component, store) => {
      const oneComp = SelectorFind.descendant(component.element(), '.one')
        .bind((elem) => component.getSystem().getByDom(elem).toOption())
        .getOrDie('Could not find "one" div');

      const twoComp = SelectorFind.descendant(component.element(), '.two')
        .bind((elem) => component.getSystem().getByDom(elem).toOption())
      ...
      return [
        ...
        Log.step(
          'TBA',
          'Check focus inside a component with focusIn set does not set focus, and calls focusIn',
        ...
        Log.step(
          'TBA',
          'Check focus inside a component with no focusIn should just focus',
          GeneralSteps.sequence([
            store.sAssertEq('Store starts empty', []),
            Step.sync(() => {
