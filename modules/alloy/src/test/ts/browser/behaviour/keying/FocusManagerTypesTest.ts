  ...
  (success, failure) => {
    GuiSetup.setup(
            ...
            AddEventsBehaviour.config('focus-manager-events', [
              AlloyEvents.run<SystemEvents.AlloyFocusShiftedEvent>(
                ...
                (_comp, se) => {
                  ...
                  const prevIndex = prevFocus
                    .map((p) => Attr.get(p, 'data-index'))
                    .getOr('{none}');
                  const newIndex = newFocus
                    .map((p) => Attr.get(p, 'data-index'))
                    .getOr('{none}');
      ...
      (doc, _body, _gui, component, store) => {
        ...
        const sFireFocusOn = (
          focusManager: FocusManagers.FocusManager,
          selector: string
        ) =>
        ...
        return [
          ...
          Logger.t(
            ...
            GeneralSteps.sequence([
              ...
              store.sAssertEq(
                'Checking highlights transitioned from none to 1',
                ['{none}->1']
              ),
              Assertions.sAssertPresence(
                'Checking a selected item',
                {
              ...
              store.sAssertEq('Checking highlights transitioned from 1 to 2', [
                '1->2'
              ]),
          ...
          Logger.t(
            ...
            GeneralSteps.sequence([
              ...
              store.sAssertEq('Checking dom transitioned from none to 2', [
                '{none}->2'
              ]),
              FocusTools.sTryOnSelector(
                'Focus should be on candidate 2',
                doc,
                '[data-index="2"]'
              ),
              store.sClear,
              ...
              store.sAssertEq('Checking dom transitioned from 2 to 1', [
                '2->1'
              ]),
              FocusTools.sTryOnSelector(
                'Focus should be on candidate 1',
                doc,
                '[data-index="1"]'
              )
            ])
