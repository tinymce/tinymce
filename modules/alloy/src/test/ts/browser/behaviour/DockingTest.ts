...
UnitTest.asynctest('DockingTest', (success, failure) => {
  ...
  GuiSetup.setup((store, doc, body) => GuiFactory.build(
    ...
    const boxWithNoPosition = () => ApproxStructure.build((s, str, arr) => s.element('div', {
      styles: {
        position: str.none(),
        left: str.none(),
        top: str.none(),
    ...
    const boxWithPosition = (position: string) => ApproxStructure.build((s, str, arr) => s.element('div', {
      styles: {
