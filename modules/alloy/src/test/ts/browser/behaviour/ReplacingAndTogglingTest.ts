...
UnitTest.asynctest(
  'Browser Test: behaviour.ReplacingAndTogglingTest',
  (success, failure) => {
    ...
    GuiSetup.setup(
      (_store, _doc, _body) =>
        GuiFactory.build({
          ...
          components: [memChild1.asSpec(), memChild2.asSpec()],
      ...
      (_doc, _body, _gui, component, _store) => {
        ...
        return [
          Step.sync(() => {
            Assertions.assertEq(
              'Assert child1 initial toggling state',
              false,
              Toggling.isOn(childComp1)
            );
            Assertions.assertEq(
              'Assert child1 initial toggling state',
              true,
              Toggling.isOn(childComp2)
            );

            // Toggle the component
            Toggling.on(childComp1);
            Assertions.assertEq(
              'Assert child1 toggling state is now on',
              true,
              Toggling.isOn(childComp1)
            );
            ...
            Replacing.set(component, [
              GuiFactory.premade(childComp1),
              GuiFactory.premade(childComp2)
            ]);

            // Assert the state hasn't been reset
            Assertions.assertEq(
              'Assert child1 toggling state is still on',
              true,
              Toggling.isOn(childComp1)
            );
            Assertions.assertEq(
              'Assert child2 toggling state is still on',
              true,
              Toggling.isOn(childComp1)
            );
          })
