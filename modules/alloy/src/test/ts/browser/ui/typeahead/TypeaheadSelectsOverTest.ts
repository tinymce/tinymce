...
} from 'ephox/alloy/api/ui/TieredMenu';
...
UnitTest.asynctest(
  'Browser Test: .ui.typeahead.TypeaheadNoSelectsOverTest',
  (success, failure) => {
    ...
    GuiSetup.setup(
      ...
      (doc, _body, gui, _component, _store) => {
        const testWithSelector = () => {
          const typeahead = gui
            .getByUid('test-type-with-selectover')
            .getOrDie();
          ...
          return [
            FocusTools.sSetFocus(
              'Focusing typeahead with selectover',
              gui.element(),
              '.with-selectover'
            ),
            ...
            steps.sAssertValue(
              'Checking non-matching typeahead menu not changing value',
              'al'
            ),
            ...
            steps.sTriggerInputEvent('Simulate typing to show menu with "Al"'),
            steps.sWaitForMenu(
              '"Typing" should activate menu and select over because of matching text'
            ),

            steps.sAssertValue(
              'Should change to "Alpha" with it selected over',
              'Alpha'
            ),
            steps.sAssertTextSelection(
              'Selects Over Al|pha|',
              'Al'.length,
              'Alpha'.length
            ),
            Keyboard.sKeydown(doc, Keys.escape(), {}),
            steps.sWaitForNoMenu(
              'Pressing escape should dismiss menu for end of this part of test'
            )
          ];
        };
        ...
        const testWithoutSelector = () => {
          const typeahead = gui
            .getByUid('test-type-without-selectover')
            .getOrDie();
          ...
          return [
            FocusTools.sSetFocus(
              'Focusing typeahead without selectover',
              gui.element(),
              '.without-selectover'
            ),
            FocusTools.sSetActiveValue(doc, 'al'),
            ...
            steps.sTriggerInputEvent('Simulate typing to show menu with "Al"'),
            steps.sWaitForMenu(
              '"Typing" should activate menu and not select over because the setting is false'
            ),

            steps.sAssertValue(
              'Should keep it as "Al" with selection at end of input',
              'Al'
            ),
            steps.sAssertTextSelection(
              'No select over. So "Al"',
              'Al'.length,
              'Al'.length
            )
          ];
        ...
        return [
          ...
          .concat(testWithSelector(), testWithoutSelector())
          .concat([GuiSetup.mRemoveStyles]);
      },
      success,
      failure
    );
