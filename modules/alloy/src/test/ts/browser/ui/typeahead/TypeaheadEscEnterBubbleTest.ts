...
UnitTest.asynctest(
  'Browser Test: .ui.typeahead.TypeaheadEscEnterBubbleTest',
  (success, failure) => {
    GuiSetup.setup(
      (store, _doc, _body) => {
      ...
      (doc, body, gui, _component, store) => {
        ...
        return [
          ...
          Keyboard.sKeydown(doc, Keys.enter(), {}),
          store.sAssertEq('Should have item1 and onExecute', [
            '1(input-div-li)',
            '***onExecute***'
          ]),

          FocusTools.sSetFocus(
            'Focusing typeahead to open preview mode',
            gui.element(),
            'input'
          ),
          ...
          store.sAssertEq('Pressing <enter> in preview mode should execute', [
            '***onExecute***'
          ]),
