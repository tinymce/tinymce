  ...
  GuiSetup.setup(
                ...
                const future = Future.pure([
                  {
                    ...
                    data: {
                      value: text + '1',
                      meta: { text: Strings.capitalize(text) + '1' }
                    }
                  },
                  {
                    type: 'item',
                    data: {
                      value: text + '2',
                      meta: { text: Strings.capitalize(text) + '2' }
                    }
                  }
                ]);
    ...
    (doc, _body, gui, component, _store) => {
      ...
      const testWithChooser = (label: string, sChooser: Step<any, any>): Step<any, any> =>
        Logger.t(
          ...
          GeneralSteps.sequence([
            Logger.t('Set some content in the typeahead', UiControls.sSetValue(typeahead.element(), 'Neo')),
            ...
            Logger.t(
              ...
              GeneralSteps.sequence([sChooser, UiFinder.sNotExists(component.element(), '[role="menu"]')])
            ),

            Logger.t(
              'Wait and ensure menu does not reappear when second input triggers',
              GeneralSteps.sequence([Step.wait(500), UiFinder.sNotExists(component.element(), '[role="menu"]')])
            )
          ])
        );
      ...
      return [
        ...
        GuiSetup.mAddStyles(doc, ['.selected-item { background-color: #cadbee; }']),
        ...
        testWithChooser(
          ...
          GeneralSteps.sequence([Keyboard.sKeydown(doc, Keys.down(), {}), Keyboard.sKeydown(doc, Keys.enter(), {})])
        ),

        testWithChooser('Using mouse to choose item', GeneralSteps.sequence([Mouse.sClickOn(component.element(), '[role="menuitem"]')])),

        testWithChooser('Using tap to choose item', GeneralSteps.sequence([Touch.sTapOn(component.element(), '[role="menuitem"]')])),
