...
UnitTest.asynctest(
  'Browser Test: .ui.typeahead.TypeaheadTest',
  (success, failure) => {
    GuiSetup.setup(
      (store, _doc, _body) => {
      ...
      (doc, _body, gui, component, _store) => {
        ...
        return [
          ...
          steps.sAssertFocusOnTypeahead(
            'Focus stays on typeahead after pressing Down'
          ),
          ...
          Step.sync(() => {
            ...
            const peo1Data = repState
              .lookup('peo1')
              .getOrDie('Should have dataset data for peo1 now');
            Assertions.assertEq(
              'Checking peo1Data',
              { value: 'peo1', meta: { text: 'Peo1' } },
              peo1Data
            );

            const peo2Data = repState
              .lookup('peo2')
              .getOrDie('Should have dataset data for peo2 now');
            Assertions.assertEq(
              'Checking peo2Data',
              { value: 'peo2', meta: { text: 'Peo2' } },
              peo2Data
            );
          }),
          ...
          steps.sAssertFocusOnTypeahead(
            'Pressing down to check for dismissing popups'
          ),
