        ...
        InlineView.sketch({
          ...
          inlineBehaviours: Behaviour.derive([
            AddEventsBehaviour.config('inline-reposition-test', [
              AlloyEvents.run(
                'test-reposition',
                store.adder('test-reposition-fired')
              )
            ])
          ])
        })
          ...
          GeneralSteps.sequence([
            ...
            Step.sync(() => {
              Assertions.assertEq(
                'Checking isOpen API',
                true,
                InlineView.isOpen(component)
              );
            })
          ])
        );

      const sCheckPosition = (
        label: string,
        element: Element,
        x: number,
        y: number
        ...
        Logger.t(
          ...
          Step.sync(() => {
            ...
            const left = parseInt(
              Css.get(element, 'left').replace('px', ''),
              10
            );
      ...
      return [
        Logger.t(
          ...
          GeneralSteps.sequence([
            ...
            sCheckPosition(
              'Check inline view has not moved',
              inline.element(),
              200,
              210
            ),
            store.sAssertEq('Broadcasting SHOULD fire reposition event', [
              'test-reposition-fired'
            ])
          ])
        ),
