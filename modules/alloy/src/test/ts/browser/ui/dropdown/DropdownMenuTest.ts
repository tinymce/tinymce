  ...
  GuiSetup.setup(
    (store, _doc, _body) => {
      ...
      const widget = Container.sketch({
        ...
        components: Arr.map(['one', 'two', 'three'], makeFlow)
      });
        ...
        menus: Obj.map(
          {
            'tools-menu': {
              ...
              items: Arr.map(
                [
                  {
                    type: 'item',
                    data: { value: 'packages', meta: { text: 'Packages' } },
                    hasSubmenu: true
                  },
                  {
                    type: 'item',
                    data: { value: 'about', meta: { text: 'About' } }
                  },
                  {
                    type: 'widget',
                    widget,
                    data: { value: 'widget', meta: {} }
                  }
                ],
            ...
            'packages': {
              // menu name should be triggering parent item so TieredMenuSpec path works
              ...
              items: Arr.map(
                [
                  {
                    type: 'item',
                    data: { value: 'sortby', meta: { text: 'SortBy' } },
                    hasSubmenu: true
                  }
                ],
                TestDropdownMenu.renderItem
            ...
            'sortby': {
              ...
              items: Arr.map(
                [
                  {
                    type: 'item',
                    data: { value: 'strings', meta: { text: 'Strings' } },
                    hasSubmenu: true
                  },
                  {
                    type: 'item',
                    data: { value: 'numbers', meta: { text: 'Numbers' } },
                    hasSubmenu: true
                  }
                ],
                TestDropdownMenu.renderItem
                ...
                [
                  {
                    ...
                    data: {
                      value: 'versions',
                      meta: { text: 'Versions', html: '<b>V</b>ersions' }
                    }
                  },
                  {
                    type: 'item',
                    data: { value: 'alphabetic', meta: { text: 'Alphabetic' } }
                  }
                ],
                TestDropdownMenu.renderItem
              )
    ...
    (doc, _body, gui, dropdown, store) => {
      ...
      const focusables: Record<string, TestFocusable> = {
        toolsMenu: {
          label: 'tools-menu',
          selector: '.menu[aria-label="Tools Menu"]'
        },
        packagesMenu: {
          label: 'packages-menu',
          selector: '.menu[aria-label="Packages Menu"]'
        },
        sortbyMenu: {
          label: 'sortby-menu',
          selector: '.menu[aria-label="Sortby Menu"]'
        },
        stringsMenu: {
          label: 'strings-menu',
          selector: '.menu[aria-label="Strings Menu"]'
        },
        numbersMenu: {
          label: 'numbers-menu',
          selector: '.menu[aria-label="Numbers Menu"]'
        },
      ...
      const sTestMenus = (
        label: string,
        stored: string[],
        focused: TestFocusable,
        active: TestFocusable[],
        background: TestFocusable[],
        others: TestFocusable[]
      ) => {
      ...
      return [
        ...
        sTestMenus(
          ...
          focusables.strings,
          [focusables.sortbyMenu],
          [focusables.toolsMenu, focusables.packagesMenu],
          [focusables.stringsMenu, focusables.numbersMenu]
        ),
