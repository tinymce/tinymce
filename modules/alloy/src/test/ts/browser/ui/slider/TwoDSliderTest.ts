...
UnitTest.asynctest(
  'Browser Test: ui.slider.TwoDSliderTest',
  (success, failure) => {
    ...
    GuiSetup.setup(
      ...
      (doc, _body, _gui, component, _store) => {
        const cGetBounds = Chain.mapper((elem: Element) =>
          elem.dom().getBoundingClientRect()
        );

        const cGetComponent = Chain.binder((elem: Element) =>
          component.getSystem().getByDom(elem)
        );
        ...
        const cGetParts = NamedChain.asChain([
          ...
          NamedChain.direct(
            'slider',
            UiFinder.cFindIn('.twod-slider-test-thumb'),
            'thumb'
          ),
          NamedChain.direct(
            'slider',
            UiFinder.cFindIn('.twod-slider-test-top-edge'),
            'tedge'
          ),
          NamedChain.direct(
            'slider',
            UiFinder.cFindIn('.twod-slider-test-left-edge'),
            'ledge'
          ),
          NamedChain.direct(
            'slider',
            UiFinder.cFindIn('.twod-slider-test-right-edge'),
            'redge'
          ),
          NamedChain.direct(
            'slider',
            UiFinder.cFindIn('.twod-slider-test-bottom-edge'),
            'bedge'
          ),
          NamedChain.direct(
            'slider',
            UiFinder.cFindIn('.twod-slider-test-spectrum'),
            'spectrum'
          ),
        ...
        const cCheckThumbAtLeft = Chain.op((parts: any) => {
          Assert.eq(
            'Thumb (' +
              ...
              ')',
            true,
            parts.ledgeRect.right > parts.thumbRect.left &&
              parts.ledgeRect.left < parts.thumbRect.left
          );
        });
        ...
        const cCheckThumbAtRight = Chain.op((parts: any) => {
          Assert.eq(
            'Thumb (' +
              ...
              ')',
            true,
            parts.redgeRect.left < parts.thumbRect.right &&
              parts.ledgeRect.left < parts.redgeRect.left
          );
        });
        ...
        const cCheckThumbAtTop = Chain.op((parts: any) => {
          Assert.eq(
            'Thumb (' +
              ...
              ')',
            true,
            parts.tedgeRect.bottom > parts.thumbRect.top &&
              parts.tedgeRect.top < parts.thumbRect.top
          );
        });
        ...
        const cCheckThumbAtBottom = Chain.op((parts: any) => {
          Assert.eq(
            'Thumb (' +
              ...
              ')',
            true,
            parts.bedgeRect.top < parts.thumbRect.bottom &&
              parts.tedgeRect.top < parts.bedgeRect.top
          );
        });

        const cCheckThumbPastRight = Chain.op((parts: any) => {
          Assert.eq(
            'Checking thumb past end of spectrum',
            true,
        ...
        const cCheckThumbBeforeLeft = Chain.op((parts: any) => {
          Assert.eq(
            'Checking thumb before start of spectrum',
            true,
        ...
        return [
          ...
          FocusTools.sTryOnSelector(
            'Focus should be on spectrum',
            doc,
            '.twod-slider-test-spectrum'
          ),
          ...
          Keyboard.sKeydown(doc, Keys.down(), {}),
          sAssertValue(
            'Checking that the thumb is now one step further right',
            { x: 49, y: 60 }
          ),
