...
const getOffset = <I extends SelectionAnchor | NodeAnchor>(
  ...
  return Option.from(win.frameElement as HTMLFrameElement)
    .map(Element.fromDom)
    .filter(hasSameOwner)
    .map(Location.absolute);
};

const getRootPoint = <I extends SelectionAnchor | NodeAnchor>(
  component: AlloyComponent,
  origin: OriginAdt,
  anchorInfo: I
): CssPosition.CssPositionAdt => {
