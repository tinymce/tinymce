...
const make: SingleSketchFactory<TieredMenuDetail, TieredMenuSpec> = (detail, rawUiSpec) => {
  ...
  const toDirectory = (container: AlloyComponent): Record<string, string[]> => Obj.map(detail.data.menus, (data, menuName) => Arr.bind(data.items, (item) => item.type === 'separator' ? [ ] : [ item.data.value ]));
