...
const findOffsetOfValue = (
  ...
  const centerMinEdge = minEdge.bind((edge: AlloyComponent) => Option.some(getXCenterOffSetOf(edge, spectrum))).getOr(minOffset);
  const centerMaxEdge = maxEdge.bind((edge: AlloyComponent) => Option.some(getXCenterOffSetOf(edge, spectrum))).getOr(maxOffset);
