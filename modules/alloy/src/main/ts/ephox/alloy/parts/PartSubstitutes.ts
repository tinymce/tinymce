...
const combine = <D extends CompositeSketchDetail, S extends CompositeSketchSpec>(
  detail: D,
  data: PartType.BasePartDetail<D, S>,
  partSpec: S,
    ...
    part.fold(
      ...
      (data) => {
        ...
        externals[data.name] = Fun.constant(
          data.factory.sketch(combine(detail, data, partSpec[PartType.original()]), partSpec) // This is missing partValidated
        );
        // no placeholders
      },
      ...
      (data) => {
        internals[data.pname] = UiSubstitutes.multiple(true, (detail, _partSpec, _partValidated) => {
          ...
          return Arr.map(units, (u) =>
            ...
            data.factory.sketch(Merger.deepMerge(data.defaults(detail, u, _partValidated), u, data.overrides(detail, u)))
          );
        });
      }
    );
