...
const subs = <D extends CompositeSketchDetail>(
  owner: string,
  detail: D,
  ...
  Arr.each(parts, (part) => {
    part.fold(
      // Internal
      (data) => {
        internals[data.pname] = UiSubstitutes.single(
          true,
      ...
      // Group
      (data) => {
        internals[data.pname] = UiSubstitutes.multiple(
          true,
          (detail, _partSpec, _partValidated) => {
            const units: Array<Record<string, any>> = (detail as any)[
              data.name
            ];
