...
export interface PartTypeAdt<S = BasePartDetail<any, any>> {
  fold: <T>(required: PartType<S, T>, external: PartType<S, T>, optional: PartType<S, T>, group: PartType<S, T>) => T;
  match: <T>(branches: { required: PartType<S, T>; external: PartType<S, T>; optional: PartType<S, T>; group: PartType<S, T> }) => T;
...
const adt: {
  ...
  group: PartType<any>;
} = Adt.generate([{ required: ['data'] }, { external: ['data'] }, { optional: ['data'] }, { group: ['data'] }]);
...
const fGroupSchema = FieldSchema.state('schema', () => [FieldSchema.option('preprocess')]);

const fDefaults = FieldSchema.defaulted('defaults', Fun.constant({}));
const fOverrides = FieldSchema.defaulted('overrides', Fun.constant({}));

const requiredSpec = ValueSchema.objOf([fFactory, fSchema, fName, fPname, fDefaults, fOverrides]);

const externalSpec = ValueSchema.objOf([fFactory, fSchema, fName, fDefaults, fOverrides]);

const optionalSpec = ValueSchema.objOf([fFactory, fSchema, fName, fPname, fDefaults, fOverrides]);

const groupSpec = ValueSchema.objOf([fFactory, fGroupSchema, fName, FieldSchema.strict('unit'), fPname, fDefaults, fOverrides]);
...
const convert = <D extends CompositeSketchDetail, S, PS extends PartSpec<D, S>, PD extends PartDetail<D, S>>(
  adtConstructor: PartType<PD>,
  partSchema: Processor
) => (spec: PS): PartTypeAdt<PD> => {
  const data = ValueSchema.asRawOrDie('Converting part type', partSchema, spec);
  return adtConstructor(data);
};

const required: <D extends CompositeSketchDetail, PS = SimpleOrSketchSpec>(p: PartSpec<D, PS>) => PartTypeAdt<PartDetail<D, PS>> = convert(
  adt.required,
  requiredSpec
) as any;
const external: <D extends CompositeSketchDetail, PS = SimpleOrSketchSpec>(
  p: ExternalPartSpec<D, PS>
) => PartTypeAdt<ExternalPartDetail<D, PS>> = convert(adt.external, externalSpec) as any;
const optional: <D extends CompositeSketchDetail, PS = SimpleOrSketchSpec>(p: PartSpec<D, PS>) => PartTypeAdt<PartDetail<D, PS>> = convert(
  adt.optional,
  optionalSpec
) as any;
const group: <D extends CompositeSketchDetail, PS = SimpleOrSketchSpec>(
  p: GroupPartSpec<D, PS>
) => PartTypeAdt<GroupPartDetail<D, PS>> = convert(adt.group, groupSpec) as any;
