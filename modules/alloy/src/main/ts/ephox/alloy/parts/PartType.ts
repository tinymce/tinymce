...
export interface PartDetail<D extends CompositeSketchDetail, PS>
  extends BasePartDetail<D, PS> {
  factory: { sketch: (detail: PS & { uid: string }) => any };
}

export interface ExternalPartDetail<D extends CompositeSketchDetail, PS>
  extends BasePartDetail<D, PS> {
  factory: { sketch: (detail: PS & { uid: string }, partSpec: PS) => any };
}

export interface GroupPartDetail<D extends CompositeSketchDetail, PS>
  extends PartDetail<D, PS> {
  unit: string;
}

export interface PartSpec<D extends CompositeSketchDetail, PS>
  extends Partial<PartDetail<D, PS>> {
  name: string;
}

export interface ExternalPartSpec<D extends CompositeSketchDetail, PS>
  extends Partial<ExternalPartDetail<D, PS>> {}

export interface GroupPartSpec<D extends CompositeSketchDetail, PS>
  extends Partial<GroupPartDetail<D, PS>> {
  name: string;
  unit: string;
}

export type OverrideHandler<D, PS> = (
  detail: D,
  spec: PS,
...
const fPname = FieldSchema.field(
  ...
  FieldPresence.defaultedThunk(
    (typeSpec: PartSpec<any, any>) =>
      '<alloy.' + Id.generate(typeSpec.name) + '>'
  ),
...
const convert = <
  ...
  S,
  PS extends PartSpec<D, S>,
  PD extends PartDetail<D, S>
>(
...
const required: <D extends CompositeSketchDetail, PS = SimpleOrSketchSpec>(
  p: PartSpec<D, PS>
) => PartTypeAdt<PartDetail<D, PS>> = convert(
  adt.required,
  requiredSpec
) as any;
const external: <D extends CompositeSketchDetail, PS = SimpleOrSketchSpec>(
  p: ExternalPartSpec<D, PS>
) => PartTypeAdt<ExternalPartDetail<D, PS>> = convert(
  adt.external,
  externalSpec
) as any;
const optional: <D extends CompositeSketchDetail, PS = SimpleOrSketchSpec>(
  p: PartSpec<D, PS>
) => PartTypeAdt<PartDetail<D, PS>> = convert(
  adt.optional,
  optionalSpec
