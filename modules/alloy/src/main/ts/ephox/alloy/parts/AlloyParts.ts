...
const schemas = (parts: PartType.PartTypeAdt[]): FieldProcessorAdt[] =>
  ...
  Arr.bind(parts, (part: PartType.PartTypeAdt) =>
    part
      .fold<Option<PartType.BasePartDetail<any, any>>>(Option.none, Option.some, Option.none, Option.none)
      .map((data) => FieldSchema.strictObjOf(data.name, data.schema.concat([Fields.snapshot(PartType.original())])))
      .toArray()
  );

const names = (parts: PartType.PartTypeAdt[]): string[] => Arr.map(parts, PartType.name);

const substitutes = <D extends CompositeSketchDetail>(owner: string, detail: D, parts: PartType.PartTypeAdt[]): Substitutions =>
  PartSubstitutes.subs(owner, detail, parts);

const components = <D extends CompositeSketchDetail>(owner: string, detail: D, internals: Substitution): AlloySpec[] =>
  UiSubstitutes.substitutePlaces(Option.some(owner), detail, detail.components, internals);
...
const getPartOrDie = <D extends CompositeSketchDetail>(component: AlloyComponent, detail: D, partKey: string): AlloyComponent =>
  getPart(component, detail, partKey).getOrDie('Could not find part: ' + partKey);

const getParts = <D extends CompositeSketchDetail>(
  component: AlloyComponent,
  detail: D,
  partKeys: string[]
): Record<string, () => Result<AlloyComponent, Error>> => {
...
const getAllParts = <D extends CompositeSketchDetail>(
  component: AlloyComponent,
  detail: D
): Record<string, () => Result<AlloyComponent, Error>> => {
  const system = component.getSystem();
...
const getPartsOrDie = <D extends CompositeSketchDetail>(
  component: AlloyComponent,
  detail: D,
  partKeys: string[]
...
const defaultUids = (baseUid: string, partTypes: PartType.PartTypeAdt[]): Record<string, string> => {
  ...
  return Objects.wrapAll(Arr.map(partNames, (pn) => ({ key: pn, value: baseUid + '-' + pn })));
};
