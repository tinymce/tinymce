...
const generate = (
  ...
  Arr.each(parts, (part) => {
    PartType.asNamedPart(part).each((np) => {
      ...
      r[np.name] = (config) => {
        const validated = ValueSchema.asRawOrDie(
          'Part: ' + np.name + ' in ' + owner,
          ValueSchema.objOf(np.schema),
          config
        );
        return {
          ...g,
...
const schemas = (parts: PartType.PartTypeAdt[]): FieldProcessorAdt[] =>
  ...
  Arr.bind(parts, (part: PartType.PartTypeAdt) =>
    part
      .fold<Option<PartType.BasePartDetail<any, any>>>(
        Option.none,
        Option.some,
        Option.none,
        Option.none
      ...
      .map((data) =>
        FieldSchema.strictObjOf(
          data.name,
          data.schema.concat([Fields.snapshot(PartType.original())])
        )
      )
      .toArray()
  );
...
const names = (parts: PartType.PartTypeAdt[]): string[] =>
  Arr.map(parts, PartType.name);

const substitutes = <D extends CompositeSketchDetail>(
  owner: string,
  detail: D,
  parts: PartType.PartTypeAdt[]
): Substitutions => PartSubstitutes.subs(owner, detail, parts);

const components = <D extends CompositeSketchDetail>(
  owner: string,
  detail: D,
  internals: Substitution
): AlloySpec[] =>
  UiSubstitutes.substitutePlaces(
    Option.some(owner),
    detail,
    detail.components,
    internals
...
const getPart = <D extends CompositeSketchDetail>(
...
const getPartOrDie = <D extends CompositeSketchDetail>(
  component: AlloyComponent,
  detail: D,
  partKey: string
): AlloyComponent =>
...
const getAllParts = <D extends CompositeSketchDetail>(
  ...
  return Obj.map(detail.partUids, (pUid, _k) =>
    Fun.constant(system.getByUid(pUid))
  );
};

const getAllPartNames = <D extends CompositeSketchDetail>(detail: D) =>
  Obj.keys(detail.partUids);
...
const defaultUids = (
  baseUid: string,
  partTypes: PartType.PartTypeAdt[]
...
const defaultUidsSchema = (
  partTypes: PartType.PartTypeAdt[]
): FieldProcessorAdt =>
  FieldSchema.field(
    'partUids',
    'partUids',
    FieldPresence.mergeWithThunk((spec: SketchSpec) =>
      defaultUids(spec.uid, partTypes)
    ),
