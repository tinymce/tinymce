...
const missingOrderError = <T>(
  ...
  Result.error([
    'The event (' +
      ...
      'can trigger it are: ' +
      JSON.stringify(
        Arr.map(tuples, (c) => c.name()),
        null,
        2
      )
  ]);

const fuse = <T extends EventFormat>(
  tuples: Array<BehaviourTuple<T>>,
  eventOrder: Record<string, string[]>,
  eventName: string
): Result<AlloyEventHandler<T>, any[]> => {
...
const combineGroups = <T extends EventFormat>(
  byEventName: Record<string, Array<BehaviourTuple<T>>>,
  eventOrder: Record<string, string[]>
) => {
