...
const field = (
  ...
  FieldSchema.defaultedObjOf(
    ...
    ).concat([FieldSchema.state('dump', Fun.identity)])
  );

const get = (data: SketchBehaviours): AlloyBehaviourRecord => data.dump;

const augment = (
  data: SketchBehaviours,
  original: Array<NamedConfiguredBehaviour<any, any>>
): AlloyBehaviourRecord => ({
  ...data.dump,
  ...derive(original)
});
