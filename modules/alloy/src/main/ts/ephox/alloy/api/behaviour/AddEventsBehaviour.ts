...
import {
  ...
  AlloyEventRecord,
  derive
} from '../events/AlloyEvents';
import {
  AlloyBehaviour,
  create as createBehaviour,
...
const events = (
  name: string,
  eventHandlers: Array<AlloyEventKeyAndHandler<any>>
): AlloyBehaviour<any, any> => {
  const events: AlloyEventRecord = derive(eventHandlers);

  return createBehaviour({
    fields: [FieldSchema.strict('enabled')],
    name,
    active: {
...
const config = (
  name: string,
  eventHandlers: Array<AlloyEventKeyAndHandler<any>>
): NamedConfiguredBehaviour<any, any> => {
