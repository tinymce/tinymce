...
export interface CoordAdt<T = number> {
  fold: <U>(offset: DragCoords<T, U>, absolute: DragCoords<T, U>, fixed: DragCoords<T, U>) => U;
  match: <U>(branches: { offset: DragCoords<T, U>; absolute: DragCoords<T, U>; fixed: DragCoords<T, U> }) => U;
...
} = Adt.generate([{ offset: ['x', 'y'] }, { absolute: ['x', 'y'] }, { fixed: ['x', 'y'] }]);
...
const transform = (changes: CoordTransform[]) => (x: number, y: number): Position =>
  Arr.foldl(changes, (rest, f) => f(rest), Position(x, y));
...
const withinRange = (
  coord1: CoordAdt<number>,
  coord2: CoordAdt<number>,
  xRange: number,
  yRange: number,
  scroll: Position,
  origin: Position
...
const absorb = (
  partialCoord: CoordAdt<Option<number>>,
  originalCoord: CoordAdt<number>,
  scroll: Position,
  origin: Position
): CoordAdt<number> => {
...
export { offset, absolute, fixed, asFixed, asAbsolute, asOffset, withinRange, getDeltas, toStyles, translate, absorb, toString };
