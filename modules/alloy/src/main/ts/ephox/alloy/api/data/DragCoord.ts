...
export type CoordStencil = (
  coord: CoordAdt<number>,
  scroll: Position,
  origin: Position
) => Position;
/*
...
const subtract = (change: Position): CoordTransform => (point) =>
  point.translate(-change.left(), -change.top());

const add = (change: Position): CoordTransform => (point) =>
  point.translate(change.left(), change.top());

const transform = (changes: CoordTransform[]) => (
  x: number,
  y: number
): Position => Arr.foldl(changes, (rest, f) => f(rest), Position(x, y));

const asFixed: CoordStencil = (
  coord: CoordAdt<number>,
  scroll: Position,
  origin: Position
) =>
