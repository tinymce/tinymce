...
const makeApi = <A, R>(
  f: (api: A, comp: AlloyComponent, ...rest: any[]) => R
) =>
  FunctionAnnotator.markAsSketchApi(
    (component: AlloyComponent, ...rest: any[] /* , ... */) =>
      f.apply(
        undefined,
        ([component.getApis<any>()] as any).concat([component].concat(rest))
      ),
    f
  );

export { makeApi, premade, getPremade };
