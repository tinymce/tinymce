...
const factory: CompositeSketchFactory<ToolbarDetail, ToolbarSpec> = (detail, components, _spec, _externals) => {
  ...
  const getGroupContainer = (component: AlloyComponent) =>
    detail.shell ? Option.some(component) : AlloyParts.getPart(component, detail, 'groups');
  ...
  return {
    ...
    behaviours: SketchBehaviours.augment(detail.toolbarBehaviours, extra.behaviours),
    apis: {
      setGroups
    },
    domModification: {
