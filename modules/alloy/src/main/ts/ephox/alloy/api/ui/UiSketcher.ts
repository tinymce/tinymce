...
const single = <D extends SingleSketchDetail, S extends SingleSketchSpec>(owner: string, schema: ValueProcessorTypes[], factory: SingleSketchFactory<D, S>, spec: S): SketchSpec => {
  const specWithUid = supplyUid<S>(spec);
  const detail = SpecSchema.asRawOrDie<D, S>(owner, schema, specWithUid, [ ], [ ]);
  return factory(detail, specWithUid);
};

const composite = <D extends CompositeSketchDetail, S extends CompositeSketchSpec>(owner: string, schema: ValueProcessorTypes[], partTypes: PartTypeAdt[], factory: CompositeSketchFactory<D, S>, spec: S): SketchSpec => {
