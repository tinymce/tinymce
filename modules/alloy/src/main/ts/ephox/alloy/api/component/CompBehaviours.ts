...
const getBehaviours = (spec: { behaviours?: AlloyBehaviourRecord }): Array<AlloyBehaviour<any, any, any>> => {
  ...
  const keys = Arr.filter(Obj.keys(behaviours), (k: BehaviourName) => behaviours[k] !== undefined);
  return Arr.map(keys, (k) => behaviours[k].me);
};

const generateFrom = (
  spec: { behaviours?: AlloyBehaviourRecord },
  all: Array<AlloyBehaviour<any, any, any>>
): BehaviourBlob.BehaviourData<any, any, any> => BehaviourBlob.generateFrom(spec, all);
