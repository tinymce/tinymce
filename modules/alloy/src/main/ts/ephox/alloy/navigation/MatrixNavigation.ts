...
export type MatrixNavigationFunc<A> = (
  matrix: A[][],
  startRow: number,
  startCol: number
) => Option<MatrixNavigationOutcome<A>>;
...
const toCell = <A>(
  ...
  Option.from(matrix[rowIndex]).bind((row) =>
    Option.from(row[columnIndex]).map((cell) => ({
      rowIndex,
      columnIndex,
      cell
    }))
