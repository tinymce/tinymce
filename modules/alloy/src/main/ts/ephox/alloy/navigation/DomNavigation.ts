...
const horizontal = (
  ...
  const isDisabledButton = (candidate: Element<HTMLElement>) =>
    Node.name(candidate) === 'button' &&
    Attr.get(candidate, 'disabled') === 'disabled';

  const tryCycle = (
    initial: number,
    index: number,
    candidates: Array<Element<HTMLElement>>
  ...
  return DomPinpoint.locateVisible(container, current, selector).bind(
    (identified) => {
