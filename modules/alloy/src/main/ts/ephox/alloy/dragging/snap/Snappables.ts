...
const getCoords = <E>(
  ...
  snapInfo: SnapsConfig<E>,
  coord: DragCoord.CoordAdt,
  delta: Position
): DragCoord.CoordAdt =>
  Presnaps.get(component, snapInfo).fold(
    () => coord,
    (fixed) =>
...
const findMatchingSnap = <E>(
  snaps: Array<SnapConfig<E>>,
  newCoord: DragCoord.CoordAdt,
  scroll: Position,
  origin: Position
): Option<SnapOutput<E>> =>
  Arr.findMap(snaps, (snap) => {
    const sensor = snap.sensor;
    const inRange = DragCoord.withinRange(
      newCoord,
      sensor,
      snap.range.left(),
      snap.range.top(),
...
const findClosestSnap = <E>(
  ...
  return matchSnap.orThunk(
    (): Option<SnapOutput<E>> => {
      const bestSnap = Arr.foldl(
        snaps,
        (acc: SnapCandidate<E>, snap: SnapConfig<E>): SnapCandidate<E> => {
          const sensor = snap.sensor;
          const deltas = DragCoord.getDeltas(
            newCoord,
            sensor,
            snap.range.left(),
            snap.range.top(),
...
export { moveOrSnap, stopDrag, snapTo };
