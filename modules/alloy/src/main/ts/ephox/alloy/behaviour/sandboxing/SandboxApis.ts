...
const openWhileCloaked = (
  sandbox: AlloyComponent,
  sConfig: SandboxingConfig,
  sState: SandboxingState,
  data: AlloySpec,
  transaction: () => void
) => {
...
const isPartOf = (sandbox: AlloyComponent, sConfig: SandboxingConfig, sState: SandboxingState, queryElem: Element) =>
  isOpen(sandbox, sConfig, sState) && sState.get().exists((data) => sConfig.isPartOf(sandbox, data, queryElem));
...
export { cloak, decloak, open, openWhileCloaked, close, isOpen, isPartOf, getState, setContent };
