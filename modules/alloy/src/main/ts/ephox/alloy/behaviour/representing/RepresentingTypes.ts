...
export interface RepresentingBehaviour
  extends Behaviour.AlloyBehaviour<RepresentingConfigSpec, RepresentingConfig> {
  config: (
    config: RepresentingConfigSpec
  ) => Behaviour.NamedConfiguredBehaviour<
    RepresentingConfigSpec,
    RepresentingConfig
  >;
...
interface BaseStoreConfig<T, S extends RepresentingState> {
  ...
  manager: {
    setValue: (
      comp: AlloyComponent,
      config: RepresentingConfig,
      state: RepresentingState,
      data: RepresentingData
    ) => void;
    getValue: (
      comp: AlloyComponent,
      config: RepresentingConfig,
...
export interface MemoryStoreConfig
  extends BaseStoreConfig<RepresentingData, MemoryRepresentingState> {}
...
export interface ManualStoreConfig
  extends BaseStoreConfig<RepresentingData, ManualRepresentingState> {
  getValue: (comp: AlloyComponent) => RepresentingData;
  setValue: (comp: AlloyComponent, data: RepresentingData) => void;
}
...
export interface DatasetStoreConfig<T extends ItemDataTuple>
  extends BaseStoreConfig<T, DatasetRepresentingState> {
  getFallbackEntry: (key: DatasetStoreKey) => T;
  getDataKey: (comp: AlloyComponent) => DatasetStoreKey;
  setValue: (comp: AlloyComponent, data: T) => void;
}
...
export interface RepresentingConfigSpec extends Behaviour.BehaviourConfigSpec {
  store:
    | MemoryStoreConfigSpec
    | ManualStoreConfigSpec
    | DatasetStoreConfigSpec<RepresentingData>;
...
export interface RepresentingConfig extends Behaviour.BehaviourConfigDetail {
  ...
  store:
    | MemoryStoreConfig
    | ManualStoreConfig
    | DatasetStoreConfig<RepresentingData>;
  onSetValue: (comp: AlloyComponent, data: RepresentingData) => void;
}
