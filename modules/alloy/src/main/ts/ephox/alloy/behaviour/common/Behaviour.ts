...
): AlloyEvents.AlloyEventKeyAndHandler<CustomEvent> =>
  AlloyEvents.runOnExecute((component) => {
    executor(component, bConfig, bState);
  });

const loadEvent = <C extends BehaviourConfigSpec, S extends BehaviourState>(
  bConfig: C,
  bState: S,
  f: Executor<C, S>
): AlloyEvents.AlloyEventKeyAndHandler<CustomEvent> =>
...
const wrapApi = <D extends BehaviourConfigDetail, S extends BehaviourState>(
  bName: string,
  apiFunction: BehaviourApiFunc<D, S>,
  apiName: string
) => {
...
const doCreate = <
  ...
  E extends BehaviourExtraRecord<E>
>(
  configSchema: Processor,
  schemaSchema: FieldProcessorAdt,
  name: string,
  active: BehaviourActiveSpec<D, S>,
  apis: A,
  extra: E,
