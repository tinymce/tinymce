...
const generateFrom = (
  spec: { behaviours?: BehaviourRecord },
  all: Array<AlloyBehaviour<BehaviourConfigSpec, BehaviourConfigDetail, BehaviourState>>
): BehaviourData<BehaviourConfigSpec, BehaviourConfigDetail, BehaviourState> => {
  /*
  ...
  const schema: FieldProcessorAdt[] = Arr.map(all, (a) =>
    ...
    FieldSchema.optionObjOf(a.name(), [FieldSchema.strict('config'), FieldSchema.defaulted('state', NoState)])
  );

  type B = Record<string, Option<BehaviourSpec<BehaviourConfigSpec, BehaviourConfigDetail, BehaviourState>>>;
  const validated = ValueSchema.asRaw<B>('component.behaviours', ValueSchema.objOf(schema), spec.behaviours).fold(
    (errInfo) => {
...
const getBehaviours = <C extends BehaviourConfigSpec, D extends BehaviourConfigDetail, S extends BehaviourState>(
  bData: BehaviourData<C, D, S>
): Array<AlloyBehaviour<C, D, S>> => bData.list;

const getData = <C extends BehaviourConfigSpec, D extends BehaviourConfigDetail, S extends BehaviourState>(
  bData: BehaviourData<C, D, S>
): Record<string, () => Option<BehaviourConfigAndState<D, S>>> => bData.data;

export { generateFrom, getBehaviours, getData };
