...
const cleanParameters = (parameters: string[]) =>
  Arr.map(parameters, (p) => (Strings.endsWith(p, '/*') ? p.substring(0, p.length - '/*'.length) : p));
...
const getAnnotation = <T extends Function>(f: FunctionWithAnnotation<T>) => Option.from(f.toFunctionAnnotation?.());
