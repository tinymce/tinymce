...
export type FunctionWithAnnotation<T extends Function> = T & {
  toFunctionAnnotation?: () => { name: string; parameters: string[] };
};

const markAsBehaviourApi = <T extends Function>(
  f: T,
  apiName: string,
  apiFunction: Function
): FunctionWithAnnotation<T> => {
...
const cleanParameters = (parameters: string[]) =>
  Arr.map(parameters, (p) =>
    Strings.endsWith(p, '/*') ? p.substring(0, p.length - '/*'.length) : p
  );

const markAsExtraApi = <T extends Function>(
  f: T,
  extraName: string
): FunctionWithAnnotation<T> => {
...
const markAsSketchApi = <T extends Function>(
  f: T,
  apiFunction: Function
): FunctionWithAnnotation<T> => {
  const delegate = apiFunction.toString();
...
const getAnnotation = <T extends Function>(f: FunctionWithAnnotation<T>) =>
  Option.from(f.toFunctionAnnotation?.());
