...
const init = (): LayeredState => {
  ...
  const lookupMenu = (menuValue: string): Option<MenuPreparation> => Obj.get(menus.get(), menuValue);

  const lookupItem = (itemValue: string): Option<string> => Obj.get(expansions.get(), itemValue);
