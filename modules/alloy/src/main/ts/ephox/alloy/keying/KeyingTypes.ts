...
const defaultExecute = (component: AlloyComponent, simulatedEvent: NativeSimulatedEvent, focused: Element): Option<boolean> => EditableFields.inside(focused) && KeyMatch.inSet(Keys.SPACE())(simulatedEvent.event()) ? Option.none() : doDefaultExecute(component, simulatedEvent, focused);
...
const stopEventForFirefox: KeyRuleHandler<GeneralKeyingConfig, any> = (_component: AlloyComponent, _simulatedEvent: NativeSimulatedEvent) => Option.some<boolean>(true);
