...
const schema: FieldProcessorAdt[] = [
  FieldSchema.strictObjOf('selectors', [FieldSchema.strict('row'), FieldSchema.strict('cell')]),
...
const execute: KeyRuleHandler<MatrixConfig, Stateless> = (component, simulatedEvent, matrixConfig) =>
  Focus.search(component.element()).bind((focused) => matrixConfig.execute(component, simulatedEvent, focused));
...
): DomMovement.ElementMover<MatrixConfig, Stateless> => (element, focused, matrixConfig) => {
