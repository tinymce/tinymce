...
const execute: KeyRuleHandler<MatrixConfig, Stateless> = (
  component,
  simulatedEvent,
  matrixConfig
) =>
  Focus.search(component.element()).bind((focused) =>
    matrixConfig.execute(component, simulatedEvent, focused)
  );

const toMatrix = (rows: Element[], matrixConfig: MatrixConfig): Element[][] =>
  Arr.map(rows, (row) =>
    SelectorFilter.descendants(row, matrixConfig.selectors.cell)
  );

const doMove = (
  ifCycle: MatrixNavigation.MatrixNavigationFunc<Element>,
  ifMove: MatrixNavigation.MatrixNavigationFunc<Element>
): DomMovement.ElementMover<MatrixConfig, Stateless> => (
  element,
  focused,
  matrixConfig
) => {
  const move = matrixConfig.cycles ? ifCycle : ifMove;
  return SelectorFind.closest(focused, matrixConfig.selectors.row).bind(
    (inRow) => {
      const cellsInRow = SelectorFilter.descendants(
        inRow,
        matrixConfig.selectors.cell
      );

      return DomPinpoint.findIndex(cellsInRow, focused).bind((colIndex) => {
        const allRows = SelectorFilter.descendants(
          element,
...
const moveRight = doMove(
  MatrixNavigation.cycleRight,
...
const getKeydownRules: () => Array<
  KeyRules.KeyRule<MatrixConfig, Stateless>
> = Fun.constant([
  KeyRules.rule(
    KeyMatch.inSet(Keys.LEFT()),
    DomMovement.west(moveLeft, moveRight)
  ),
  KeyRules.rule(
