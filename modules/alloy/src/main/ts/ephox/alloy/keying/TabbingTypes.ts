...
const create = (cyclicField: FieldProcessorAdt) => {
  ...
  const findInitial = (
    ...
    const tabstops: Element[] = SelectorFilter.descendants(
      component.element(),
  ...
  const goFromTabstop = (
    ...
    cycle(tabstops, stopIndex, (elem: Element) =>
      isTabstop(tabbingConfig, elem)
    ).fold(
      () =>
  ...
  const go = (
    ...
    return findCurrent(component, tabbingConfig).bind((tabstop) => {
      // focused component
      const optStopIndex = Arr.findIndex(
        tabstops,
        Fun.curry(Compare.eq, tabstop)
      );
  ...
  const goBackwards: KeyRuleHandler<TabbingConfig, Stateless> = (
    component,
    simulatedEvent,
    tabbingConfig
  ) => {
    const navigate = tabbingConfig.cyclic
      ? ArrNavigation.cyclePrev
      : ArrNavigation.tryPrev;
    return go(component, simulatedEvent, tabbingConfig, navigate);
  };

  const goForwards: KeyRuleHandler<TabbingConfig, Stateless> = (
    component,
    simulatedEvent,
    tabbingConfig
  ) => {
  ...
  const execute: KeyRuleHandler<TabbingConfig, Stateless> = (
    component,
    simulatedEvent,
    tabbingConfig
  ) => tabbingConfig.onEnter.bind((f) => f(component, simulatedEvent));

  const exit: KeyRuleHandler<TabbingConfig, Stateless> = (
    component,
    simulatedEvent,
    tabbingConfig
