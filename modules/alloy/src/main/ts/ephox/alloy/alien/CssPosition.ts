...
export interface CssPositionAdt {
  fold: <T>(screen: (point: Position) => T, absolute: (point: Position, sx: number, sy: number) => T) => T;
  match: <T>(branches: { screen: (point: Position) => T; absolute: (point: Position, sx: number, sy: number) => T }) => T;
...
} = Adt.generate([{ screen: ['point'] }, { absolute: ['point', 'scrollLeft', 'scrollTop'] }]);

const toFixed = (pos: CssPositionAdt): Position =>
  // TODO: Use new ADT methods
  pos.fold(Fun.identity, (point, scrollLeft, scrollTop) => point.translate(-scrollLeft, -scrollTop));
