...
const isAriaPartOf = (component: AlloyComponent, queryElem: Element): boolean =>
  AriaOwner.find(queryElem).exists((owner: Element) => isPartOf(component, owner));

const isPartOf = (component: AlloyComponent, queryElem: Element): boolean =>
  PredicateExists.closest(queryElem, (el: Element) => Compare.eq(el, component.element()), Fun.constant(false)) ||
