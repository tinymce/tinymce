...
const isDangerous = (event: EventArgs<KeyboardEvent>): boolean => {
  ...
  return (
    keyEv.which === Keys.BACKSPACE()[0] &&
    !Arr.contains(['input', 'textarea'], Node.name(event.target())) &&
    !SelectorExists.closest(event.target(), '[contenteditable="true"]')
  );
};
...
const setup = (container: Element, rawSettings: {}): { unbind: () => void } => {
  ...
  const simpleEvents = Arr.map(
    ...
    (type) =>
      DomEvent.bind(container, type, (event) => {
