import{r as n,j as g}from"./iframe-DMoLddr3.js";import{b as S}from"./Bem-2mkHu_ih.js";import{b as _}from"./Optional-CLt1CVr3.js";import"./Strings-DbDkfDAK.js";const H=e=>{const a=e.rows,t=e.value;e.value="",e.rows=1;const s=e.scrollHeight;return e.rows=a,e.value=t,s},b=({maxHeight:e,singleRowHeight:a})=>{if(e.unit==="rows")return Math.max(e.value,1);const t=Math.floor(e.value/a);return Math.max(t,1)},q=({minHeight:e,singleRowHeight:a})=>{if(e.unit==="rows")return Math.max(e.value,1);const t=Math.ceil(e.value/a);return Math.max(t,1)},z=({minRows:e,maxRows:a,singleRowHeight:t,textarea:s})=>{s.rows=e;const{scrollHeight:c}=s,p=Math.min(Math.max(e,Math.ceil(c/t)),a);s.rows=p},E={unit:"rows",value:1},N={unit:"rows",value:4},i=n.forwardRef(({maxHeight:e=N,minHeight:a=E,className:t,value:s,onChange:c,...p},o)=>{const u=n.useRef(null),[r,V]=n.useState(1);n.useLayoutEffect(()=>{u.current&&V(H(u.current))},[]);const v=n.useMemo(()=>q({minHeight:a,singleRowHeight:r}),[a,r]),f=n.useMemo(()=>b({maxHeight:e,singleRowHeight:r}),[e,r]);return n.useLayoutEffect(()=>{u.current&&z({textarea:u.current,minRows:v,maxRows:f,singleRowHeight:r})},[s,v,f,r]),g.jsx("textarea",{...p,className:`${S("tox-textarea")} ${t??""}`,value:s,onChange:l=>{c&&c(l.target.value)},ref:l=>{u.current=l,o&&(typeof o=="function"?o(l):_(o)&&(o.current=l))}})});try{i.displayName="AutoResizingTextarea",i.__docgenInfo={description:"",displayName:"AutoResizingTextarea",props:{maxHeight:{defaultValue:{value:`{
  unit: 'rows',
  value: 4
