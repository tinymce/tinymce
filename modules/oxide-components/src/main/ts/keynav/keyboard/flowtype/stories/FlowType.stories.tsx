...
export const FlowKeyingWithCycles: Story = {
  ...
  play: async ({ canvasElement, step }) => {
    ...
    const firstChild: HTMLElement | null = container.querySelector('.one');
    firstChild?.focus();
    ...
    await step('Test execute', async () => {
      ...
      await expect(store, 'Check that execute has fired on the right target').toEqual([ 'item.execute: one' ]);
      clearStore(store);
    });

    await step('Test escape', async () => {
      await userEvent.keyboard('{Escape}');
      await expect(store, 'Check that escape handler has fired').toEqual([ 'flow.onEscape' ]);
    });
  },
};
