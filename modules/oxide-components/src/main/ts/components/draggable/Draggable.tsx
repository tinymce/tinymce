...
import type { DraggableProps, DraggableHandleProps, Shift, Position, Boundries, CssPosition, CssSize } from './internals/types';
...
const getPosition = (position: CssPosition, size?: CssSize) => {
  if (Type.isNullable(size)) {
    return { top: position.top, left: position.left };
  }
  return {
    top: `min(${position.top}, 100% - ${size.height})`,
    left: `min(${position.left}, 100% - ${size.width})`
  };
};

const getPositioningStyles = (shift: Shift, position: CssPosition | Position, isDragging: boolean, declaredSize?: CssSize): React.CSSProperties => {
  if (isDragging) {
    return { transform: getTransform(shift), ...getPosition(normalizePosition(position), declaredSize) };
  }
  return getPosition(normalizePosition(position), declaredSize);
};
...
const Root = forwardRef<HTMLDivElement, DraggableProps>(({ children, style, initialPosition = { top: 0, left: 0 }, declaredSize, ...props }, ref) => {
  ...
  const positioningStyles = getPositioningStyles(shift, position, isDragging, declaredSize);
