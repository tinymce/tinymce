define(
  ...
  function (Unicode, Fun, Option, Spot, Injection, Wrapper, Wraps, Struct) {
    var point = function (universe, start, soffset, finish, foffset) {
     ...
     */
    var scan = function (universe, start, soffset, finish, foffset) {
      return universe.property().parent(start).bind(function (parent) {
        var cursor = Spot.point(start, soffset);
        var canReuse = isSpan(universe, parent) && universe.property().children(parent).length === 1 && isUnicode(universe, start);
    ...
    var isSpan = function (universe, elem) {
      return universe.property().name(elem) === 'span';
    };

    var wrap = function (universe, start, soffset, finish, foffset) {
      var doc = start.dom().ownerDocument;
      var nuSpan = function () {
        return Wraps(universe, universe.create().nu('span', doc));
      };

      var wrappers = Wrapper.reuse(universe, start, soffset, finish, foffset, Fun.curry(isSpan, universe), nuSpan);
    ...
    var spans = function (universe, start, soffset, finish, foffset) {
      var wrapper = isCollapsed(universe, start, soffset, finish, foffset) ? point : wrap;
      return wrapper(universe, start, soffset, finish, foffset);
    };
