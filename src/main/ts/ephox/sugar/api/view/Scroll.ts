...
var setToElement = function (win: Window, element: Element) {
...
var preserve = function (doc: Element, f) {
  var before = get(doc);
  f();
  var after = get(doc);
...
var capture = function (doc: Element) {
  var previous = Option.none<Position>();
...
var intoView = function (element: Element, alignToTop: boolean) {
...
var intoViewIfNeeded = function (element: Element, container: Element) {
  var containerBox = (container.dom() as HTMLElement).getBoundingClientRect();
  var elementBox = (element.dom() as HTMLElement).getBoundingClientRect();
