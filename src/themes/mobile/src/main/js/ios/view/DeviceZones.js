define(
  ...
  function (Css, Height, window, Devices, Orientation) {
    // Green zone is the area below the toolbar and above the keyboard, its considered the viewable region that is not obstructed by the keyboard.
    // If the keyboard is down, then the Green Zone is larger.
    ...
    var accountableKeyboardHeight = function () {
      ...
      // This is our attempt to detect when we are in a webview. If the DOM window height is smaller than the actual screen height by about the size of a keyboard,
      // we assume that's because a keyboard is causing it to be that small.
      // We will improve this at a later date.
    ...
    var updatePadding = function (contentBody, socket) {
      ...
      // TBIO-3878 Changed the element that was receiving the padding from the iframe to the body of the iframe's document.
      // The reasoning for this is that the syncHeight function of IosSetup.js relies on the scrollHeight of the body to set the height of the iframe itself.
      // If we don't set the padding-bottom on the body, the scrollHeight is too short, effectively disappearing the content from view.
