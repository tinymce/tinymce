define(
  ...
  function (Focus, Css, WindowSelection, clearInterval, setInterval) {
    ...
    var setSelectionAtTouch = function (editorApi, touchEvent) {
      // shortTextFix, when text is short body height is short too, tapping at the bottom of the editor should set a selection.
      // We don't set body height to 100% because of side effects, so we resort to a mousedown on the iDoc, it is a clean place, and very specific to this issue
      // On a vanilla CE, with body height 100%, event sequence: touchstart, touchend, mousemove, mousedown, FOCUS, mouseup, click
      // This is why we fire focus on mousedown, to match the natural sequence.
      Focus.focus(editorApi.body());

      // then set the selection to the end, last cursor position
      // Note: the reason why there is a flicker when we touch the bottom, is because of the native scroll cursor into view,
    ...
    var onOrientationReady = function (refreshView) {
      ...
      // done here instead of nomad toolbar fixup since that is tied to window scroll, which does not fire on landscape
