...
const extractFromAnchor = (editor, settings, anchor: HTMLAnchorElement, selection) => {
  ...
  const defaultTarget: Option<string> = Settings.hasDefaultLinkTarget(settings) ? Option.some(Settings.getDefaultLinkTarget(settings)) : Option.none();
  const target: Option<string> = anchor ? Option.from(dom.getAttrib(anchor, 'target')) : defaultTarget;
