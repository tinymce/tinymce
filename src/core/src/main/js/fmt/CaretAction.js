...
define(
  ...
  function (RangeUtils, TreeWalker, ExpandRange, FormatUtils, MatchFormat, Zwsp, Fun, Tools) {
    ...
    var performCaretAction = function (ed, applyFormat, type, name, vars, similar) {
      ...
      function applyCaretFormat() {
        ...
        if (text && offset > 0 && offset < text.length &&
          ...
          rng = ExpandRange.expandRng(ed, rng, ed.formatter.get(name));
          rng = new RangeUtils(dom).split(rng);

          // Apply the format to the range
          applyFormat(ed, name, vars, rng);
          ...
          if (!caretContainer || textNode.nodeValue !== ZWSP) {
            ...
            applyFormat(ed, name, vars, caretContainer);
          } else {
            applyFormat(ed, name, vars, caretContainer);
          }
      ...
      function removeCaretFormat() {
        ...
        while (node) {
          if (MatchFormat.matchNode(ed, node, name, vars, similar)) {
            formatNode = node;
            break;
          }
        ...
        if (hasContentAfter) {
          ...
          // Expand the range to the closest word and split it at those points
          rng = ExpandRange.expandRng(ed, rng, ed.formatter.get(name), true);
          rng = new RangeUtils(dom).split(rng);

          // Remove the format from the range
          ed.formatter.remove(name, vars, rng);
          ...
          var block = dom.getParent(formatNode, Fun.curry(FormatUtils.isTextBlock, ed));
