 ...
 * inadvertently lost.
 *
 * This plugin is developed and maintained by Todd Northrop aka Speednet.
 */
...
(function() {
	...
	tinymce.create("tinymce.plugins.AutoSavePlugin", {
		...
		///		A reference to the TinyMCE editor instance that contains this AutoSave plugin instance.
		/// </field>
		/// <field name="url" type="String" mayBeNull="false">
		///		The URL of the folder containing the AutoSave plugin. Does not include a trailing slash.
		/// </field>
		/// <field name="key" type="String" mayBeNull="false">
		///		A string value identifying the storage and settings for the plugin, as set by autosave_key.
		/// </field>
		...
		///		Receives the Boolean value specified in the autosave_showsaveprogress configuration option,
		...
		/// 
		/// autosave_key - (String, default = editor id) A string value used to identify the autosave
		/// storage and settings to use for the plug instance. If autosave_key is not specified, then
		/// the editor's id property is used. If you set the autosave_key for all editors to the same value,
		/// that would create a single autosave storage instance and a single set of autosave settings to use
		/// with all editors. Because each key maintains its own plugin settings, autosave_key can also be
		...
		/// 
		/// autosave_interval_seconds - (Number, default = 60) The number of seconds between automatic saves.
		/// When the editor is first displayed, an autosave will not occur for at least this amount of time.
		/// 
		/// autosave_minlength - (Number, default = 50) The minimum number of characters that must be in the
		...
		/// 
		/// autosave_retention_minutes - (Number, default = 20) The number of minutes since the last autosave
		/// that content will remain in the rescue storage space before it is automatically expired.
		/// 
		/// autosave_oninit - (String, default = null) The name of a function to call immediately after the
		/// AutoSave plugin instance is initialized. Can include dot-notation, e.g., "myObject.myFunction".
		...
		/// 
		/// autosave_showsaveprogress - (Boolean, default = true) When true, the toolbar button will show a
		/// brief animation every time an autosave occurs.
		/// 
		/// COMMANDS:
		/// 
		/// Available TinyMCE commands are:
		/// 	mceAutoSave - Perform an auto-save
		/// 	mceAutoSaveRestore - Restore auto-saved content into the editor
		/// 
		/// PUBLIC PROPERTIES:
		/// 
		/// Available public properties of the AutoSave plugin are:
		/// 	editor (Object)
		/// 	url (String)
		/// 	key (String)
		/// 	onPreSave (String)
		...
		/// PUBLIC METHODS:
		/// 
		/// Available public methods of the AutoSave plugin are:
		...
		/// HTML 5 gets wider support, the AutoSave plugin will use it automatically. In Windows Vista,
		...
		init: function (ed, url) {
			...
			// Either 'autosave_interval_seconds' or 'autosave_interval' can be used, but 'autosave_interval_seconds' provides better clarity.
			...
			// Either 'autosave_retention_minutes' or 'autosave_retention' can be used, but 'autosave_retention_minutes' provides better clarity.
			...
			// Specified as 'autosave_minlength' in the config.
			...
			// Specified as 'autosave_showsaveprogress' in the config.
			...
			// Register commands
			ed.addCommand("mceAutoSave", s.saveDelegate);
			ed.addCommand("mceAutoSaveRestore", s.restoreDelegate);

			// Register restore button
			ed.addButton(pluginName, {
				title: pluginName + ".restore_content",
				cmd: "mceAutoSaveRestore",
			...
			// Call autosave_oninit, if specified
			...
			if (ed.getParam(pluginName + "_ask_beforeunload", true))
		...
		clear: function () {
			/// <summary>
			///		Removes the autosave content from storage. Disables the 'autosave' toolbar button.
			/// </summary>
		...
		setProgressImage: function (url) {
			...
			///		temporarily replaces the image on the AutoSave toolbar button every
			///		time an auto-save occurs. The default value is
			///		"[tinymce]/plugins/autosave/images/progress.gif". Can be set any time
	...
	function save() {
		...
		///		animates the toolbar button. Enables the 'autosave' button to indicate
	...
	function restore() {
		/// <summary>
		///		Called when the user clicks the 'autosave' button on the toolbar.
	...
	tinymce.PluginManager.add(pluginName, tinymce.plugins.AutoSavePlugin);
